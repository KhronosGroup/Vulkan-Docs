...
== Attributes of tag:enum Tags
  ...
  * attr:protect - optional.
    ...
    a boolean expression using the same syntax as attr:depends:
  ** `,` for logical OR
  ** `+` for logical AND
  ** `()` for grouping
+
...
+
Single tokens generate `#ifdef` directives in the output header, while
expressions generate `#if` directives with `defined()` macros around each
term.
+
Examples:
+
