...
tag:feature specifies a required (or removed) individual feature, as
described in the "`Features`" chapter of the <<vulkan-spec, Vulkan API
Specification>>.
The tag has no content, but contains attributes:

  * attr:name - feature name.
    Name of the feature, and also the name of a member of the corresponding
    attr:struct structure.
...
with a attr:depends attribute that includes more than one API version or
extension using logical operators is currently not supported, and an error
will be generated.
