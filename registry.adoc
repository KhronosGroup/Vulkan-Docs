...
_Vulkan 1.4 - A Specification_ (December 2, 2024).
...
classes of features with the use of _API names_.
...
order should not be important):
  ...
  * <<tag-dynamicstates,tag:dynamicstates>> - defines properties for
    dynamic state.
    Only one tag is used, wrapping all the1 `VkDynamicState` enums.
  ...
  * <<tag-videocodecs,tag:videocodecs>> - Defines video codecs
  * <<tag-videocodec,tag:videocodec>> - Defines a video codec category or
    specific video codec
  * <<tag-videoprofiles,tag:videoprofiles>> - Defines video profiles
    supported by a specific video codec
  * <<tag-videoprofilemember,tag:videoprofilemember>> - Defines video
    profiles with respect to specific video profile structure members
  * <<tag-videoprofile,tag:videoprofile>> - Defines a video profile with
    respect to specific video profile structure members
  * <<tag-videocapabilities,tag:videocapabilities>> - Specifies a capability
    structure that applies to the video codec category or specific video
    codec
  * <<tag-videoformat,tag:videoformat>> - Defines a video format category
    that applies to the video codec category or specific video codec
  * <<tag-videoformatproperties,tag:videoformatproperties>> - Specifies a
    video format property structure that applies to a video format category
  * <<tag-videorequirecapabilities,tag:videorequirecapabilities>> - Defines
    a video capability prerequisite
...
`HANDLE`, the tag:type tag can insert a `#include` directive to pull in that
header.


== Shared Attributes of tag:type Tags

All tag:type tags may include these attributes:

  * attr:alias - optional (unless not allowed by other tags, as defined
    below).
  ...
  * attr:category - optional (unless not allowed by other tags, as defined
    below).
  ...
  * attr:deprecated - optional.
    Indicates that this type has been marked as legacy.
    Possible values are:
  ** `"true"` - marked as legacy, but no explanation given.
  ** `"aliased"` - an old name not following Vulkan conventions.
     The equivalent alias following Vulkan conventions should be used
     instead.
  * attr:name - optional (unless required by other tags, as described
    below).
    Name of this type (if not defined in the tag body).

[[tag-types:declarations]]
== C Type Declarations

Several tag:type attr:category categories permit or require a C type
declaration in the tag content.
This is free-form text which must: be legal C, but the content can, and
sometimes must contain embedded tag metadata:

  * tag:type - optional.
    May appear multiple times.
    Contains text which is a valid type name found in another tag:type
    tag, and indicates that this type must be previously defined for the
    definition of the type to succeed.
  * tag:name - contains the name of this type (if not defined in the
    attr:name attribute).
  * tag:comment - optional.
    Contains an arbitrary string (unused).

=== Constraints on Type Categories

There is not a hard restriction on which sorts of definitions may be made in
a given category, but the intended purpose of each category should be
respected.
In particular, when generating header files, types within each category are
grouped together, and categories are generated in the order given by the
following list.
...
The intended purpose of each category is:
  ...
  * `define` - C macro `#define` directives.
  * `enum` - enumerated types.
  * `funcpointer` - C function pointer typedefs.
  * `handle` - scalar API types defined using platform-dependent macros,
     such as `VkInstance`.
  * `include` (`#include`) directives).
  * `struct` and `union` - the corresponding C aggregate types.

== Contents of tag:type Tags

The valid combinations of additional attributes, and valid contents of
tag:type tags are defined below.

=== Dependency Relationships

When *both* the attr:name and attr:requires tags are present, but the
attr:category tag is not present, the tag simply defines a dependency
relationship - whatever required the specified attr:name will also require
the specified attr:requires.

This form is used when referring to external header files that define
multiple external types, such as the `X11/Xlib.h` header which defines X11
types such as `Display` and `VisualID`.

// The schema currently allows "deprecated=" tags but does nothing with them

No additional attributes are allowed for dependency relationships, including
attr:alias.

No content is allowed for dependency relationships.


=== Base Types - attr:category `"basetype"`

Base types are scalar or incomplete types, either external such as Android's
`ANativeWindow` or Vulkan types such as `VkBool32`.

No additional attributes are allowed for base types, including attr:alias.

Base type content consists of a <<tag-types:declarations, C Type
Declaration>>.

=== Bitmask Types - attr:category `"bitmask"`

Bitmask types are scalar types representing a mask of 32 or 64 bits, as
typedefs of `VkFlags` or `VkFlags64`.

If the attr:alias attribute is specified, the attr:name attribute must also
be specified, and no content is allowed.

Otherwise, the attr:name is optional and is normally not provided.
Other attributes which may be specified in this case are:

  * attr:requires - containing the name of a 32-bit `Vk*FlagBits` tag:enum
    type defining individual flag values.
  * attr:bitvalues - contains the name of a 64-bit `Vk*FlagBits2` tag:enum
    type defining individual flag values.
    This is used instead of attr:requires because these types do not define
    an actual C enumerated types for their flag values, defining them simply
    as `uint64_t` values.

Bitmask type content consists of a <<tag-types:declarations, C Type
Declaration>>.

=== Macro Definitions - attr:category `"define"`

Macro definitions are uncommon and used for purposes such as the Vulkan API
version defined by the header file, or for assembling a version number from
major, minor, release, and variant numbers.

The attr:name is optional and is normally not provided, being specified
in the content instead.

Macro definition content consists of a <<tag:types:declarations, C Type
Declaration>>.
However, unlike most such declarations, the content does not actually define
a type.
Instead, the content is a legal C `#define` statement.

=== Enumeranted Type References - attr:category `"enum"`

Enumeranted types refer to tag:enums tags where individual enumerants for
the type are defined.

The attr:name attribute must be specified.

If the attr:alias attribute is specified, attr:name is declared as an alias
of that enumerated type.

Otherwise, attr:name must match the attr:name of a tag:enum which actually
defines individual enumerant values.
Those values are used to generate the type declaration.

[NOTE]
====
The tag:enums tag is not redundant.
It is simpler to use tag:enums as a container that is indirectly referred
to, than to have included the enumerant definitions inside the type tag.
====

No content is allowed.

=== Function Pointers - attr:category `"funcpointer"`

Function pointer types represent callback functions passed by an application
to Vulkan API commands for purposes such as allocations and debugging.

The attr:name is optional and is normally not provided.

No additional attributes are allowed for function pointer types, including
attr:alias.

The content of a function pointer tag is structurally identical to the
content of a <<tag-command:content, tag:command>> tag.
Refer to that tag for the allowed content.
However, instead of generating a declaratio of a function prototype and
corresponding pointer to that function, only the function pointer is
generated.

[NOTE]
====
Many attributes are supported for tag:command tag:param tags, primarily used
to generate valid usage statements, which are only relevant when calling
*into* Vulkan - not in callbacks *from* it.

While these attributes are syntactically legal for function pointer types,
they are not used when generating the function pointer and should not
be included.
====

=== Handle Types - attr:category `"handle"`

Handle types are scalar types representing a handle to an object in the
Vulkan implementation, such as `VkDevice` or `VkInstance`.
They are defined using compile time macros, since the representation of a
handle differs depending on the ABI and API variant being compiled for.
...
Other attributes which may be specified in this case are:
  ...
  * attr:parent - optional.
    Name of another handle type which this type is a subclass of, such as
    `parent="VkDevice"` for a `VkQueue` handle.
  * attr:objtypeenum - required.
    Name of a `VkObjectType` enumerant which specifies this handle type.

Handle content consists of a <<tag:types:declarations, C Type Declaration>>.
The declaration should use the `VK_DEFINE_HANDLE` or
`VK_DEFINE_NON_DISPATCHABLE_HANDLE` macros with the handle tag:name as an
argument.

=== Include Files - attr:category `"include"`

Include file definitions are uncommon.
They are used to define C `#include` files needed by the Vulkan headers to
define external types, including video codec headers; and to pull in the
platform-dependent `vk_platform.h` header included with the Vulkan SDK.

The attr:name attribute is required.
It is usually the pathname of the external header file to include.

No additional attributes are allowed for function pointer types, including
attr:alias.

If present, include file content should be a C `#include` directive for the
specified attr:name.

[NOTE]
====
Include files which are external headers, such as `X11/Xlib.h`, usually do
not have tag content.
These headers can be included under application control from the
platform-dependent `vulkan.h` header, or explicitly included by the
application.

Include files which are Vulkan video codec headers are not
platform-dependent, but included with the Vulkan SDK.
Tag content for these files explicitly includes them.
====

=== Structure Types - attr:category `"struct"` or `"union"`

Structure and union types define the corresponding C types.

If the attr:alias attribute is specified, the attr:name attribute must also
be specified, and no content is allowed.

Otherwise, the attr:name is required.
Many other attributes may be specified

Other attributes which may be specified in this case are:

// Should we even allow "false" values here? Inconsistent with attr:returnedonly.
  * attr:allowduplicate - optional.
    Must be `"true"` or `"false"`.
    The default value if unspecified is `"false"`.
    If `"true"`, then more than one instance of this type may be present in
    a `pNext` chain.
  * attr:requiredlimittype - optional.
    If provided, the attribute value must be `"true"`.
    The default value if unspecified is `"false"`.
    If `"true"`, then this type's tag:member tags, other than `sType` and
    `pNext`, must include attr:limittype attributes.
  * attr:returnedonly - optional.
    If provided, the attribute value must be `"true"`.
    The default value if unspecified is `"false"`.
    If `"true"`, it indicates that this type will always be filled in
    (returned by) the API from a query, rather than filled in by an
    application passing it to the API.
    In this case, the type's `pNext` tag:member must be declared `const`.
  * attr:structextends - optional.
    This is a comma-separated list of structures whose `pNext` chain can
    include this type.
    This should only list the top-level structure that is extended, for all
    possible extending structures.
    This will generate a validity statement on the top level structure that
    validates the entire chain at once, rather than each extending structure
    repeating the list of valid structs.
    There is no need to set the attr:noautovalidity attribute on the `pNext`
    members of extending structures.

Struct and union content is a series of tag:member tags defining the members
of the aggregate type, in order, interleaved with any number of tag:comment
tags.

==== Structure Member (tag:member) Tags

The tag:member tag defines the type and name of a structure or union member.
Member tags may define many attributes providing metadata about that member.

==== Attributes of tag:member Tags

  * attr:altlen - if the attr:len attribute (see below) is also specified,
    and contains a `latexmath:` equation, this contains one or more of the
    other options for representing an array length defined for that
    attribute, separated by commas (one for each array indirection).
    This attribute is for consumers of the XML who need to generate
    validation code from the allowed length without wanting to parse
    arbitrary LaTeX formatting.
  * attr:api - optional <<schema:apiname, API names>> for which this
    ...
    attribute.
    May be used to address subtle incompatibilities.
    This is the only case in which the same tag:member tag:name may appear
    multiple times.
  * attr:deprecated - optional.
    Indicates that this member has been marked as legacy.
    Possible values are:
  ** `"true"` - marked as legacy, but no explanation given.
  ** `"unused"` - this member is not used by the implementation and must be 0.
  * attr:externsync - optional.
    If provided, the attribute value must be `"true"`.
    The default value if unspecified is `"false"`.
    If `"true"`, it indicates whether the member should be externally
    synchronized when accessed by Vulkan.
  * attr:featurelink - optional.
    This attribute is only used for members representing a boolean API
    feature, when the anchor link in the Vulkan Specification does not match
    the member tag:name.
    Used for features in extensions that were later promoted but with
    changes.
    The attribute value should be a feature name for which a corresponding
    `features-**name**` anchor exists.
  * attr:flagsextend - optional.
    Name of a structure extending the structure this member belongs to.
    If present in the `pNext` chain, the attr:flagsextendmember of that
    structure will be used, instead of the flags value specified by this
    member.
  * attr:flagsextendmember - optional.
    Name of a member within the attr:flagsextend structure specifying the
    flags value that is used, instead of the flags value specified by this
    member.
    attr:flagsextend and attr:flagsextendmember are used to inform
    generation of implicit valid usage statements and validation layer code.
    If one of attr:flagsextend and attr:flagsextendmember is defined, the
    other must be defined as well.
  * attr:len - optional.
    Valid length of the data array described by the member, in array
    elements.
    If the member is a static array, the length must be less than or equal
    to the size of the array.
    If not present for a static array, all elements of the array are
    considered valid.
    ...
    each level of array indirection), separated by commas.
  ...
  ** an equation in math markup for incorporation in the specification (a
     ...
     the structure. Use the attr:altlen attribute in this case to provide
     both human- and code-friendly representations of the length.
  ** the name of another tag:member of the structure specifying the length
     for the corresponding indirection;
  ** `"null-terminated"`, indicating a pointer to a null-terminated UTF-8
     string;
  ** `"1"` indicating it is just a pointer (used for nested pointers);
  ** a C99 expression that refers to another member of the structure,
     indirectly specifying the length for the corresponding indirection;
  ** or a custom expression that cannot be used directly by code generation
     and needs manual handling in generators.
  * attr:limittype - optional.
    Specifies the type of a device limit corresponding to the tag:member.
    ...
    `VkSparseImageFormatProperties2`, `VkVideoCapabilitiesKHR`,
    `VkVideoFormatPropertiesKHR`, and structures extending or members of
    any of those structures.
    The attribute value describes how a value should be compared with the
    value of a member in order to check whether it fits the limit.
    Valid attr:limittype values are:
  ** `"min"` and `"max"` denote minimum and maximum limits.
     They may also apply to arrays and `VkExtent*D`. For `VkBool32`, if the
     property is `VK_TRUE`, `"min"` indicates the presence of a requirement,
     while `"max"` indicates the presence of a capability, respectively.
  ** `"pot"` denotes a value that must be a power of two.
  ...
  ** `"bitmask"` corresponds to bitmasks, where set bits indicate the
  ...
  ** `"noauto"` limits cannot be trivially compared and must be manually
     handled in generators.
     `"noauto"` is the default value, if attr:limittype is needed but
     unspecified.
  * attr:noautovalidity - optional.
    If provided, the attribute value must be `"true"`.
    The default value if unspecified is `"false"`.
    If `"true"`, it prevents automatic validity language being generated for
    the tag:member.
    Note that this only suppresses item-specific validity - parenting
    relationships, for example, are still generated.
  * attr:objecttype - optional.
    Only applicable for members which are Vulkan object handles.
    The attribute value is the name of another tag:member, whose value must
    be a `VkObjectType` or `VkDebugReportObjectTypeEXT` value specifying the
    type of object the handle refers to.
  * attr:optional - optional.
    ...
    Structure members with tag:name `pNext` must always be specified with
...
--
  * attr:selection - optional.
    For a member of a union, attr:selection identifies one or more
    comma-separated values of the attr:selector that indicate this member
    is valid.
  * attr:selector - optional.
    For a member of a union, attr:selector is the name of another member of
    the type that is used to select which of that union's members are valid.
  * attr:stride - optional.
    If the member is an array, attr:stride is the name of another member of
    the type containing the byte stride between consecutive elements in the
    array.
    The array is assumed to be tightly packed if omitted.
  * attr:values - optional.
    This attribute must be provided for the `sType` tag:member of a
    structure, but may not be provided for other members.
    The attribute value is a comma-separated list of names of enumerants
    that are valid for the structure type.
    Usually there is only a single value.

==== Contents of tag:member Tags

Member content consists of a <<tag:types:declarations, C Type Declaration>>.
In addition to the allowed tags for such a declaration,
the following tags are supported:

  * tag:name tag - required.
    Contains the struct or union member name being described.
    Unlike other C type declarations, the tag:name tag must always be
    provided in tag:member tags.
  ** attr:alias - optional. Another union member name or extension name which
    this member is an alias of.
    Alias chaining (A aliases B aliases C) is not supported.
  * tag:enum - optional.
    May appear multiple times.
    Contains a valid enumerant name found in another tag:type tag, and which
    must be previously defined for the definition of the struct or union to
    be complete.
    Normally this is used only to semantically tag static array lengths.
  * tag:comment - optional.
    Contains an arbitrary string (unused).
...
<types>
    ...
    <type category="struct" name="VkPhysicalDeviceVulkan12Features" structextends="VkPhysicalDeviceFeatures2,VkDeviceCreateInfo">
        <member><type>VkBool32</type> <name alias="VK_KHR_sampler_mirror_clamp_to_edge">samplerMirrorClampToEdge</name></member>
        <member><type>VkBool32</type> <name alias="VkPhysicalDevice8BitStorageFeatures::storageBuffer8BitAccess">storageBuffer8BitAccess</name><comment>8-bit integer variables supported in StorageBuffer</comment></member>
    </type>
</types>
...
== Attributes of tag:enum Tags
  ...
  * attr:api - optional comma-separated list of <<schema:apiname, API
    ...
    May be used to address subtle incompatibilities.
  * attr:deprecated - optional.
    Indicates that this enum has been marked as legacy.
    Possible values are:
  ** `"true"` - marked as legacy, but no explanation given.
  ** `"unused"` - this enum is not used by the implementation and must not be passed to the API.
  ** `"aliased"` - an old name not following Vulkan conventions.
     The equivalent alias following Vulkan conventions should be used
     instead.
  * attr:type - may be used only when attr:value is specified.
    ...
    type of attr:value, although only `uint8_t`, `uint32_t`, `uint64_t`,
    and `float` are currently meaningful.
  ...
  * attr:protect - optional.
    An additional preprocessor token or boolean expression used to protect an
    enum definition with conditional compilation directives in generated
    headers.
    May be a single preprocessor token (e.g. `VK_USE_PLATFORM_WIN32_KHR`) or
    a boolean expression using the same syntax as attr:depends, extended with
    support for logical NOT:
  ** `,` for logical OR
  ** `+` for logical AND
  ** `()` for grouping
  ** `!` prefix on a token for logical NOT
+
`,` and `+` are of equal precedence, and lower than `()`.
Expressions are evaluated left-to-right for operators of the same precedence.
+
Single tokens (with no operators) generate `#ifdef` directives in the output
header, while expressions generate `#if` directives with `defined()` macros
around each term (and `!defined()` for negated terms).
+
Examples:
+
  * `protect="VK_A"` generates `#ifdef VK_A`
  * `protect="VK_A,VK_B"` generates `#if defined(VK_A) || defined(VK_B)`
  * `protect="VK_A+VK_B"` generates `#if defined(VK_A) && defined(VK_B)`
  * `protect="(VK_A+VK_B),VK_C"` generates
    `#if (defined(VK_A) && defined(VK_B)) || defined(VK_C)`
  * `protect="VK_A+!VK_B"` generates `#if defined(VK_A) && !defined(VK_B)`

[NOTE]
====
Tagging two enums with the same value and no other context will result in an
error during spec generation, to avoid accidental aliasing.
However there are times where aliasing is deliberate and desirable.
Using attr:alias on an tag:enum indicates that two values are deliberate
aliases of each other.
For example, the following tag defines `VK_ENUM_TYPE_NEW` as a new alias of
`VK_ENUM_TYPE_ORIGINAL`:

[source,xml]
--------------------------------------
<enum extends="VkEnumType" name="VK_ENUM_TYPE_NEW" alias="VK_ENUM_TYPE_ORIGINAL">
--------------------------------------

Typically this will result in a C definition explicitly aliasing the other:

[source,c]
----
VK_ENUM_TYPE_NEW = VK_ENUM_TYPE_ORIGINAL;
----

Providing a attr:bitpos or attr:value tag alongside the attr:alias tag will
still indicate that the aliasing is deliberate:

[source,xml]
--------------------------------------
<enum extends="VkEnumType" value="0" name="VK_ENUM_TYPE_NEW" alias="VK_ENUM_TYPE_ORIGINAL">
--------------------------------------

However the generated C will use the attr:bitpos or attr:value instead:
...
In this case the allowed attributes include:
  ...
  * attr:tasks - optional.
    A comma-separated list of tasks this command performs, as described in
    the "`Queue Operation`" section of the <<vulkan-spec, Vulkan API
    Specification>>.
    The tasks may include `"action"`, `"indirection"`, `"state"`, and
    `"synchronization"`.
  * attr:queues - optional.
    A comma-separated list of command queue types this command can be placed
    on.
    The queue types are Vulkan `VkQueueFlagBits` names.
  * attr:successcodes - optional.
    A comma-separated list of possible successful return codes from the
    command.
    The codes are Vulkan `VkResult` code names.
  * attr:errorcodes - optional.
    A comma-separated list of possible error return codes from the command.
    The codes are Vulkan `VkResult` code names.
+
--
[NOTE]
====
attr:errorcodes must include `VK_ERROR_UNKNOWN` and
`VK_ERROR_VALIDATION_FAILED` even if there are no other possible error
codes.
====
--
  ...
  * attr:conditionalrendering - required for `vkCmd*` commands.
    Not allowed for other commands.
    A value of `"true"` specifies that this command is affected by
    conditional rendering, and `"false"` that it is not affected.
  * attr:allownoqueues - optional.
    A value of `"true"` specifies that this command can be used in a device
    with no queues. If not present, the value is assumed to be `"false"`.
  * attr:export - optional.
    A comma-separated list of names exporting this command.
    The names are arbitrary words, but usually correspond to Vulkan loader
    or library names.
  * attr:comment - optional.
    Arbitrary string (unused).
...
=== Attributes of tag:param Tags
  ...
  ** an equation in math markup for incorporation in the specification (a
     ...
     the command;
  ** the name of another parameter of the command specifying the length
     for the corresponding indirection;
  ** `"null-terminated"`, indicating a pointer to a null-terminated UTF-8
     string;
  ** `"1"` to indicate it is just a pointer (used for nested pointers);
  ** a C99 expression that refers to another parameter of the command
     indirectly specifying the length for the corresponding indirection;
  ** or a custom expression that cannot be used directly by code generation
     and needs manual handling.
  * attr:altlen - if the attr:len attribute is specified, and contains a
    `latexmath:` equation, this contains one or more of the other options
    above, separated by commas (one for each array indirection).
...
--
  ...
  * attr:externsync - optional.
    ...
    If the parameter needs to be externally synchronized under specific
    conditions, a value of `"maybe"` is used to indicate that the exact rule is
    specified in the Valid Usage language.
    If only a certain member of an object or elements of an array are
    modified, an expression string may be provided, describing the member which
    is modified.
    This expression may be prefixed by `maybe:` to indicate conditional
    external synchronization requirements specified in the Valid Usage
    language.
    For example, the `vkSetDebugUtilsObjectNameEXT` command includes attr:externsync
    attributes for the `pNameInfo` parameter indicating that a specific
    member of the parameter is externally synchronized:
+
--
[source,xml]
--------------------------------------
...
Some API features may be defined for internal usage only, and not exposed to
the public. These are tagged with the `apitype` attribute set to `internal`.
These features are not emitted in the generated headers and other artifacts.
Instead, they are merged into the first public feature that depends on them.
This is done automatically by the generator scripts. If you wish to avoid
this merging behavior, you can specify the `-no-internal-api-merging` option
to the genvk.py script.
...
== Attributes of tag:feature Tags
  ...
  * attr:apitype - optional.
    Whether the API block should be exposed publicly (default) or if it is for
    internal usage. Valid values are `internal` or `public`.
  ...
  * attr:number - required, but deprecated and will eventually be removed.
...
[NOTE]
====
attr:number is deprecated, replaced by attr:depends.
attr:number is retained only for backwards compatibility with downstream
consumers, and will eventually be removed.
====
--
...
--
  ...
  * attr:protect - optional.
    An additional preprocessor token or boolean expression used to protect a
    feature definition.
    Usually another feature or extension attr:name.
    Used when the definition of a feature or extension requires another to
    be defined first.
    Supports boolean expressions using the same syntax as attr:depends
    (see attr:protect under <<tag-enum,tag:enum>> for details).
  * attr:comment - optional.
    Arbitrary string (unused).
...
Zero or more <<tag-required,tag:require, tag:remove, and tag:deprecate
...
== Attributes of tag:extension Tags
  ...
  * [[depends-expressions]] attr:depends - optional.
    String containing a boolean expression of one or more API core versions
    and extension names.
...
--
  ...
  * attr:promotedto - optional.
    ...
    As discussed in the "`Extending Vulkan`" chapter of the <<vulkan-spec,
    Vulkan API Specification>>, the `promotedto` relationship is not a
    promise of exact API-level compatibility, and replacing use of one
    interface with the other must not be done purely mechanically.
  ...
  * attr:nofeatures - optional.
    `"true"` if this extension is intended to not have any
    <<tag-required-feature, individual tag:feature tags>>, `"false"`
    otherwise.
    Defaults to `"false"` if not specified.
  * attr:comment - optional.
    Arbitrary string (unused).
...
[[tag-required]]
= Required, Removed, and Deprecated Interfaces (tag:require, tag:remove, and tag:deprecate Tags)
...
A tag:deprecate block defines a set of interfaces marked as 'legacy' by a
tag:feature.
This will mark the tagged functionality in the spec as legacy or superseded
by this version.
Extensions should never deprecate interfaces, although this usage is allowed
by the schema.
For historical reasons, the tag:deprecate block is still called 'deprecate',
but we now use the terms 'legacy' or 'superseded' in the specification instead.

Except for the tag name and behavior, the contents of tag:require, tag:remove
and tag:deprecate tags are identical.


== Attributes of tag:require, tag:remove, and tag:deprecate Tags

  * attr:profile - optional.
    String name of an API profile.
    Interfaces in the tag are only required, removed, or deprecated if the
    specified profile is being generated.
    If not specified, interfaces are required, removed, or deprecated for all
    API profiles.
  * attr:comment - optional.
    Arbitrary string (unused).
  * attr:api - optional comma-separated list of <<schema:apiname, API
    names>> requiring, removing, or deprecating these interfaces.
    Interfaces in the tag are only required, removed, or deprecated if the
    requested API name matches an element of the attribute.
    If not specified, interfaces are required, removed, or deprecated for all
    APIs.
...
These attributes are allowed only for a tag:require tag.
  ...
  * attr:depends - optional, and only for tag:require tags.
    ...
    of the tag:extension attr:depends attribute, except that feature
    Booleans may additionally be included using C struct notation (e.g.
    `VkStructName::parameterName`).
    Terms which are feature Booleans are true if the corresponding feature
    is enabled for device level functionality, or simply supported
    otherwise.
...
[[tag-required-remove]]
== Attributes of tag:remove Tags

These attributes are allowed only for a tag:remove tag.

  * attr:reasonlink - optional, and only used when generating
    <<tag-required-feature, individual tag:feature documentation>>.
    String containing the name of an asciidoc anchor in the <<vulkan-spec,
    Vulkan API Specification>> to link to, explaining the removal.

[[tag-required-deprecate]]
== Attributes of tag:deprecate Tags

These attributes are allowed only for a tag:deprecate tag.

  * attr:explanation - a link to an explanation in the specification of why
    the tagged functionality is deprecated.

== Contents of tag:require, tag:remove, and tag:deprecate Tags
...
tag:command specifies a required, removed, or deprecated command defined in a
tag:commands block.
The tag has no content, but contains attributes:
  ...
  * attr:comment - optional.
    Arbitrary string (unused).
  * attr:protect - optional.
    If present, generates additional compile time conditional protection
    around the command definition in generated C headers.
    Body must be a name that can be tested using the C preprocessor
    `defined()` directive, and should be a single API core version name,
    extension name, or a symbol defined by the header boilerplate discussed
    in the "`API Boilerplate`" appendix of the Vulkan Specification.
+
--
[NOTE]
====
attr:protect exists for unusual dependencies that cannot be fully determined
by the generator scripts that create the header files.
In particular, a provisional extension appearing in `vulkan_beta.h` with an
interaction with a platform extension appearing in a header other than
`vulkan_core.h` will need to use this attribute to prevent compilation
failures when the platform header is not included prior to the beta header.
====
--
  * attr:supersededby - optional.
    Name of command it is superseded by, tag:deprecate block only.


[[tag-required-enum]]
=== Enum Tags

tag:enum specifies a required, removed, or deprecated enumerant defined in an tag:enums
block.
All forms of this tag support the following attributes:
  ...
  * attr:api - optional comma-separated list of <<schema:apiname, API
    ...
    May be used to address subtle incompatibilities.
  * attr:supersededby - optional.
    Name of enum it is superseded by, tag:deprecate block only.
...
enumerant:
  ...
  * attr:protect - define a preprocessor protection symbol or boolean
    expression for the enum in the same fashion as an tag:enum tag in an
    <<tag-enum,tag:enums>> block.
    See attr:protect under <<tag-enum,tag:enum>> for syntax details.
...
tag:type specifies a required, removed, or deprecated type defined in a tag:types
block.
...
The tag has no content, but contains attributes:
  ...
  * attr:comment - optional.
    Arbitrary string (unused).
  * attr:supersededby - optional.
    Name of type it is superseded by, tag:deprecate block only.
  * attr:protect - optional.
    If present, generates additional compile time conditional protection
    around the type definition in generated C headers.
    The allowed values and the use cases for this tag
    are the same as the attr:protect attribute of tag:command
    tags in <<tag-required-command, tag:require>> tags.
...
[[tag-required-feature]]
=== Individual Feature Tags

tag:feature specifies a required, removed, or deprecated feature, as described in the
"`Features`" chapter of the <<vulkan-spec, Vulkan API Specification>>.
The tag has no content, but contains attributes:

  * attr:name - feature name(s).
    Can be a single element or a comma-separated list.
    Each element is the name of the feature, and also the name of a member of
    the corresponding attr:struct structure.
    If multiple elements are present, indicates that at least one of the
    elements must be supported.
    Multiple tags should be used to require multiple features.
  * attr:struct - feature structure name.
    Name of the structure queried to determine support for the feature.
    Must only ever be a single struct name, and all elements of attr:name
    must be from the same structure.
  * attr:comment - optional.
    Arbitrary string (unused).
  * attr:supersededby - optional.
    Name of the feature, command, or other interface it is superseded by,
    tag:deprecate block only.

[NOTE]
====
This tag has the same name as the <<tag-feature, API Features>> tag.
This is an unfortunate consequence of overloading the term "`feature`",
originally used in the XML schema to refer to a core version of the API but
also used in the <<vulkan-spec, Vulkan API Specification>> to refer to an
individual capability introduced by a new extension or core version.

Fortunately, there is no ambiguity in XML parsing since the two tag:feature
tags are used at different structural levels.
====

[WARNING]
====
Please note that including an individual tag:feature tag in a tag:require block
with a attr:depends attribute that includes parentheses for complex dependencies
is currently not supported, and an error will be generated.
This will be fixed in future, as it is intended to be supported, but it was
not necessary for feature requirements as initially proposed.
====
...
== Attributes of tag:component Tags
  ...
  ** `"BOOL"` - boolean integer values where 0 is false, and non-zero is
     true.
  ** `"SFIXED5"` - signed fractional integer values that get converted to
     floating-point in the range [eq]#[-1024,1023.96875]#
  ...
  ** `"SRGB"` - R, G, and B components are unsigned normalized values that
     represent values using sRGB nonlinear encoding, while the A component
     (if one exists) is a regular unsigned normalized value
  ** `"SSCALED"` - signed integer values that get converted to
     floating-point in the range [-2^n-1^,2^n-1^-1]
  ** `"UFLOAT"` - unsigned floating-point numbers (used by packed, shared
     exponent, and some compressed formats)
  ** `"UINT"` - unsigned integer values in the range [0,2^n^-1]
  ** `"UNORM"` - unsigned normalized values in the range [0,1]
  ** `"USCALED"` - unsigned integer values that get converted to
...
[[tag-dynamicstates]]
= Dynamic States (tag:dynamicstates Tag)

The tag:dynamicstates tag contains definitions of each of the logical pipeline
dynamic states which are defined for the API.

== Attributes of tag:dynamicstates Tags

None.

== Contents of tag:dynamicstates Tags

A tag:dynamicstates block contains zero or more tag:dynamicstate tags, each
describing a logical dynamic state, in arbitrary order.

[[tag-dynamicstate]]
= Dynamic State (tag:dynamicstate Tag)

Dynamic states are described in individual tag:dynamicstate tags.
This tag groups related Vulkan commands and contains information specifying
the shader stages, pipeline substates, and conditions associated with the logical state.

== Attributes of tag:dynamicstate Tags

  * attr:name - required.
    The logical name of the dynamic state group.
    Example: `name="viewport"`.
  * attr:shaderstage - optional.
    The shader stage or stages affected by the dynamic state.
    Example: `shaderstage="VK_SHADER_STAGE_ALL"`.
  * attr:pipelinesubstate - optional.
    A comma-separated list specifying the Graphics Pipeline Library sub-states this dynamic state maps to.
    Allowed tokens:
  ** `"Vertex Input"`
  ** `"Pre-Rasterization Shader"`
  ** `"Fragment Shader"`
  ** `"Fragment Output"`
  * attr:requiresrasterization - optional.
    Set to `"true"` if setting/using this dynamic state requires rasterization to be enabled (`rasterizerDiscardEnable` set to `VK_FALSE`).

== Contents of tag:dynamicstate Tags

One or more <<tag-dynamicstatecmd,tag:dynamicstatecmd>> tags.
Zero or more <<tag-enable,tag:enable>> tags, and zero or more <<tag-statecondition,tag:statecondition>> tags, in arbitrary order.

== Example of a tag:dynamicstate Tag

[source,xml]
--------------------------------------
<dynamicstate name="lineStipple"
              shaderstage="VK_SHADER_STAGE_ALL"
              pipelinesubstate="Pre-Rasterization Shader"
              requiresrasterization="true">
    <dynamicstatecmd name="vkCmdSetLineStipple" pipeline="VK_DYNAMIC_STATE_LINE_STIPPLE"/>
    <enable struct="VkPhysicalDeviceLineRasterizationFeaturesKHR" feature="stippledRectangularLines" requires="VK_KHR_line_rasterization"/>
    <statecondition state="lineStippleEnable" special="stippledLineTopology"/>
</dynamicstate>
--------------------------------------

[[tag-dynamicstatecmd]]
= Dynamic State Command (tag:dynamicstatecmd Tag)

The tag:dynamicstatecmd tag maps a specific Vulkan command to its associated `VkDynamicState` enum.

== Attributes of tag:dynamicstatecmd Tags

  * attr:name - required.
    The name of the Vulkan command used to set the dynamic state.
    Example: `name="vkCmdSetViewport"`.
  * attr:pipeline - required.
    The name of the `VkDynamicState` enum required to use this command dynamically.
    Example: `pipeline="VK_DYNAMIC_STATE_VIEWPORT"`.
  * attr:pipelineonly - optional.
    Set to `"true"` if the command is only applicable to pipelines and cannot be used with shader objects.

== Contents of tag:dynamicstatecmd Tags

None.

[[tag-statecondition]]
= State Condition (tag:statecondition Tag)

The tag:statecondition tag specifies a precursor dynamic state or special execution context condition required prior to drawing.
If there are multiple conditions they are AND together

== Attributes of tag:statecondition Tags

  * attr:state - optional.
    The name of another dynamic state value that must be set/enabled prior to drawing.
    Example: `state="VK_DYNAMIC_STATE_DEPTH_TEST_ENABLE"`.
  * attr:special - optional.
    A keyword describing a specific context required for the rule.
    This is done because some requirement are quite complex and more than fits in an XML file
    Example: `special="lineTopology"`

== Contents of tag:statecondition Tags
...
== Attributes for API Feature Enables
  ...
  * attr:struct - required.
    An API feature structure name, matching a tag:type attr:name attribute
    of a structure tag:type.
...
== Attributes of tag:syncstage Tags
  ...
  * attr:name - required.
    A Vulkan `VkPipelineStageFlagBits2` name.
  * attr:alias - optional.
    A semantically equivalent Vulkan `VkPipelineStageFlagBits` name which is
    an alias of attr:name.
...
== Attributes of tag:syncaccess Tags
  ...
  * attr:name - required.
    A Vulkan `VkAccessFlagBits2` name.
  * attr:alias - optional.
    A semantically equivalent Vulkan `VkAccessFlagBits` name which is an
    alias of attr:name.
...
Zero or more tag:syncpipelinestage tags describing each pipeline stage in
logical order.
Stages have extra attributes describing special ordering within the list.


[[tag-syncpipelinestage]]
= Sync Pipeline Stage (tag:syncpipelinestage Tag)

The tag:syncpipelinestage tag contains definitions of a stage for a
tag:syncpipeline.

== Attributes of tag:syncpipelinestage Tags

  * order - optional.
    Must be `"None"` if present, indicating ???.
  * before - optional.
    A Vulkan `VkPipelineStageFlagBits2` name of a pipeline stage following
    this stage.
  * after - optional.
    A Vulkan `VkPipelineStageFlagBits2` name of a pipeline stage preceding
    this stage.

== Contents of tag:syncpipeline Tags

A Vulkan `VkPipelineStageFlagBits2` name of this pipeline stage.


[[tag-syncsupport]]
= Sync Support (tag:syncsupport Tag)

The tag:syncsupport tag defines support for a given Sync element.

== Attributes of tag:syncsupport Tags

  * attr:queues - optional.
    A comma-separated list of queue types that are supported.
    Each queue type is a Vulkan `VkQueueFlagBits` name.
  * attr:stage - optional.
    A comma-separated list of <<tag-syncstage, sync stage names>> that are
    supported.
...
The tag:syncequivalent tag defines a one-to-many relationship of Sync
elements

== Attributes of tag:syncequivalent Tags

  * attr:stage - optional.
    A comma-separated list of <<tag-syncstage, sync stages>> that are the OR
    logical equivalence of the parent <<tag-syncstage, tag:syncstage>>.
  * attr:access - optional.
    A comma-separated list of <<tag-syncaccess, sync accesses>> that are the
    OR logical equivalence of the parent <<tag-syncstage, tag:syncstage>>.
...
The tag:videocodec tag defines a video codec category (e.g. decode or
encode) or specific video codec (e.g. H.264 decode).

== Attributes of tag:videocodec Tags

  * attr:name - descriptive name of the video codec category or specific
    video codec
...
The tag:videoprofiles tag defines video profiles supported by a specific
...
== Attributes of tag:videoprofiles Tags
  ...
  * attr:struct - name of the video profile info structure corresponding to
    the specific video codec


[[tag-videoprofilemember]]
= Video Profile Member (tag:videoprofilemember Tag)

The tag:videoprofilemember tag defines video profiles with respect to
specific video profile structure members.
...
The tag:videoprofile tag defines a video profile with respect to specific
video profile structure members.

== Attributes of tag:videoprofile Tags

  * attr:name - descriptive name of the video profile
  * attr:value - value of the video profile structure member in case of this
    video profile


[[tag-videocapabilities]]
= Video Capabilities (tag:videocapabilities Tag)

The tag:videocapabilities tag specifies a capability structure that applies
to the video codec category or specific video codec.
...
The tag:videoformat tag defines a video format category that applies to the
...
== Attributes of tag:videoformat Tags
  ...
  * attr:usage - image usage flags expected to be specified in
...
[[tag-videoformatproperties]]
= Video Format Properties (tag:videoformatproperties Tag)

The tag:videoformatproperties tag specifies a video format property
structure that applies to a video format category.
...
[[tag-videorequirecapabilities]]
= Video Capability Prerequisites (tag:videorequirecapabilities Tag)

The tag:videorequirecapabilities tag can be used inside
<<tag-videoformat,tag:videoformat>> tags to define
video capability prerequisites for video format categories.
If more tag:videorequirecapabilities elements are specified, then the
overall precondition is the conjunction of the individual preconditions.

== Attributes of tag:videorequirecapabilities Tags

  * attr:struct - name of the capability structure used to determine the
    precondition
  * attr:member - name of the capability structure member used to determine
    the precondition
  * attr:value - the value the member should have to meet the precondition
...
Add (if not present) a nested tag:require block labeled
...
Go to the tag:types tag and search for the nested block labeled
...
= Change Log
  ...
  * 2026-09-17 - Add attr:protect attribute to the <<tag-required-command,
    tag:type>> tag in tag:require blocks (internal issue 4950).
  * 2026-09-09 - Add attr:protect attribute to the <<tag-required-command,
    tag:command>> tag in tag:require blocks (internal issue 4950).
  * 2026-07-01 - Allow <<tag-required-feature, tag:feature>> tags inside
    tag:deprecate blocks, so a feature structure member can be marked legacy
    without adding attr:deprecated directly to its tag:member tag (internal
    MR 8301).
  * 2026-06-15 - Add attr:alias attribute to the tag:name tag within structure
    and union tag:member tags to support member aliasing.
  * 2026-04-07 - Add support for `!` (logical NOT) prefix on tokens in
    attr:protect boolean expressions, generating `!defined()` in output
    headers.
  * 2026-03-31 - Add attr:flagsextend and attr:flagsextendmember attributes
    to tag:struct tag:member tags, to specify a 64-bit flags value replacing
    a 32-bit flags value (internal issue 4589).
  * 2026-03-04 - Update the `"ignored"` value for the attr:deprecated
    attribute to match updated spec (internal issue 4725), rename to "unused".
  * 2026-02-18 - Update the tag:enum and tag:member attr:deprecated
    attribute to describe `"ignored"` consistently with the generators and
    specification (internal MR 7998).
  * 2026-01-16 - Add support for boolean expressions in the attr:protect
    attribute of several tags.
  * 2026-01-14 - Update the tag:type attr:category `funcpointer` schema to
    support better semantic tagging, using the same syntax as tag:command
    tags.
    Heavily refactor the <<tag-type, tag:type>> tag descriptions to
    separately describe the different categories of types, and their allowed
    schema (internal issue 5).
  * 2025-11-11 - Update the attr:returnedonly attribute of <<tag-type,
    structure type tags>> to note that `pNext` members must not be `const`
    if attr:returnedonly is true (internal issue 2734).
  * 2025-07-31 - Update the attr:queues attribute to use Vulkan flag bit
    names instead of colloquial simplifications of them, add descriptions of
    a few missing tags and attributes, and use more consistent phrasing in
    describing attribute values which are comma-separated lists of separate
    values (internal issue 4393).
  * 2025-06-18 - Allow attr:selection attributes of members of <<tag-type,
    `"union"`>> types to include multiple comma-separated selections of the
    same member (internal issue 4246).
  * 2025-06-11 - Add <<tag-command, command>> attribute
    attr:conditionalrendering to specify if a `vkCmd*` command may be
    affected by conditional rendering (internal issue 4287).
  * 2025-05-07 - Add "maybe" and "maybe:<expression>" to attr:externsync
    (internal issues 4175 and 4217)
  * 2025-04-23 - Add <<tag-required, tag:deprecate>> blocks to features.
  * 2025-03-19 - Establish and clarify the use of "min" and "max" limit
    types for boolean limits and remove the, now obsolete, "not" limit type.
  * 2025-02-19 - Clean up description of the tag:member attr:limittype
    `"exact"` value (internal MR 7142).
  * 2024-11-12 - Add tag:extension attr:nofeatures attribute to identify
    extensions which are known to not have <<tag-required-feature,
    individual tag:feature tags>> (internal issue 3946).
  * 2024-10-14 - Add ability to include features in 'depends' attributes for
    require blocks, as well as enabling simple lists of or/and dependencies
  * 2024-09-25 - Add ability to have multiple features as an or'd list of
    requirements
  ...
  * 2024-08-22 - Add <<tag-required-remove, tag:remove>> attribute
    ...
    requirements for individual Vulkan features to the schema (internal
    issue 3478).
  ...
  * 2024-05-28 - Deprecate the tag:feature attr:number attribute with the
    new attr:depends tag (public issue 2327).
  ...
  * 2024-03-20 - Add a NOTE to the tag:command attr:errorcodes attribute
    that certain errors should never be
    included in the attribute (internal issue 3824).
