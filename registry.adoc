...
[NOTE]
====
If the attr:returnedonly attribute is true, the `pNext` member of that
structure must not be `const`.
This is now enforced by CI.
====
--
  ...
  * attr:requiredlimittype - only applicable if `"category"` is `struct` or
    `union`.
...
==== Attributes of tag:member Tags
  ...
  ** an equation in math markup for incorporation in the specification (a
     ...
     the structure;
  ** the name of another member of the structure specifying the length
...
--
  ...
  * attr:limittype - only applicable for members of
    ...
    This type describes how a value should be compared with the value of a
  ...
  * attr:objecttype - only applicable for members which are `uint64_t`
    ...
    handle refers to.
  * attr:featurelink - only applicable for members representing a Boolean API
    feature. Specifies that the feature has a link in the specification that
    does not match the name of the feature. Typically for features in
    extensions that were later promoted but with changes.
