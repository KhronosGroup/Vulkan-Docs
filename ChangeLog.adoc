...
Change log for September 25, 2026 Vulkan 1.4.364 spec update:

Github Issues

  * Fix various typos, grammar, and markup issues (public PRs 2818, 2819,
    2820, 2821).

Internal Issues

  * Merge a few common draw dispatch and data graph VUs (internal issue
    4813).
  * Improve beta and platform header generation scripts (internal issue
    4950).
  * Fix `optional` XML attribute for
    ...
    VkShaderCreateInfoEXT::pSetLayouts, and
    VkShaderCreateInfoEXT::pPushConstantRanges to ban non-zero array length
    counts with NULL pointers (internal issue 4987, internal MR 8573).
  * Clarify description of
    VK_ADDRESS_COMMAND_UNKNOWN_STORAGE_BUFFER_USAGE_BIT_KHR in
    VK_KHR_device_address_commalds proposal document (internal issue 4972).
  * Add dynamic state tags to XML and autogenerate a dynamic state table
    (internal MR 8147).
  * Add missing `structextends` XML attribute pointing to
    VkPipelineCreateInfoKHR, and remove `optional` attribute from its pNext
    member (internal MR 8561).
  * Match spec language for VertexOrderCw / VertexOrderCcw to corresponding
    VUs and CTS code (internal MR 8569).
  * Fix `structextends` attribute of VkMemoryBarrierAccessFlags3KHR in XML
    (internal MR 8572).
  * Improve VkPhysicalDeviceMemoryBudgetPropertiesEXT description of
    heapBudget and heapUsage members (internal MR 8575).
  * Add VUs constraining firstAttachment + attachmentCount for
    vkCmdSetColor* commands (internal MR 8576).


'''

Change log for September 18, 2026 Vulkan 1.4.363 spec update:

Github Issues

  * Clarify point size support in the <<primsrast-points, Points>> section
    of the primitive rasterization chapter (public issue 2800).
  * Improve the description of alignment requirements for
    vkGetBufferDeviceAddress (public issue 2802).
  * Clarify opacity microap declaration requirements by traversal kind in
    the <<ray-opacity-micromap, Ray Opacity Micromap>> section and the
    corresponding proposal document and extension appendices (public issue
    2810).
  * Fix <<shaders-scope-subgroup, Subgroup scope>> GLSL example (public
    issue 2813).
  * Fix Runtime SPIR-V VU 10167 for
    cooperativeMatrixFlexibleDimensionsMaxDimension when
    cooperativeMatrixFlexibleDimensions is enabled (public issue 2815).

Internal Issues

  * Add a NOTE to VkMemoryPropertyFlagBits explaining some best practices
    around memory types (internal issue 4798).
  * Clarify restrictions on barriers within a render pass instance in a new
    subsection of <<common-render-pass-data-races, Common Render Pass Data
    Races (Informative)>> (internal issue 4845).
  * Add a NOTE to <<queries-occlusion, Occlusion Queries>> about
    implementations counting per color outputs (internal issue 4913).
  * Remove VK_PIPELINE_STAGE_2_COPY_INDIRECT_BIT_KHR from
    VK_PIPELINE_STAGE_2_ALL_GRAPHICS_BIT equivalences in XML sync stage
    markup (internal issue 4957).
  * Move clip/cull SPIR-V validation from VkPipelineShaderStageCreateInfo
    and VkShaderCreateInfoEXT to the Runtime SPIR-V VUs, and handle the case
    when the limit is zero (internal issue 4965).
  * Update generated SPIR-V in the VK_KHR_shader_abort proposal document
    example (internal issue 4971).
  * Combine shader object and pipeline common draw dispatch VUs 08119 and
    08605 for sparse descriptor buffers (internal MR 8412).
  * Add multi-instance VUs for dma buf external memory in
    VkMemoryAllocateInfo and VkMemoryGetFdInfoKHR (internal MR 8480).
  * Add maxCombinedImageSamplerDescriptorCount for Android hardware buffers
    with external formats in vkGetDescriptorEXT,
    vkWriteResourceDescriptorsEXT and VkDescriptorPoolSize (internal MR
    8499).
  * Add ShaderObject to vkCmdSetRasterizationStreamEXT VU 07481 (internal MR
    8528).
  * Add ShaderObject to common draw VUs 08715 and 08716 (internal MR 8529).
  * Add ShaderObject to common draw VUs 07473 and 07483 (internal MR 8530).
  * Modify validity generator to normalize validation of bitmasks with other
    types instead of treating 0 as a valid bit mask (internal MR 8536).
  * Add Vulkan object support for the `objecttype` XML attribute (internal
    MR 8546).
  * Clarifications to vkCmdBlitImage (internal MR 8551).
  * Add missing `len` attribute to XML for VkPipelineBinaryKeyKHR::key
    (internal MR 8556).
  * Require that opacity micromap deserialization data exactly match
    serialized data in the <<acceleration-structure-def, Acceleration
    Structures>> chapter
    (internal MR 8563).
...
Change log for September 4, 2026 Vulkan 1.4.362 spec update:

Github Issues

  * Add an interaction between KHR_maintenance11 and
    KHR_device_address_commands (public PR 2804).

Internal Issues

  * Clarify zero extent regions in vkCmdBlitImage2 (internal issue 4929).
  * Refactor cooperative matrix type / muladd matching rules in new sections
    of the <<shaders, shaders>> chapter and use those to make corresponding
    runtime SPIR-V VUs more clear (internal issue 4932).
  * Improve descriptions of and references to
    ...
    (internal MR 8498).
  * Account for cases where VK_ANDROID_external_format_resolve is used with
    nullColorAttachmentWithExternalFormatResolve == VK_FALSE in
    VkRenderingAttachmentInfo VUs (internal MR 8503).
  * Align attachmentCount VUs VkGraphicsPipelineCreateInfo VU 06055 with
    dynamic rendering (internal MR 8510).
  * Add XML `optional` attribute for VkPrivateDataSlotCreateInfo::flags to
    correct the corresponding VU (internal MR 8513).
  * Remove vkCreateImageView VU 12444 (internal MR 8514).
  * Revert host image copy Android hardware buffers VUs 12500 and 09048
    (internal MR 8519).
  * Remove redundant VkImageViewCreateInfo VUs 12504 and 12505 (internal MR
    8520).
  * Add error precedence rules to the <<fundamentals-errorcodes, Error
    Codes>> section in cases where a command could return multiple codes
    (internal MR 8523).
...
Change log for August 28, 2026 Vulkan 1.4.361 spec update:

Github Issues

  * Allow vkBeginCommandBuffer to be called when a command buffer is in the
    recording state (public issue 2782).
  * Improved VK_EXT_descriptor_heap align limits (public PR 2787).
  * Minor improvements to `reg.py` script (public PR 2791).

Internal Issues

  * Add VkDescriptorGetInfoEXT and VkResourceDescriptorInfoEXT VUs for
    vkWriteResourceDescriptorsEXT range limits (internal issue 4814).
  * Include task shaders in a NOTE for
    VkSamplerCustomBorderColorCreateInfoEXT (internal issue 4940).
  * Validate VK_SHADER_CREATE_NO_TASK_SHADER_BIT_EXT in common draw mesh VUs
    regardless of the taskShader feature (internal issue 4944).
  * Miscellaneous fixes to VUs for Android hardware buffers, descriptors,
    and image views (internal MR 8443).
  * Relax render pass and pipeline input attachment mapping constraints in
    some common draw VUs (internal MR 8446).
  * Add missing heap ImageView usage VUs for vkWriteResourceDescriptorsEXT
    and VkResourceDescriptorInfoEXT (internal MR 8461).
  * Add standalone SPIR-V VU to ban Offset for Input structure members
    (internal MR 8466).
  * Fix VK_EXT_image_compression_control dependency in XML (internal MR
    8479).
  * Add vkGetPhysicalDeviceQueueFamilyProperties2 to some VUs where it is
    missing (internal MR 8481).
  * Remove VK_IMAGE_TILING_DRM_FORMAT_MODIFIER from
    VkDeviceImageMemoryRequirements VU 12399 (internal MR 8482).
  * Move Mesh VUs from common draw to common draw mesh file (internal MR
    8485).
  * Fix XML alias between VK_NV_cooperative_matrix2 and
    VK_EXT_cooperative_matrix_maintenance1 SPIR-V capabilities (internal MR
    8491).
  * Update VK_NV_low_latency2 to revision 3, including vkQueuePresentKHR and
    vkQueueSubmit frame attribution (internal MR 8495).
  * Remove unnecessary condition from common draw dispatch VU 09212
    (internal MR 8496).
  * Remove redundant depth/stencil common draw VUs 06172 - 06177 inclusive
    (internal MR 8500).
...
Change log for August 14, 2026 Vulkan 1.4.360 spec update:

Github Issues

  * Add VU disallowing the use of vkGetDeviceImageMemoryRequirements with
    parameters that represent images created with potentially >1 format
    modifier and disjoint memory planes (public issue 2309).
  * Clarify NOTE about alignment requirements for device addresses in push
    data, and mention mapping to buffer declarations. (public issue 2666).
  * Fix slink markup (public PR 2784).
  * Remove redundant `len="1"` attribute from
    vkGetDeviceSubpassShadingMaxWorkgroupSizeHUAWEI::pMaxWorkgroupSize
    (public PR 2785).

Internal Issues

  * Remove "`consecutive`" from <<synchronization-dependencies-chains,
    execution dependency chains>> and clarify how they work (internal issue
    4618).
  * Clarify common draw VUs 13362 and add new common draw VU and
    VkShaderCreateInfoEXT VU for VK_SHADER_CREATE_INDEPENDENT_SETS_BIT_KHR
    (internal issue (4848).
  * Clarify vkCmdResetEvent VUs 03834 and 03835, and vkCmdResetEvent2 VUs
    03831 and 03832 to refer to submission order (internal issue 4851).
  * Clarify when VkRenderingAttachmentInfo::resolveImage must or must not be
    NULL (internal issues 4880, 4881).
  * Fix VK_EXT_tooling_info code sample to set sType for queried structures
    (internal issue 4924).
  * Merge common draw VUs 04141 and 07492 for VK_NV_viewport_swizzle
    (internal issue 4938).
  * Add `optional="true"` attribute to XML for
    vkGetShaderInstrumentationValuesARM::pMetricValues (internal MR 8456).
  * Add missing `depends` attribute for VK_EXT_image_tiling_control and
    VK_ARM_data_graph_neural_accelerator_statistics (internal MR 8459).
  * Add missing VU for VkImageViewCreateInfo::usage when
    VK_IMAGE_USAGE_FRAGMENT_DENSITY_MAP_BIT_EXT is set (internal MR 8460).
  * Remove no longer needed asciidoc conditionals from VkImageViewCreateInfo
    VUs (internal MR 8463).
  * Reword common copy buffer VUs 00117 and 12482 to match new common copy
    image VUs (internal MR 8464).
  * Fix typo in VK_KHR_video_encode_av1 proposal document (internal MR
    8473).
...
Change log for August 7, 2026 Vulkan 1.4.359 spec update:

Github Issues

  * Fix ordering of <<textures-lod-query, LOD Query>> return value (public
    issue 2777).

Internal Issues

  * Simplify VkRenderingAttachmentInfo resolve-image layout VUs (internal
    issue 4600).
  * Add VkRenderingAttachmentInfo VUs for dynamic rendering with multiple
    mip levels (internal issue 4918).
  * Update VkRenderingAttachmentInfo VU 12256 to not require resolve mode
    without VK_ATTACHMENT_STORE_OP_STORE (internal MR 8424).
  * Fix VkImageCreateInfo VUs for VK_EXT_image_tiling_control that were
    always disallowing VkImageTilingControlCreateInfoEXT for linear image
    tiling (internal MR 8448).
...
Internal Issues
  ...
  * Clarify update-after-bind limits with respect to update-after-bind
    feature support (internal MR 8361).
...
Github Issues
  ...
  * Add VkRenderingAttachmentInfo VU to disallow 3D image views in dynamic
    rendering (public issue 2766).
  * Fix typo in definition of "`Invalid`" command buffer state (public PR
    2768).

Internal Issues

  * Add a NOTE to the <<textures-operation-validation,
    Instruction/Format/Sampler/Image View Validation>> section on validation
    of type and format combinations that result in undefined values
    (internal issues 4128, 4363).
  * Add missing PROFILE_IDC tokens to XML for 4:2:2 and 10bit 4:2:0 formats
    (internal issue 4832).
  * Update VK_EXT_opacity_micromap appendix to clarify differences from
    VK_KHR_opacity_micromap (internal issue 4842).
  * Update VkSamplerCustomBorderColorCreateInfoEXT to describe undefined
    handling of custom border colors on compute-only queues (internal issue
    4888).
  * Fix weight sampler phase calculation for VK_QCOM_image_processing
    (internal MR 8215).
  * Add vulkan_object loading of struct member alias attribute (internal MR
    8374).
  * Fix common draw dispatch VUs 11636, 11637, 11638 for micromaps and
    acceleration structures (internal MR 8376).
  * Minor tweaks for inclusive language (internal MR 8392).
  * Fully expose entity details in print_vulkan_object.py (internal MR
    8393).
  * Add common draw dispatch VU requiring dynamically accessed descriptors
    in descriptor heaps to be valid (internal MR 8398).
  * Extend VK_NV_low_latency to expose legacy NVIDIA Reflex entry points
    (internal MR 8399).
  * Fix incorrect vendor suffix in ExecutionMode names in
    VK_AMD_shader_early_and_late_fragment_tests VUIDs (internal MR 8400).
  * Fix common blit image VU 00230 to affect on color images (internal MR
    8406).
  * Fix RayTracingOpacityMicromapKHR spirvcapability tag in XML (internal MR
    8408).
  * Update structextends attribute of VkValidationFeaturesEXT in XML
    (internal MR 8410).
  * Remove gen_framework_c.py from CTS CI jobs (internal MR 8414).


'''

Change log for July 3, 2026 Vulkan 1.4.356 spec update:

Github Issues

  * Clarify HitAttributeKHR preservation for closest hits in the
    <<ray-traversal, Ray Traversal>> chapter (public issue 2681).

Internal Issues

  * Deprecate host-side acceleration structure and micromap build commands
    in XML (internal issue 4094).
  * Add a NOTE to <<interfaces-iointerfaces-locations, Location and
    Component Assignment>> acknowledging that implementations advertising a
    conformanceVersion less than 1.4.6.0 may consume all four Component
    slots in each Location slot (internal issue 4216).
  * Fix member alias attributes in VkPhysicalDeviceVulkan14Features XML
    (internal issue 4889).
  * Adjust language in the <<ray-tracking-repack> section to correct how we
    interact with future reconvergence specification work (internal MR
    8313).
  * Add VkGraphicsPipelineCreateInfo VU for VK_FORMAT_E5B9G9R9_UFLOAT_PACK32
    (internal MR 8383).
  * Move vkCreateImage VU 12442 to VkImageCreateInfo (internal MR 8388).
...
Change log for June 26, 2026 Vulkan 1.4.355 spec update:
...
Github Issues
  ...
  * Clarify mapping of image texels to memory in the introduction to the
    <<images, Images>> chapter and the <<memory-model-memory-location,
    Memory Location>> section (public issue 2757).

Internal Issues

  * Add static maxPushDataSize VU to runtime SPIR-V VUs (internal MR 8292).
  * Reword standalone SPIR-V VU 09565 and move it from the runtime SPIR-V VU
    section (internal MR 8299).
  * Update the solution space section of the proposal template (internal MR
    8311).
  * Ban SHADER_RECORD mappings in non-ray tracing stages in common shader
    descriptor heap VUs (internal issue 4849).
  * Clarify ray query confirm and commit flow in the
    <<ray-intersection-confirmation, Ray Intersection Confirmation>>,
    <<ray-closest-hit-determination, Ray Closest Hit Determination>>, and
    <<ray-result-determination, Ray Result Determination>> sections
    (internal MR 8339).
  * Remove redundant vkCmdExecuteGeneratedCommandsEXT VU 11150 (internal MR
    8340).
  * Add common heap embedded sampler VU file to refactor identical VUs for
    several different structures (internal MR 8345).
  * Clarify that both Set and Binding decorations are required in standalone
    SPIR-V VU 06677 (internal issue 4853)
  * Revert common draw dispatch VUs 12450 and 12451, which were already
    covered by VUS 11300 and 11304 (internal MR 8349).
  * Add VkDescriptorSetAndBindingMappingEXT VUs for pushDataOffset and
    pushAddressOffset limits (internal MR 8350).
  * Add an explicit definition of the DenormPreserve execution mode to the
    <<spirvenv-precision-operation, Precision and Operation of SPIR-V
    Instructions>> section (internal MR 8353).
  * Clarify acceleration structure descriptor alignment for the
    bufferDescriptorAlignment limit (internal MR 8357).
...
Change log for June 5, 2026 Vulkan 1.4.353 spec update:

Internal Issues

  * Clarify type construction language around generic acceleration
    structures in VkCopyAccelerationStructureModeKHR
    VkCopyMemoryToAccelerationStructureInfoKHR, and
    VkAccelerationStructureCreateInfoKHR (internal issue 3953).
  * Remove redundant definition of extensions and features required by
    Roadmap appendices from those chapters (internal issue 4779).
  * Clarify that out of bounds image writes are always discarded in the
    <<images-writes, Image Writes>> and
    shaders-execution-memory-access-bounds, Shader Out-of-Bounds Memory
    Access>> sections (internal issue 4777).
  * Add descriptor heaps VUs to disallow opaque descriptor data for embedded
    samplers (internal issue 4788).
  * Fix VK_PIPELINE_LAYOUT_CREATE_INDEPENDENT_SETS_BIT_EXT dependency in XML
    (internal issue 4811).
  * Fix asciidoctor warnings when building less than core + all extension
    specs (internal issues 4820, 4835, 4836).
  * Convert "`Unknown link macro target`" warnings in spec build to errors,
    so they will cause CI failure (internal issues 4820, 4839).
  * Update description of
    vkCmdSetPrimitiveRestartIndexEXT::primitiveRestartIndex (internal issue
    4828).
  * Remove standalone SPIR-V OpBufferPointerEXT explicit layout VU 11346
    (internal issue 4837).
  * Improve profiles definition in `xml/profiles/VP_KHR_roadmap.json`
    (internal issue 4785).
  * Fix makePushDataSize calculation in common shader descriptor heap VU
    11316 (internal MR 8291).
  * Add common copy image VU to ban any depth or stencil copies with MSAA on
    a non-graphics queue (internal MR 8295).
  * Add common draw dispatch VU for indirect buffers using scalar alignment
    (internal MR 8308).
  * Small fixes for VkGpaSampleBeginInfoAMD and vkGetGpaSessionResultsAMD
    specification language and XML (internal MR 8322).
  * Add VkShaderCreateInfoEXT VU banning creation with ray tracing stages
    (internal MR 8327).
...
Change log for May 15, 2026 Vulkan 1.4.352 spec update:

Github Issues

  * Refer to extension feature structure from feature tags in extensions,
    rather than promoted structure (public issue 2722).
  * Fix race condition in antora-prep.py (public PR 2747).
  * Fix broken link in Resource Descriptors section (public PR 2748).
  * Refer to image from VK_KHR_opacity_micromap proposals document correctly
    (public issue 2749).

Internal Issues

  * Clarify description of descriptor heap indexing in the
    VK_EXT_descriptor_heap extension proposal (internal issue 4803).
  * Add VK_DESCRIPTOR_MAPPING_SOURCE_INDIRECT_ADDRESS_EXT mapping from being
    a descriptor array to common shader descriptor heap VU 11318 (internal
    issue 4804).
  * Add VkGraphicsPipelineCreateInfo VU to ban mixing renderPass with
    dynamic rendering (internal issue 4805).
  * Add VkShaderCreateInfoEXT VU disallowing VK_SHADER_CREATE_INDEPENDENT_SETS_BIT_KHR
    with compute shaders
    (internal MR 8266).
  * Improve formatting of titles on paragraph blocks so they are rendered
    like subsection headings (internal MR 8282).
...
Change log for May 8, 2026 Vulkan 1.4.351 spec update:

Internal Issues

  * Add missing required limits table entries for
    VK_ARM_tensors (internal issue 4791).
  * Modify vkCmdExecuteGeneratedCommandsEXT VU 11062
    to only ban DGC + viewmask when using indirectExecutionSet
    (internal MR 8241).
  * Clarify that VK_FORMAT_*_SINT types are <<spirvenv-tensor-formats,
    compatible with signless integer types>> in SPIR-V, for VK_ARM_tensors
    (internal MR 8074).
...
Change log for May 1, 2026 Vulkan 1.4.350 spec update:

Github Issues

  * Revert VkGraphicsPipelineCreateInfo VUs 06853 / 06854 to be
    unconditional again (public issue 2697).
  * Update Actions in Github CI script for Node.js 24, and update REUSE
    exception file from .reuse/dep5 -> REUSE.toml (public PR 2713).
  * Change links in proposals to fix errors and be easier to convert for
    inclusion in the docs site (public PR 4795).
  * Revise VK_EXT_ray_tracing_invocation_reorder to support
    hitObjectRecordFromQueryEXT overload (public SPIRV-Registry issue 373).

Internal Issues

  * Revert <<interfaces-iointerfaces-locations, matrix component
    consumption>> to match OpenGL (internal issue 4325).
  * Clarify which commands can be used with VkRenderingFlagBitsKHR bit
    VK_RENDERING_CONTENTS_SECONDARY_COMMAND_BUFFERS_BIT, and align with the
    wording for VK_SUBPASS_CONTENTS_SECONDARY_COMMAND_BUFFERS. (internal
    issue 4437).
  * Rewrite common access mask VUs 03927, 06256, and 06257, which were
    overzealous (internal issue 4680).
  * Clarify that shader side effects are not ordered across stages of a
    pipeline in the <<shaders-execution, Shader Execution>> section
    (internal issue 4718).
  * Revert `len` XML attribute for VkDeviceCreateInfo::ppEnabledLayerNames,
    which was breaking Vulkan-Hpp, but suppress implicit VU generation for
    `deprecated="unused"` in generator scripts (internal issue 4757).
  * Tag vkSetHdrMetadata::pSwapchains as `externsync="true"` in XML
    (internal issue 4774).
  * Update description of VkDeviceFaultInfoEXT to refer to the (separately
    specified) lengths of its array members (internal issue 4776).
  * Require VK_BUFFER_USAGE_2_INDIRECT_BUFFER_BIT_KHR be set in the usage
    flags for the buffer underlying
    VkGeneratedCommandsInfoEXT::indirectAddress (internal issue 4780).
  * Clarify reserved range restrictions for vkCmdBindSamplerHeapEXT, and fix
    calculation of reserved range size (internal issue 4782).
  * Fix structure::member markup in a few places where pname: macros were
    missing, and add a CI check to prevent recurrence (internal MR 8208).
  * Clarify common draw dispatch VUs 08600 / 08601, and remove 08602
    (internal MR 8210).
  * Fix minor issues with Vulkan object scripts (internal MRs 8212, 8213).
  * Remove VK_DESCIPTOR_TYPE_TENSOR_ARM from VkResourceDescriptorInfoEXT VU
    11213, as this is already covered in another VU (internal MR 8223).
  * Fix `depends` attributes for VK_KHR_shader_constant_data,
    VK_KHR_shader_abort, and VK_KHR_device_fault (internal MR 8225).
  * Remove VkSurfaceCapabilities2EXT VUID 01246, which should not have been
    a VU since this is a query command (internal MR 8131).
  * Clarify `deprecated="unused"` in the registry schema and schema
    description document (internal MR 8231).
  * Add missing VkDescriptorSetAndBindingMappingEXT CombinedImageSampler
    heap VU (internal MR 8242).
  * Update use of pyparsing module to replace deprecated method names with
    current PEP8-compliant ones (internal MR 8243).
  * Default to compact output of print_vulkan_object.py (internal MR 8244).
...
Github Issues
  ...
  * Update TaskPayloadWorkgroupEXT access to be dynamic (public PR 2677).
  * Fix example in registry schema document (public PR 2688).
  * Mention both VkAccessFlagBits2 and VkAccessFlagBits enums in the
    <<synchronization-access-types-supported, Supported Access Types>>
    table, and link to that table from the VkAccessFlagBits2 refpage (public
    Vulkan-Site issue 161).

Internal Issues

  * Tag original synchronization commands as deprecated in XML and add a
    <<deprecation-sync2>> section to the Legacy appendix (internal issue
    3903).
  * Describe descriptors independently of descriptor sets / buffers / heaps
    (internal issue 4011).
  * Honor `depends` attributes in XML `<require>` tags when generating
    headers (internal issue 4074).
  * Add cooperativeMatrixRobustBufferAccess and nullDescriptor VUs (internal
    issue 4557).
  * Add INDEPENDENT_SETS to VkGraphicsPipelinCreateInfo VU 06679 (internal
    MR 8087).
  * Tag 32-bit Vk*Flags types which have corresponding 64-bit types as
    `supersededby` in XML and add a <<legacy-flagbits>> section to the
    Legacy appendix (internal issue 4614).
  * Require that oversized buffers are not created for heaps in
    vkCreateBuffer (internal issue 4688).
  * Fix indexes in examples in the VK_EXT_descriptor_heap proposal document
    (internal issue 4701).
  * Add `supersededby` tags to some existing deprecation tags in XML and
    incorporate them in legacy warnings (internal issue 4717).
  * Add `definingRequirements` field to Vulkan Object APIs (internal MR
    7872).
  * Fix Roadmap 2026 VUID errors in `VP_KHR_roadmap.json` profile (internal
    MR 8029).
  * Add legacy warnings for extensions superseding extensions (internal MR
    8060).
  * Add missing `len` parameters to XML for
    VkRenderPassPerformanceCountersByRegionBeginInfoARM (internal MR 8079).
  * Add VK_PIPELINE_LAYOUT_CREATE_INDEPENDENT_SETS_BIT_EXT to
    VkGraphicsPipelineCreateInfo VU 06681 (internal MR 8081).
  * Tag commands deprecated by <<legacy-renderpass2, renderpass2>> as
    `supersededby` in XML (internal MR 8086).
  * Remove `provisional` attribute for VK_NV_present_metering (internal MR
    8098).
...
Github Issues
  ...
  * State that a vkAcquireNextImage*KHR fence happens-after semaphore
    (public issue 2278).
  * Use more consistent <<commandbuffers-lifecycle, command buffer state
    definitions>> (public issue 2647).
  * Remove advice about SPIR-V decorations in the VK_EXT_descriptor_heap
    proposal document, since it cannot apply definitions>> (public issue
    2666).
  * Add anchors for tables that did not have them, and corresponding markup
    guidance in the style guide (public issue 2670).

Internal Issues

  * Fix language and markup issues for vkWaitForPresentKHR and
    VkPresentId2KHR (internal issue 4070).
  * Restore description of FRAGMENT_REGION_BIT in VK_EXT_custom_resolve
    proposal (internal issue 4569).
  * Add interaction between tensors and
    {fragment,vertex}PipelineStoreAndAtomics features (internal issue 4613).
  * Fix some pipeline library flag VUs by moving them from
    VkPipelineLibraryCreateInfoKHR to VkGraphicsPipelineCreateInfo and
    VkRayTracingPipelineCreateInfoKHR (internal issue 4659).
  * Add new common render pass QFOT/Layout transition restriction VUs to
    wait events commands (internal issue 4672).
  * Fix vkGetSwapchainTimeDomainPropertiesEXT to require pTimeDomains and
    pTimeDomainIds to both be `NULL`, or both be non-`NULL` (internal merge
    request 7966).
  * Add VK_ARM_tensors interactions with several other extensions (internal
    MR 7988).
  * Add OpUntypedArrayLengthKHR to common shader descriptor heap VU 11378
    (internal MR 8002).
  * Update <<tessellation, shader object tessellation>> Spacing*
    descriptions to match VUs (internal MR 8009).
  * Add Roadmap 2022 limit for maxPerStageDescriptorSamples to the
    <<limits-types, Required Limit Types>> table (internal MR 8012).
  * Add CI test to prevent UNRESOLVED and PROPOSED issues in published
    extension appendices (internal MR 8026).
  * Mark VkCopyMemoryToImageIndirectInfoKHR::flags,
    VkCopyMemoryIndirectInfoKHR::srcCopyFlags, and
    VkCopyMemoryIndirectInfoKHR::dstCopyFlags as `optional` in XML (internal
    MR 8028).
  * Fixes to VkDataGraphPipelineCreateInfoARM VUs and XML attributes
    (internal MR 8028).
  * Remove redundant VkBuildPartitionedAccelerationStructureInfoNV VU 10561
    (internal MR 8032).
  * Remove implicit VU for
    VkBuildPartitionedAccelerationStructureInfoNV::scratchData (internal MR
    8033).
  * Add missing `<spirvextension>` XML tag for
    SPV_QCOM_cooperative_matrix_conversion (internal MR 8038).
  * Minor consistency and markup fixes to
    VkSubsampledImageFormatPropertiesEXT, <<descriptorheaps-writing, Writing
    Descriptors>>, vkWriteSamplerDescriptorsEXT, and
    vkWriteResourceDescriptorsEXT (internal MR 8044).
...
Internal Issues
  ...
  * Add Roadmap 2026 (internal MR 7937).

'''

Change log for January 23, 2026 Vulkan 1.4.340 spec update:
...
Internal Issues
  ...
  * Fix <<fragmentdensi1ymap-fetch-density-value, fragment density map texel
    size calculation>> equation (internal MR 7355).
  * Remove inaccurate and unnecessary open/closed GOP definitions from video
    encode descriptions (internal MR 7954).
  * Remove redundant common draw VU 07748 (internal MR 7968).
  * Update VkBufferCreateInfo VU 09641 to refer to the
    <<resources-effective-buffer-usage, effective buffer usage>>, rather
    than just `usage` (internal MR 7974).
  * Tighten wording around common draw dispatch VU 08114 and in the
    <<descriptor-validity, descriptor validity>> section (internal MR 7984).
  * Remove overlap and simplify VK_ARM_data_graph (internal MR 7989).
  * Fix XML dependencies for VK_QCOM_tile_shading (internal MR 7990).

New Extensions

  * VK_KHR_internally_synchronized_queues
  * VK_EXT_descriptor_heap
  ** Update VK_NVX_image_view_handle for VK_EXT_descriptor_heap interaction
  * VK_EXT_shader_subgroup_partitioned
  * VK_NV_push_constant_bank_support

'''

Change log for January 16, 2026 Vulkan 1.4.339 spec update:

Github Issues

  * Clarify VkMemoryAllocateFlagBits VK_MEMORY_ALLOCATE_DEVICE_ADDRESS_BIT
    definition by removing wording around vkGetBufferOpaqueCaptureAddress
    where not needed (public issue 2636).
  * Rewrite common draw VUs for feedback loop layouts
    and corresponding language in the specification body
    discussing <<renderpass-feedbackloop, feedback loops>>
    (public issue 2644).
  * Make message from building video codec headers diagnostic, instead of
    warning, and remove redundant `codec_headers` target in CI scripts
    (public issue 2655).
  * Update Vulkan-Hpp generator stage in github CI to enable C++23 test and
    run in the same container as other CI stages (public issue 2654).

Internal Issues

  * Update <type category="funcpointer"> schema to simplify format and
    include more information, representing function pointers in the same way
    as <command> tags. Update RNC schema and schema document to separately
    validate different categories of <type> tags (internal issue 5).
+
[NOTE]
====
This is a backwards-incompatible, breaking schema change. We are usually
very reluctant to do this, but there was sufficiently internal demand and
the scope of the change is small. We have updated all generators using
funcpointers in the Vulkan-Docs repository, as well as the Vulkan-Hpp
generator, to accommodate the change. Other downstream XML consumers may
need to modify how they interpret these tags.
====
  * Fix builtin VUs for MeshShader (internal issue 4142).
  * Remove old vkDestroyPipelineLayout VU 02004 (internal issue 4625).
  * Move VK_EXT_pipeline_creation_cache_control and
    VK_EXT_pipeline_protected_access VUs for different pipeline types to
    common validity (internal MR 7945).
  * Fix XML `depends` logic for VK_KHR_present_id (internal MR 7956).
  * Update generated markup for the extension appendices so that each
    extension starts on a new PDF page (internal MR 7957).
  * Remove redundant VkMemoryMapPlacedInfoEXT VU 09576 (internal MR 7965).
  * Skip spirvcapability `xml_consistency.py` check for non-vulkan apis
    (internal MR 7975).
  * Remove Vulkan SC JSON tooling, now found in the VulkanSC-pcutil
    repository (internal MR 7976).

'''

Change log for January 10, 2026 Vulkan 1.4.338 spec update:

Github Issues

  * Factor out function-specific scoping of semaphore signal / wait (public
    issue 1175).
  * Define <<resources-external-image-layout-rules, Image Layout Rules with
    External Memory>> to use with D3D 10+11 interop and add corresponding
    VkImageMemoryBarrier VUs (public issue 1436)
  * Fix various markup macro and whitespace typos (public PRs 2637, 2638,
    2639, 2642).
  * Do not run CI when pushing tags (public PR 2635).

Internal Issues

  * Make definition of max work group count limits consistent for mesh and
    task shaders (internal issue 4479)
  * Simplify definition of <<synchronization-framebuffer-regions,
    Framebuffer Region Dependencies>> (internal issue 4535).
  * Fix some VkPresentTimingInfoEXT VUs (internal issues 4571, 4573).
  * Remove redundant VkDescriptorGetInfoEXT VUs (internal MR 7669).
  * Fix XML validity attributes and VUID 09956 for VK_ARM_data_graph
    (internal MR 7833).
  * Add missing maxMeshMultiviewViewCount VUs for
    VkGraphicsPipelineCreateInfo (internal MR 7911).
  * Add indirect addressing clarifications to cluster acceleration structure
    VUs (internal MR 7924.
  * Fix typo OpImageWeightedSampleQCOM -> OpImageSampleWeightedQCOM
    (internal MR 7929).
  * Combine some related VUs for vkUpdateDescriptorSets and
    VkGraphicsPipelineCreateInfo (internal MR 7931).
  * Add missing vkUpdateDescriptorSets VU for VK_ARM_tensor (internal MR
    7933).
  * Simplify VkBufferCreateInfo VU 09641 (internal MR 7934).
  * Combine some VkSubpassDescription VUs (internal MR 7938).
  * Add ratified spec build to CI (internal MR 7939).
  * Add missing VK_QCOM_tile_memory_heap interactions to vkBindImageMemory
    and VkBindImageMemoryInfo VUs (internal MR 7940).
  * Fix vkCmdBindTileMemoryQCOM `NULL` typo (internal MR 7941).
  * Add missing PerPrimitiveEXT index bounds VU for
    <<spirvenv-module-validation-runtime, Runtime SPIR-V Validation>>
    (internal MR 7943).
  * Add `scripts/nextfreebits.py` to summarize remaining bits which may be
    reserved in bitmask types (internal MR 7948).
  * Fixes and additions for VK_QCOM_tile_memory_heap in common subpass
    description, VkFramebufferCreateInfo, and VkRenderPassBeginInfo VUs
    (internal MR 7950).
  * Update CI to use newer Docker image (internal MR 7969).

'''

Change log for December 19, 2025 Vulkan 1.4.337 spec update:

Internal Issues

  * Clarify the definition of the <<features-ycbcr2plane444Formats,
    ycbcr2plane444Formats>> feature, and add VUs restrict related formats
    when the feature is not enabled (internal issue 4029).
  * Make the <<limits-maxPerStageDescriptorInputAttachments,
    pname:maxPerStageDescriptorInputAttachments>> limit the max index for
    common subpass description and VkRenderingInputAttachmentIndexInfo VUs,
    and add an explanatory NOTE in its definition (internal issue 4506).
  * Reword custom resolve overdraw normative language in
    vkCmdBeginCustomResolveEXT and add "`Overdraw`" as a glossary term
    (internal issue 4510).
  * Merge KHR and remaining vendor extension markup in separate files into
    the chapters they are included from (internal issue 4516).
  * Clarify shader indices in VkRayTracingShaderGroupCreateInfoKHR VUs
    (internal issue 4568).
  * Add missing vkCmdBeginRendering VUs to check the VkImageLayout for both
    VK_EXT_fragment_density_map and VK_KHR_fragment_shading_rate (internal
    issue 4577).
  * Specify that
    VkAccelerationStructureInstanceKHR::accelerationStructureReference
    refers to a BLAS (internal issue 4578).
  * Add VkDescriptorDataEXT buffer alignment VUs (internal issue 4580).
  * Fix text markup in some equations in the
    <<textures-weightimage-filteroperation, Weight Sampling Operation>>
    section (internal issue 4583).
  * Fix ordering of "`if`" and "`must`" clauses in some MSRTSS VUs (internal
    issue 4592).
  * Fix some, and add some new common indirect memory to image command and
    memory to image copy VUs (internal MR 7897).
  * Add missing VK_BASE_VERSION_1_3 to some markup conditionals referring
    only to VK_KHR_synchronization2 in VUs (internal MR 7906).
  * Clarify some attachment layout VUs as validated on the GPU timeline for
    vkCmdBeginRendering (internal MR 7909).
  * Add VU requiring the primitiveFragmentShadingRateMeshShader feature to
    be enabled when using PrimitiveShadingRateKHR in the MeshEXT execution
    model (internal MR 7910).
  * Disable VK_OHOS_native_buffer (internal MR 7917).
  * Reorder <<pipelines-ray-tracing, Ray Tracing Pipelines>> section and add
    subsections (internal MR 7920).
  * Embed required image usage flags into VkImageViewCreateInfo VU 04441
    instead of referring to an external list in a different part of the
    specification (internal MR 7927).
...
Internal Issues
  ...
  * Add <<descriptorsets-inputattachment, allowed ImageLayout for
    InputAttachment>> (internal issue 4567).
...
Internal Issues
  ...
  * Evaluate complex dependencies in XML at syncgenerator runtime, instead
    ...
    to VK_KHR_ray_tracing_pipeline (internal issue 4133).
...
Github Issues
  ...
  * Cleanup VkComputePipelineCreateInfo layout reuse restriction language
    (public issue 2612).
  * De-alias VkPhysicalDeviceRayTracingInvocationReorderFeaturesNV from
    VkPhysicalDeviceRayTracingInvocationReorderFeaturesEXT and restore the
    original enum value for
    VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_RAY_TRACING_INVOCATION_REORDER_FEATURES_NV,
    after realizing the aliasing was done incorrectly and that the
    underlying SPIR-V extensions were not identical (public issue 2620).

Internal Issues

  * Clarify that VkSparseImageFormatProperties::imageGranularity is always
    in terms of texels (internal issue 4430).
  * Merge remaining vendor and EXT extension markup in separate files into
    the chapters they are included from (internal issue 4516).
  * Add VkDescriptorSetLayoutBinding VU to disallow mix and matching of
    immutable YCbCr samplers (internal issue 4521).
  * Clarify YCbCr shader restrictions in runtime SPIR-V VUs and the
    <<textures-operation-validation, Instruction/Sampler/Image View
    Validation>> section (internal issues 4532, 4544).
  * Use "`for each element`" instead of "`for any element`" in all relevant
    VUs (internal issue 4534).
  * Add a NOTE that images created with vkImportMetalTextureInfoEXT require
    no memory binding operations (internal MR 6738).
  * Add a NOTE at the end of the <<features-requirements, Feature
    Requirements>> section that the formatRgba10x6WithoutYCbCrSampler
    feature may not be supported on older implementations (internal MR
    7445).
  * Add a common image copy VU banning depth/stencil copies when the source
    and destination aspects do not match (internal MR 7770).
  * Clarify that VkSamplerCreateInfo::addressModeW is ignored with
    unnormalizedCoordinates (internal MR 7796)
  * Add missing VK_INDIRECT_COMMANDS_TOKEN_TYPE_DRAW_INDEXED_EXT to
    VkIndirectCommandsLayoutCreateInfoEXT VU 11104 (internal MR 7805).
  * Add missing vkCmdExecuteGeneratedCommandsEXT VU constraining compute
    commands inside a render pass instance (internal MR 7807).
  * Remove un-needed conditional markup from standalone SPIR-V VUs (internal
    MR 7836).
  * Fix descriptorCount restriction in common pipeline create info VU 07991
    (internal MR 7842).
  * Use globally consistent phrasing when describing a requirement for USAGE
    flags to be set (internal MR 7842).
...
Github Issues
  ...
  * Generate refpage aliases in Antora site build and remove now-redundant
    'alias' attributes from refpage markup (public PRs 2610, 2611).
  * Remove 'returnedonly' XML attribute from
    VkPhysicalDeviceFragmentDensityMapLayeredFeaturesVALVE (public PR 2607).

Internal Issues

  * Require contiguous memory per plane for multiplanar images, instead of
    for the entire image, in common image copy and image layout transition
    VUs (internal issue 3344).
  * Attempt to clarify <<interfaces-raypipeline, ray payload matching
    requirements>> (internal issue 4373).
  * Clarify fragment density map texel size calculation with dynamic
    rendering in the <<fragmentdensitymap-fetch-density-value, Fetch Density
    Value>> section (internal issue 4480).
  * Improve host synchronization NOTE for VkBufferMemoryBarrier (internal
    issue 4504).
  * Add VkQueryPoolCreateInfo VU to require queue family support for
    status-only query pool creation (internal issue 4511).
  * Make chapter markup filenames consistent with the extension names they
    document (internal issue 4516).
  * Merge AMD extension markup in separate files into the chapters it is
    included from (internal issue 4516).
  * Add missing VUs that VkDeviceOrHostAddressKHR cannot be null in
    ray-tracing extensions (internal MR 7613).
  * Add conditional markup to glossary for compute- and graphics-specific
    terms (internal MR 7752).
  * Add missing common push descriptor template VU (internal MR 7772).
  * Replace use of refpage attribute in SPIR-V VUIDs with `Standalone` or
    `Runtime` as appropriate (internal MR 7773).
  * Add missing VK_KHR_get_physical_device_properties2 dependencies to XML
    (internal MR 7777).
  * Add autogenerated "Equivalent to" for promoted APIs (internal MR 7779)
    and add CI test for old usage (internal MR 7798).
  * Take VkShaderModuleCreateInfo into account for pipeline binary matching
    in VkPipelineBinaryInfoKHR (internal MR 7785).
  * Replace "`indifferent of`" with "`regardless of`" and add CI test
    (internal MR 7799).
...
Change log for October 31, 2025 Vulkan 1.4.331 spec update:

Internal Issues

  * Clarify treatment of zero shader binding table addresses in common ray
    tracing VUs (internal issue 4460).
  * Add vkGetDescriptorSetLayoutSizeEXT VU for descriptor set layout with
    the PUSH_DESCRIPTOR bit set (internal issue 4473).
  * Add introductory paragraph to the <<dense-geometry-format, Dense
    Geometry Format Version 1>> section (internal issue 4519).
  * Add a runtime SPIR-V VU to ensure that any pointer a BDA uses is inside
    a valid VkBuffer (internal MR 7754).
  * Refactor <<queries, Queries>> chapter for parts which are optional in
    Vulkan Base compute / graphics profiles (internal MR 7756).
  * Add missing VkPhysicalDeviceMemoryDecompressionPropertiesEXT VU that
    decompressionMethods is valid (internal MR 7760).
  * Remove `const` from VkPhysicalDevicePresentationPropertiesOHOS::pNext
    (internal MR 7762).
  * Clarify <<textures-blockmatch, Block Matching>> blockSize behavior with
    `VK_QCOM_image_processing` (internal MR 7764).
...
Internal Issues
  ...
  * Update common copy image VUs to account for texel block extent (internal
    issue 4113)
...
Internal Issues
  ...
  * Add runtime SPIR-V VUs for workgroup layout storage rules (internal
    issue 4255).
  * Update VkBufferDeviceAddressInfo and vkGetBufferOpaqueCaptureAddress to
...
Internal Issues
  ...
  * Update VkImageCreateInfo VU 06722 and add new VU to fix mutable image
    ...
    6774).
  * Remove unhelpful (because unobservable) additional notes about float
...
Internal Issues
  ...
  * All VK_IMAGE_CREATE_PROTECTED_BIT to report different memoryTypes in the
    implementation guarantees of properties about memory requirements in
...
Internal Issues
  ...
  * Add a common draw vertex binding VU for primitiveRestartEnable list
    topology, corresponding to VkPipelineInputAssemblyStateCreateInfo VU
...
Internal Issues
  ...
  * Fix names of apiext:VK_NV_cuda_kernel_launch enums for use with
...
Internal Issues
  ...
  * Correct type of flink:vkQueueNotifyOutOfBandNV::pname:pQueueTypeInfo
    (internal MR 6179).
  * Add XML `len` attribute for slink:VkFrameBoundaryEXT::pname:pTag
    (internal MR 6180).
...
Internal Issues:
  ...
  * Fix typo in apiext:VK_QCOM_image_processing for required value of
    code:unnormalizedCoordinates for the input code:sampler (internal merge
    request 5564).
...
Internal Issues:
  ...
  * Add pname:samplerFilterMinmax feature valid usage statement to
    slink:VkSamplerCreateInfo (internal issue 2747).
...
GitHub Issues:
  ...
  * Fix spelling and add backwards-compatibility aliases for some
    elink:VkPipelineCreateFlagBits values (public issue 1676).
  ...
  * Add restriction to the <<formats-mandatory-features-depth-stencil,
    mandatory format support table>> for depth/stencil formats preventing
...
Internal Issues:
  ...
  * Fix a few enumerant and member names in the provisional video extensions
    to comply with the spec style guidelines (internal issue 2710).
...
Internal Issues:
  ...
  * Define the slink:VkPipelineCacheHeaderVersionOne structure in `vk.xml`
...
Internal Issues:
  ...
  * Copy over import/export behavior for sync file descriptor value `-1`
    ...
    (internal issue 2274).
  * State that a derivative group is a quad scope instance in the
...
Internal Issues:
  ...
  * Add valid usage statement to slink:VkImageCreateInfo requiring that
    ...
    request 3988).
  * Clarify that the ptext:maxPerStageDescriptorUpdateAfterBind*
...
Internal Issues:
  ...
  * Add language to
    slink:VkPipelineCoverageModulationStateCreateInfoNV documenting
...
Internal Issues:
  ...
  * Document restrictions on image queries for {YCbCr} formats in the
    ...
    for slink:VkImageFormatProperties and slink:VkImageCreateInfo
    (internal issue 1361).
...
Internal Issues:
  ...
  * Changed all members of slink:VkPhysicalDevicePCIBusInfoPropertiesEXT to
    ...
    issue 1492).
  * Set spec version to 1 for `VK_GOOGLE_hlsl_functionality1` and
...
Public Issues:
  ...
  * Replace references to `VK_NV_dedicated_allocation` with links to the
    corresponding slink:VkMemoryDedicatedRequirements and
    slink:VkMemoryDedicatedAllocateInfo structures in the description
...
GitHub Issues:
  ...
  * Add missing validity rule to flink:vkCmdBeginRenderPass requiring
    compatibility between slink:VkAttachmentDescription pname:initialLayout
...
GitHub Issues:
  ...
  * Minor fixes to ref pages to track descriptions of memory bits that
    ...
    sname:VkPipelineMultisampleStateCreateInfo (public issues 8, 13).
