...
known, it is noted below.
  ...
  *** Node Package Manager (`npm`, version: 5.8.0), for installing Lunr
      and to precalculate math at build time for html
...
parts you don't use) completely before trying to install.
  ...
  * `escape-string-regexp` and `he` Node.js package
...
The Node.js is best obtained through `nvm`.
Follow installation instructions at https://github.com/creationix/nvm#install-script.
Then install Node.js by running `nvm install stable`.
Verify your installation by `node --version`.
Install our Node.js package dependencies by running `npm install`.
