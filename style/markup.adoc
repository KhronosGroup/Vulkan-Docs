...
Use `Apache-2.0` as the license identifier for such files.
...
attributes which refer to different paths in the document tree:
  ...
  * `\{config}` - path to configuration files used in the specification
    toolchain.
...
Chapters, sections, and tables should always have anchors, following the
naming convention <<markup,discussed above>>.
Anchors to other parts of the document may be inserted as needed.
In addition, the autogenerated include files defining commands, structures,
enumerations, and flags all include anchors whose name is the name of the
...
cross-reference.
There are several different toolchains used for various forms of asciidoctor
output, and not all of them treat anchors without alt-text the same way.
...
Tables should always be given an anchor, following the convention in the
<<markup-structure, Document Structure>> section for section anchors, but
appending "`-table`" to the anchor name.

Tables should be preceded with a short title.
If a longer

[source,asciidoc]
.Example Markup
----
[[markup-normative-terminology-macros-table]]
...
This directory is referred to as `pass:[{images}]` so that there is a
consistent path no matter what directory the file including the images is
in.
...
By using the `pass:[{imageopts}]` asciidoc attribute, this behavior can be
controlled.


[[markup-sample-section-captions]]
=== Longer Captions

Asciidoc restricts captions in figures and tables to be a single line in the
source document, rendered as a single paragraph.
If a longer or more complexly formatted caption is required, follow the
figure or table markup with a sidebar block including the full caption,
preceded by a link to the figure or table.
...
.Caption
****
In the <<markup-caption-figure, Figure caption>> diagram, the diagram
represents
...
[NOTE]
====
As of July 2024, we no longer use the `.Note` title on NOTE blocks.
====
...
[NOTE]
====
Editing Notes are now rarely used in the Specification markup.
Instead, file a gitlab or GitHub issue.
====

Editing notes usually only appear in internal (non-published) versions of
documents, via asciidoc conditionals.
If they are internal issues that should not be visible in public, file a
gitlab issue instead.
They usually tag places where an outstanding gitlab or GitHub issue is being
...
It is good practice to include a gitlab or GitHub issue number, or link to
the issue, in the editing note.
====
\endif::editing-notes[]
----
...
| regardless of | indifferent of |
| _handle_ is not dlink{cl}VK_NULL_HANDLE
                | _handle_ is a valid structure |
| member        | field          |
| ename{cl}enumerant specifies
                | ename{cl}enumerant indicates (denotes)
...
| pname{cl}parameter are/is
                | pname{cl}parameter specifies (denotes, indicates)
                | In cases when _are_ or _is_ are not grammatically
                  appropriate, _specifies_ may be used instead.
| pname{cl}parameter_ is
                | the value of pname{cl}parameter_ is
                | In rare cases, _the value of_ is appropriate. See the
                  existing specification language for examples.
| pname{cl}parameter_ is a _typename_ containing / controlling / defining /
  describing / specifying / etc.
                | pname{cl}parameter_ is a _typename_ that/which contains
                  (controls, defines, describes, specifies, etc.)
                | Commonly used for more nuanced descriptions of parameters
                  or structure members
| If the <<features-name, featureName>> feature is enabled / is not enabled
                | If <<features-name, featureName>> is enabled / is not enabled / is disabled
                | For consistency, and to make clear that the internal link
                  to a feature description does in fact refer to a feature
                  name, which is not obvious otherwise.
...
| _nothing_     | or the equivalent
                | When describing promoted or aliased APIs.
                  Similar phrasing is now generated by scripts.
