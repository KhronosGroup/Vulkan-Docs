...
The `<name>` portion of version, extension, and layer names is a concise
name describing its purpose or functionality.
...
require a new chapter of the specification, localize such changes into a new
asciidoc file.
Include the new chapter file at a relevant point in `vkspec.adoc` so it is
incorporated in the specification build, with appropriate asciidoc
conditional protections so it is only included when needed for the
specification being built.
An example can be found in `chapters/videocoding.adoc`.
...
Such changes should be inlined in existing chapters.
...
--
  ...
  * Extensions usually make significant additions and changes to the Vulkan
    ...
    For example, `VK_EXT_debug_marker` adds a new section of the
    "`Debugging`" chapter in `chapters/debugging.adoc`.
    in that file:
+
[source,asciidoc]
.Example Markup
...
----
  * In every other place where the extension alters the behavior of the core
    ...
    For example, `VK_KHR_surface` adds a new valid usage statement to
    `vkEndCommandBuffer` in `chapters/cmdbuffers.adoc`, which is protected
    as follows:
+
[source,asciidoc]
.Example Markup
----
... previously defined valid usage statements
\ifdef::VK_EXT_debug_marker[]
...
\endif::VK_KHR_foo[]
\ifdef::VK_KHR_foo+VK_KHR_fum[]
... discussion of interactions between VK_KHR_foo and VK_KHR_fum when both
are defined ...
\endif::VK_KHR_foo+VK_KHR_fum[]
----
  ...
  * For vendor extensions, changes made to existing core Specification
    source files and to `vk.xml` normally fall under the Contributor License
    Agreement, which assigns copyright to Khronos.
    Entirely new files added by vendors can use their own copyright
    attribution.
    Vendors may add their own copyright attribution when they make changes
    to existing markup files, although this is discouraged.
    In all cases, the copyright *license* terms must be the same as the rest
    of the Specification repository.
  * In most cases, there will be no new files added to the Specification,
    only changes.
    When a new chapter and/or appendix is added, there will be new files
    `extensions/*extension_name*.adoc` and/or
...
The script `scripts/nextfreebits.py` can be used to determine which bits are
not yet used or reserved in the XML.
When run with a specific `Vk*FlagBits*` name, it will report only on that
type.
Otherwise it will report on all bitmask types in the XML.

It is also possible, but more difficult to determine the next free bit for a
given bitmask type by manually searching the XML.
Just looking at the generated headers is not sufficient, because bits
reserved in disabled extension are not included in the headers.
