...
New specification language should, whenever possible, closely follow related
examples in the existing specification documents.

Whenever possible follow existing API design patterns.
Some specific patterns to follow (this list is very incomplete) include:

  * Behavior of queries which return arrays of unknown length (see e.g.
    `vkEnumeratePhysicalDevices`).
  * Behavior of creation commands which create multiple objects at once (see
...
the "`and`" or "`or`" separating the last item.
...
The passive voice is often imprecise.

There are too many examples to exhaustively list them.
Some that have come up include:

  * "`Applications should:`" instead of "`It is recommended that`"
  * "`To create a command buffer, call`" instead of "`A command buffer is
    created by calling`" or "`The application may create a command pool by
    calling`"
...
[NOTE]
====
American spelling is enforced in CI and the `check-spelling` Makefile target
by the `codespell` builtin dictionary `en-GB_to_en-US`.
If any exceptions to the dictionary are needed, they can be added in
`config/CI/codespell-allowed`.
If specific files need to be excluded, they can be added to the `skip` entry
in `config/CI/codespellrc`.

There are a few exceptions to `color` in the Vulkan Video extensions, such
as etext:VK_VIDEO_ENCODE_H264_CAPABILITY_SEPARATE_COLOUR_PLANE_BIT_EXT.
This is because those extensions refer to externally defined video
...
This process is ongoing as new material and sometimes, new terms to avoid
are added.

If use of a term on the list is absolutely unavoidable, the CI script check
can be bypassed by inserting a "`word joiner`" character into the markup of
that term.
Rendered HTML output will show this term as a single word and can be
searched as such.

Asciidoctor predefines the attribute `\{wj}` for this purpose:

[source,asciidoc]
.Example Markup
----
The external DRM API supported by the VK_EXT_acquire_drm_display
extension uses the term "`DRM mast{wj}er` permissions".
----
...
[[writing-mustbe]]
=== Avoid Use of "`set to`" Where Possible

When describing a parameter or member that is required to be, or not to be a
specific value, avoid extraneous use of "`set to`".
In particular, when constraining a value passed or returned to the API write
"`pname:value must: be zero`" (or other simple scalar value) instead of
"`pname:value must: be set to zero`".

However, in more complicated sentences with multiple clauses and values
described, "`set to`" sometimes makes language flow more naturally.
...
|====
| Use Spelling  | Instead Of     | Comments
| bit plane     | bit{wj}plane       |
| compile time  | compile-{wj}time   | Per Wikipedia "`compile time`"
| closest hit   | closest-{wj}hit    |
| color space   | color{wj}space     | Not currently checked due to API naming conventions
| cube map      | cube{wj}map        |
| double-buffer | double{wj}buffer   |
| entry point   | entry-{wj}point, +
                  entry{wj}point
                                     | Except if needed to disambiguate from
                                       surrounding terms
| flat shading  | flat{wj}shading    |
| GitHub        | Git{wj}hub         | Site's preferred spelling
| indirect (drawing/dispatching) command
                | indirect command (draw/dispatch)
                                     | Even though related commands are
                                       named ftext:vk*Draw*Indirect
| LOD           | lo{wj}d, +
                  level of detail, +
                  level-{wj}of-detail| Acronym for "`Level of Detail`"
| mip level +
  mip layer +
  mip size  +
  mip tail
                | mip{wj}level +
                  mip{wj}layer +
                  mip{wj}size  +
                  mip{wj}tail        | "`mipmap *term*`" may be used in time +
                                       mip{wj}level is not currently checked due to API naming conventions

| ray tracing   | ray{wj}tracing     |
| render pass   | render{wj}pass     | Not currently checked due to API naming conventions
| reuse         | re-{wj}use         |
| runtime       | run time, +
                  run-{wj}time
                | Arbitrary choice for consistency
| side effect   | side-{wj}effect    |
| use case      | use-{wj}case       |
|====
...
specifying values; bitmasks which affect the operation of a command; or
...
"`You/The application can create a command pool by calling
...
protected by asciidoc conditionals>> for the new extension.
...
expressions) is marked up using LaTeX math notation, which is either passed
...
| `{\rm A}`                | `++\mathrm{A}++`      |
| `\text{for }`            | `\text{++for++}\ `    | Text ending in spaces is unpredictable - favor escaped spaces after text
|====
...
As of July 2024, we are using KaTeX v0.16.10.

See the mtex2MML repository for a
link:https://github.com/gjtorikian/mtex2MML/blob/main/SUPPORTED.md[list of
...
Use a short introductory sentence written in the <<write-active, active
voice>> when describing commands, and follow similar existing examples.
...
section headers, as asciidoc cannot render such nested content correctly.
...
related sections of the specification using asciidoc `xref` macros.
...
core versions with asciidoc conditionals.
...
When a valid usage statement must apply independently to every element of an
array parameter or member, write it starting with "`For each element`"
instead of "`For any element`".
...
and/or core API versions are enabled at runtime, use appropriate asciidoc
conditionals around such clauses.

[NOTE]
====
Prior to specification update 1.3.255, there were stronger restrictions on
placement of asciidoc conditionals that have been relaxed, allowing writing
such valid usage statements in a more natural manner and with less
...
[[sample-writing-common-vu]]
== Writing Common Explicit Valid Usage Statements
...
text than a simple copy paste (e.g. only two commands consume a single valid
usage statement), the original VUs can be left intact.
...
Attributes which can be set on the block are:
  ...
  * *type* - type of the interface, which must match the directory name
    ...
    `freeform`. or `spirv`.
    This attribute is required.
  * *alias* - list of whitespace-separated names of other pages which this
    page also describes.
    Used to generate redirections so that multiple names can refer to the
    same page.
    This is rarely used, and only for non-API pages.
    API aliases are also redirected, but are derived from the XML during
  ...
  * *xrefs* - list of whitespace-separated names of other reference pages
    ...
    dependency information in `vk.xml`.
    In some cases, such as referring between `*FlagBits` and `*Flags` types,
    this additional tagging is useful.
    This attribute is optional.
...
----
+
There are no examples of this usage in the current Vulkan Specification, but
it has been needed in the past and may again in the future.
+
