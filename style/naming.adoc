...
Type names are prefixed with `Vk`, except for external type names used when
integrating with platform APIs and window systems.

Type names must not match the names of the command parameter or structure
member they apply to.

[source,c]
.Example
----
// Disallowed, type name identical to member name
typedef struct VkExample {
...
Structures which extend a base structure through its pname:pNext chain
...
This includes the begin, end, range, and max values added to enumeranted
type definitions by the generator scripts.
...
Mip::
    ...
    If referring to something associated with a mipmap, such as levels,
    sampling mode, size, tail images, etc., use "`mip`" as a standalone
    prefix word, e.g. pname:maxMipLevels, ename:VK_REMAINING_MIP_LEVELS,
    etc.
