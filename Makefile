...
# spec sources.
# The sources in the partials/ directory are used since that has all
# source files, and some of the files not in pages/ have refpage
# content.

ANTORAREFPATH = antora/refpages/modules/refpages
ANTORALOGFILE = antora/refpages/refpage.log
# Same as SPECFILES but pointing to rewritten partials
ANTORASPECFILES = $(wildcard $(ANTORA_SPECMODULE)/partials/chapters/[A-Za-z]*.adoc $(ANTORA_SPECMODULE)/partials/chapters/*/[A-Za-z]*.adoc $(ANTORA_SPECMODULE)/partials/appendices/[A-Za-z]*.adoc)

setup_refpages_antora: setup_spec_antora xrefmaps $(GENREF) $(SCRIPTS)/reflib.py $(PYAPIMAP)
	$(QUIET)$(MKDIR) $(ANTORAREFPATH)
	$(PYTHON) $(GENREF) -basedir $(ANTORAREFPATH) \
	    -log $(ANTORALOGFILE) -extpath $(ANTORA_SPECMODIULE)/partials/appendices \
	    $(EXTOPTIONS) $(ANTORASPECFILES)
...
# Omit antora/features/modules/features/nav.adoc and
# antora/refpages/modules/refpages/nav.adoc which are generated, but
# also checked in.
CLEAN_ANTORA_PATHS = \
	$(ANTORA_FILELIST) \
	...
	antora/features/modules/features/imag,es \
	antora/refpages/modules/refpages/pages \
