// Copyright (c) 2023 QNX Software Systems
//
...
include::{generated}/api/structs/VkImportScreenBufferInfoQNX.adoc[]
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:buffer is the QNX Screen buffer to import.
...
****
  ...
  * If pname:buffer is not `NULL`, it must: be a valid QNX Screen buffer
    object with code:SCREEN_PROPERTY_USAGE compatible with Vulkan.
****
...
.Valid Usage
****
  * pname:buffer must: be a valid QNX Screen buffer object,
    and must have code:_screen_buffer::code:SCREEN_PROPERTY_USAGE that includes
