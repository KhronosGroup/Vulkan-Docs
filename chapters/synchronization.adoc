...
For any pair of execution dependencies, *ExeDep~A~* and *ExeDep~B~*, an
_execution dependency chain_ exists if the intersection of *ScopedOps~2~* in
*ExeDep~A~* and *ScopedOps~1~* in *ExeDep~B~* is not an empty set.
...
results in a <<memory-model-access-data-race,data race>>.
====
...
ifdef::VK_BASE_VERSION_1_3,VK_KHR_synchronization2[]
[open,refpage='VkPipelineStageFlagBits2',desc='Pipeline stage flags for VkPipelineStageFlags2',type='enums']
--
...
ifdef::VK_VERSION_1_3,VK_KHR_copy_commands2[flink:vkCmdResolveImage2 and]
...
ifdef::VK_EXT_opacity_micromap[]
  * ename:VK_PIPELINE_STAGE_2_MICROMAP_BUILD_BIT_EXT specifies the execution
    of <<micromap, micromap commands>> for slink:VkMicromapEXT objects.
...
ifdef::VK_KHR_copy_memory_indirect[]
  * ename:VK_PIPELINE_STAGE_2_COPY_INDIRECT_BIT_KHR specifies the stage of
...
[NOTE]
====
The etext:TOP and etext:BOTTOM pipeline stages are legacy, and applications
should prefer ename:VK_PIPELINE_STAGE_2_ALL_COMMANDS_BIT and
...
[open,refpage='VkPipelineStageFlags2',desc='64-bit mask of pipeline stage flags',type='flags']
--
...
ifdef::VK_EXT_memory_decompression[]
  * ename:VK_PIPELINE_STAGE_2_MEMORY_DECOMPRESSION_BIT_EXT specifies the
    execution of decompression commands with flink:vkCmdDecompressMemoryEXT
...
For the decompression pipeline, the following stages occur in this order:
  ...
  * ename:VK_PIPELINE_STAGE_2_MEMORY_DECOMPRESSION_BIT_EXT
endif::VK_EXT_memory_decompression[]
...
include::{generated}/sync/pipelineOrders/data_graph.adoc[]
...
The _access type_ is a function of the <<descriptors, descriptor type>>
...
ifdef::VK_BASE_VERSION_1_3,VK_KHR_synchronization2[]
[open,refpage='VkAccessFlagBits2',desc='Access flags for VkAccessFlags2',type='enums']
--
Bits which can: be set in the pname:srcAccessMask and pname:dstAccessMask
members of slink:VkMemoryBarrier2, slink:VkImageMemoryBarrier2, and
slink:VkBufferMemoryBarrier2, specifying access behavior, are:
...
ifdef::VK_KHR_copy_memory_indirect[]
    ...
    ename:VK_PIPELINE_STAGE_2_COPY_INDIRECT_BIT_KHR pipeline stage.
endif::VK_KHR_copy_memory_indirect[]
...
ifdef::VK_GRAPHICS_VERSION_1_4,VK_KHR_maintenance5[flink:vkCmdBindIndexBuffer2 and]
  ...
  * ename:VK_ACCESS_2_UNIFORM_READ_BIT specifies read access to a
    <<descriptors-uniformbuffer, uniform buffer>> in any shader pipeline
    stage.
...
ifdef::VK_HUAWEI_subpass_shading[]
  ...
  * ename:VK_ACCESS_2_SHADER_SAMPLED_READ_BIT specifies read access to a
    <<descriptors-uniformtexelbuffer, uniform texel buffer>> or
    <<descriptors-sampledimage, sampled image>> in any shader pipeline
    stage.
  * ename:VK_ACCESS_2_SHADER_STORAGE_READ_BIT specifies read access to a
    <<descriptors-storagebuffer, storage buffer>>,
ifdef::VK_COMPUTE_VERSION_1_2,VK_EXT_buffer_device_address,VK_buffer_device_address[]
    <<descriptors-physical-storage-buffer, physical storage buffer>>,
endif::VK_COMPUTE_VERSION_1_2,VK_EXT_buffer_device_address,VK_buffer_device_address[]
    <<descriptors-storagetexelbuffer, storage texel buffer>>, or
    <<descriptors-storageimage, storage image>> in any shader pipeline
    stage.
...
ifdef::VK_EXT_memory_decompression[]
  * ename:VK_ACCESS_2_MEMORY_DECOMPRESSION_READ_BIT_EXT specifies read
    access to memory in decompression commands
    flink:vkCmdDecompressMemoryEXT and
    flink:vkCmdDecompressMemoryIndirectCountEXT.
    Such access occurs in
    ename:VK_PIPELINE_STAGE_2_MEMORY_DECOMPRESSION_BIT_EXT pipeline stage.
  * ename:VK_ACCESS_2_MEMORY_DECOMPRESSION_WRITE_BIT_EXT specifies write
    access to memory in decompression commands
    flink:vkCmdDecompressMemoryEXT and
    flink:vkCmdDecompressMemoryIndirectCountEXT.
    Such access occurs in
...
ifdef::VK_QCOM_tile_shading[]
  * ename:VK_ACCESS_2_SHADER_TILE_ATTACHMENT_READ_BIT_QCOM specifies read
    access to a <<renderpass-tile-shading-attachment-access, tile
    attachment>>.
    Such access occurs in the ename:VK_PIPELINE_STAGE_2_FRAGMENT_SHADER_BIT
    or ename:VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT pipeline stages.
  * ename:VK_ACCESS_2_SHADER_TILE_ATTACHMENT_WRITE_BIT_QCOM specifies write
    access to a <<renderpass-tile-shading-attachment-access, tile
    attachment>>.
    Such access occurs in the ename:VK_PIPELINE_STAGE_2_FRAGMENT_SHADER_BIT
    or ename:VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT pipeline stages.
...
ifdef::VK_ARM_data_graph[]
  * ename:VK_ACCESS_2_DATA_GRAPH_READ_BIT_ARM specifies read access to
    resources in the ename:VK_PIPELINE_STAGE_2_DATA_GRAPH_BIT_ARM pipeline
    stage.
  * ename:VK_ACCESS_2_DATA_GRAPH_WRITE_BIT_ARM specifies write access to
    resources in the ename:VK_PIPELINE_STAGE_2_DATA_GRAPH_BIT_ARM pipeline
    stage.
endif::VK_ARM_data_graph[]
ifdef::VK_EXT_descriptor_heap[]
  * ename:VK_ACCESS_2_SAMPLER_HEAP_READ_BIT_EXT specifies read access to a
    sampler heap in any shader pipeline stage.
  * ename:VK_ACCESS_2_RESOURCE_HEAP_READ_BIT_EXT specifies read access to a
...
described in more detail for elink:VkAccessFlagBits.
The <<synchronization-access-types-supported, Supported Access Types>> table
...
[open,refpage='VkAccessFlags2',desc='64-bit mask of access flags',type='flags']
--
...
ifdef::VK_KHR_device_address_commands[slink:VkMemoryRangeBarrierKHR,]
slink:VkMemoryBarrier2, slink:VkBufferMemoryBarrier2, or
slink:VkImageMemoryBarrier2 to provide additional access flags beyond those
available in elink:VkAccessFlagBits2.
...
ifdef::VK_HUAWEI_subpass_shading[]
  ...
  * ename:VK_ACCESS_SHADER_READ_BIT specifies read access to a
    <<descriptors-uniformtexelbuffer, uniform texel buffer>>,
...
ifdef::VK_ARM_tensors[]
    <<descriptors-storagetensor,storage tensor>>,
...
ifdef::VK_EXT_shader_tile_image[]
  ...
  * ename:VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT specifies write
    ...
    ename:VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT or
    ename:VK_PIPELINE_STAGE_LATE_FRAGMENT_TESTS_BIT pipeline stages.
  * ename:VK_ACCESS_TRANSFER_READ_BIT specifies read access to an
...
ifdef::VK_ARM_tensors[]
    ...
    or buffer in a <<copies, copy>> operation.
ifdef::VK_BASE_VERSION_1_3,VK_KHR_synchronization2[]
    Such access occurs in the ename:VK_PIPELINE_STAGE_2_ALL_TRANSFER_BIT
    pipeline stage.
endif::VK_BASE_VERSION_1_3,VK_KHR_synchronization2[]
  * ename:VK_ACCESS_TRANSFER_WRITE_BIT specifies write access to an
ifndef::VK_ARM_tensors[]
    image
endif::VK_ARM_tensors[]
ifdef::VK_ARM_tensors[]
...
<<renderpass-per-tile-execution-model,per-tile execution model>> is enabled,
the ename:VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT and
ename:VK_PIPELINE_STAGE_DRAW_INDIRECT_BIT are included as framebuffer-space
pipeline stages.
endif::VK_QCOM_tile_shading[]
...
operations in the first <<synchronization-dependencies-scopes,
synchronization scope>> to all framebuffer operations in the second
synchronization scope.

Framebuffer-local dependencies instead partition the framebuffer operations
into _framebuffer regions_, and dependencies are only formed between
framebuffer regions with overlapping [eq]#(x, y, layer,
...
ifdef::VK_QCOM_render_pass_shader_resolve,VK_EXT_custom_resolve[]
     ...
     ename:VK_SUBPASS_DESCRIPTION_FRAGMENT_REGION_BIT_EXT,
endif::VK_QCOM_render_pass_shader_resolve,VK_EXT_custom_resolve[]
     the regions are further split to only cover a single [eq]#sample#.
ifdef::VK_QCOM_tile_shading[]
  ** If the dependency is between commands that enable
     <<renderpass-tile-shading,tile shading>>, instead of a single
     [eq]#(x,y)# position, each region covers multiple [eq]#(x,y)# positions
     according to the tile size and the specified apron.
endif::VK_QCOM_tile_shading[]
ifdef::VK_GRAPHICS_VERSION_1_1,VK_KHR_multiview[]
  * If the ename:VK_DEPENDENCY_VIEW_LOCAL_BIT flag is specified in a
    synchronization command, each region covers a single [eq]#(view)#.
endif::VK_GRAPHICS_VERSION_1_1,VK_KHR_multiview[]
ifdef::VK_EXT_rasterization_order_attachment_access,VK_ARM_rasterization_order_attachment_access[]
  * Each region in an automatic framebuffer-local dependency inserted
    ...
    covers a single [eq]#(x,y,layer,sample)# position.
...
or [eq]#sample# dimensions, all coordinates for that dimension are included
in each region.
...
ifdef::VK_BASE_VERSION_1_3,VK_KHR_synchronization2[]
    ...
    from lowest index to highest.
  . The fence signal operation defined by the pname:fence parameter of the
    following commands
ifdef::VK_KHR_swapchain[]
ifdef::VK_BASE_VERSION_1_1,VK_KHR_device_group[]
    and the slink:VkAcquireNextImageInfoKHR::pname:fence member of the
    variable referred to by the pname:pAcquireInfo parameter of
    ...
    command:
  ** flink:vkQueueSubmit
ifdef::VK_BASE_VERSION_1_3,VK_KHR_synchronization2[  ** flink:vkQueueSubmit2]
ifndef::VKSC_VERSION_1_0[  ** flink:vkQueueBindSparse]
ifdef::VK_KHR_swapchain[  ** flink:vkAcquireNextImageKHR]

Semaphore signal operations defined by a single slink:VkSubmitInfo
ifdef::VK_BASE_VERSION_1_3,VK_KHR_synchronization2[or slink:VkSubmitInfo2]
...
ifdef::VK_BASE_VERSION_1_1,VK_KHR_external_fence[]
...
endif::VK_BASE_VERSION_1_1,VK_KHR_external_fence[]
...
[[synchronization-fence-object-status-codes-table]]
...
ifdef::VK_BASE_VERSION_1_3,VK_KHR_synchronization2[or flink:vkQueueSubmit2]
...
****
  ...
  * [[VUID-VkImportFenceWin32HandleInfoKHR-handle-01539]]
    ...
    pname:handleType in <<external-fence-handle-types-compatibility,
...
ifndef::VK_KHR_external_fence_win32,VK_KHR_external_fence_fd[]
  * platform-specific fence import structures (the current specification
    build has none enabled)
endif::VK_KHR_external_fence_win32,VK_KHR_external_fence_fd[]
...
dependency
ifndef::VK_BASE_VERSION_1_2,VK_KHR_timeline_semaphore[]
...
ifdef::VK_BASE_VERSION_1_1,VK_KHR_external_semaphore[]
...
endif::VK_BASE_VERSION_1_1,VK_KHR_external_semaphore[]
...
If slink:VkSemaphoreSciSyncCreateInfoNV is included in the pname:pNext chain
of slink:VkSemaphoreCreateInfo,
slink:VkSemaphoreSciSyncCreateInfoNV::pname:semaphorePool may: be accessed
...
ifdef::VK_BASE_VERSION_1_2,VK_KHR_timeline_semaphore[]
[open,refpage='VkSemaphoreTypeCreateInfo',desc='Structure specifying the type of a newly created semaphore',type='structs']
--
...
[open,refpage='VkSemaphoreType',desc='Specifies the type of a semaphore object',type='enums']
--
...
ifdef::VK_NV_external_sci_sync[]
  * [[VUID-VkExportSemaphoreCreateInfo-pNext-05120]]
    ...
    structure, the <<features-sciSyncSemaphore,
    slink:VkPhysicalDeviceExternalSciSyncFeaturesNV::pname:sciSyncSemaphore>>
    and <<features-sciSyncExport,
    slink:VkPhysicalDeviceExternalSciSyncFeaturesNV::pname:sciSyncExport>>
    features must: be enabled
...
the stext:NvSciSyncObj that was imported is released when the device is
destroyed.
...
The first <<synchronization-dependencies-scopes, synchronization scope>> of
...
<<synchronization-semaphores-signaling,semaphore signal operation>> for each
...
[open,refpage='vkGetSemaphoreCounterValue',desc='Query the current state of a timeline semaphore',type='protos']
--
...
[open,refpage='vkWaitSemaphores',desc='Wait for timeline semaphores on the host',type='protos']
--
...
[open,refpage='VkSemaphoreWaitInfo',desc='Structure containing information about the semaphore wait condition',type='structs']
--
...
[open,refpage='VkSemaphoreWaitFlagBits',desc='Bitmask specifying additional parameters of a semaphore wait operation',type='enums']
--
...
[open,refpage='VkSemaphoreWaitFlags',desc='Bitmask of VkSemaphoreWaitFlagBits',type='flags']
--
...
[open,refpage='vkSignalSemaphore',desc='Signal a timeline semaphore on the host',type='protos']
--
...
[open,refpage='VkSemaphoreSignalInfo',desc='Structure containing information about a semaphore signal operation',type='structs']
--
...
****
  ...
  * [[VUID-VkImportSemaphoreWin32HandleInfoKHR-handle-01542]]
    If pname:handle is not `NULL`, it must: obey any requirements listed for
    pname:handleType in <<external-semaphore-handle-types-compatibility,
...
ifndef::VK_KHR_external_semaphore_win32,VK_KHR_external_semaphore_fd,VK_FUCHSIA_external_semaphore[]
  * platform-specific semaphore import structures (the current specification
    build has none enabled)
endif::VK_KHR_external_semaphore_win32,VK_KHR_external_semaphore_fd,VK_FUCHSIA_external_semaphore[]
...
ifdef::VK_COMPUTE_VERSION_1_3,VK_KHR_synchronization2[]
  * ename:VK_EVENT_CREATE_DEVICE_ONLY_BIT specifies that host event commands
    will not be used with this event.
endif::VK_COMPUTE_VERSION_1_3,VK_KHR_synchronization2[]

ifndef::VK_COMPUTE_VERSION_1_3,VK_KHR_synchronization2[]
[NOTE]
====
...
[[synchronization-event-object-status-codes-table]]
...
ifdef::VK_COMPUTE_VERSION_1_3,VK_KHR_synchronization2[]
[open,refpage='vkCmdSetEvent2',desc='Set an event object to signaled state',type='protos']
--
...
If ename:VK_DEPENDENCY_ASYMMETRIC_EVENT_BIT_KHR is not set in
pname:pDependencyInfo->dependencyFlags, future
endif::VK_KHR_maintenance9[]
ifndef::VK_KHR_maintenance9[Future]
flink:vkCmdWaitEvents2 commands rely on all values of each element in
pname:pDependencyInfo matching exactly with those used to signal the
corresponding event.
ifdef::VK_KHR_maintenance9[]
If ename:VK_DEPENDENCY_ASYMMETRIC_EVENT_BIT_KHR is set, fname:vkCmdSetEvent2
must: only include the <<synchronization-pipeline-stages-masks, source stage
mask>> of the first synchronization scope in
pname:pDependencyInfo->pMemoryBarriers[0].pname:srcStageMask.
endif::VK_KHR_maintenance9[]
...
ifdef::VK_KHR_maintenance9[or ename:VK_DEPENDENCY_ASYMMETRIC_EVENT_BIT_KHR]
...
ifdef::VK_KHR_maintenance9[]
  * [[VUID-vkCmdSetEvent2-dependencyFlags-10785]]
    If the pname:dependencyFlags member of pname:pDependencyInfo includes
    ename:VK_DEPENDENCY_ASYMMETRIC_EVENT_BIT_KHR, the
    pname:bufferMemoryBarrierCount and pname:imageMemoryBarrierCount members
    of pname:pDependencyInfo must: be `0`
  * [[VUID-vkCmdSetEvent2-dependencyFlags-10786]]
    If the pname:dependencyFlags member of pname:pDependencyInfo includes
    ename:VK_DEPENDENCY_ASYMMETRIC_EVENT_BIT_KHR, the
    pname:memoryBarrierCount member of pname:pDependencyInfo must: be `1`
  * [[VUID-vkCmdSetEvent2-dependencyFlags-10787]]
    If the pname:dependencyFlags member of pname:pDependencyInfo includes
    ename:VK_DEPENDENCY_ASYMMETRIC_EVENT_BIT_KHR, the pname:srcAccessMask,
    pname:dstStageMask, and pname:dstAccessMask members of
    pname:pDependencyInfo->pMemoryBarriers[0] must: be `0`
endif::VK_KHR_maintenance9[]
****
...
[open,refpage='VkDependencyInfo',desc='Structure specifying dependency information for a synchronization command',type='structs']
--
...
ifdef::VK_ARM_tensors[]
  * [[VUID-VkDependencyInfo-pNext-09754]]
    If a slink:VkTensorDependencyInfoARM structure is included in the
    pname:pNext chain, a slink:VkTensorMemoryBarrierARM structure must: not
...
ifdef::VK_COMPUTE_VERSION_1_3,VK_KHR_synchronization2[]
[open,refpage='vkCmdResetEvent2',desc='Reset an event object to non-signaled state',type='protos']
--
...
include::{chapters}/commonvalidity/stage_mask_2_common.adoc[]
  ...
  * [[VUID-vkCmdResetEvent2-event-03831]]
    ...
    and the execution of any earlier flink:vkCmdWaitEvents in
    <<synchronization-submission-order,submission order>> that includes
    pname:event in its pname:pEvents parameter
  * [[VUID-vkCmdResetEvent2-event-03832]]
    There must: be an execution dependency between fname:vkCmdResetEvent2
    and the execution of any earlier flink:vkCmdWaitEvents2 in
    <<synchronization-submission-order,submission order>> that includes
    pname:event in its pname:pEvents parameter
ifdef::VK_BASE_VERSION_1_1,VK_KHR_device_group[]
...
include::{chapters}/commonvalidity/stage_mask_common.adoc[]
  ...
  * [[VUID-vkCmdResetEvent-event-03834]]
    ...
    the execution of any earlier flink:vkCmdWaitEvents in
    <<synchronization-submission-order,submission order>> that includes
    pname:event in its pname:pEvents parameter
ifdef::VK_COMPUTE_VERSION_1_3,VK_KHR_synchronization2[]
  * [[VUID-vkCmdResetEvent-event-03835]]
    There must: be an execution dependency between fname:vkCmdResetEvent and
    the execution of any earlier flink:vkCmdWaitEvents2 in
    <<synchronization-submission-order,submission order>> that includes
    pname:event in its pname:pEvents parameter
endif::VK_COMPUTE_VERSION_1_3,VK_KHR_synchronization2[]
ifdef::VK_BASE_VERSION_1_1,VK_KHR_device_group[]
...
ifdef::VK_COMPUTE_VERSION_1_3,VK_KHR_synchronization2[]
[open,refpage='vkCmdWaitEvents2',desc='Wait for one or more events',type='protos']
--
:refpage: vkCmdWaitEvents2
...
.Valid Usage
****
include::{chapters}/commonvalidity/fine_sync_renderpass_common.adoc[]
  ...
  * [[VUID-vkCmdWaitEvents2-pEvents-10788]]
    For each element [eq]#i# of pname:pEvents,
ifdef::VK_KHR_maintenance9[]
    if the pname:dependencyFlags member of the [eq]##i##th element of
    pname:pDependencyInfos does not include
    ename:VK_DEPENDENCY_ASYMMETRIC_EVENT_BIT_KHR, and
endif::VK_KHR_maintenance9[]
    if that event is signaled by flink:vkCmdSetEvent2, that command's
    pname:dependencyInfo parameter must: be exactly equal to the [eq]##i##th
    element of pname:pDependencyInfos
ifdef::VK_KHR_maintenance9[]
  * [[VUID-vkCmdWaitEvents2-pEvents-10789]]
    For each element [eq]#i# of pname:pEvents, if the pname:dependencyFlags
    member of the [eq]##i##th element of pname:pDependencyInfos includes
    ename:VK_DEPENDENCY_ASYMMETRIC_EVENT_BIT_KHR, that event must: be
    signaled by flink:vkCmdSetEvent2 with
    ename:VK_DEPENDENCY_ASYMMETRIC_EVENT_BIT_KHR
  * [[VUID-vkCmdWaitEvents2-pEvents-10790]]
    For each element [eq]#i# of pname:pEvents, if the pname:dependencyFlags
    member of the [eq]##i##th element of pname:pDependencyInfos includes
    ename:VK_DEPENDENCY_ASYMMETRIC_EVENT_BIT_KHR, the union of
    pname:srcStageMask members of all elements of pname:pMemoryBarriers,
    pname:pBufferMemoryBarriers, and pname:pImageMemoryBarriers of the
    [eq]##i##th element of pname:pDependencyInfos must: equal
    pname:pDependencyInfos->pMemoryBarriers[0].pname:srcStageMask in the
    flink:vkCmdSetEvent2 command
endif::VK_KHR_maintenance9[]
  * [[VUID-vkCmdWaitEvents2-pEvents-03839]]
    For each element [eq]#i# of pname:pEvents, if that event is signaled by
  ...
  * [[VUID-vkCmdWaitEvents2-pEvents-03840]]
    For each element [eq]#i# of pname:pEvents, if barriers in the
    [eq]##i##th element of pname:pDependencyInfos include only host
    operations, the [eq]##i##th element of pname:pEvents must: be signaled
    before flink:vkCmdWaitEvents2 is executed
  * [[VUID-vkCmdWaitEvents2-pEvents-03841]]
    For each element [eq]#i# of pname:pEvents, if barriers in the
    [eq]##i##th element of pname:pDependencyInfos do not include host
    operations, the [eq]##i##th element of pname:pEvents must: be signaled
    by a corresponding flink:vkCmdSetEvent2 that occurred earlier in
    <<synchronization-submission-order,submission order>>
...
ifdef::VK_EXT_attachment_feedback_loop_layout[]
  ** ename:VK_DEPENDENCY_FEEDBACK_LOOP_BIT_EXT
...
ifdef::VK_KHR_maintenance8[]
  ...
  * [[VUID-vkCmdWaitEvents2-dependencyFlags-03844]]
    ...
    pname:srcStageMask member of any element of the pname:pMemoryBarriers,
    pname:pBufferMemoryBarriers, or pname:pImageMemoryBarriers members of
    pname:pDependencyInfos must: not include
    ename:VK_PIPELINE_STAGE_2_HOST_BIT
...
ifdef::VK_COMPUTE_VERSION_1_3,VK_KHR_synchronization2[flink:vkCmdResetEvent2,]
...
ifdef::VK_BASE_VERSION_1_3,VK_KHR_synchronization2[]
[open,refpage='vkCmdPipelineBarrier2',desc='Insert a memory dependency',type='protos']
--
...
include::{chapters}/commonvalidity/fine_sync_renderpass_common.adoc[]
include::{chapters}/commonvalidity/pipeline_barrier_common.adoc[]
  * [[VUID-vkCmdPipelineBarrier-oldLayout-10758]]
    If called within a render pass instance using a slink:VkRenderPass
    object, the pname:oldLayout member of any image memory barrier included
...
ifdef::VK_GRAPHICS_VERSION_1_3,VK_KHR_dynamic_rendering[]
  * [[VUID-vkCmdPipelineBarrier-oldLayout-10759]]
    If called within a render pass instance started with
    flink:vkCmdBeginRendering, the pname:oldLayout member of any image
  ...
  * [[VUID-vkCmdPipelineBarrier-srcStageMask-09633]]
    ...
    ename:VK_PIPELINE_STAGE_HOST_BIT, for each element of
...
include::{generated}/api/enums/VkDependencyFlagBits.adoc[]
  ...
  * ename:VK_DEPENDENCY_BY_REGION_BIT specifies that dependencies will be
    split into multiple <<synchronization-framebuffer-regions,
    framebuffer-local regions>> according to the [eq]#(x,y,layer,sample)#
    coordinates.
ifdef::VK_GRAPHICS_VERSION_1_1,VK_KHR_multiview[]
  * ename:VK_DEPENDENCY_VIEW_LOCAL_BIT specifies that dependencies will be
    split into multiple <<synchronization-framebuffer-regions,
    framebuffer-local regions>> according to the [eq]#view#.
endif::VK_GRAPHICS_VERSION_1_1,VK_KHR_multiview[]
ifdef::VK_BASE_VERSION_1_1,VK_KHR_device_group[]
...
ifdef::VK_KHR_synchronization2,VK_COMPUTE_VERSION_1_3[]
ifdef::VK_KHR_maintenance9[]
  * ename:VK_DEPENDENCY_ASYMMETRIC_EVENT_BIT_KHR specifies that the access
    scopes of flink:vkCmdSetEvent2 and flink:vkCmdWaitEvents2 do not need to
    match for a given event when it is specified in both commands, and the
    access scope of flink:vkCmdSetEvent2 is empty.
endif::VK_KHR_maintenance9[]
endif::VK_KHR_synchronization2,VK_COMPUTE_VERSION_1_3[]
--
...
ifdef::VK_BASE_VERSION_1_3,VK_KHR_synchronization2[]
[open,refpage='VkMemoryBarrier2',desc='Structure specifying a global memory barrier',type='structs']
--
...
[[synchronization-memory-range-barriers]]
...
[open,refpage='VkMemoryRangeBarriersInfoKHR',desc='Structure specifying memory range barriers',type='structs']
--
:refpage: VkMemoryRangeBarriersInfoKHR

The sname:VkMemoryRangeBarriersInfoKHR structure is defined as:

include::{generated}/api/structs/VkMemoryRangeBarriersInfoKHR.adoc[]

  * pname:sType is a elink:VkStructureType value identifying this structure.
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:memoryRangeBarrierCount is the length of the
    pname:pMemoryRangeBarriers array
  * pname:pMemoryRangeBarriers is a pointer to an array of
    slink:VkMemoryRangeBarrierKHR structures defining memory dependencies
    between accesses to specified memory ranges.

When this structure is included in the pname:pNext chain of
slink:VkDependencyInfo, it defines a set of
<<synchronization-dependencies-memory, memory dependencies>>, as well as
<<synchronization-queue-transfers, queue family ownership transfer
operations>>, for a specified set of memory ranges.

If this structure is not included in the pname:pNext chain of
slink:VkDependencyInfo, it is equivalent to specifying it with a
pname:memoryRangeBarrierCount of 0.

Each member of pname:pMemoryRangeBarriers defines a separate
<<synchronization-dependencies-memory, memory dependency>>.

include::{generated}/validity/structs/VkMemoryRangeBarriersInfoKHR.adoc[]
--

[open,refpage='VkMemoryRangeBarrierKHR',desc='Structure specifying a memory range barrier',type='structs']
--
:refpage: VkMemoryRangeBarrierKHR

The sname:VkMemoryRangeBarrierKHR structure is defined as:

include::{generated}/api/structs/VkMemoryRangeBarrierKHR.adoc[]
  ...
  * pname:dstQueueFamilyIndex is the destination queue family for a
    <<synchronization-queue-transfers, queue family ownership transfer>>.
  * pname:addressRange is a slink:VkDeviceAddressRangeKHR structure
...
This structure defines a <<synchronization-dependencies-memory, memory
dependency>> limited to an address range, and can: define a
<<synchronization-queue-transfers, queue family ownership transfer
...
limited to only memory accesses to memory in pname:addressRange.

If the buffer from which pname:address was queried was created with a
sharing mode of ename:VK_SHARING_MODE_EXCLUSIVE, and
pname:srcQueueFamilyIndex is not equal to pname:dstQueueFamilyIndex, this
memory range barrier defines a <<synchronization-queue-transfers, queue
...
include::{chapters}/commonvalidity/barrier_queue_family_common.adoc[]
  * [[VUID-VkMemoryRangeBarrierKHR-address-13087]]
    If the buffer from which pname:address was queried was created with a
    sharing mode of ename:VK_SHARING_MODE_EXCLUSIVE, and
    pname:srcQueueFamilyIndex and pname:dstQueueFamilyIndex are not equal,
    pname:srcQueueFamilyIndex must: be
...
ifdef::VK_EXT_queue_family_foreign[]
  ...
  * [[VUID-VkMemoryRangeBarrierKHR-address-13088]]
    ...
    sharing mode of ename:VK_SHARING_MODE_EXCLUSIVE, and
    pname:srcQueueFamilyIndex and pname:dstQueueFamilyIndex are not equal,
    pname:dstQueueFamilyIndex must: be
ifdef::VK_VERSION_1_1,VK_KHR_external_memory[]
    ename:VK_QUEUE_FAMILY_EXTERNAL,
ifdef::VK_EXT_queue_family_foreign[]
    ename:VK_QUEUE_FAMILY_FOREIGN_EXT,
...
include::{generated}/validity/structs/VkMemoryRangeBarrierKHR.adoc[]
--
...
ifdef::VK_BASE_VERSION_1_3,VK_KHR_synchronization2[]
[open,refpage='VkBufferMemoryBarrier2',desc='Structure specifying a buffer memory barrier',type='structs']
--
...
ename:VK_MEMORY_PROPERTY_HOST_COHERENT_BIT have to be flushed with
flink:vkFlushMappedMemoryRanges before they can be accessed safely on the
device.
Similarly, device writes to such memory have to be invalidated with
flink:vkInvalidateMappedMemoryRanges before they can be accessed safely on
the host.

Memory allocated with ename:VK_MEMORY_PROPERTY_HOST_COHERENT_BIT does not
...
length of a buffer or allocation following a given pname:offset should be
used.
It can: be specified for slink:VkBufferMemoryBarrier::pname:size,
flink:vkMapMemory::pname:size, and other similar structures.
...
ifdef::VK_BASE_VERSION_1_3,VK_KHR_synchronization2[]
[open,refpage='VkImageMemoryBarrier2',desc='Structure specifying an image memory barrier',type='structs']
--
...
[[synchronization-image-barrier2-ignored-layout]]
If pname:srcQueueFamilyIndex and pname:dstQueueFamilyIndex are equal and
pname:oldLayout and pname:newLayout are also equal, the layout values are
ignored and the image contents are preserved regardless of the values of
pname:oldLayout, pname:newLayout, and the current layout of the image.

ifdef::VK_KHR_maintenance9[]
If pname:image is a 3D image created with
ename:VK_IMAGE_CREATE_2D_ARRAY_COMPATIBLE_BIT and the
<<features-maintenance9, pname:maintenance9>> feature is enabled, the
pname:baseArrayLayer and pname:layerCount members of pname:subresourceRange
specify the subset of slices of the 3D image affected by the memory barrier,
including the layout transition.
Any slices of a 3D image not included in pname:subresourceRange are not
affected by the memory barrier and remain in their existing layout.

[NOTE]
====
Enabling the <<features-maintenance9, pname:maintenance9>> feature modifies
the behavior of image barriers targeting 3D images created with
ename:VK_IMAGE_CREATE_2D_ARRAY_COMPATIBLE_BIT.
Previously, a pname:layerCount equal to 1 would cover the entire 3D image,
but this has a different meaning when the pname:maintenance9 feature is
enabled.
Linking this behavioral change solely to the pname:maintenance9 feature
...
pname:maintenance9 feature is not enabled, and the application uses
pname:layerCount equal to 1 on this kind of 3D image.
pname:layerCount can be set to ename:VK_REMAINING_ARRAY_LAYERS instead,
...
breaking changes should the pname:maintenance9 feature be enabled in the
...
:imagelayoutnotignored: <<synchronization-image-barrier2-ignored-layout, layouts are not ignored>>,
...
ifdef::VK_BASE_VERSION_1_3,VK_KHR_synchronization2[]
[[synchronization-image-barrier-ignored-layout]]
If the <<features-synchronization2, pname:synchronization2>> feature is
enabled, pname:srcQueueFamilyIndex and pname:dstQueueFamilyIndex are equal,
and pname:oldLayout and pname:newLayout are also equal, the layout values
...
ifdef::VK_KHR_synchronization2[]
:imagelayoutnotignored: <<synchronization-image-barrier-ignored-layout, layouts are not ignored>>,
endif::VK_KHR_synchronization2[]
ifndef::VK_KHR_synchronization2[]
:imagelayoutnotignored:
endif::VK_KHR_synchronization2[]
...
ifdef::VK_KHR_external_memory_win32[]
  * [[VUID-VkImageMemoryBarrier-dstQueueFamilyIndex-12331]]
    If pname:dstQueueFamilyIndex is ename:VK_QUEUE_FAMILY_EXTERNAL and
    pname:image was created with
    ename:VK_EXTERNAL_MEMORY_HANDLE_TYPE_D3D11_TEXTURE_BIT or
    ename:VK_EXTERNAL_MEMORY_HANDLE_TYPE_D3D11_TEXTURE_KMT_BIT in
    slink:VkExternalMemoryImageCreateInfo::pname:handleTypes,
    pname:newLayout must: be ename:VK_IMAGE_LAYOUT_GENERAL
  * [[VUID-VkImageMemoryBarrier-srcQueueFamilyIndex-12332]]
    If pname:srcQueueFamilyIndex is ename:VK_QUEUE_FAMILY_EXTERNAL and
    pname:image was created with
    ename:VK_EXTERNAL_MEMORY_HANDLE_TYPE_D3D11_TEXTURE_BIT or
    ename:VK_EXTERNAL_MEMORY_HANDLE_TYPE_D3D11_TEXTURE_KMT_BIT in
    slink:VkExternalMemoryImageCreateInfo::pname:handleTypes,
    pname:oldLayout must: be ename:VK_IMAGE_LAYOUT_GENERAL or
    ename:VK_IMAGE_LAYOUT_UNDEFINED
...
[open,refpage='vkTransitionImageLayout',desc='Perform an image layout transition on the host',type='protos']
--
...
[open,refpage='VkHostImageLayoutTransitionInfo',desc='Structure specifying the parameters of a host-side image layout transition',type='structs']
--
...
****
  * [[VUID-VkHostImageLayoutTransitionInfo-image-09055]]
    ...
    ename:VK_IMAGE_USAGE_HOST_TRANSFER_BIT usage flag set
...
include::{chapters}/commonvalidity/image_layout_transition_common.adoc[]
  ...
  * [[VUID-VkHostImageLayoutTransitionInfo-oldLayout-09230]]
    If pname:oldLayout is not ename:VK_IMAGE_LAYOUT_UNDEFINED,
ifdef::VK_EXT_zero_initialize_device_memory[ename:VK_IMAGE_LAYOUT_ZERO_INITIALIZED_EXT,]
    or ename:VK_IMAGE_LAYOUT_PREINITIALIZED, it must: be one of the layouts
    in slink:VkPhysicalDeviceHostImageCopyProperties::pname:pCopySrcLayouts
...
ifdef::VK_ARM_tensors[]
[[synchronization-tensor-memory-barriers]]
...
[open,refpage='VkTensorMemoryBarrierARM',desc='Structure specifying a tensor memory barrier',type='structs']
--
The sname:VkTensorMemoryBarrierARM structure is defined as:

include::{generated}/api/structs/VkTensorMemoryBarrierARM.adoc[]
  ...
  * pname:dstQueueFamilyIndex is the destination queue family for a
    <<synchronization-queue-transfers, queue family ownership transfer>>.
  * pname:tensor is a handle to the tensor whose backing memory is affected
    by the barrier.
...
Both <<synchronization-dependencies-access-scopes, access scopes>> are
limited to only memory accesses to pname:tensor.

If pname:tensor was created with ename:VK_SHARING_MODE_EXCLUSIVE, and
pname:srcQueueFamilyIndex is not equal to pname:dstQueueFamilyIndex, this
memory barrier defines a <<synchronization-queue-transfers, queue family
transfer operation>>.
When executed on a queue in the family identified by
...
****
  * [[VUID-VkTensorMemoryBarrierARM-tensor-09755]]
    If pname:tensor was created with a sharing mode of
  ...
  * [[VUID-VkTensorMemoryBarrierARM-tensor-09756]]
    ...
    pname:dstQueueFamilyIndex must: both be either
    ename:VK_QUEUE_FAMILY_IGNORED, or a valid queue family (see
...
include::{generated}/validity/structs/VkTensorMemoryBarrierARM.adoc[]
--

[open,refpage='VkTensorDependencyInfoARM',desc='Structure specifying tensor dependency information for a synchronization command',type='structs']
--
The sname:VkTensorDependencyInfoARM structure is defined as:

include::{generated}/api/structs/VkTensorDependencyInfoARM.adoc[]

  * pname:sType is a elink:VkStructureType value identifying this structure.
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:tensorMemoryBarrierCount is the length of the
    pname:pTensorMemoryBarriers array.
  * pname:pTensorMemoryBarriers is a pointer to an array of
    slink:VkTensorMemoryBarrierARM structures defining memory dependencies
    between tensors.

include::{generated}/validity/structs/VkTensorDependencyInfoARM.adoc[]
--
endif::VK_ARM_tensors[]
...
[open,refpage='VK_QUEUE_FAMILY_EXTERNAL',desc='External queue family index sentinel',type='consts']
--
...
ifdef::VK_BASE_VERSION_1_1,VK_KHR_device_group[device group or]
...
to/from ename:VK_QUEUE_FAMILY_EXTERNAL.
====
endif::VK_EXT_queue_family_foreign[]
...
ename:VK_IMAGE_LAYOUT_UNDEFINED is also required.
...
[NOTE]
====
Even though flink:vkQueuePresentKHR does not have a fence parameter, it does
accept a fence through slink:VkSwapchainPresentFenceInfoEXT.
====
endif::VK_EXT_swapchain_maintenance1[]
ifndef::VK_EXT_swapchain_maintenance1[]
Even though flink:vkQueuePresentKHR does not accept a fence, for the purpose
of fname:vkQueueWaitIdle it behaves as if it signals a fence payload that is
managed by the implementation and has no associated slink:VkFence handle.
See the flink:vkQueuePresentKHR documentation for details.
endif::VK_EXT_swapchain_maintenance1[]
endif::VK_KHR_swapchain[]
...
[open,refpage='vkGetCalibratedTimestampsKHR',desc='Query calibrated timestamps',type='protos']
--
...
[open,refpage='VkCalibratedTimestampInfoKHR',desc='Structure specifying the input parameters of a calibrated timestamp query',type='structs']
--
...
[open,refpage='VkTimeDomainKHR',desc='Supported time domains',type='enums']
--
...
ifdef::VK_EXT_present_timing[]
  * ename:VK_TIME_DOMAIN_PRESENT_STAGE_LOCAL_EXT specifies a time domain
...
include::{generated}/api/structs/VkSwapchainCalibratedTimestampInfoEXT.adoc[]
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:swapchain is the swapchain to retrieve the swapchain-local
    timestamp from.
  ...
  * pname:timeDomainId is the id for the opaque time domain being
    calibrated.

pname:timeDomainId must: be an id previously reported by
flink:vkGetSwapchainTimeDomainPropertiesEXT for pname:swapchain.
If the pname:timeDomainId is no longer supported by the pname:swapchain,
...
****
  * [[VUID-VkSwapchainCalibratedTimestampInfoEXT-timeDomain-12228]]
    If the pname:timeDomain member of the slink:VkCalibratedTimestampInfoKHR
    structure in this structure's pname:pNext chain is
    ename:VK_TIME_DOMAIN_PRESENT_STAGE_LOCAL_EXT, pname:presentStage must:
