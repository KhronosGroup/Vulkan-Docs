...
ifdef::VK_EXT_shader_tile_image[]
    <<renderpass-load-store-ops, subpass load operations>> in the
...
chain, this structure is ignored.
If slink:VkExportFenceCreateInfo is present in the pname:pNext chain of
slink:VkFenceCreateInfo with a NvSciSync pname:handleType, but either
slink:VkExportFenceSciSyncInfoNV is not present in the pname:pNext chain, or
if it is present but pname:pAttributes is set to `NULL`, flink:vkCreateFence
...
chain, this structure is ignored.
If slink:VkExportSemaphoreCreateInfo is present in the pname:pNext chain of
slink:VkSemaphoreCreateInfo with a NvSciSync pname:handleType, but either
slink:VkExportSemaphoreSciSyncInfoNV is not present in the pname:pNext
chain, or if it is but pname:pAttributes is set to `NULL`,
flink:vkCreateSemaphore will return ename:VK_ERROR_INITIALIZATION_FAILED.
