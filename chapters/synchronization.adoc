...
ifdef::VK_ARM_tensors[]
    <<descriptorsets-storagetensor,storage tensor>>,
...
ifdef::VK_KHR_maintenance9[]
  * ename:VK_DEPENDENCY_ASYMMETRIC_EVENT_BIT_KHR specifies that
    flink:vkCmdSetEvent2 must: only include the
    <<synchronization-pipeline-stages-masks, source stage mask>> of the
    first synchronization scope, and that flink:vkCmdWaitEvents2 must:
