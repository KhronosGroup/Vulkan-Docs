...
include::{chapters}/commonvalidity/compute_graph_pipeline_create_info_common.adoc[]
  ...
  * [[VUID-VkExecutionGraphPipelineCreateInfoAMDX-layout-09130]]
    If pname:layout is not dlink:VK_NULL_HANDLE, it must: be
    <<descriptors-pipelinelayout-consistency,consistent>> with the layout of
    the shaders specified in pname:pStages
  ...
  * [[VUID-VkExecutionGraphPipelineCreateInfoAMDX-layout-09132]]
    ...
    pname:layout accessible to each shader stage that is used by the
...
include::{chapters}/commonvalidity/dispatch_graph_common.adoc[]
  ...
  * [[VUID-vkCmdDispatchGraphIndirectCountAMDX-countInfo-09160]]
    ...
    created with the ename:VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT usage flag
    set
...
captures the following dynamic state for use with draw nodes:
  ...
  * ename:VK_DYNAMIC_STATE_DEPTH_BIAS
  * ename:VK_DYNAMIC_STATE_BLEND_CONSTANTS
  * ename:VK_DYNAMIC_STATE_DEPTH_BOUNDS
