...
ifdef::VK_ANDROID_external_memory_android_hardware_buffer[]
  ...
  * [[VUID-VkMemoryAllocateInfo-pNext-12500]]
    If the parameters define an import operation, the external handle is an
    Android hardware buffer, the pname:pNext chain includes a
    slink:VkMemoryDedicatedAllocateInfo with pname:image that is not
    dlink:VK_NULL_HANDLE, the pname:image was created with a
    slink:VkExternalFormatANDROID with a non-zero pname:externalFormat, and
    the
    slink:VkAndroidHardwareBufferFormatProperties2ANDROID::pname:formatFeatures
    returned by flink:vkGetAndroidHardwareBufferPropertiesANDROID for the
    Android hardware buffer does not contain
    ename:VK_FORMAT_FEATURE_2_HOST_IMAGE_TRANSFER_BIT, the pname:image must:
    not have been created with a pname:usage containing
