...
If slink:VkExportMemoryAllocateInfo is present in the pname:pNext chain of
slink:VkMemoryAllocateInfo with the pname:handleType mask containing the
ename:VK_EXTERNAL_MEMORY_HANDLE_TYPE_SCI_BUF_BIT_NV bit, but either
sname:VkExportMemorySciBufInfoNV is not present in the pname:pNext chain, or
if it is present but pname:pAttributes is set to `NULL`,
