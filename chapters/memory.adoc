...
ename:VK_MEMORY_HEAP_TILE_MEMORY_BIT_QCOM, are only visible across the
command buffers executed in a single command buffer submission batch within
a flink:vkQueueSubmit or flink:vkQueueSubmit2 call.
If the
slink:VkPhysicalDeviceTileMemoryHeapPropertiesQCOM::pname:queueSubmitBoundary
...
Collectively, these rules define the _tile memory scope_.
...
ifdef::VK_QCOM_tile_memory_heap[]
  * ename:VK_MEMORY_HEAP_TILE_MEMORY_BIT_QCOM bit specifies that the heap
...
correction (ECC) on platforms where this is a concern.
...
ifdef::VK_AMD_device_coherent_memory[]
  * ename:VK_MEMORY_PROPERTY_DEVICE_COHERENT_BIT_AMD bit specifies that
    ...
    made <<synchronization-dependencies-available-and-visible, available and
    visible>> on the device.
    If paired with ename:VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
...
ename:VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT on any particular implementation,
there may be different properties which cater to different memory access
patterns.

ename:VK_MEMORY_PROPERTY_HOST_CACHED_BIT is expected to be used when the
...
ename:VK_MEMORY_PROPERTY_HOST_COHERENT_BIT without
ename:VK_MEMORY_PROPERTY_HOST_CACHED_BIT is expected for upload operations
...
support ename:VK_MEMORY_PROPERTY_HOST_COHERENT_BIT and
ename:VK_MEMORY_PROPERTY_HOST_CACHED_BIT together.
Either way, applications are expected to look for the first memory type
which supports ename:VK_MEMORY_PROPERTY_HOST_COHERENT_BIT and use that.
On ename:VK_PHYSICAL_DEVICE_TYPE_INTEGRATED_GPU with a unified memory
architecture, ename:VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT is less of a concern
since all memory is effectively device-local, but on
ename:VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU, there may be a distinction
between a ename:VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT memory type which is
additionally ename:VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT or not.

On discrete GPUs, using ename:VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT,
ename:VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, and
ename:VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT together is intended for small
...
flink:vkWriteResourceDescriptorsEXT and flink:vkWriteSamplerDescriptorsEXT,
endif::VK_EXT_descriptor_heap[]
ifdef::VK_EXT_descriptor_buffer[]
updating descriptors in a descriptor buffer with flink:vkGetDescriptorEXT,
...
ename:VK_MEMORY_PROPERTY_HOST_CACHED_BIT, where some types support
ename:VK_MEMORY_PROPERTY_HOST_COHERENT_BIT alongside CACHED, and some types
which do not support ename:VK_MEMORY_PROPERTY_HOST_COHERENT_BIT in
combination with CACHED.
Unless coherency is required by the application's usage patterns, the
ename:VK_MEMORY_PROPERTY_HOST_COHERENT_BIT supporting type is likely to be
less optimal.
...
include::{generated}/api/structs/VkPhysicalDeviceMemoryBudgetPropertiesEXT.adoc[]
  ...
  * pname:heapBudget is an array of ename:VK_MAX_MEMORY_HEAPS
    ...
    A heap's budget provides a guideline for how much total memory the
    current process can: use at any given time before allocations may: start
    failing or causing performance degradation.
    The values may: change based on other activity in the system that is
  ...
  * pname:heapUsage is an array of ename:VK_MAX_MEMORY_HEAPS
    ...
    one element for each memory heap.
    A heap's usage is an estimate of how much memory the current process has
    allocated from that heap.
...
If slink:VkBufferDeviceAddressAlignmentAllocateInfoVALVE is included in the
pname:pNext chain of pname:pAllocateInfo, the allocation requirements can:
...
Such alignment can: be enforced by flink:vkCreateAccelerationStructure2KHR
...
If the parameters define an <<memory-import-operation,import>> operation,
and the same memory object is imported into multiple slink:VkDeviceMemory
handles,
slink:VkBufferDeviceAddressAlignmentAllocateInfoVALVE::pname:alignment may:
...
If the pname:memoryTypeIndex belongs to a heap with the
ename:VK_MEMORY_HEAP_TILE_MEMORY_BIT_QCOM bit included in its properties,
...
all other slink:VkDeviceMemory objects bound to the same range while
executing within the same <<memory-tile-heaps, _tile memory scope_>>.
...
Higher priority allocations will be moved to device-local memory first.
...
ifdef::VK_QCOM_tile_memory_heap[]
  * [[VUID-vkAllocateMemory-tileMemoryHeap-10976]]
    ...
    enabled, pname:pAllocateInfo->memoryTypeIndex must: not identify a
    memory type that corresponds to a slink:VkMemoryHeap with the
...
However, it must: affect the following per-process values:
  ...
  * slink:VkPhysicalDeviceLimits::pname:maxMemoryAllocationCount
ifdef::VK_COMPUTE_VERSION_1_1,VK_KHR_maintenance3,VK_EXT_memory_budget[]
ifdef::VK_EXT_memory_budget[]
  * slink:VkPhysicalDeviceMemoryBudgetPropertiesEXT::pname:heapUsage
endif::VK_EXT_memory_budget[]
endif::VK_COMPUTE_VERSION_1_1,VK_KHR_maintenance3,VK_EXT_memory_budget[]
...
ifdef::VK_ARM_tensors[]
  * [[VUID-VkMemoryAllocateInfo-pNext-09858]]
    If the pname:pNext chain includes a sname:VkExportMemoryAllocateInfo
    structure, and any of the handle types specified in
    sname:VkExportMemoryAllocateInfo::pname:handleTypes require a dedicated
    allocation, as reported by
    ...
    the pname:pNext chain must: include a
    sname:VkMemoryDedicatedAllocateInfoTensorARM structure with its
    pname:tensor member set to a value other than dlink:VK_NULL_HANDLE
...
ifdef::VK_BASE_VERSION_1_1,VK_KHR_device_group[]
  * [[VUID-VkMemoryAllocateInfo-None-12523]]
    If the parameters define an import operation and the external handle
    type is ename:VK_EXTERNAL_MEMORY_HANDLE_TYPE_DMA_BUF_BIT_EXT, the number
...
ifdef::VK_EXT_external_memory_metal[]
  ...
  * [[VUID-VkMemoryAllocateInfo-pNext-10396]]
    ...
    slink:VkMemoryDedicatedAllocateInfo structure with pname:image that is
    not dlink:VK_NULL_HANDLE, the width, height, array layer dimensions, and
    mipmap levels of pname:image and the Metal MTLTexture's must: be
    identical
...
ifdef::VK_NV_external_memory_sci_buf[]
  * [[VUID-VkMemoryAllocateInfo-pNext-05097]]
    ...
    structure, the <<features-sciBufExport, pname:sciBufExport>> feature
    must: be enabled
  * [[VUID-VkMemoryAllocateInfo-pNext-05098]]
    If the pname:pNext chain includes a slink:VkImportMemorySciBufInfoNV
    structure, the <<features-sciBufImport, pname:sciBufImport>> feature
    must: be enabled
...
ifdef::VK_VALVE_buffer_device_address_allocation_alignment[]
  * [[VUID-VkMemoryAllocateInfo-flags-12510]]
    If slink:VkMemoryAllocateFlagsInfo::pname:flags does not include
    ename:VK_MEMORY_ALLOCATE_DEVICE_ADDRESS_BIT,
    slink:VkBufferDeviceAddressAlignmentAllocateInfoVALVE::pname:alignment
    must: be 0
...
ifdef::VK_QNX_external_memory_screen_buffer[QNX Screen buffer]
    ...
    , sname:VkMemoryAllocateInfo::pname:allocationSize must: be greater than
    or equal to the sname:VkMemoryRequirements::pname:size of the image
...
ifdef::VK_QNX_external_memory_screen_buffer[QNX Screen buffer]
    ...
    , sname:VkMemoryAllocateInfo::pname:allocationSize must: be greater than
    or equal to the sname:VkMemoryRequirements::pname:size of the buffer
...
ifdef::VK_QCOM_tile_memory_heap[]
[[memory-bind-tile-memory]]
[open,refpage='vkCmdBindTileMemoryQCOM',desc='Bind tile memory to a command buffer',type='protos']
--
To bind a range of tile memory to the command buffer, call:

include::{generated}/api/protos/vkCmdBindTileMemoryQCOM.adoc[]

  * pname:commandBuffer is the command buffer that the tile memory will be
    bound to.
  * pname:pTileMemoryBindInfo is dlink:VK_NULL_HANDLE or a pointer to a
    slink:VkTileMemoryBindInfoQCOM structure defining how tile memory is
    bound.

Calling flink:vkCmdBindTileMemoryQCOM when pname:pTileMemoryBindInfo is
`NULL` is equivalent to binding no tile memory to the command buffer.

include::{generated}/validity/protos/vkCmdBindTileMemoryQCOM.adoc[]
--

[open,refpage='VkTileMemoryBindInfoQCOM',desc='Structure specifying tile memory to bind',type='structs']
--
The sname:VkTileMemoryBindInfoQCOM structure is defined as:

include::{generated}/api/structs/VkTileMemoryBindInfoQCOM.adoc[]

  * pname:memory is the tile memory object to be bound.

pname:memory is used to bind this memory object to tile memory for all
subsequent commands in the pname:commandBuffer.
Tile memory contents for ranges in the heap outside the bound pname:memory
are discarded and become undefined: for the <<memory-tile-heaps, active
_tile memory scope_>> if an action command is executed.
...
the pname:pNext chain of sname:VkCommandBufferInheritanceInfo.

If this structure was not specified since recording started for
pname:commandBuffer, no tile memory is bound to the command buffer and all
contents become undefined: for the _tile memory scope_ if an action command
is executed.

.Valid Usage
****
  * [[VUID-VkTileMemoryBindInfoQCOM-memory-10726]]
    pname:memory must: have been allocated from a slink:VkMemoryHeap with
    the ename:VK_MEMORY_HEAP_TILE_MEMORY_BIT_QCOM property
****

include::{generated}/validity/structs/VkTileMemoryBindInfoQCOM.adoc[]
--
endif::VK_QCOM_tile_memory_heap[]

ifdef::VK_ARM_tensors[]
[open,refpage='VkMemoryDedicatedAllocateInfoTensorARM',desc='Specify a dedicated memory allocation tensor resource',type='structs']
--
If the pname:pNext chain includes a
sname:VkMemoryDedicatedAllocateInfoTensorARM structure, then that structure
includes a handle of the sole tensor resource that the memory can: be bound
to.

The sname:VkMemoryDedicatedAllocateInfoTensorARM structure is defined as:

include::{generated}/api/structs/VkMemoryDedicatedAllocateInfoTensorARM.adoc[]

  * pname:sType is a elink:VkStructureType value identifying this structure.
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:tensor is a handle of a tensor which this memory will be bound to.
...
****
  * [[VUID-VkMemoryDedicatedAllocateInfoTensorARM-allocationSize-09710]]
    ...
    sname:VkMemoryRequirements::pname:size of the tensor
  * [[VUID-VkMemoryDedicatedAllocateInfoTensorARM-tensor-09859]]
    If slink:VkMemoryAllocateInfo defines a memory import operation with
    handle type ename:VK_EXTERNAL_MEMORY_HANDLE_TYPE_OPAQUE_FD_BIT, the
    memory being imported must: also be a dedicated tensor allocation and
    pname:tensor must: be identical to the tensor associated with the
    imported memory
****

include::{generated}/validity/structs/VkMemoryDedicatedAllocateInfoTensorARM.adoc[]
--
endif::VK_ARM_tensors[]
...
ifdef::VK_VALVE_buffer_device_address_allocation_alignment[]
If the pname:pNext chain includes a
sname:VkBufferDeviceAddressAlignmentAllocateInfoVALVE structure, then that
structure requests a minimum buffer device address alignment for bound
buffers.

[open,refpage='VkBufferDeviceAddressAlignmentAllocateInfoVALVE',desc='Specify minimum buffer device address alignment',type='structs']
--
The slink:VkBufferDeviceAddressAlignmentAllocateInfoVALVE structure is
defined as:

include::{generated}/api/structs/VkBufferDeviceAddressAlignmentAllocateInfoVALVE.adoc[]

  * pname:sType is a elink:VkStructureType value identifying this structure.
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:alignment is the minimum alignment that implementation must:
    allocate for any buffer device addresses.

If this struct is not present in a pname:pNext chain, pname:alignment is
assumed to be 0.
If pname:alignment is 0, this alignment request is ignored.

.Valid Usage
****
  * [[VUID-VkBufferDeviceAddressAlignmentAllocateInfoVALVE-alignment-12512]]
    If pname:alignment is not 0, it must: be power-of-two
  * [[VUID-VkBufferDeviceAddressAlignmentAllocateInfoVALVE-alignment-12513]]
    If pname:alignment is not 0,
    <<features-bufferDeviceAddressAllocationAlignment,pname:bufferDeviceAddressAllocationAlignment>>
    must: be enabled
  * [[VUID-VkBufferDeviceAddressAlignmentAllocateInfoVALVE-alignment-12514]]
    pname:alignment must: be less-than or equal-to
    <<limits-maxBufferDeviceAddressAllocationAlignment,pname:maxBufferDeviceAddressAllocationAlignment>>
****

include::{generated}/validity/structs/VkBufferDeviceAddressAlignmentAllocateInfoVALVE.adoc[]
--
endif::VK_VALVE_buffer_device_address_allocation_alignment[]
...
****
  * [[VUID-VkExportMemoryAllocateInfo-handleTypes-09860]]
    ...
    reported by
ifdef::VK_ARM_tensors[slink:VkExternalTensorPropertiesARM,]
...
ifdef::VK_ARM_tensors[slink:VkExternalTensorPropertiesARM,]
  ...
  * [[VUID-VkImportMemoryWin32HandleInfoKHR-handle-01518]]
    ...
    pname:handleType in <<external-memory-handle-types-compatibility,
...
.Valid Usage
****
  * [[VUID-VkImportMemoryFdInfoKHR-handleType-09862]]
    If pname:handleType is not `0`, it must: be supported for import, as
    reported by
ifdef::VK_ARM_tensors[slink:VkExternalTensorPropertiesARM,]
    slink:VkExternalImageFormatProperties or
...
ifdef::VK_EXT_external_memory_dma_buf[]
ifdef::VK_BASE_VERSION_1_1,VK_KHR_device_group[]
  * [[VUID-VkMemoryGetFdInfoKHR-handleType-12524]]
    If pname:handleType is
    ename:VK_EXTERNAL_MEMORY_HANDLE_TYPE_DMA_BUF_BIT_EXT, pname:memory must:
...
.Valid Usage
****
  * [[VUID-VkImportAndroidHardwareBufferInfoANDROID-buffer-09863]]
...
include::{generated}/api/structs/VkAndroidHardwareBufferFormatResolvePropertiesANDROID.adoc[]
  ...
  * pname:colorAttachmentFormat is a elink:VkFormat specifying the format of
    ...
    the ename:VK_FORMAT_FEATURE_COLOR_ATTACHMENT_BIT in
    ...
    If external format resolves are not supported, this value will be
...
include::{generated}/api/structs/VkMemoryGetZirconHandleInfoFUCHSIA.adoc[]
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:memory is the slink:VkDeviceMemory being exported.
  * pname:handleType is a elink:VkExternalMemoryHandleTypeFlagBits value
...
include::{generated}/api/structs/VkImportMetalTextureInfoEXT.adoc[]
  ...
  * pname:mtlTexture is the Metal `id<MTLTexture>` object that is to
...
[NOTE]
====
Due to `id<MTLTexture>` already being backed by memory, images created with
sname:VkImportMetalTextureInfoEXT in the pname:pNext of the
slink:VkImageCreateInfo will be treated as bound to a VkDeviceMemory.
====
...
chain, or it is included but pname:pAttributes is `NULL`,
...
include::{generated}/api/protos/vkGetPhysicalDeviceExternalMemorySciBufPropertiesNV.adoc[]
  ...
  * pname:handle is the stext:NvSciBufObj handle which will be imported.
...
****
  ...
  * [[VUID-VkImportScreenBufferInfoQNX-buffer-08967]]
    If pname:buffer is not `NULL`, it must: be a pointer to a
    <<memory-external-screen-buffer-validity,valid QNX Screen buffer>>
...
ifdef::VK_EXT_zero_initialize_device_memory[]
  ...
  * [[VUID-VkMemoryAllocateFlagsInfo-flags-10761]]
    If the allocation uses protected memory, then pname:flags must: not
    contain ename:VK_MEMORY_ALLOCATE_ZERO_INITIALIZE_BIT_EXT
...
ifdef::VK_BASE_VERSION_1_2,VK_KHR_buffer_device_address[]
  ...
  * ename:VK_MEMORY_ALLOCATE_DEVICE_ADDRESS_CAPTURE_REPLAY_BIT specifies
    ...
    If this bit is set, ename:VK_MEMORY_ALLOCATE_DEVICE_ADDRESS_BIT must:
    also be set.
endif::VK_BASE_VERSION_1_2,VK_KHR_buffer_device_address[]
ifdef::VK_EXT_zero_initialize_device_memory[]
  * ename:VK_MEMORY_ALLOCATE_ZERO_INITIALIZE_BIT_EXT specifies that the
...
[open,refpage='VkMemoryOpaqueCaptureAddressAllocateInfo',desc='Request a specific address for a memory allocation',type='structs']
--
...
ifdef::VK_EXT_map_memory_placed[]
  ...
  * pname:ppData is a pointer to a code:void* variable in which a
    ...
    returned.
    The value of the returned pointer minus pname:offset must: be aligned to
...
ifdef::VK_BASE_VERSION_1_4,VK_KHR_map_memory2[]
Calling fname:vkMapMemory is equivalent to calling flink:vkMapMemory2 with
an empty pname:pNext chain.
endif::VK_BASE_VERSION_1_4,VK_KHR_map_memory2[]
...
slink:VkMemoryMapInfo::pname:flags, specifying additional properties of a
memory map, are:
...
ifdef::VK_BASE_VERSION_1_4,VK_KHR_map_memory2[]
[open,refpage='vkMapMemory2',desc='Map a memory object into application address space',type='protos']
--
To retrieve a host virtual address pointer to a region of a mappable memory
object, call:

ifdef::VK_BASE_VERSION_1_4[]
include::{generated}/api/protos/vkMapMemory2.adoc[]
...
include::{generated}/api/protos/vkMapMemory2KHR.adoc[]
  ...
  * pname:device is the logical device that owns the memory.
  * pname:pMemoryMapInfo is a pointer to a slink:VkMemoryMapInfo structure
  ...
  * pname:ppData is a pointer to a `void *` variable in which is returned a
    ...
    This pointer minus slink:VkMemoryMapInfo::pname:offset must: be aligned
    to at least slink:VkPhysicalDeviceLimits::pname:minMemoryMapAlignment.
...
include::{generated}/validity/protos/vkMapMemory2.adoc[]
--

[open,refpage='VkMemoryMapInfo',desc='Structure containing parameters of a memory map operation',type='structs']
--
The sname:VkMemoryMapInfo structure is defined as:

include::{generated}/api/structs/VkMemoryMapInfo.adoc[]
...
include::{generated}/validity/structs/VkMemoryMapInfo.adoc[]
--
endif::VK_BASE_VERSION_1_4,VK_KHR_map_memory2[]
...
sname:VkMemoryMapInfo::pname:flags and the pname:pNext chain of
slink:VkMemoryMapInfo includes a sname:VkMemoryMapPlacedInfoEXT structure,
...
sname:VkMemoryMapPlacedInfoEXT::pname:pPlacedAddress and fname:vkMapMemory2
will return ename:VK_SUCCESS.
If it cannot place the map at the requested address for any reason, the
memory object is left unmapped and fname:vkMapMemory2 will return
ename:VK_ERROR_MEMORY_MAP_FAILED.
...
include::{generated}/api/structs/VkMemoryMapPlacedInfoEXT.adoc[]
  ...
  * pname:pPlacedAddress is the virtual address at which to place the
    address.
    If sname:VkMemoryMapInfo::pname:flags does not contain
...
****
  ...
  * [[VUID-VkMemoryMapPlacedInfoEXT-pPlacedAddress-09578]]
    ...
    sname:VkMemoryMapInfo::pname:size must: not overlap any existing Vulkan
...
The second <<synchronization-dependencies-scopes, synchronization scope>> is
empty.
...
whole aligned set of <<limits-nonCoherentAtomSize,
pname:nonCoherentAtomSize>> bytes that the memory location exists in will
also be made available as if they were written by the host.
For example, with a pname:nonCoherentAtomSize of 128, if an application
...
host <<synchronization,synchronization operations>> that depend on device
operations such as flink:vkWaitForFences must be executed beforehand.
...
has to be a dependency chain along the following lines:
  ...
  . Signal on the device (e.g. a <<synchronization-fences-signaling,fence
    signal operation>>)
  . Wait on the host (e.g. flink:vkWaitForFences)
...
way, each whole aligned set of <<limits-nonCoherentAtomSize,
pname:nonCoherentAtomSize>> bytes that the memory location exists in will
...
ifdef::VK_BASE_VERSION_1_4,VK_KHR_map_memory2[]
Calling fname:vkUnmapMemory is equivalent to calling flink:vkUnmapMemory2
...
ifdef::VK_BASE_VERSION_1_4,VK_KHR_map_memory2[]
[open,refpage='vkUnmapMemory2',desc='Unmap a previously mapped memory object',type='protos']
--
To unmap a memory object once host access to it is no longer needed by the
application, call:

ifdef::VK_BASE_VERSION_1_4[]
include::{generated}/api/protos/vkUnmapMemory2.adoc[]
endif::VK_BASE_VERSION_1_4[]
...
include::{generated}/api/protos/vkUnmapMemory2KHR.adoc[]
  ...
  * pname:device is the logical device that owns the memory.
  * pname:pMemoryUnmapInfo is a pointer to a slink:VkMemoryUnmapInfo
...
include::{generated}/validity/protos/vkUnmapMemory2.adoc[]
--

[open,refpage='VkMemoryUnmapInfo',desc='Structure containing parameters of a memory unmap operation',type='structs']
--
The sname:VkMemoryUnmapInfo structure is defined as:

include::{generated}/api/structs/VkMemoryUnmapInfo.adoc[]
...
include::{generated}/api/structs/VkMemoryUnmapInfoKHR.adoc[]
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:flags is a bitmask of elink:VkMemoryUnmapFlagBits specifying
    additional parameters of the memory map operation.
...
ifdef::VK_EXT_map_memory_placed[]
  * [[VUID-VkMemoryUnmapInfo-flags-09579]]
    ...
    <<features-memoryUnmapReserve, pname:memoryUnmapReserve>> feature must:
    be enabled
ifdef::VK_EXT_external_memory_host[]
...
include::{generated}/validity/structs/VkMemoryUnmapInfo.adoc[]
--

ifdef::VK_EXT_map_memory_placed[]
[open,refpage='VkMemoryUnmapFlagBits',desc='Bitmask specifying additional parameters of a memory unmap',type='enums']
--
Bits which can: be set in slink:VkMemoryUnmapInfo::pname:flags, specifying
additional properties of a memory unmap, are:

include::{generated}/api/enums/VkMemoryUnmapFlagBits.adoc[]

ifdef::VK_KHR_map_memory2[]
include::{generated}/api/enums/VkMemoryUnmapFlagBitsKHR.adoc[]
endif::VK_KHR_map_memory2[]

  * ename:VK_MEMORY_UNMAP_RESERVE_BIT_EXT requests that the virtual address
    range currently occupied by the memory map remain reserved after the
    flink:vkUnmapMemory2 call completes.
    Future system memory map operations or calls to flink:vkMapMemory or
    flink:vkMapMemory2 will not return addresses in that range unless the
    range has since been unreserved by the client or the mapping is
    explicitly placed in that range by calling flink:vkMapMemory2 with
    ...
    mapped and flink:vkUnmapMemory2 will return
    ename:VK_ERROR_MEMORY_MAP_FAILED.
--
endif::VK_EXT_map_memory_placed[]

[open,refpage='VkMemoryUnmapFlags',desc='Bitmask of VkMemoryUnmapFlagBits',type='flags']
--
include::{generated}/api/flags/VkMemoryUnmapFlags.adoc[]

ifdef::VK_KHR_map_memory2[]
include::{generated}/api/flags/VkMemoryUnmapFlagsKHR.adoc[]
endif::VK_KHR_map_memory2[]

tname:VkMemoryUnmapFlags is a bitmask type for setting a mask of zero or
more elink:VkMemoryUnmapFlagBits.
--
endif::VK_BASE_VERSION_1_4,VK_KHR_map_memory2[]
...
Protected memory adds the following concepts:
  ...
  * Resources:
  ** Unprotected images
ifdef::VK_ARM_tensors[, unprotected tensors, ]
     and unprotected buffers, to which unprotected memory can: be bound
  ** Protected images
ifdef::VK_ARM_tensors[, protected tensors, ]
...
with ename:VK_PIPELINE_CREATE_PROTECTED_ACCESS_ONLY_BIT may only be recorded
in protected command buffers, and pipelines created with
ename:VK_PIPELINE_CREATE_NO_PROTECTED_ACCESS_BIT may only be recorded in
...
An slink:VkImage or slink:VkBuffer
ifdef::VK_ARM_tensors[]
or slink:VkTensorARM
endif::VK_ARM_tensors[]
can: be bound to the imported or exported slink:VkDeviceMemory object if it
is created with
ename:VK_EXTERNAL_MEMORY_HANDLE_TYPE_ANDROID_HARDWARE_BUFFER_BIT_ANDROID.
...
ifdef::VK_ANDROID_external_format_resolve[]
or rendered to as a <<features-externalFormatResolve,resolve attachment>>
endif::VK_ANDROID_external_format_resolve[]
.
...
3::
    ...
    It would be invalid to take an Android Hardware Buffer with a format of
...
[open,refpage='vkGetDeviceMemoryOpaqueCaptureAddress',desc='Query an opaque capture address of a memory object',type='protos']
--
...
****
  ...
  * [[VUID-vkGetDeviceMemoryOpaqueCaptureAddress-pInfo-10727]]
    pname:pInfo->memory must: have been allocated using the
    ename:VK_MEMORY_ALLOCATE_DEVICE_ADDRESS_CAPTURE_REPLAY_BIT flag
...
[open,refpage='VkDeviceMemoryOpaqueCaptureAddressInfo',desc='Structure specifying the memory object to query an address for',type='structs']
--
