...
****
  ...
  * [[VUID-{refpage}-size-11366]]
    If pname:size is not 0 and the buffer from which pname:address was
    queried is non-sparse, then the buffer must: be bound completely and
