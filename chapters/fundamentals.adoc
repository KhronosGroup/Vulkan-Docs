...
ifdef::VK_BASE_VERSION_1_1[protected memory management,]
...
Each device supports a number of <<devsandqueues-queues,queues>>, which
provide an interface for <<devsandqueues-submission, submitting work>> for
execution on the device.

<<devsandqueues-submission,_Queue submission commands_>> are used to submit
work, along with a set of <<synchronization, synchronization primitives>>
...
Many commands for queues are recorded into <<commandbuffers,_command
buffers_>> first, before the command buffers are then submitted to a queue
for execution.
...
Synchronization::
    _Synchronization commands_ impose ordering constraints on action
    commands, by introducing explicit
    <<synchronization-dependencies,execution and memory dependencies>>.
...
ifndef::VK_BASE_VERSION_1_3,VK_EXT_private_data[Non-dispatchable]
...
Vulkan objects passed to a recording command (ftext:vkCmd*) may: be accessed
...
into, or while the command buffer is in the <<commandbuffers-lifecycle,
_pending state_>>, unless explicitly stated otherwise.
...
ifdef::VK_BASE_VERSION_1_0,VK_EXT_buffer_device_address,VK_KHR_buffer_device_address[]
[open,refpage='VkDeviceAddress',desc='Vulkan device address type',type='basetypes']
--
:refpage: VkDeviceAddress
...
****
  * [[VUID-VkDeviceAddress-size-11364]]
    A valid basetype:VkDeviceAddress must: be equal to the sum of an address
    retrieved from a slink:VkBuffer via flink:vkGetBufferDeviceAddress, and
    any offset in the range [eq]#[0, pname:size)#, where pname:size is the
    value of slink:VkBufferCreateInfo::pname:size used to create that
    slink:VkBuffer
  * [[VUID-{refpage}-None-10894]]
    If a basetype:VkDeviceAddress was retrieved from a non-sparse buffer,
...
--
endif::VK_BASE_VERSION_1_0,VK_EXT_buffer_device_address,VK_KHR_buffer_device_address[]
...
The pname:pAllocator parameter can: be a non-`NULL` value, in which case
...
ifdef::VKSC_VERSION_1_0[]
The pname:pAllocator parameter must: be `NULL`.
Refer to the <<memory-allocation,Memory Allocation>> chapter for further
details.
endif::VKSC_VERSION_1_0[]
...
[[fundamentals-binaryresults]]
=== Opaque Binary Data Results

Some query commands of the form ftext:vkGet* retrieve opaque binary data in
the form of a byte array and have a possible result code of
ename:VK_ERROR_NOT_ENOUGH_SPACE_KHR.
Such commands always have two pointer arguments as follows:

  * A binary data size pointer pointing to a code:size_t variable,
    conventionally named as ftext:p*Size where ftext:* is the capitalized
    form of the name of the retrieved binary data.
  * A pointer to a byte array where the binary data is retrieved,
    conventionally named as ftext:p* where ftext:* is the capitalized form
...
and the command will return ename:VK_ERROR_NOT_ENOUGH_SPACE_KHR instead of
ename:VK_SUCCESS.

If the return code is ename:VK_SUCCESS or
ename:VK_ERROR_NOT_ENOUGH_SPACE_KHR, the total size of the binary data that
can: be retrieved is returned in the variable pointed to by the binary size
pointer.

If multiple binaries are being retrieved,
ename:VK_ERROR_NOT_ENOUGH_SPACE_KHR will be returned if any input binary
...
[NOTE]
====
If ename:VK_ERROR_NOT_ENOUGH_SPACE_KHR is returned with a command that
...
historical reasons, primarily that the ename:VK_ERROR_NOT_ENOUGH_SPACE_KHR
...
Additionally, if the <<features-maintenance5,pname:maintenance5>> feature is
...
slink:VkPhysicalDevice object as its first parameter.
...
ifdef::VK_BASE_VERSION_1_3,VK_KHR_format_feature_flags2,VK_KHR_synchronization2[]
...
etext:FlagBits2.
For example, a flag value of type tlink:VkAccessFlags2 must: contain only
bit flags defined by elink:VkAccessFlagBits2.
...
====
--
endif::VK_BASE_VERSION_1_3,VK_KHR_format_feature_flags2,VK_KHR_synchronization2[]


[[fundamentals-effective-flags]]
===== Effective Flags
...
ifdef::VK_BASE_VERSION_1_4[* slink:VkBufferUsageFlags2CreateInfo]
ifdef::VK_KHR_extended_flags[* slink:VkImageUsageFlags2CreateInfoKHR]
ifndef::VK_BASE_VERSION_1_4,VK_KHR_extended_flags[* none present in this build]
...
ifdef::VK_BASE_VERSION_1_1,VK_KHR_external_memory,VK_KHR_external_semaphore,VK_KHR_external_fence[]
  * ename:VK_ERROR_INVALID_EXTERNAL_HANDLE An external handle is not a valid
    handle of the specified type.
endif::VK_BASE_VERSION_1_1,VK_KHR_external_memory,VK_KHR_external_semaphore,VK_KHR_external_fence[]
...
ifdef::VK_EXT_present_timing[]
  * ename:VK_ERROR_PRESENT_TIMING_QUEUE_FULL_EXT A present operation on a
    swapchain created with ename:VK_SWAPCHAIN_CREATE_PRESENT_TIMING_BIT_EXT
  ...
  * ename:VK_ERROR_VALIDATION_FAILED A command failed because invalid usage
...
ifdef::VK_BASE_VERSION_1_4,VK_KHR_global_priority,VK_EXT_global_priority[]
  * ename:VK_ERROR_NOT_PERMITTED The driver implementation has denied a
...
If ename:VK_ERROR_UNKNOWN is returned, the application should be checked
...
ename:VK_ERROR_VALIDATION_FAILED if a violation of valid usage is detected.
...
ename:VK_INCOMPLETE would be returned, reporting of the runtime error
...
=== Host Address Ranges
[open,refpage='VkHostAddressRangeEXT',desc='Structure specifying a host address range',type='structs']
--
A host address range indicates a range of host memory.

include::{generated}/api/structs/VkHostAddressRangeEXT.adoc[]

  * pname:address is a host memory address.
  * pname:size is the size of the range.

include::{generated}/validity/structs/VkHostAddressRangeEXT.adoc[]
--

[open,refpage='VkHostAddressRangeConstEXT',desc='Structure specifying a constant host address range',type='structs']
--
A constant host address range indicates a range of host memory that cannot
be altered.

include::{generated}/api/structs/VkHostAddressRangeConstEXT.adoc[]

  * pname:address is a read-only host memory address.
  * pname:size is the size of the range.

include::{generated}/validity/structs/VkHostAddressRangeConstEXT.adoc[]
--

endif::VK_EXT_descriptor_heap[]

ifdef::VK_KHR_device_address_commands,VK_EXT_descriptor_heap,VK_KHR_copy_memory_indirect[]
=== Device Address Ranges

ifdef::VK_KHR_device_address_commands,VK_EXT_descriptor_heap[]
[open,refpage='VkDeviceAddressRangeKHR',desc='Structure specifying a device address range',type='structs']
--
:refpage: VkDeviceAddressRangeKHR

A device address range indicates a sized range of device memory.

include::{generated}/api/structs/VkDeviceAddressRangeKHR.adoc[]

ifdef::VK_EXT_descriptor_heap[]
include::{generated}/api/structs/VkDeviceAddressRangeEXT.adoc[]
endif::VK_EXT_descriptor_heap[]

  * pname:address is 0 or a basetype:VkDeviceAddress specifying the start of
    the range.
  * pname:size is a basetype:VkDeviceSize specifying the size of the range.

.Valid Usage
****
include::{chapters}/commonvalidity/device_address_range_common.adoc[]
****

include::{generated}/validity/structs/VkDeviceAddressRangeKHR.adoc[]
--
endif::VK_KHR_device_address_commands,VK_EXT_descriptor_heap[]

ifdef::VK_KHR_device_address_commands,VK_KHR_copy_memory_indirect[]
[open,refpage='VkStridedDeviceAddressRangeKHR',desc='Structure specifying a device address range with a stride',type='structs']
--
:refpage: VkStridedDeviceAddressRangeKHR

A strided device address range is defined by the structure:

include::{generated}/api/structs/VkStridedDeviceAddressRangeKHR.adoc[]

  * pname:address is a basetype:VkDeviceAddress specifying the start of the
    range.
  * pname:size is a basetype:VkDeviceSize specifying the size of the range.
  * pname:stride is a basetype:VkDeviceSize specifying the stride of
...
include::{chapters}/commonvalidity/device_address_range_common.adoc[]
  * [[VUID-VkStridedDeviceAddressRangeKHR-stride-10957]]
    pname:stride must: be less than or equal to pname:size
****

include::{generated}/validity/structs/VkStridedDeviceAddressRangeKHR.adoc[]
--
...
[open,refpage='VkAddressCommandFlagBitsKHR',desc='Bitmask specifying address copy parameters',type='enums']
--
Bits which can: be set in a tlink:VkAddressCommandFlagsKHR mask are:

include::{generated}/api/enums/VkAddressCommandFlagBitsKHR.adoc[]

  * ename:VK_ADDRESS_COMMAND_PROTECTED_BIT_KHR specifies that an address
    range is allocated from protected memory.
  * ename:VK_ADDRESS_COMMAND_FULLY_BOUND_BIT_KHR specifies that an address
    range will be fully bound to physical memory when accessed.
  * ename:VK_ADDRESS_COMMAND_STORAGE_BUFFER_USAGE_BIT_KHR specifies that all
    buffers containing any part of an address range were created with the
    ename:VK_BUFFER_USAGE_STORAGE_BUFFER_BIT usage.
  * ename:VK_ADDRESS_COMMAND_UNKNOWN_STORAGE_BUFFER_USAGE_BIT_KHR specifies
    that whether buffers containing an any part of an address range were
    created with ename:VK_BUFFER_USAGE_STORAGE_BUFFER_BIT usage or not is
    unknown.
ifdef::VK_EXT_transform_feedback[]
  * ename:VK_ADDRESS_COMMAND_TRANSFORM_FEEDBACK_BUFFER_USAGE_BIT_KHR
    ...
    allocated with the
    ename:VK_BUFFER_USAGE_TRANSFORM_FEEDBACK_BUFFER_BIT_EXT usage.
  * ename:VK_ADDRESS_COMMAND_UNKNOWN_TRANSFORM_FEEDBACK_BUFFER_USAGE_BIT_KHR
    ...
    ename:VK_BUFFER_USAGE_TRANSFORM_FEEDBACK_BUFFER_BIT_EXT usage or not is
    unknown.
endif::VK_EXT_transform_feedback[]

If neither ename:VK_ADDRESS_COMMAND_STORAGE_BUFFER_USAGE_BIT_KHR nor
ename:VK_ADDRESS_COMMAND_UNKNOWN_STORAGE_BUFFER_USAGE_BIT_KHR are specified,
the address range must: not be aliased with any buffer allocated with the
ename:VK_BUFFER_USAGE_STORAGE_BUFFER_BIT.
ifdef::VK_EXT_transform_feedback[]
If neither ename:VK_ADDRESS_COMMAND_TRANSFORM_FEEDBACK_BUFFER_USAGE_BIT_KHR
nor ename:VK_ADDRESS_COMMAND_UNKNOWN_TRANSFORM_FEEDBACK_BUFFER_USAGE_BIT_KHR
are specified, the address range must: not be aliased with any buffer
allocated with the ename:VK_BUFFER_USAGE_TRANSFORM_FEEDBACK_BUFFER_BIT_EXT.
endif::VK_EXT_transform_feedback[]

[NOTE]
====
Buffers can return overlapping address ranges if they are bound to
overlapping ranges of a slink:VkDeviceMemory object.
Applications should ensure that if they do this kind of aliasing, that they
consistently either include or do not include
ename:VK_BUFFER_USAGE_STORAGE_BUFFER_BIT
ifdef::VK_EXT_transform_feedback[and ename:VK_BUFFER_USAGE_TRANSFORM_FEEDBACK_BUFFER_BIT_EXT]
on aliased buffers when considering these flags.
====
--

[open,refpage='VkAddressCommandFlagsKHR',desc='Bitmask of VkAddressCommandFlagBitsKHR',type='flags']
--
include::{generated}/api/flags/VkAddressCommandFlagsKHR.adoc[]

tname:VkAddressCommandFlagsKHR is a bitmask type for setting a mask of zero
or more elink:VkAddressCommandFlagBitsKHR.
--
endif::VK_KHR_device_address_commands[]
endif::VK_KHR_device_address_commands,VK_KHR_copy_memory_indirect[]
endif::VK_KHR_device_address_commands,VK_EXT_descriptor_heap,VK_KHR_copy_memory_indirect[]
...
prefixing each capital letter except the first with etext:_, converting the
entire resulting string to upper case, and prefixing it with
etext:VK_STRUCTURE_TYPE_.
...
ifdef::hidden[]
// tag::scremoved[]
  * elink:VkStructureType (legacy aliases)
