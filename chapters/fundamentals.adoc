...
A enumerant is valid if:
  ...
  * If the enumerant is defined by an extension or core version which adds
    that value to the enumerated type, it is valid only if the corresponding
    extension is enabled, or is a supported <<device-extensions, physical
    device extension>>; or the corresponding core version is supported.
