...
ifdef::VK_NV_device_generated_commands_compute[]
  * If token is of type ename:VK_INDIRECT_COMMANDS_TOKEN_TYPE_DISPATCH_NV or
    ename:VK_INDIRECT_COMMANDS_TOKEN_TYPE_PIPELINE_NV then pname:pipelineBindPoint must:
    be ename:VK_PIPELINE_BIND_POINT_COMPUTE
endif::VK_NV_device_generated_commands_compute[]
****
...
****
  * [[VUID-VkBindShaderGroupIndirectCommandNV-None-02944]]
    The current bound pipeline, as well as the pipelines it may
...
void cmdProcessSequence(cmd, pipeline, indirectCommandsLayout, pIndirectCommandsStreams, s)
{
  for (uint32_t t = 0; t < indirectCommandsLayout.tokenCount; t++){
    ...
    VK_INDIRECT_COMMANDS_TOKEN_TYPE_PIPELINE_NV:
      VkPipeline* computePipeline = input;
