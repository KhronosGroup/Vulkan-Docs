...
ifdef::VK_NV_device_generated_commands_compute[]
  * If pname:pipelineBindPoint is of type ename:VK_PIPELINE_BIND_POINT_COMPUTE, then
...
The order of execution of individual draws
ifdef::VK_NV_device_generated_commands_compute[]
or dispatches
endif::VK_NV_device_generated_commands_compute[]
through this command may: execute in any order and may: not necessarily be
in the same order as specified in sname:VkGeneratedCommandsInfoNV::pname:pStreams
.Valid Usage
****
...
ifdef::VK_NV_device_generated_commands_compute[]
  * If pname:pipelineBindPoint is of type ename:VK_PIPELINE_BIND_POINT_COMPUTE, then
    pname:pipeline must: be `NULL`. Instead, the pipeline's device address is
    provided as a token in sname:VkIndirectCommandsLayoutTokenNV with
    pname:tokentype as ename:VK_INDIRECT_COMMANDS_TOKEN_TYPE_PIPELINE_NV
endif::VK_NV_device_generated_commands_compute[]
...
ifdef::VK_VERSION_1_1[]
  ...
  * [[VUID-vkCmdPreprocessGeneratedCommandsNV-deviceGeneratedCommands-02928]]
    ...
    feature must: be enabled
ifdef::VK_NV_device_generated_commands_compute[]
  * If pname:pGeneratedCommandsInfo's pname:pipelineBindPoint is of type
    ename:VK_PIPELINE_BIND_POINT_COMPUTE then a valid descriptor set must: be bound
    before calling fname:vkCmdPreprocessGeneratedCommandsNV
endif::VK_NV_device_generated_commands_compute[]
****
