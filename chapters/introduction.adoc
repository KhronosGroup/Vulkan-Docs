...
to that runtime so that it may be interrogated at any time.  For example, such information could be the version number, name, date, release build number or a combination of these that is unique and comprehensible
...
(For example, <<fundamentals-validusage, Valid Usage>> sections only address
...
[[spirv-spec]] John Kessenich, Boaz Ouriel, Raun Krisch, and Diego Novillo.
_SPIR-V Specification, Version 1.6.7, Revision 1, Unified_ (March, 2026).
https://registry.khronos.org/spir-v/ .
