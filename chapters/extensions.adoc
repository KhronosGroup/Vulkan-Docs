...
[[extendingvulkan-functionalitylevels]]
...
instance-level, physical-device-level, and device-level.
  ...
  * slink:VkInstance and any objects created from a slink:VkInstance other
    than slink:VkPhysicalDevice are instance-level.
  * slink:VkPhysicalDevice is the only physical-device-level object.
  * slink:VkDevice and any objects created from a slink:VkDevice are
    device-level.

The level of a command is the same as the level of its first parameter - if
the first parameter is not a <<fundamentals-objectmodel-overview,
dispatchable handle>>, it is a global command.
Different levels of functionality may: be advertised in different ways.


ifdef::VK_BASE_VERSION_1_1[]
[[extendingvulkan-instanceanddeviceversions]]
...
flink:vkGetPhysicalDeviceProperties or flink:vkGetPhysicalDeviceProperties2,
...
[open,refpage='VK_API_VERSION_1_4',desc='Return API version number for Vulkan 1.4',type='defines',xrefs='vkCreateInstance vkGetPhysicalDeviceProperties']
--
dname:VK_API_VERSION_1_4 returns the API version number for Vulkan 1.4.0.

include::{generated}/api/defines/VK_API_VERSION_1_4.adoc[]
--
...
pname:pPropertyCount will be `0`, and ename:VK_SUCCESS is returned.
endif::VKSC_VERSION_1_0[]
...
slink:VkDeviceCreateInfo are legacy and their values must: be ignored by
implementations.
ifndef::VKSC_VERSION_1_0[]
...
Instance extensions add new <<extendingvulkan-functionalitylevels,global or
instance-level functionality>> to the API, outside of the core
specification.
Instance extensions may: also add <<extendingvulkan-functionalitylevels,
...
Device extensions add new <<extendingvulkan-functionalitylevels,device-level
functionality>> to the API, outside of the core specification.
ifdef::VK_BASE_VERSION_1_1,VK_KHR_get_physical_device_properties2[]
If
ifdef::VK_BASE_VERSION_1_1[Vulkan 1.1]
ifdef::VK_BASE_VERSION_1_1+VK_KHR_get_physical_device_properties2[or]
ifdef::VK_KHR_get_physical_device_properties2[apiext:VK_KHR_get_physical_device_properties2]
is required by the extension, it may: also add new
<<extendingvulkan-functionalitylevels,physical-device-level functionality>>.
endif::VK_BASE_VERSION_1_1,VK_KHR_get_physical_device_properties2[]
...
If the `apiext:VK_KHR_ray_tracing_pipeline` extension is advertised as
supported by this query, the `apiext:VK_KHR_pipeline_library` extension
must: also be supported.
endif::VK_KHR_ray_tracing_pipeline[]

ifdef::VK_EXT_descriptor_heap[]
If the `apiext:VK_EXT_descriptor_heap` extension is advertised as supported
by this query, the `apiext:VK_KHR_shader_untyped_pointers` extension must:
also be supported.
...
removal of <<legacy,legacy functionality>>, and the modification, addition,
...
Extensions may: be marked as <<legacy,deprecated>> when they are no longer
