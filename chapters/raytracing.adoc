...
Within the ray tracing pipeline, a <<glossary-pipeline-trace-ray, pipeline
trace ray>> instruction can: be called to perform a <<ray-traversal,ray
...
| code:OpTraceRayKHR |X|X|X|X|
ifdef::VK_NV_ray_tracing_motion_blur[]
| code:OpTraceRayMotionNV |X|X|X|X|
endif::VK_NV_ray_tracing_motion_blur[]
| code:OpReportIntersectionKHR | |X| | |
| code:OpExecuteCallableKHR | | | | |X
ifdef::VK_NV_ray_tracing_invocation_reorder[]
| code:OpHitObjectTraceRayNV |X|X| | |
| code:OpHitObjectTraceRayMotionNV |X|X| | |
| code:OpHitObjectExecuteShaderNV | | |X|X|
endif::VK_NV_ray_tracing_invocation_reorder[]
ifdef::VK_EXT_ray_tracing_invocation_reorder[]
| code:OpHitObjectExecuteShaderEXT | | |X|X|
| code:OpHitObjectTraceRayEXT |X|X| | |
| code:OpHitObjectReorderExecuteShaderEXT | | |X|X|
| code:OpHitObjectTraceReorderExecuteEXT |X|X|X|X|
ifdef::VK_NV_ray_tracing_motion_blur[]
| code:OpHitObjectTraceRayMotionEXT |X|X| | |
| code:OpHitObjectTraceMotionReorderExecuteEXT |X|X|X|X|
endif::VK_NV_ray_tracing_motion_blur[]
...
[[ray-tracing-repack]]
An _invocation repack instruction_ is a ray tracing instruction where the
...
ifdef::VK_EXT_ray_tracing_invocation_reorder[]
code:OpReorderThreadWithHintEXT, code:OpReorderThreadWithHitObjectEXT,
code:OpHitObjectTraceRayEXT, code:OpHitObjectReorderExecuteShaderEXT,
...
ifdef::VK_KHR_ray_tracing_pipeline,VK_NV_cluster_acceleration_structure[]
  * [[VUID-vkCmdTraceRaysKHR-allowClusterAccelerationStructure-10578]]
    ...
    slink:VkRayTracingPipelineClusterAccelerationStructureCreateInfoNV::pname:allowClusterAccelerationStructure
...
include::{generated}/api/protos/vkCmdBindInvocationMaskHUAWEI.adoc[]
  ...
  * pname:imageView is an image view handle specifying the invocation mask
    image pname:imageView may: be dlink:VK_NULL_HANDLE, which is equivalent
...
****
  ...
  * [[VUID-vkCmdBindInvocationMaskHUAWEI-imageView-04979]]
    ...
    created with the ename:VK_IMAGE_USAGE_INVOCATION_MASK_BIT_HUAWEI usage
    flag set
  ...
  * [[VUID-vkCmdBindInvocationMaskHUAWEI-width-04981]]
    Thread mask image resolution must: match the pname:width and
    pname:height in flink:vkCmdTraceRaysKHR
...
To dispatch ray tracing for the `apiext:VK_NV_ray_tracing` extension use:
...
if the code:DescriptorHeapEXT capability is not declared,
endif::VK_EXT_descriptor_heap[]
when indexing resources based on values in the code:ShaderRecordBufferKHR
storage class, the index should be decorated as code:NonUniform.
====
...
parameters to the <<glossary-pipeline-trace-ray, pipeline trace ray>>
instructions.
...
By enabling the <<features-rayTracingValidation, ray tracing validation>>
feature, warnings and errors can: be delivered straight from a ray tracing
implementation to the application through a <<debugging-debug-messengers,
messenger callback>> registered with the implementation, where they can: be
