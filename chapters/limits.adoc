...
include::{generated}/api/structs/VkPhysicalDeviceMaintenance5PropertiesKHR.adoc[]
  ...
  * [[{anchor-prefix}fragops]]pname:nonStrictSinglePixelWideLinesUseParallelogram
    is a boolean value indicating whether non-strict lines with a width of
    1.0 are rasterized as parallelograms or using Bresenham's algorithm.
  * [[{anchor-prefix}fragops]]pname:nonStrictWideLinesUseParallelogram is a
    boolean value indicating whether non-strict lines with a width greater
    than 1.0 are rasterized as parallelograms or using Bresenham's
    algorithm.
