...
include::{generated}/api/structs/VkPhysicalDeviceMaintenance7PropertiesKHR.adoc[]
  ...
  * [[limits-robustFragmentShadingRateAttachmentAccess]]
    ...
    rates for missing texels are defined by <<textures-texel-replacement,
    texel replacement for invalid texels>>.
...
interchangeable with ename:VK_IMAGE_LAYOUT_GENERAL in pname:pCopyDstLayouts.
...
If the <<features-unifiedImageLayouts, pname:unifiedImageLayouts>> feature
is enabled, the implementation must: include all the image layouts that are
interchangeable with ename:VK_IMAGE_LAYOUT_GENERAL in pname:pCopySrcLayouts.
endif::VK_KHR_unified_image_layouts[]
