...
When the <<features-transformFeedbackPreservesProvokingVertex,
pname:transformFeedbackPreservesProvokingVertex>> feature is not enabled,
implementations
...
ifdef::VK_EXT_provoking_vertex[]
When the <<features-transformFeedbackPreservesProvokingVertex,
pname:transformFeedbackPreservesProvokingVertex>> feature is enabled, then
...
the <<features-transformFeedbackPreservesProvokingVertex,
pname:transformFeedbackPreservesProvokingVertex>> feature is enabled.
...
corresponding ename:VK_QUERY_TYPE_TRANSFORM_FEEDBACK_STREAM_EXT query will
increment as normal.
...
[open,refpage='vkCmdBindTransformFeedbackBuffers2EXT',desc='Bind transform feedback memory ranges to a command buffer',type='protos']
--
:refpage: vkCmdBindTransformFeedbackBuffers2EXT

To bind transform feedback memory ranges to a command buffer for use in
subsequent drawing commands, call:

include::{generated}/api/protos/vkCmdBindTransformFeedbackBuffers2EXT.adoc[]
  ...
  * pname:pBindingInfos is a pointer to an array of
    slink:VkBindTransformFeedbackBuffer2InfoEXT structures specifying the
    ranges of memory to be used to capture transform feedback data.

Element [eq]#i# of pname:pBindingInfos replaces the current state for the
transform feedback binding [eq]#pname:firstBinding {plus} i#, for [eq]#i# in
[eq]#[0,pname:bindingCount)#.

.Valid Usage
****
include::{chapters}/commonvalidity/xform_feedback_bind_common.adoc[]
  * [[VUID-vkCmdBindTransformFeedbackBuffers2EXT-addressRange-13090]]
    The pname:addressRange.address member of all elements of
    pname:pBindingInfos must: be a multiple of 4
  * [[VUID-vkCmdBindTransformFeedbackBuffers2EXT-addressRange-13091]]
    The buffer that the pname:addressRange of each element of
    pname:pBindingInfos was queried from must: have been created with
    ename:VK_BUFFER_USAGE_TRANSFORM_FEEDBACK_BUFFER_BIT_EXT
  * [[VUID-vkCmdBindTransformFeedbackBuffers2EXT-addressRange-13092]]
    The pname:addressRange.size member of all elements of
    pname:pBindingInfos must: be less than or equal to
    sname:VkPhysicalDeviceTransformFeedbackPropertiesEXT::pname:maxTransformFeedbackBufferSize
****

include::{generated}/validity/protos/vkCmdBindTransformFeedbackBuffers2EXT.adoc[]
--

[open,refpage='VkBindTransformFeedbackBuffer2InfoEXT',desc='Bind transform feedback buffer info',type='structs']
--
:refpage: VkBindTransformFeedbackBuffer2InfoEXT

sname:VkBindTransformFeedbackBuffer2InfoEXT is defined as:

include::{generated}/api/structs/VkBindTransformFeedbackBuffer2InfoEXT.adoc[]
...
include::{generated}/validity/structs/VkBindTransformFeedbackBuffer2InfoEXT.adoc[]
--
...
--
:refpage: vkCmdBindTransformFeedbackBuffersEXT
...
include::{generated}/api/protos/vkCmdBindTransformFeedbackBuffersEXT.adoc[]
  ...
  * pname:pSizes is `NULL` or a pointer to an array of basetype:VkDeviceSize
    ...
    If pname:pSizes is `NULL`, it is equivalent to setting a pname:pSizes
    array where every element is ename:VK_WHOLE_SIZE.
...
[[transform-feedback-effective-size]]
When an element of pname:pSizes[i] is ename:VK_WHOLE_SIZE, or pname:pSizes
is `NULL`, the effective range is calculated by taking the size of
pname:pBuffers[i] minus pname:pOffsets[i].
Otherwise, the effective range is equal to the element in pname:pSizes[i].
...
include::{chapters}/commonvalidity/xform_feedback_bind_common.adoc[]
  ...
  * [[VUID-vkCmdBindTransformFeedbackBuffersEXT-pBuffers-02360]]
    ...
    ename:VK_BUFFER_USAGE_TRANSFORM_FEEDBACK_BUFFER_BIT_EXT usage flag set
  * [[VUID-vkCmdBindTransformFeedbackBuffersEXT-pOffsets-02363]]
    All elements of pname:pOffsets plus the
    <<transform-feedback-effective-size,effective size>> of the element,
...
ifdef::VK_KHR_device_address_commands[]
[open,refpage='vkCmdBeginTransformFeedback2EXT',desc='Make transform feedback active in the command buffer',type='protos']
--
:refpage: vkCmdBeginTransformFeedback2EXT
:firstcounter: firstCounterRange
:countercount: counterRangeCount
:bindCommand: vkCmdBindTransformFeedbackBuffers2EXT

Transform feedback for specific transform feedback buffers is made active by
calling:

include::{generated}/api/protos/vkCmdBeginTransformFeedback2EXT.adoc[]

  * pname:commandBuffer is the command buffer into which the command is
    recorded.
  * pname:firstCounterRange is the index of the first transform feedback
    buffer corresponding to pname:pCounterInfos[0].
  * pname:counterRangeCount is the size of the pname:pCounterRanges array.
  * pname:pCounterInfos is `NULL` or a pointer to an array of
    slink:VkBindTransformFeedbackBuffer2InfoEXT structures defining memory
    ranges containing counters used to resume transform feedback from a
    previous location.

If pname:pCounterInfos is `NULL`, it is equivalent to an array of
slink:VkBindTransformFeedbackBuffer2InfoEXT structures with each element
having a pname:addressRange.size of 0.

If the pname:addressRange.size of an element of pname:pCounterInfos at index
[eq]#i# is 0, transform feedback data written to code:XfbBuffer with a
code:XFB code:Buffer code:Number equal to the sum of [eq]#i# and
pname:firstCounterRange will be written starting at an offset of 0 in the
bound transform feedback buffer.
If the pname:addressRange.size of an element of pname:pCounterInfos at index
[eq]#i# is not 0, it will instead be written starting at an offset equal to
the 32-bit value found at pname:pCounterInfos[i].addressRange.address.
...
.Valid Usage
****
include::{chapters}/commonvalidity/xform_feedback_begin_common.adoc[]
  * [[VUID-vkCmdBeginTransformFeedback2EXT-pCounterInfos-13093]]
    Each element of pname:pCounterInfos must: have a pname:addressRange.size
    that is either 0 or greater than or equal to 4
  * [[VUID-vkCmdBeginTransformFeedback2EXT-pCounterInfos-13094]]
    For each element of pname:pCounterInfos with a non-zero address, the
    buffer the address was queried from must: have been created with the
    ename:VK_BUFFER_USAGE_TRANSFORM_FEEDBACK_COUNTER_BUFFER_BIT_EXT usage
    flag set
****

include::{generated}/validity/protos/vkCmdBeginTransformFeedback2EXT.adoc[]
--

[open,refpage='vkCmdEndTransformFeedback2EXT',desc='Make transform feedback inactive in the command buffer',type='protos']
--
:refpage: vkCmdEndTransformFeedback2EXT
:countercount: counterRangeCount

Transform feedback for specific transform feedback buffers is made inactive
by calling:

include::{generated}/api/protos/vkCmdEndTransformFeedback2EXT.adoc[]

  * pname:commandBuffer is the command buffer into which the command is
    recorded.
  * pname:firstCounterBuffer is the index of the first transform feedback
    buffer corresponding to pname:pCounterInfos[0].
  * pname:counterRangeCount is the size of the pname:pCounterInfos array.
  * pname:pCounterInfos is `NULL` or a pointer to an array of
    slink:VkBindTransformFeedbackBuffer2InfoEXT structures defining memory
...
If the pname:addressRange.size of an element of pname:pCounterInfos at index
[eq]#i# is 0, no data is written to the address range.
If the pname:addressRange.size of an element of pname:pCounterInfos at index
[eq]#i# is not 0, the byte offset, where the next vertex data would be
written to in the transform feedback buffer at a binding equal to the sum of
[eq]#i# and pname:firstCounterBuffer, is written to that range's
pname:address.

.Valid Usage
****
include::{chapters}/commonvalidity/xform_feedback_end_common.adoc[]
...
include::{generated}/validity/protos/vkCmdEndTransformFeedback2EXT.adoc[]
--
endif::VK_KHR_device_address_commands[]

[open,refpage='vkCmdBeginTransformFeedbackEXT',desc='Make transform feedback active in the command buffer',type='protos']
--
:refpage: vkCmdBeginTransformFeedbackEXT
:firstcounter: firstCounterBuffer
:countercount: counterBufferCount
:bindCommand: vkCmdBindTransformFeedbackBuffersEXT
...
include::{generated}/api/protos/vkCmdBeginTransformFeedbackEXT.adoc[]
  ...
  * pname:pCounterBuffers is `NULL` or a pointer to an array of
    ...
    fname:vkCmdEndTransformFeedbackEXT and
    fname:vkCmdBeginTransformFeedbackEXT, with
    ename:VK_PIPELINE_STAGE_TRANSFORM_FEEDBACK_BIT_EXT as the source and
    destination stages,
    ename:VK_ACCESS_TRANSFORM_FEEDBACK_COUNTER_WRITE_BIT_EXT as the source
    access and ename:VK_ACCESS_TRANSFORM_FEEDBACK_COUNTER_READ_BIT_EXT as
...
include::{chapters}/commonvalidity/xform_feedback_begin_common.adoc[]
  ...
  * [[VUID-vkCmdBeginTransformFeedbackEXT-pCounterBuffers-02372]]
    ...
    dlink:VK_NULL_HANDLE it must: have been with the
    ename:VK_BUFFER_USAGE_TRANSFORM_FEEDBACK_COUNTER_BUFFER_BIT_EXT usage
    flag set
****
...
--
:refpage: vkCmdEndTransformFeedbackEXT
:countercount: counterBufferCount
...
include::{chapters}/commonvalidity/xform_feedback_end_common.adoc[]
  ...
  * [[VUID-vkCmdEndTransformFeedbackEXT-pCounterBuffers-02380]]
    For each buffer handle in the pname:pCounterBuffers array that is not
    dlink:VK_NULL_HANDLE it must: have been created with the
    ename:VK_BUFFER_USAGE_TRANSFORM_FEEDBACK_COUNTER_BUFFER_BIT_EXT usage
    flag set
****
...
will use the ename:VK_PROVOKING_VERTEX_MODE_FIRST_VERTEX_EXT mode.
...
sname:VkPhysicalDeviceLimits::pname:maxClipDistances application-defined
half-spaces.

The cull volume is the intersection of up to
sname:VkPhysicalDeviceLimits::pname:maxCullDistances application-defined
...
is determined by the explicit size of the built-in arrays code:ClipDistance
and code:CullDistance, respectively, declared as an output in the interface
...
state is only affected by the <<dynamic-state-current-value, current value>>
of pname:depthClampEnable if the pipeline was not created with the
slink:VkPipelineRasterizationDepthClipStateCreateInfoEXT structure.
If slink:VkPipelineRasterizationDepthClipStateCreateInfoEXT was included in
...
slink:VkPipelineRasterizationDepthClipStateCreateInfoEXT::pname:depthClipEnable
value used to create the currently active pipeline.
If slink:VkPipelineRasterizationDepthClipStateCreateInfoEXT was not
specified, depth clipping is set to the inverse of the
<<dynamic-state-current-value, current value>> of pname:depthClampEnable.
...
ifndef::VK_GRAPHICS_VERSION_1_1,VK_KHR_maintenance2[]
...
****
  * [[VUID-VkPipelineViewportDepthClipControlCreateInfoEXT-negativeOneToOne-06470]]
    ...
    not enabled, pname:negativeOneToOne must: be ename:VK_FALSE
****
...
The vertex's framebuffer coordinates [eq]#(x~f~, y~f~)# and depth [eq]#z~f~#
are given by
...
sname:VkPhysicalDeviceLineRasterizationProperties::pname:lineSubPixelPrecisionBits.
...
[open,refpage='vkCmdSetViewportWithCount',desc='Set the viewport count and viewports dynamically for a command buffer',type='protos']
--
...
[open,refpage='vkCmdSetScissorWithCount',desc='Set the scissor count and scissor rectangular bounds dynamically for a command buffer',type='protos']
--
...
ifndef::VK_GRAPHICS_VERSION_1_2,VK_EXT_shader_viewport_index_layer,VK_NV_viewport_array2[]
...
ifdef::VK_GRAPHICS_VERSION_1_1,VK_KHR_maintenance1,VK_AMD_negative_viewport_height[]
...
endif::VK_GRAPHICS_VERSION_1_1,VK_KHR_maintenance1,VK_AMD_negative_viewport_height[]
