...
<<memory-model-memory-location,memory locations>> within the memory they are
bound to.
...
If the coordinates are <<shaders-execution-memory-access-bounds, out of
bounds>>, the write is discarded.
ifndef::VKSC_VERSION_1_0[]
...
ifdef::VK_EXT_image_sliced_view_of_3d[]
  * [eq]#z += slink:VkImageViewSlicedCreateInfoEXT::pname:sliceOffset#
...
Query operations do not access any texels in the image.
  ...
  * code:OpImageQuerySizeLod returns the values of [eq]#width~level~#,
    ...
    slink:VkImageSubresourceRange::pname:baseMipLevel.
ifdef::VK_EXT_image_sliced_view_of_3d[]
  ...
  * code:OpImageQuerySize is equivalent to code:OpImageQuerySizeLod with
    [eq]#level# set to slink:VkImageSubresourceRange::pname:baseMipLevel.
  * code:OpImageQueryLevels returns the value of
    slink:VkImageSubresourceRange::pname:levelCount specified for the image
    view used.
