...
ifdef::VK_GRAPHICS_VERSION_1_2,VK_EXT_separate_stencil_usage[]
  * [[VUID-vkCmdClearDepthStencilImage-pRanges-02658]]
    ...
    <<VkImageStencilUsageCreateInfo,separate stencil usage>>, it must have
...
ifdef::VK_GRAPHICS_VERSION_1_2,VK_EXT_separate_stencil_usage[]
    ...
    the ename:VK_IMAGE_USAGE_TRANSFER_DST_BIT usage flag must: have been set
    in the slink:VkImageCreateInfo::pname:usage used to create pname:image
  * [[VUID-vkCmdClearDepthStencilImage-pRanges-02660]]
    If the pname:aspect member of any element of pname:pRanges includes
    ename:VK_IMAGE_ASPECT_DEPTH_BIT, pname:image must: have been created
    with the ename:VK_IMAGE_USAGE_TRANSFER_DST_BIT usage flag set
...
[[clears-filling-memory]]
== Filling Memory

[open,refpage='vkCmdFillMemoryKHR',desc='Fill a memory range with a fixed 4-byte bit pattern',type='protos']
--
:refpage: vkCmdFillMemoryKHR

To fill a memory range with a fixed 4-byte bit pattern, call:

include::{generated}/api/protos/vkCmdFillMemoryKHR.adoc[]

  * pname:commandBuffer is the command buffer into which the command will be
    recorded.
  * pname:pDstRange is a pointer to the slink:VkDeviceAddressRangeKHR
  ...
  * pname:data is the 4-byte word written repeatedly to the destination
    range to fill pname:size bytes of data.

[NOTE]
====
The bit pattern of pname:data is determined by the host endianness.
...
include::{chapters}/commonvalidity/address_flags_common.adoc[]
  ...
  * [[VUID-vkCmdFillMemoryKHR-pDstRange-13001]]
    pname:pDstRange->address must: be a multiple of 4
  * [[VUID-vkCmdFillMemoryKHR-pDstRange-13002]]
    pname:pDstRange->size must: be a multiple of 4
  ...
  * [[VUID-vkCmdFillMemoryKHR-commandBuffer-13004]]
    If pname:commandBuffer is a protected command buffer and
    <<limits-protectedNoFault, pname:protectedNoFault>> is not supported,
    pname:dstFlags must: include ename:VK_ADDRESS_COMMAND_PROTECTED_BIT_KHR
****

include::{generated}/validity/protos/vkCmdFillMemoryKHR.adoc[]
--


[[clears-updating-memory]]
== Updating Memory

[open,refpage='vkCmdUpdateMemoryKHR',desc='Update the content of device memory from host memory',type='protos']
--
:refpage: vkCmdUpdateMemoryKHR

To update device memory inline in a command buffer, call:

include::{generated}/api/protos/vkCmdUpdateMemoryKHR.adoc[]
...
command is called, and then copied to pname:dstRange when the command is
executed on a device.

[NOTE]
====
Due to the data being copied into command buffers, <<copies-memory-ranges,
memory range copies>> are recommended for larger data transfers, to avoid
...
include::{chapters}/commonvalidity/address_flags_common.adoc[]
  ...
  * [[VUID-vkCmdUpdateMemoryKHR-pDstRange-13007]]
    pname:pDstRange->size must: be less than or equal to 65536
  * [[VUID-vkCmdUpdateMemoryKHR-dataSize-13008]]
    pname:dataSize must: be less than or equal to pname:pDstRange->size
...
include::{generated}/validity/protos/vkCmdUpdateMemoryKHR.adoc[]
--
...
ifndef::VK_BASE_VERSION_1_1,VK_KHR_maintenance1[The]
