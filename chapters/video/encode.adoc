...
ifdef::VK_KHR_video_encode_quantization_map[]
  * The image subregion corresponding to a
    <<encode-quantization-map,quantization map>> used in the video encode
    operation with access ename:VK_ACCESS_2_VIDEO_ENCODE_READ_BIT_KHR.
...
ifdef::VK_KHR_unified_image_layouts[]
    , unless the <<features-unifiedImageLayoutsVideo,
    pname:unifiedImageLayoutsVideo>> feature is enabled, in which case it
    may: be in the ename:VK_IMAGE_LAYOUT_GENERAL layout
...
ifdef::VK_KHR_video_encode_quantization_map[]
  * If the image subresource is used in the video encode operation as a
    <<encode-quantization-map,quantization map>>, then it must: be in the
    ename:VK_IMAGE_LAYOUT_VIDEO_ENCODE_QUANTIZATION_MAP_KHR layout
ifdef::VK_KHR_unified_image_layouts[]
    , unless the <<features-unifiedImageLayoutsVideo,
    pname:unifiedImageLayoutsVideo>> feature is enabled, in which case it
    may: be in the ename:VK_IMAGE_LAYOUT_GENERAL layout
...
ifdef::VK_KHR_video_encode_av1[]
  * If the used video codec operation is
    ...
    codec-specific aspects of the video encoding process are performed as
    defined in the <<encode-av1,AV1 Encode Operations>> section.
...
picture resource>>, regardless of whether <<encode-ref-pic-setup,reference
...
--
ifdef::VK_KHR_video_encode_quantization_map[]
  * ename:VK_VIDEO_ENCODE_CAPABILITY_QUANTIZATION_DELTA_MAP_BIT_KHR
    specifies support for using <<encode-quantization-delta-map,quantization
    delta maps>>.
  * ename:VK_VIDEO_ENCODE_CAPABILITY_EMPHASIS_MAP_BIT_KHR specifies support
    for using <<encode-emphasis-map,emphasis maps>>.
endif::VK_KHR_video_encode_quantization_map[]
...
[open,refpage='VkVideoEncodeFeedback2CapabilitiesKHR',desc='Structure describing additional video encode feedback capabilities for a video profile',type='structs']
--
The sname:VkVideoEncodeFeedback2CapabilitiesKHR structure is defined as:

include::{generated}/api/structs/VkVideoEncodeFeedback2CapabilitiesKHR.adoc[]

  * pname:sType is a elink:VkStructureType value identifying this structure.
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:maxPerPartitionFeedbackEntries specifies the maximum number of
    picture partitions for which the implementation supports capturing per
    picture partition video encode feedback values.
  * pname:supportedPerPartitionEncodeFeedbackFlags is a bitmask of
    elink:VkVideoEncodePerPartitionFeedbackFlagBitsKHR values specifying the
...
include::{generated}/validity/structs/VkVideoEncodeFeedback2CapabilitiesKHR.adoc[]
--
...
ifdef::VK_KHR_video_encode_av1[]
  * [[VUID-vkGetPhysicalDeviceVideoEncodeQualityLevelPropertiesKHR-pQualityLevelInfo-10305]]
    If pname:pQualityLevelInfo->pVideoProfile->videoCodecOperation is
    ename:VK_VIDEO_CODEC_OPERATION_ENCODE_AV1_BIT_KHR, then the pname:pNext
    chain of pname:pQualityLevelProperties must: include a
    slink:VkVideoEncodeAV1QualityLevelPropertiesKHR structure
endif::VK_KHR_video_encode_av1[]
****
...
ename:VK_ERROR_NOT_ENOUGH_SPACE_KHR error status, and writes zero to
pname:pDataSize.
====
...
ifdef::VK_KHR_video_encode_quantization_map[]
  * If there is a bound video session parameters object created with
    ...
    <<encode-quantization-map-texel-size,quantization map texel size>> the
...
ifdef::VK_KHR_video_encode_av1[]
  ** If the bound video session object was created with an
     <<encode-av1-profile,AV1 encode profile>>, then let
     pname:maxCodingBlockSize be equal to the maximum AV1 superblock size
     that may: be used by the video encode operation derived as the maximum
     of the superblock sizes corresponding to the
     elink:VkVideoEncodeAV1SuperblockSizeFlagBitsKHR bits set in
     slink:VkVideoEncodeAV1CapabilitiesKHR::pname:superblockSizes, as
...
ifdef::VK_KHR_video_encode_intra_refresh[]
  * If the bound video session was created with the video codec operation
    ename:VK_VIDEO_CODEC_OPERATION_ENCODE_H264_BIT_KHR and with the
    <<encode-intra-refresh-modes,intra refresh mode>>
    ename:VK_VIDEO_ENCODE_INTRA_REFRESH_MODE_PER_PICTURE_PARTITION_BIT_KHR,
    pname:pEncodeInfo->flags includes
    ename:VK_VIDEO_ENCODE_INTRA_REFRESH_BIT_KHR, and the pname:pNext chain
    of pname:pEncodeInfo includes a slink:VkVideoEncodeIntraRefreshInfoKHR
    structure, then let `uint32_t intraRefreshH264SliceIndex` be the
    <<encode-intra-refresh-index,intra refresh index>> specified in
    slink:VkVideoEncodeIntraRefreshInfoKHR::pname:intraRefreshIndex.
...
ifdef::VK_KHR_video_encode_intra_refresh[]
  * If the bound video session was created with the video codec operation
    ename:VK_VIDEO_CODEC_OPERATION_ENCODE_H265_BIT_KHR and with the
    <<encode-intra-refresh-modes,intra refresh mode>>
    ename:VK_VIDEO_ENCODE_INTRA_REFRESH_MODE_PER_PICTURE_PARTITION_BIT_KHR,
    pname:pEncodeInfo->flags includes
    ename:VK_VIDEO_ENCODE_INTRA_REFRESH_BIT_KHR, and the pname:pNext chain
...
ifdef::VK_KHR_video_encode_av1[]
  * If the bound video session object was created with an
    <<encode-av1-profile,AV1 encode profile>>, then:
  ** If the pname:primaryReferenceCdfOnly member of the
     slink:VkVideoEncodeAV1PictureInfoKHR structure included in the
     pname:pEncodeInfo->pNext chain is set to ename:VK_TRUE, then let
     `int32_t cdfOnlyReferenceIndex` be the value of
     slink:VkVideoEncodeAV1PictureInfoKHR::pname:pStdPictureInfo->primary_ref_frame.
...
****
  ...
  * [[VUID-vkCmdEncodeVideoKHR-None-08318]]
    ...
    been created with the currently set <<encode-quality-level,video encode
    quality level>> for the bound video session at the time the command is
...
ifdef::VK_KHR_video_maintenance1[]
  * [[VUID-vkCmdEncodeVideoKHR-pNext-08360]]
    ...
    slink:VkVideoInlineQueryInfoKHR::pname:queryCount must: equal
    pname:opCount
  ...
  * [[VUID-vkCmdEncodeVideoKHR-pEncodeInfo-08210]]
    ...
    created with the ename:VK_IMAGE_USAGE_VIDEO_ENCODE_SRC_BIT_KHR usage
    flag set
...
ifdef::VK_KHR_video_encode_quantization_map[]
  * [[VUID-vkCmdEncodeVideoKHR-pEncodeInfo-10306]]
    If pname:pEncodeInfo->flags includes
    ename:VK_VIDEO_ENCODE_WITH_QUANTIZATION_DELTA_MAP_BIT_KHR, then the
    bound video session must: have been created with
    ename:VK_VIDEO_SESSION_CREATE_ALLOW_ENCODE_QUANTIZATION_DELTA_MAP_BIT_KHR
  * [[VUID-vkCmdEncodeVideoKHR-pEncodeInfo-10307]]
    If pname:pEncodeInfo->flags includes
    ename:VK_VIDEO_ENCODE_WITH_EMPHASIS_MAP_BIT_KHR, then the bound video
    session must: have been created with
    ename:VK_VIDEO_SESSION_CREATE_ALLOW_ENCODE_EMPHASIS_MAP_BIT_KHR
  * [[VUID-vkCmdEncodeVideoKHR-pEncodeInfo-10308]]
    If the current <<encode-rate-control-modes,rate control mode>> is
    ename:VK_VIDEO_ENCODE_RATE_CONTROL_MODE_DEFAULT_KHR or
    ename:VK_VIDEO_ENCODE_RATE_CONTROL_MODE_DISABLED_BIT_KHR, then
    pname:pEncodeInfo->flags must: not include
    ename:VK_VIDEO_ENCODE_WITH_EMPHASIS_MAP_BIT_KHR
  * [[VUID-vkCmdEncodeVideoKHR-pEncodeInfo-10309]]
    If pname:pEncodeInfo->flags includes
    ename:VK_VIDEO_ENCODE_WITH_QUANTIZATION_DELTA_MAP_BIT_KHR or
    ename:VK_VIDEO_ENCODE_WITH_EMPHASIS_MAP_BIT_KHR, then the pname:pNext
    chain of pname:pEncodeInfo must: include a
    slink:VkVideoEncodeQuantizationMapInfoKHR structure with its
    pname:quantizationMap member specifying a valid sname:VkImageView handle
  * [[VUID-vkCmdEncodeVideoKHR-pEncodeInfo-10310]]
    If pname:pEncodeInfo->flags includes
    ename:VK_VIDEO_ENCODE_WITH_QUANTIZATION_DELTA_MAP_BIT_KHR or
    ename:VK_VIDEO_ENCODE_WITH_EMPHASIS_MAP_BIT_KHR, then the
    sname:VkImageView specified by the pname:quantizationMap member of the
    slink:VkVideoEncodeQuantizationMapInfoKHR structure included in the
    pname:pNext chain must: be <<video-profile-compatibility,compatible>>
  ...
  * [[VUID-vkCmdEncodeVideoKHR-pEncodeInfo-10311]]
    ...
    sname:VkImageView specified by the pname:quantizationMap member of the
    slink:VkVideoEncodeQuantizationMapInfoKHR structure included in the
    pname:pNext chain of pname:pEncodeInfo must: have been created with the
    ename:VK_IMAGE_USAGE_VIDEO_ENCODE_QUANTIZATION_DELTA_MAP_BIT_KHR usage
    flag set
  * [[VUID-vkCmdEncodeVideoKHR-pEncodeInfo-10312]]
    ...
    slink:VkVideoEncodeQuantizationMapInfoKHR structure included in the
    pname:pNext chain of pname:pEncodeInfo must: have been created with the
    ename:VK_IMAGE_USAGE_VIDEO_ENCODE_EMPHASIS_MAP_BIT_KHR usage flag set
  * [[VUID-vkCmdEncodeVideoKHR-pNext-10313]]
    If an instance of the slink:VkVideoEncodeQuantizationMapInfoKHR
    structure is included in the pname:pNext chain of pname:pEncodeInfo, its
    pname:quantizationMap member is not dlink:VK_NULL_HANDLE,
    pname:commandBuffer is an unprotected command buffer, and
    <<limits-protectedNoFault, pname:protectedNoFault>> is not supported,
    then slink:VkVideoEncodeQuantizationMapInfoKHR::pname:quantizationMap
    must: not have been created from a protected image
  * [[VUID-vkCmdEncodeVideoKHR-pNext-10314]]
    If an instance of the slink:VkVideoEncodeQuantizationMapInfoKHR
    structure is included in the pname:pNext chain of pname:pEncodeInfo and
    its pname:quantizationMap member is not dlink:VK_NULL_HANDLE, then the
    image subresource range referenced by pname:quantizationMap must: be in
    the ename:VK_IMAGE_LAYOUT_VIDEO_ENCODE_QUANTIZATION_MAP_KHR layout at
...
ifdef::VK_KHR_unified_image_layouts[]
  ...
  * [[VUID-vkCmdEncodeVideoKHR-pNext-10315]]
    If an instance of the slink:VkVideoEncodeQuantizationMapInfoKHR
    structure is included in the pname:pNext chain of pname:pEncodeInfo and
    its pname:quantizationMap member is not dlink:VK_NULL_HANDLE, and there
  ...
  * [[VUID-vkCmdEncodeVideoKHR-pNext-10316]]
    If an instance of the slink:VkVideoEncodeQuantizationMapInfoKHR
    structure is included in the pname:pNext chain of pname:pEncodeInfo, its
    pname:quantizationMap member is not dlink:VK_NULL_HANDLE, and there is a
    bound video session parameters object created with
    ename:VK_VIDEO_SESSION_PARAMETERS_CREATE_QUANTIZATION_MAP_COMPATIBLE_BIT_KHR,
    then pname:quantizationMapExtent must: equal
    [eq]#{lceil}pname:pEncodeInfo->srcPictureResource.codedExtent /
    pname:quantizationMapTexelSize{rceil}#
endif::VK_KHR_video_encode_quantization_map[]
ifdef::VK_KHR_video_encode_intra_refresh[]
  * [[VUID-vkCmdEncodeVideoKHR-pEncodeInfo-10837]]
    If pname:pEncodeInfo->flags includes
    ename:VK_VIDEO_ENCODE_INTRA_REFRESH_BIT_KHR, then the
    <<encode-intra-refresh-modes,intra refresh mode>> the bound video
    session was created with must: not be
    ename:VK_VIDEO_ENCODE_INTRA_REFRESH_MODE_NONE_KHR
  * [[VUID-vkCmdEncodeVideoKHR-pEncodeInfo-10838]]
    If pname:pEncodeInfo->flags includes
    ename:VK_VIDEO_ENCODE_INTRA_REFRESH_BIT_KHR, then
    pname:pEncodeInfo->activeReferencePictureCount must: be less than or
    equal to
    slink:VkVideoEncodeIntraRefreshCapabilitiesKHR::pname:maxIntraRefreshActiveReferencePictures,
    as returned by flink:vkGetPhysicalDeviceVideoCapabilitiesKHR for the
    video profile the bound video session was created with
  * [[VUID-vkCmdEncodeVideoKHR-pEncodeInfo-10839]]
    If pname:pEncodeInfo->flags includes
    ename:VK_VIDEO_ENCODE_INTRA_REFRESH_BIT_KHR, then the pname:pNext chain
    of pname:pEncodeInfo must: include a
    slink:VkVideoEncodeIntraRefreshInfoKHR structure
  * [[VUID-vkCmdEncodeVideoKHR-pEncodeInfo-10840]]
    If pname:pEncodeInfo->flags includes
    ename:VK_VIDEO_ENCODE_INTRA_REFRESH_BIT_KHR, then the
    pname:intraRefreshCycleDuration member of the
    slink:VkVideoEncodeIntraRefreshInfoKHR structure included in the
    pname:pNext chain of pname:pEncodeInfo must: not be zero
  * [[VUID-vkCmdEncodeVideoKHR-pEncodeInfo-10841]]
    If pname:pEncodeInfo->flags includes
    ename:VK_VIDEO_ENCODE_INTRA_REFRESH_BIT_KHR, then the
    pname:intraRefreshIndex member of the
    slink:VkVideoEncodeIntraRefreshInfoKHR structure included in the
    pname:pNext chain of pname:pEncodeInfo must: be less than
    slink:VkVideoEncodeIntraRefreshInfoKHR::pname:intraRefreshCycleDuration
  * [[VUID-vkCmdEncodeVideoKHR-pNext-10842]]
    If the pname:pNext chain of any element of
    pname:pEncodeInfo->pReferenceSlots includes a
    slink:VkVideoReferenceIntraRefreshInfoKHR structure with a non-zero
    pname:dirtyIntraRefreshRegions member, then pname:pEncodeInfo->flags
    must: include ename:VK_VIDEO_ENCODE_INTRA_REFRESH_BIT_KHR
  * [[VUID-vkCmdEncodeVideoKHR-pNext-10843]]
    If the pname:pNext chain of any element of
    pname:pEncodeInfo->pReferenceSlots includes a
    slink:VkVideoReferenceIntraRefreshInfoKHR structure with a non-zero
    pname:dirtyIntraRefreshRegions member, then for each such element
    pname:dirtyIntraRefreshRegions must: equal
    pname:intraRefreshCycleDuration minus pname:intraRefreshIndex where
    pname:intraRefreshCycleDuration and pname:intraRefreshIndex are the
    members of the slink:VkVideoEncodeIntraRefreshInfoKHR structure included
    in the pname:pNext chain of pname:pEncodeInfo
  * [[VUID-vkCmdEncodeVideoKHR-pNext-10844]]
    If the pname:pNext chain of pname:pEncodeInfo includes a
    slink:VkVideoEncodeIntraRefreshInfoKHR structure, then its
    pname:intraRefreshCycleDuration member must: be zero or must: be between
    `2` and
    slink:VkVideoEncodeIntraRefreshCapabilitiesKHR::pname:maxIntraRefreshCycleDuration,
    as returned by flink:vkGetPhysicalDeviceVideoCapabilitiesKHR for the
    video profile the bound video session was created with
endif::VK_KHR_video_encode_intra_refresh[]
ifdef::VK_KHR_video_encode_h264[]
  ...
  * [[VUID-vkCmdEncodeVideoKHR-naluSliceEntryCount-08312]]
    ...
    pname:minCodingBlockExtent.height
ifdef::VK_KHR_video_encode_intra_refresh[]
  * [[VUID-vkCmdEncodeVideoKHR-pEncodeInfo-10845]]
    If the bound video session was created with the video codec operation
    ename:VK_VIDEO_CODEC_OPERATION_ENCODE_H264_BIT_KHR and with an
    <<encode-intra-refresh-modes,intra refresh mode>> other than
    ename:VK_VIDEO_ENCODE_INTRA_REFRESH_MODE_PER_PICTURE_PARTITION_BIT_KHR,
    pname:pEncodeInfo->flags includes
    ename:VK_VIDEO_ENCODE_INTRA_REFRESH_BIT_KHR, and
    slink:VkVideoEncodeIntraRefreshCapabilitiesKHR::pname:partitionIndependentIntraRefreshRegions
    is ename:VK_FALSE, as returned by
    flink:vkGetPhysicalDeviceVideoCapabilitiesKHR for the video profile the
    bound video session was created with, then the pname:naluSliceEntryCount
    member of the slink:VkVideoEncodeH264PictureInfoKHR structure included
    in the pname:pNext chain of pname:pEncodeInfo must: equal `1`
  * [[VUID-vkCmdEncodeVideoKHR-pEncodeInfo-10846]]
    If the bound video session was created with the video codec operation
    ename:VK_VIDEO_CODEC_OPERATION_ENCODE_H264_BIT_KHR and with the
    <<encode-intra-refresh-modes,intra refresh mode>>
    ename:VK_VIDEO_ENCODE_INTRA_REFRESH_MODE_PER_PICTURE_PARTITION_BIT_KHR,
    and pname:pEncodeInfo->flags includes
    ename:VK_VIDEO_ENCODE_INTRA_REFRESH_BIT_KHR, then the
    pname:naluSliceEntryCount member of the
    slink:VkVideoEncodeH264PictureInfoKHR structure included in the
    pname:pNext chain of pname:pEncodeInfo must: equal the
    pname:intraRefreshCycleDuration member of the
    slink:VkVideoEncodeIntraRefreshInfoKHR structure included in the
    pname:pNext chain of pname:pEncodeInfo
  * [[VUID-vkCmdEncodeVideoKHR-intraRefreshH264SliceIndex-10847]]
    If pname:intraRefreshH264SliceIndex is defined, then
    pname:pNaluSliceEntries[pname:intraRefreshH264SliceIndex].pname:pStdSliceHeader->slice_type
    must: be code:STD_VIDEO_H264_SLICE_TYPE_I in the
    slink:VkVideoEncodeH264PictureInfoKHR structure included in the
    pname:pNext chain of pname:pEncodeInfo
  * [[VUID-vkCmdEncodeVideoKHR-pEncodeInfo-10848]]
    If the bound video session was created with the video codec operation
    ename:VK_VIDEO_CODEC_OPERATION_ENCODE_H264_BIT_KHR and with the
    <<encode-intra-refresh-modes,intra refresh mode>>
    ename:VK_VIDEO_ENCODE_INTRA_REFRESH_MODE_PER_PICTURE_PARTITION_BIT_KHR,
    pname:pEncodeInfo->flags includes
    ename:VK_VIDEO_ENCODE_INTRA_REFRESH_BIT_KHR, and
    slink:VkVideoEncodeIntraRefreshCapabilitiesKHR::pname:nonRectangularIntraRefreshRegions
    is ename:VK_FALSE, as returned by
    flink:vkGetPhysicalDeviceVideoCapabilitiesKHR for the video profile the
    bound video session was created with, then the pname:naluSliceEntryCount
    member of the slink:VkVideoEncodeH264PictureInfoKHR structure included
    in the pname:pNext chain of pname:pEncodeInfo must: be less than or
    equal to pname:minCodingBlockExtent.height
  * [[VUID-vkCmdEncodeVideoKHR-h264PictureType-10849]]
    If the bound video session was created with the video codec operation
    ename:VK_VIDEO_CODEC_OPERATION_ENCODE_H264_BIT_KHR,
    pname:h264PictureType is code:STD_VIDEO_H264_PICTURE_TYPE_B, and
    slink:VkVideoEncodeH264CapabilitiesKHR::pname:flags, as returned by
    flink:vkGetPhysicalDeviceVideoCapabilitiesKHR for the video profile the
    bound video session was created with, does not include
    ename:VK_VIDEO_ENCODE_H264_CAPABILITY_B_PICTURE_INTRA_REFRESH_BIT_KHR,
    then pname:pEncodeInfo->flags must: not include
    ename:VK_VIDEO_ENCODE_INTRA_REFRESH_BIT_KHR
endif::VK_KHR_video_encode_intra_refresh[]
  * [[VUID-vkCmdEncodeVideoKHR-flags-10850]]
    If the bound video session was created with the video codec operation
    ename:VK_VIDEO_CODEC_OPERATION_ENCODE_H264_BIT_KHR and
    slink:VkVideoEncodeH264CapabilitiesKHR::pname:flags, as returned by
    flink:vkGetPhysicalDeviceVideoCapabilitiesKHR for the video profile the
    ...
    pname:pNaluSliceEntries[[eq]#i#].pname:pStdSliceHeader->slice_type must:
    be identical for all elements [eq]#i# of the pname:pNaluSliceEntries
    member of the slink:VkVideoEncodeH264PictureInfoKHR structure included
    in the pname:pNext chain of pname:pEncodeInfo
ifdef::VK_KHR_video_encode_intra_refresh[]
    , except for element index [eq]#i# equal to
    pname:intraRefreshH264SliceIndex, if pname:intraRefreshH264SliceIndex is
    defined
endif::VK_KHR_video_encode_intra_refresh[]
...
ifdef::VK_KHR_video_encode_h265[]
  ...
  * [[VUID-vkCmdEncodeVideoKHR-naluSliceSegmentEntryCount-08313]]
    ...
    pname:minCodingBlockExtent.height
ifdef::VK_KHR_video_encode_intra_refresh[]
  * [[VUID-vkCmdEncodeVideoKHR-pEncodeInfo-10851]]
    If the bound video session was created with the video codec operation
    ename:VK_VIDEO_CODEC_OPERATION_ENCODE_H265_BIT_KHR and with an
    <<encode-intra-refresh-modes,intra refresh mode>> other than
    ename:VK_VIDEO_ENCODE_INTRA_REFRESH_MODE_PER_PICTURE_PARTITION_BIT_KHR,
    pname:pEncodeInfo->flags includes
    ename:VK_VIDEO_ENCODE_INTRA_REFRESH_BIT_KHR, and
    ...
    pname:naluSliceSegmentEntryCount member of the
    slink:VkVideoEncodeH265PictureInfoKHR structure included in the
    pname:pNext chain of pname:pEncodeInfo must: equal `1`
  * [[VUID-vkCmdEncodeVideoKHR-pEncodeInfo-10852]]
    If the bound video session was created with the video codec operation
    ename:VK_VIDEO_CODEC_OPERATION_ENCODE_H265_BIT_KHR and with the
    <<encode-intra-refresh-modes,intra refresh mode>>
    ename:VK_VIDEO_ENCODE_INTRA_REFRESH_MODE_PER_PICTURE_PARTITION_BIT_KHR,
    ...
    pname:naluSliceSegmentEntryCount member of the
    slink:VkVideoEncodeH265PictureInfoKHR structure included in the
    pname:pNext chain of pname:pEncodeInfo must: equal
    slink:VkVideoEncodeIntraRefreshInfoKHR::pname:intraRefreshCycleDuration
  * [[VUID-vkCmdEncodeVideoKHR-intraRefreshH265SliceSegmentIndex-10853]]
    If pname:intraRefreshH265SliceSegmentIndex is defined, then
    pname:pNaluSliceSegmentEntries[pname:intraRefreshH265SliceSegmentIndex].pname:pStdSliceSegmentHeader->slice_type
    must: be code:STD_VIDEO_H265_SLICE_TYPE_I in the
    slink:VkVideoEncodeH265PictureInfoKHR structure included in the
    pname:pNext chain of pname:pEncodeInfo
  * [[VUID-vkCmdEncodeVideoKHR-pEncodeInfo-10854]]
    If the bound video session was created with the video codec operation
    ename:VK_VIDEO_CODEC_OPERATION_ENCODE_H265_BIT_KHR and with the
    <<encode-intra-refresh-modes,intra refresh mode>>
    ename:VK_VIDEO_ENCODE_INTRA_REFRESH_MODE_PER_PICTURE_PARTITION_BIT_KHR,
    ...
    pname:pNext chain of pname:pEncodeInfo must: be less than or equal to
    pname:minCodingBlockExtent.height
  * [[VUID-vkCmdEncodeVideoKHR-h265PictureType-10855]]
    If the bound video session was created with the video codec operation
    ename:VK_VIDEO_CODEC_OPERATION_ENCODE_H265_BIT_KHR,
    pname:h265PictureType is code:STD_VIDEO_H265_PICTURE_TYPE_B, and
    slink:VkVideoEncodeH265CapabilitiesKHR::pname:flags, as returned by
    flink:vkGetPhysicalDeviceVideoCapabilitiesKHR for the video profile the
    bound video session was created with, does not include
    ename:VK_VIDEO_ENCODE_H265_CAPABILITY_B_PICTURE_INTRA_REFRESH_BIT_KHR,
    then pname:pEncodeInfo->flags must: not include
    ename:VK_VIDEO_ENCODE_INTRA_REFRESH_BIT_KHR
endif::VK_KHR_video_encode_intra_refresh[]
  * [[VUID-vkCmdEncodeVideoKHR-flags-10856]]
    If the bound video session was created with the video codec operation
    ename:VK_VIDEO_CODEC_OPERATION_ENCODE_H265_BIT_KHR and
    slink:VkVideoEncodeH265CapabilitiesKHR::pname:flags, as returned by
    ...
    then
    pname:pNaluSliceSegmentEntries[[eq]#i#].pname:pStdSliceSegmentHeader->slice_type
    must: be identical for all elements [eq]#i# of the
    pname:pNaluSliceSegmentEntries member of the
    slink:VkVideoEncodeH265PictureInfoKHR structure included in the
    pname:pNext chain of pname:pEncodeInfo
ifdef::VK_KHR_video_encode_intra_refresh[]
    , except for element index [eq]#i# equal to
    pname:intraRefreshH265SliceSegmentIndex, if
    pname:intraRefreshH265SliceSegmentIndex is defined
endif::VK_KHR_video_encode_intra_refresh[]
...
ifdef::VK_KHR_video_encode_av1[]
  * [[VUID-vkCmdEncodeVideoKHR-pNext-10317]]
    ...
    ename:VK_VIDEO_CODEC_OPERATION_ENCODE_AV1_BIT_KHR, then the pname:pNext
    chain of pname:pEncodeInfo must: include a
    slink:VkVideoEncodeAV1PictureInfoKHR structure
  * [[VUID-vkCmdEncodeVideoKHR-pEncodeInfo-10318]]
    If the bound video session was created with the video codec operation
    ename:VK_VIDEO_CODEC_OPERATION_ENCODE_AV1_BIT_KHR and
    pname:pEncodeInfo->pSetupReferenceSlot is not `NULL`, then the
    pname:pNext chain of pname:pEncodeInfo->pSetupReferenceSlot must:
    include a slink:VkVideoEncodeAV1DpbSlotInfoKHR structure
  ...
  * [[VUID-vkCmdEncodeVideoKHR-constantQIndex-10320]]
    If the bound video session was created with the video codec operation
    ename:VK_VIDEO_CODEC_OPERATION_ENCODE_AV1_BIT_KHR and the current
    <<encode-rate-control-modes,rate control mode>> is not
    ename:VK_VIDEO_ENCODE_RATE_CONTROL_MODE_DISABLED_BIT_KHR, then the
    pname:constantQIndex member of the slink:VkVideoEncodeAV1PictureInfoKHR
    structure included in the pname:pNext chain of pname:pEncodeInfo must:
    be zero
  * [[VUID-vkCmdEncodeVideoKHR-constantQIndex-10321]]
    ...
    pname:constantQIndex member of the slink:VkVideoEncodeAV1PictureInfoKHR
    structure included in the pname:pNext chain of pname:pEncodeInfo must:
    be between slink:VkVideoEncodeAV1CapabilitiesKHR::pname:minQIndex and
  ...
  * [[VUID-vkCmdEncodeVideoKHR-flags-10322]]
    If the bound video session was created with the video codec operation
    ename:VK_VIDEO_CODEC_OPERATION_ENCODE_AV1_BIT_KHR and
    slink:VkVideoEncodeAV1CapabilitiesKHR::pname:flags does not include
    ename:VK_VIDEO_ENCODE_AV1_CAPABILITY_FRAME_SIZE_OVERRIDE_BIT_KHR, as
    returned by flink:vkGetPhysicalDeviceVideoCapabilitiesKHR for the video
    profile the bound video session was created with, then
    slink:VkVideoEncodeAV1PictureInfoKHR::pname:pStdPictureInfo->flags.frame_size_override_flag
    must: be zero for the slink:VkVideoEncodeAV1PictureInfoKHR structure
    included in the pname:pNext chain of pname:pEncodeInfo
  * [[VUID-vkCmdEncodeVideoKHR-flags-10323]]
    ...
    returned by flink:vkGetPhysicalDeviceVideoCapabilitiesKHR for the video
    profile the bound video session was created with, then
    pname:pEncodeInfo->srcPictureResource.codedExtent.width must: equal
    [eq]#code:StdVideoAV1SequenceHeader::code:max_frame_width_minus_1 {plus}
    1# of the <<encode-av1-active-sequence-header,active AV1 sequence
    header>>
  * [[VUID-vkCmdEncodeVideoKHR-flags-10324]]
    ...
    returned by flink:vkGetPhysicalDeviceVideoCapabilitiesKHR for the video
    profile the bound video session was created with, then
    pname:pEncodeInfo->srcPictureResource.codedExtent.height must: equal
    [eq]#code:StdVideoAV1SequenceHeader::code:max_frame_height_minus_1
    {plus} 1# of the <<encode-av1-active-sequence-header,active AV1 sequence
    header>>
  * [[VUID-vkCmdEncodeVideoKHR-flags-10325]]
    If the bound video session was created with the video codec operation
    ename:VK_VIDEO_CODEC_OPERATION_ENCODE_AV1_BIT_KHR and
    slink:VkVideoEncodeAV1CapabilitiesKHR::pname:flags does not include
    ename:VK_VIDEO_ENCODE_AV1_CAPABILITY_MOTION_VECTOR_SCALING_BIT_KHR, as
    returned by flink:vkGetPhysicalDeviceVideoCapabilitiesKHR for the video
    profile the bound video session was created with, then for each element
    [eq]#i# of pname:pEncodeInfo->pReferenceSlots
    pname:pEncodeInfo->pReferenceSlots[i].pname:pPictureResource->codedExtent
    must: match pname:pEncodeInfo->srcPictureResource.codedExtent
  * [[VUID-vkCmdEncodeVideoKHR-predictionMode-10326]]
    If the bound video session was created with the video codec operation
    ename:VK_VIDEO_CODEC_OPERATION_ENCODE_AV1_BIT_KHR and the
    pname:predictionMode member of the slink:VkVideoEncodeAV1PictureInfoKHR
    structure included in the pname:pNext chain of pname:pEncodeInfo is
    ename:VK_VIDEO_ENCODE_AV1_PREDICTION_MODE_INTRA_ONLY_KHR, then
    slink:VkVideoEncodeAV1PictureInfoKHR::pname:pStdPictureInfo->frame_type
    must: be code:STD_VIDEO_AV1_FRAME_TYPE_KEY or
    code:STD_VIDEO_AV1_FRAME_TYPE_INTRA_ONLY
  * [[VUID-vkCmdEncodeVideoKHR-pStdPictureInfo-10327]]
    If the bound video session was created with the video codec operation
    ename:VK_VIDEO_CODEC_OPERATION_ENCODE_AV1_BIT_KHR and
    pname:pStdPictureInfo->frame_type for the pname:pStdPictureInfo member
    of the slink:VkVideoEncodeAV1PictureInfoKHR structure included in the
    pname:pNext chain of pname:pEncodeInfo is
    code:STD_VIDEO_AV1_FRAME_TYPE_KEY or
    code:STD_VIDEO_AV1_FRAME_TYPE_INTRA_ONLY, then
    slink:VkVideoEncodeAV1PictureInfoKHR::pname:predictionMode must: be
    ename:VK_VIDEO_ENCODE_AV1_PREDICTION_MODE_INTRA_ONLY_KHR
  * [[VUID-vkCmdEncodeVideoKHR-maxSingleReferenceCount-10328]]
    If the bound video session was created with the video codec operation
    ename:VK_VIDEO_CODEC_OPERATION_ENCODE_AV1_BIT_KHR and
    slink:VkVideoEncodeAV1CapabilitiesKHR::pname:maxSingleReferenceCount is
    zero, as returned by flink:vkGetPhysicalDeviceVideoCapabilitiesKHR for
    the video profile the bound video session was created with, then the
    pname:predictionMode member of the slink:VkVideoEncodeAV1PictureInfoKHR
    structure included in the pname:pNext chain of pname:pEncodeInfo must:
    not be ename:VK_VIDEO_ENCODE_AV1_PREDICTION_MODE_SINGLE_REFERENCE_KHR
  * [[VUID-vkCmdEncodeVideoKHR-predictionMode-10329]]
    If the bound video session was created with the video codec operation
    ename:VK_VIDEO_CODEC_OPERATION_ENCODE_AV1_BIT_KHR and the
    pname:predictionMode member of the slink:VkVideoEncodeAV1PictureInfoKHR
    structure included in the pname:pNext chain of pname:pEncodeInfo is
    ename:VK_VIDEO_ENCODE_AV1_PREDICTION_MODE_SINGLE_REFERENCE_KHR, then
    there must: be at least one non-negative element of
    slink:VkVideoEncodeAV1PictureInfoKHR::pname:referenceNameSlotIndices
    with element index [eq]#i# that does not equal
    pname:cdfOnlyReferenceIndex and for which bit index [eq]#i# is set in
    slink:VkVideoEncodeAV1CapabilitiesKHR::pname:singleReferenceNameMask, as
    returned by flink:vkGetPhysicalDeviceVideoCapabilitiesKHR for the video
    profile the bound video session was created with
  * [[VUID-vkCmdEncodeVideoKHR-maxUnidirectionalCompoundReferenceCount-10330]]
    If the bound video session was created with the video codec operation
    ename:VK_VIDEO_CODEC_OPERATION_ENCODE_AV1_BIT_KHR and
    slink:VkVideoEncodeAV1CapabilitiesKHR::pname:maxUnidirectionalCompoundReferenceCount
    is zero, as returned by flink:vkGetPhysicalDeviceVideoCapabilitiesKHR
  ...
  * [[VUID-vkCmdEncodeVideoKHR-predictionMode-10331]]
    ...
    slink:VkVideoEncodeAV1PictureInfoKHR::pname:referenceNameSlotIndices
    with element indices [eq]#i# and [eq]#j# where [eq]#(i,j) {elem}
    {(0,1),(0,2),(0,3),(4,6)}#, such that neither element equals
    pname:cdfOnlyReferenceIndex and for which bit indices [eq]#i# and
    [eq]#j# are set in
  ...
  * [[VUID-vkCmdEncodeVideoKHR-maxBidirectionalCompoundReferenceCount-10332]]
    If the bound video session was created with the video codec operation
    ename:VK_VIDEO_CODEC_OPERATION_ENCODE_AV1_BIT_KHR and
    slink:VkVideoEncodeAV1CapabilitiesKHR::pname:maxBidirectionalCompoundReferenceCount
    is zero, as returned by flink:vkGetPhysicalDeviceVideoCapabilitiesKHR
    for the video profile the bound video session was created with, then the
    pname:predictionMode member of the slink:VkVideoEncodeAV1PictureInfoKHR
    structure included in the pname:pNext chain of pname:pEncodeInfo must:
  ...
  * [[VUID-vkCmdEncodeVideoKHR-predictionMode-10333]]
    ...
    then there must: be at least two non-negative elements of
    slink:VkVideoEncodeAV1PictureInfoKHR::pname:referenceNameSlotIndices
    with element indices [eq]#i {elem} {0,1,2,3}# and [eq]#j {elem}
    ...
    video profile the bound video session was created with
ifdef::VK_KHR_video_encode_intra_refresh[]
  * [[VUID-vkCmdEncodeVideoKHR-predictionMode-10857]]
    If the bound video session was created with the video codec operation
    ename:VK_VIDEO_CODEC_OPERATION_ENCODE_AV1_BIT_KHR, the
    pname:predictionMode member of the slink:VkVideoEncodeAV1PictureInfoKHR
    structure included in the pname:pNext chain of pname:pEncodeInfo is
    ename:VK_VIDEO_ENCODE_AV1_PREDICTION_MODE_UNIDIRECTIONAL_COMPOUND_KHR or
    ename:VK_VIDEO_ENCODE_AV1_PREDICTION_MODE_BIDIRECTIONAL_COMPOUND_KHR,
    and slink:VkVideoEncodeAV1CapabilitiesKHR::pname:flags, as returned by
    flink:vkGetPhysicalDeviceVideoCapabilitiesKHR for the video profile the
    bound video session was created with, does not include
    ename:VK_VIDEO_ENCODE_AV1_CAPABILITY_COMPOUND_PREDICTION_INTRA_REFRESH_BIT_KHR,
    then pname:pEncodeInfo->flags must: not include
    ename:VK_VIDEO_ENCODE_INTRA_REFRESH_BIT_KHR
endif::VK_KHR_video_encode_intra_refresh[]
  * [[VUID-vkCmdEncodeVideoKHR-referenceNameSlotIndices-10334]]
    If the bound video session was created with the video codec operation
    ename:VK_VIDEO_CODEC_OPERATION_ENCODE_AV1_BIT_KHR, then each element of
    the pname:referenceNameSlotIndices array member of the
    slink:VkVideoEncodeAV1PictureInfoKHR structure included in the
    pname:pNext chain of pname:pEncodeInfo must: either be negative or must:
  ...
  * [[VUID-vkCmdEncodeVideoKHR-pExtensionHeader-10336]]
    If the bound video session was created with the video codec operation
    ename:VK_VIDEO_CODEC_OPERATION_ENCODE_AV1_BIT_KHR and the
    pname:pExtensionHeader member of
    slink:VkVideoEncodeAV1PictureInfoKHR::pname:pStdPictureInfo for the
    slink:VkVideoEncodeAV1PictureInfoKHR structure included in the
    pname:pNext chain of pname:pEncodeInfo is not `NULL`, then
    pname:pExtensionHeader->temporal_id must: be less than
  ...
  * [[VUID-vkCmdEncodeVideoKHR-pExtensionHeader-10337]]
    ...
    slink:VkVideoEncodeAV1PictureInfoKHR structure included in the
    pname:pNext chain of pname:pEncodeInfo is not `NULL`, then
    pname:pExtensionHeader->spatial_id must: be less than
    slink:VkVideoEncodeAV1CapabilitiesKHR::pname:maxSpatialLayerCount, as
    returned by flink:vkGetPhysicalDeviceVideoCapabilitiesKHR for the video
    profile the bound video session was created with
  * [[VUID-vkCmdEncodeVideoKHR-pEncodeInfo-10338]]
    ...
    pname:pEncodeInfo->pSetupReferenceSlot is not `NULL`, then the
    pname:pExtensionHeader member of
    slink:VkVideoEncodeAV1DpbSlotInfoKHR::pname:pStdReferenceInfo for the
    slink:VkVideoEncodeAV1DpbSlotInfoKHR structure included in the
    pname:pNext chain of pname:pEncodeInfo->pSetupReferenceSlot and the
    pname:pExtensionHeader member of
    slink:VkVideoEncodeAV1PictureInfoKHR::pname:pStdPictureInfo for the
    slink:VkVideoEncodeAV1PictureInfoKHR structure included in the
    pname:pNext chain of pname:pEncodeInfo must: both be `NULL` or not
    `NULL`
  * [[VUID-vkCmdEncodeVideoKHR-pEncodeInfo-10339]]
    If the bound video session was created with the video codec operation
    ename:VK_VIDEO_CODEC_OPERATION_ENCODE_AV1_BIT_KHR,
    pname:pEncodeInfo->pSetupReferenceSlot is not `NULL`, and the
    pname:pExtensionHeader member of
    slink:VkVideoEncodeAV1DpbSlotInfoKHR::pname:pStdReferenceInfo for the
    slink:VkVideoEncodeAV1DpbSlotInfoKHR structure included in the
    pname:pNext chain of pname:pEncodeInfo->pSetupReferenceSlot is not
    `NULL`, then pname:pExtensionHeader->temporal_id must: equal
    pname:pStdPictureInfo->pExtensionHeader->temporal_id in the
    slink:VkVideoEncodeAV1PictureInfoKHR structure included in the
    pname:pNext chain of pname:pEncodeInfo
  * [[VUID-vkCmdEncodeVideoKHR-pEncodeInfo-10340]]
    ...
    slink:VkVideoEncodeAV1DpbSlotInfoKHR structure included in the
    pname:pNext chain of pname:pEncodeInfo->pSetupReferenceSlot is not
    `NULL`, then pname:pExtensionHeader->spatial_id must: equal
    pname:pStdPictureInfo->pExtensionHeader->spatial_id in the
    slink:VkVideoEncodeAV1PictureInfoKHR structure included in the
    pname:pNext chain of pname:pEncodeInfo
  * [[VUID-vkCmdEncodeVideoKHR-pExtensionHeader-10341]]
    ...
    ename:VK_VIDEO_CODEC_OPERATION_ENCODE_AV1_BIT_KHR and the
    pname:pExtensionHeader member of
    slink:VkVideoEncodeAV1DpbSlotInfoKHR::pname:pStdReferenceInfo for any of
    the slink:VkVideoEncodeAV1DpbSlotInfoKHR structures included in the
    pname:pNext chain of any element of pname:pEncodeInfo->pReferenceSlots
    is not `NULL`, then pname:pExtensionHeader->temporal_id must: be less
    than slink:VkVideoEncodeAV1CapabilitiesKHR::pname:maxTemporalLayerCount,
    as returned by flink:vkGetPhysicalDeviceVideoCapabilitiesKHR for the
    video profile the bound video session was created with
  * [[VUID-vkCmdEncodeVideoKHR-pExtensionHeader-10342]]
    ...
    the slink:VkVideoEncodeAV1DpbSlotInfoKHR structures included in the
    pname:pNext chain of any element of pname:pEncodeInfo->pReferenceSlots
    is not `NULL`, then pname:pExtensionHeader->spatial_id must: be less
    than slink:VkVideoEncodeAV1CapabilitiesKHR::pname:maxSpatialLayerCount,
    as returned by flink:vkGetPhysicalDeviceVideoCapabilitiesKHR for the
    video profile the bound video session was created with
  * [[VUID-vkCmdEncodeVideoKHR-pTileInfo-10343]]
    If the bound video session was created with the video codec operation
    ename:VK_VIDEO_CODEC_OPERATION_ENCODE_AV1_BIT_KHR and the
    pname:pTileInfo member of
    slink:VkVideoEncodeAV1PictureInfoKHR::pname:pStdPictureInfo for the
    slink:VkVideoEncodeAV1PictureInfoKHR structure included in the
    pname:pNext chain of pname:pEncodeInfo is not `NULL`, then
    pname:pTileInfo->TileCols must: be greater than `0`
  * [[VUID-vkCmdEncodeVideoKHR-pTileInfo-10344]]
    ...
    slink:VkVideoEncodeAV1PictureInfoKHR structure included in the
    pname:pNext chain of pname:pEncodeInfo is not `NULL`, then
    pname:pTileInfo->TileRows must: be greater than `0`
  * [[VUID-vkCmdEncodeVideoKHR-pTileInfo-10345]]
    ...
    slink:VkVideoEncodeAV1PictureInfoKHR structure included in the
    pname:pNext chain of pname:pEncodeInfo is not `NULL`, then
    pname:pTileInfo->TileCols must: be less than or equal to
    slink:VkVideoEncodeAV1CapabilitiesKHR::pname:maxTiles.width, as returned
    by flink:vkGetPhysicalDeviceVideoCapabilitiesKHR for the video profile
    the bound video session was created with
  * [[VUID-vkCmdEncodeVideoKHR-pTileInfo-10346]]
    ...
    slink:VkVideoEncodeAV1PictureInfoKHR structure included in the
    pname:pNext chain of pname:pEncodeInfo is not `NULL`, then
    pname:pTileInfo->TileRows must: be less than or equal to
    slink:VkVideoEncodeAV1CapabilitiesKHR::pname:maxTiles.height, as
    returned by flink:vkGetPhysicalDeviceVideoCapabilitiesKHR for the video
    profile the bound video session was created with
  * [[VUID-vkCmdEncodeVideoKHR-pTileInfo-10347]]
    ...
    slink:VkVideoEncodeAV1PictureInfoKHR structure included in the
    pname:pNext chain of pname:pEncodeInfo is not `NULL`, then
    [eq]#{lceil}pname:pEncodeInfo->srcPictureResource.codedExtent.width /
    pname:pTileInfo->TileCols{rceil}# must: be between
    slink:VkVideoEncodeAV1CapabilitiesKHR::pname:minTileSize.width and
    slink:VkVideoEncodeAV1CapabilitiesKHR::pname:maxTileSize.width,
  ...
  * [[VUID-vkCmdEncodeVideoKHR-pTileInfo-10348]]
    ...
    slink:VkVideoEncodeAV1PictureInfoKHR structure included in the
    pname:pNext chain of pname:pEncodeInfo is not `NULL`, then
    [eq]#{lceil}pname:pEncodeInfo->srcPictureResource.codedExtent.height /
    pname:pTileInfo->TileRows{rceil}# must: be between
    slink:VkVideoEncodeAV1CapabilitiesKHR::pname:minTileSize.height and
    slink:VkVideoEncodeAV1CapabilitiesKHR::pname:maxTileSize.height,
    inclusive, as returned by flink:vkGetPhysicalDeviceVideoCapabilitiesKHR
    for the video profile the bound video session was created with
  * [[VUID-vkCmdEncodeVideoKHR-pStdPictureInfo-10349]]
    If the bound video session was created with the video codec operation
    ename:VK_VIDEO_CODEC_OPERATION_ENCODE_AV1_BIT_KHR, then
    slink:VkVideoEncodeAV1PictureInfoKHR::pname:pStdPictureInfo->flags.segmentation_enabled
    for the slink:VkVideoEncodeAV1PictureInfoKHR structure included in the
    pname:pNext chain of pname:pEncodeInfo must: be zero
  * [[VUID-vkCmdEncodeVideoKHR-pStdPictureInfo-10350]]
    If the bound video session was created with the video codec operation
    ename:VK_VIDEO_CODEC_OPERATION_ENCODE_AV1_BIT_KHR, then
    slink:VkVideoEncodeAV1PictureInfoKHR::pname:pStdPictureInfo->pSegmentation
    for the slink:VkVideoEncodeAV1PictureInfoKHR structure included in the
    pname:pNext chain of pname:pEncodeInfo must: be `NULL`
endif::VK_KHR_video_encode_av1[]
****
...
ifndef::VK_KHR_video_encode_quantization_map,VK_KHR_video_encode_intra_refresh[]
  * pname:flags is reserved for future use.
endif::VK_KHR_video_encode_quantization_map,VK_KHR_video_encode_intra_refresh[]
ifdef::VK_KHR_video_encode_quantization_map,VK_KHR_video_encode_intra_refresh[]
  * pname:flags is a bitmask of elink:VkVideoEncodeFlagBitsKHR indicating
...
****
  * [[VUID-VkVideoEncodeInfoKHR-dstBuffer-08236]]
    ...
    ename:VK_BUFFER_USAGE_VIDEO_ENCODE_DST_BIT_KHR usage flag set
...
ifdef::VK_KHR_video_encode_quantization_map,VK_KHR_video_encode_intra_refresh[]
[open,refpage='VkVideoEncodeFlagBitsKHR',desc='Video encode flags',type='enums']
--
Bits which can: be set in slink:VkVideoEncodeInfoKHR::pname:flags,
specifying video encode flags, are:

include::{generated}/api/enums/VkVideoEncodeFlagBitsKHR.adoc[]

ifdef::VK_KHR_video_encode_quantization_map[]
  * ename:VK_VIDEO_ENCODE_WITH_QUANTIZATION_DELTA_MAP_BIT_KHR specifies the
    use of a <<encode-quantization-delta-map,quantization delta map>> in the
    issued <<video-encode-operations,video encode operations>>.
  * ename:VK_VIDEO_ENCODE_WITH_EMPHASIS_MAP_BIT_KHR specifies the use of an
    <<encode-emphasis-map,emphasis map>> in the issued
    <<video-encode-operations,video encode operations>>.
endif::VK_KHR_video_encode_quantization_map[]
ifdef::VK_KHR_video_encode_intra_refresh[]
  * ename:VK_VIDEO_ENCODE_INTRA_REFRESH_BIT_KHR enables
    <<encode-intra-refresh,intra refresh>> for the encoded picture.
endif::VK_KHR_video_encode_intra_refresh[]
--
endif::VK_KHR_video_encode_quantization_map,VK_KHR_video_encode_intra_refresh[]

[open,refpage='VkVideoEncodeFlagsKHR',desc='Bitmask of VkVideoEncodeFlagBits',type='flags']
--
...
In conjunction with <<encode-intra-refresh-reference-pictures,restricting
reference picture prediction>>, this enables avoiding error propagation
...
[[encode-intra-refresh-cycle]]
An _intra refresh cycle_ is a series of encode operations using intra
...
[[encode-intra-refresh-cycle-duration]]
The _intra refresh cycle duration_ is the number of encode operations in an
intra refresh cycle.

image::{images}/encode_intra_refresh_cycle_duration.svg[align="center",title="Intra refresh cycle duration",opts="{imageopts}"]

[[encode-intra-refresh-regions]]
For the purposes of performing an intra refresh cycle, the coded extent is
divided into a set of _intra refresh regions_.
...
image::{images}/encode_intra_refresh_regions.svg[align="center",title="Examples of picture partitioning into intra refresh regions",opts="{imageopts}"]

[[encode-intra-refresh-index]]
...
refresh cycle.
Therefore each encode operation in an intra refresh cycle has an _intra
refresh index_ that provides the following information:
...
code:error_resilient_mode flag for the first frame encoded as part of the
intra refresh cycle in order to avoid any error propagation of CDF data.
====
endif::VK_KHR_video_encode_av1[]

[[encode-dirty-intra-refresh-regions]]
An intra refresh region of a picture encoded using intra refresh is
considered _clean_ once the encode operation within the intra refresh cycle
that encoded that intra refresh region using intra prediction is complete.
Otherwise, the intra refresh region is considered _dirty_.

The number of _dirty intra refresh regions_ of a picture encoded using intra
...
=== Intra Refresh Capabilities

[open,refpage='VkVideoEncodeIntraRefreshCapabilitiesKHR',desc='Structure describing video encode intra refresh capabilities for a video profile',type='structs']
--
When calling flink:vkGetPhysicalDeviceVideoCapabilitiesKHR with
pname:pVideoProfile->videoCodecOperation specifying an encode operation, the
slink:VkVideoEncodeIntraRefreshCapabilitiesKHR structure can: be included in
the pname:pNext chain of the slink:VkVideoCapabilitiesKHR structure to
retrieve capabilities specific to video encode intra refresh.

The sname:VkVideoEncodeIntraRefreshCapabilitiesKHR structure is defined as:

include::{generated}/api/structs/VkVideoEncodeIntraRefreshCapabilitiesKHR.adoc[]

  * pname:sType is a elink:VkStructureType value identifying this structure.
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:intraRefreshModes is a bitmask of
    elink:VkVideoEncodeIntraRefreshModeFlagBitsKHR values indicating the set
    of supported <<encode-intra-refresh-modes,intra refresh modes>>.
  * pname:maxIntraRefreshCycleDuration specifies the maximum supported
    <<encode-intra-refresh-cycle-duration,intra refresh cycle duration>>.
  * pname:maxIntraRefreshActiveReferencePictures is the maximum number of
    <<active-reference-pictures,active reference pictures>> when encoding
    pictures with <<encode-intra-refresh,intra refresh>> enabled.
    ...
    slink:VkVideoCapabilitiesKHR::pname:maxActiveReferencePictures and the
    maximum requested at video session creation time in
    slink:VkVideoSessionCreateInfoKHR::pname:maxActiveReferencePictures.
  * pname:partitionIndependentIntraRefreshRegions specifies whether the
    implementation supports intra refresh regions that are independent of
    the picture partitioning used during encoding.
    If it is ename:VK_TRUE, then pictures can: be encoded with multiple
...
[NOTE]
====
This capability is only indicative for <<encode-av1-profile,AV1 encode
profiles>> and does not impose any restrictions on the application as
...
====
  ...
  * pname:nonRectangularIntraRefreshRegions specifies whether the
...
include::{generated}/validity/structs/VkVideoEncodeIntraRefreshCapabilitiesKHR.adoc[]
--


[[encode-intra-refresh-modes]]
=== Intra Refresh Modes

[open,refpage='VkVideoEncodeIntraRefreshModeFlagBitsKHR',desc='Video encode intra refresh modes',type='enums']
--
The intra refresh modes are defined with the following enums:

include::{generated}/api/enums/VkVideoEncodeIntraRefreshModeFlagBitsKHR.adoc[]

  * ename:VK_VIDEO_ENCODE_INTRA_REFRESH_MODE_NONE_KHR specifies that intra
    refresh must: not be used.
  * ename:VK_VIDEO_ENCODE_INTRA_REFRESH_MODE_PER_PICTURE_PARTITION_BIT_KHR
    specifies the use of _per picture partition intra refresh_.
    In this mode each intra refresh region [eq]#i# corresponds to the
    encoded picture partition [eq]#i#.
  * ename:VK_VIDEO_ENCODE_INTRA_REFRESH_MODE_BLOCK_BASED_BIT_KHR specifies
    the use of any _block-based intra refresh_.
  ...
  * ename:VK_VIDEO_ENCODE_INTRA_REFRESH_MODE_BLOCK_ROW_BASED_BIT_KHR
    specifies the use of _block-row-based intra refresh_.
    This mode is a block-based intra refresh mode where each intra refresh
    region encompasses a set of coding block rows.
  * ename:VK_VIDEO_ENCODE_INTRA_REFRESH_MODE_BLOCK_COLUMN_BASED_BIT_KHR
    specifies the use of _block-column-based intra refresh_.
    This mode is a block-based intra refresh mode where each intra refresh
    region encompasses a set of coding block columns.
--

Implementations reporting support for
ename:VK_VIDEO_ENCODE_INTRA_REFRESH_MODE_BLOCK_ROW_BASED_BIT_KHR and/or
ename:VK_VIDEO_ENCODE_INTRA_REFRESH_MODE_BLOCK_COLUMN_BASED_BIT_KHR in
slink:VkVideoEncodeIntraRefreshCapabilitiesKHR::pname:intraRefreshModes are
also required: to report support for
ename:VK_VIDEO_ENCODE_INTRA_REFRESH_MODE_BLOCK_BASED_BIT_KHR.
...
ename:VK_VIDEO_ENCODE_INTRA_REFRESH_MODE_BLOCK_BASED_BIT_KHR on such
...
[open,refpage='VkVideoEncodeIntraRefreshModeFlagsKHR',desc='Bitmask of VkVideoEncodeIntraRefreshModeFlagBitsKHR', type='flags']
--
include::{generated}/api/flags/VkVideoEncodeIntraRefreshModeFlagsKHR.adoc[]

tname:VkVideoEncodeIntraRefreshModeFlagsKHR is a bitmask type for setting a
mask of zero or more elink:VkVideoEncodeIntraRefreshModeFlagBitsKHR.
--

The used intra refresh mode is selected at <<video-session-creation,video
session creation time>> by including an instance of the
slink:VkVideoEncodeSessionIntraRefreshCreateInfoKHR structure in the
pname:pNext chain of the slink:VkVideoSessionCreateInfoKHR structure and
specifying one of the supported intra refresh modes, as returned in
slink:VkVideoEncodeIntraRefreshCapabilitiesKHR::pname:intraRefreshModes, in
its pname:intraRefreshMode member.

[open,refpage='VkVideoEncodeSessionIntraRefreshCreateInfoKHR',desc='Video encode session intra refresh parameters',type='structs']
--
The sname:VkVideoEncodeSessionIntraRefreshCreateInfoKHR structure is defined
as:

include::{generated}/api/structs/VkVideoEncodeSessionIntraRefreshCreateInfoKHR.adoc[]

  * pname:sType is a elink:VkStructureType value identifying this structure.
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:intraRefreshMode is a
    elink:VkVideoEncodeIntraRefreshModeFlagBitsKHR specifying the used
    <<encode-intra-refresh-modes,intra refresh mode>>.

include::{generated}/validity/structs/VkVideoEncodeSessionIntraRefreshCreateInfoKHR.adoc[]
--


=== Encoding Pictures with Intra Refresh

If the video encode operation is issued with the
ename:VK_VIDEO_ENCODE_INTRA_REFRESH_BIT_KHR flag, the picture will be
encoded with intra refresh, resulting in the <<encode-intra-refresh-regions,
intra refresh region>> identified by the specified
<<encode-intra-refresh-index,intra refresh index>> to be encoded with intra
prediction.

Intra refresh parameters are specified to video encode operations by
including an instance of the slink:VkVideoEncodeIntraRefreshInfoKHR
structure in the pname:pNext chain of the slink:VkVideoEncodeInfoKHR
structure specified to the video encode command.

[open,refpage='VkVideoEncodeIntraRefreshInfoKHR',desc='Structure specifying video encode intra refresh parameters',type='structs']
--
The sname:VkVideoEncodeIntraRefreshInfoKHR structure is defined as:

include::{generated}/api/structs/VkVideoEncodeIntraRefreshInfoKHR.adoc[]

  * pname:sType is a elink:VkStructureType value identifying this structure.
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:intraRefreshCycleDuration is the used
    <<encode-intra-refresh-cycle-duration,intra refresh cycle duration>>.
  * pname:intraRefreshIndex is the <<encode-intra-refresh-index,intra
    refresh index>> of the encoded picture.

include::{generated}/validity/structs/VkVideoEncodeIntraRefreshInfoKHR.adoc[]
--


[[encode-intra-refresh-reference-pictures]]
=== Referencing Pictures Encoded with Intra Refresh

When using pictures encoded with intra refresh as an
<<active-reference-pictures,active reference picture>> in a video encode
operation, applications may: want to limit the set of
<<encode-intra-refresh-regions,intra refresh regions>> of the reference
picture that are used to predict samples of the encoded picture to exclude
<<encode-dirty-intra-refresh-regions,dirty intra refresh regions>> of the
...
In order to limit the set of <<encode-intra-refresh-regions,intra refresh
regions>> of an <<active-reference-pictures,active reference picture>> used
for sample prediction, the application must: specify an instance of the
slink:VkVideoReferenceIntraRefreshInfoKHR structure in the pname:pNext chain
of the slink:VkVideoReferenceSlotInfoKHR structure specifying the active
reference picture in question.

[open,refpage='VkVideoReferenceIntraRefreshInfoKHR',desc='Structure specifying video reference picture intra refresh parameters',type='structs']
--
The sname:VkVideoReferenceIntraRefreshInfoKHR structure is defined as:

include::{generated}/api/structs/VkVideoReferenceIntraRefreshInfoKHR.adoc[]

  * pname:sType is a elink:VkStructureType value identifying this structure.
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:dirtyIntraRefreshRegions is the number of
    <<encode-dirty-intra-refresh-regions,dirty intra refresh regions>> in
    the reference picture.

include::{generated}/validity/structs/VkVideoReferenceIntraRefreshInfoKHR.adoc[]
--
...
<<encode-dirty-intra-refresh-regions,dirty intra refresh regions>>, the
number of dirty intra refresh regions must: equal the used
<<encode-intra-refresh-cycle-duration,intra refresh cycle duration>> minus
the <<encode-intra-refresh-index,intra refresh index>> of the encoded
picture.
...
Given an <<active-reference-pictures,active reference picture>> with
pname:dirtyIntraRefreshRegions number of
<<encode-dirty-intra-refresh-regions,dirty intra refresh regions>> and an
<<encode-intra-refresh-cycle-duration,intra refresh cycle duration>> of
pname:intraRefreshCycleDuration, the samples within intra refresh region
index [eq]#i# of the encoded picture are allowed to be predicted by samples
within intra refresh region index [eq]#j# of the active reference picture in
question if and only if any of the following conditions are true:

  * The picture is encoded with ename:VK_VIDEO_ENCODE_INTRA_REFRESH_BIT_KHR
    and [eq]#i# is greater than the pname:intraRefreshIndex of the encoded
    picture.
  * The picture is encoded with ename:VK_VIDEO_ENCODE_INTRA_REFRESH_BIT_KHR,
    [eq]#i# is less than the pname:intraRefreshIndex of the encoded picture,
    and [eq]#j# is less than the difference of
    pname:intraRefreshCycleDuration and pname:dirtyIntraRefreshRegions.
...
ifdef::VK_KHR_video_encode_av1[]
  ** When using an <<encode-av1-profile,AV1 encode profile>>,
     implementations will use the quantizer index value specified in
     slink:VkVideoEncodeAV1PictureInfoKHR::pname:constantQIndex to control
     the quality of the encoded picture.
endif::VK_KHR_video_encode_av1[]
...
ifdef::VK_KHR_video_encode_av1[]
  * If the video session was created with an <<encode-av1-profile,AV1 encode
    profile>>:
  ** The values of the members of the
     slink:VkVideoEncodeAV1RateControlInfoKHR structure, if one is specified
     in the pname:pNext chain of the slink:VkVideoEncodeRateControlInfoKHR
     used to configure the rate control state.
  ** The values of the members of any
     slink:VkVideoEncodeAV1RateControlLayerInfoKHR structures included in
     the pname:pNext chain of a slink:VkVideoEncodeRateControlLayerInfoKHR
     structure used to configure the state of a rate control layer.
endif::VK_KHR_video_encode_av1[]
...
ifdef::VK_KHR_video_encode_av1[]
  * If the video session was created with the video codec operation
    ename:VK_VIDEO_CODEC_OPERATION_ENCODE_AV1_BIT_KHR, then this index
...
ifdef::VK_KHR_video_encode_av1[]
  * <<encode-av1-rate-control,AV1 Encode Rate Control>>
endif::VK_KHR_video_encode_av1[]
...
****
  ...
  * [[VUID-VkVideoEncodeRateControlInfoKHR-layerCount-08358]]
    ...
    must: be less than or equal to pname:virtualBufferSizeInMs
ifdef::VK_KHR_video_encode_h264[]
...
ifdef::VK_KHR_video_encode_av1[]
  * [[VUID-VkVideoEncodeRateControlInfoKHR-videoCodecOperation-10351]]
    If the pname:videoCodecOperation of the used video profile is
    ename:VK_VIDEO_CODEC_OPERATION_ENCODE_AV1_BIT_KHR, the pname:pNext chain
    this structure is included in also includes an instance of the
    slink:VkVideoEncodeAV1RateControlInfoKHR structure, and pname:layerCount
    is greater than `1`, then pname:layerCount must: equal
    slink:VkVideoEncodeAV1RateControlInfoKHR::pname:temporalLayerCount
endif::VK_KHR_video_encode_av1[]
****
...
ifdef::VK_KHR_video_encode_quantization_map[]
[[encode-quantization-map]]
== Video Encode Quantization Maps

Quantization maps are slink:VkImage objects that are used in video encode
...
[[encode-quantization-map-texel-size]]
The size of the rectangular block of texels each quantization map texel
covers is referred to as the _quantization map texel size_.

The extent of the image subresource used as a quantization map when encoding
a picture with a coded extent of [eq]#(pname:width,pname:height)# thus has
to be at least [eq]#({lceil}pname:width / pname:texelSize.width{rceil},
{lceil}pname:height / pname:texelSize.height{rceil})#, where pname:texelSize
is the used quantization map texel size.

In particular, the quantization map texel at location
[eq]#(pname:x,pname:y)# contains relative quantization parameter values used
when encoding the pname:texelSize sized rectangular block of the
<<encode-input-picture,encode input picture>> starting at the texel location
[eq]#(pname:x {times} pname:texelSize.width, pname:y {times}
pname:texelSize.height)#.

[[encode-quantization-map-fetched-texel]]
...
[eq]#(pname:width,pname:height)# at the texel location
[eq]#(pname:x,pname:y)# in the <<encode-input-picture,encode input
picture>>:
  ...
  * If the size of the codec-specific coding block matches the used
    ...
    the texel location [eq]#(pname:x / pname:texelSize.width, pname:y /
    pname:texelSize.height)#.
  * If the size of the codec-specific coding block is smaller than the used
    quantization map texel size, then the fetched quantization map value
    corresponding to the codec-specific coding block is the texel value at
    the texel location [eq]#({lfloor}pname:x /
    pname:texelSize.width{rfloor}, {lfloor}pname:y /
    pname:texelSize.height{rfloor})#.
  * If the size of the codec-specific coding block is larger than the used
    ...
    values in the subregion starting at texel location [eq]#(pname:x /
    pname:texelSize.width,pname:y / pname:texelSize.height)# with a size
    [eq]#({lceil}pname:width / pname:texelSize.width{rceil},
    {lceil}pname:height / pname:texelSize.height{rceil})#.
...
This specification supports the following types of quantization maps:
  ...
  * <<encode-quantization-delta-map,Quantization delta maps>>
  * <<encode-emphasis-map,Emphasis maps>>


[[encode-quantization-delta-map]]
...
<<encode-rate-control-modes,rate control mode>>, including
...
ifdef::VK_KHR_video_encode_h264[]
  * The behavior of quantization delta maps used with an
    <<encode-h264-profile,H.264 encode profile>> is described in the
    <<encode-h264-quantization>> and <<encode-h264-qp-delta-map>> sections.
endif::VK_KHR_video_encode_h264[]
ifdef::VK_KHR_video_encode_h265[]
  * The behavior of quantization delta maps used with an
    <<encode-h265-profile,H.265 encode profile>> is described in the
    <<encode-h265-quantization>> and <<encode-h265-qp-delta-map>> sections.
endif::VK_KHR_video_encode_h265[]
ifdef::VK_KHR_video_encode_av1[]
  * The behavior of quantization delta maps used with an
    <<encode-av1-profile,AV1 encode profile>> is described in the
    <<encode-av1-quantization>> and <<encode-av1-qindex-delta-map>>
    sections.
endif::VK_KHR_video_encode_av1[]
...
session is not ename:VK_VIDEO_ENCODE_RATE_CONTROL_MODE_DEFAULT_KHR or
ename:VK_VIDEO_ENCODE_RATE_CONTROL_MODE_DISABLED_BIT_KHR.
...
ifdef::VK_KHR_video_encode_h264[]
  * The behavior of emphasis maps used with an <<encode-h264-profile,H.264
    encode profile>> is described in the <<encode-h264-quantization>>
    section.
endif::VK_KHR_video_encode_h264[]
ifdef::VK_KHR_video_encode_h265[]
  * The behavior of emphasis maps used with an <<encode-h265-profile,H.265
    encode profile>> is described in the <<encode-h265-quantization>>
    section.
endif::VK_KHR_video_encode_h265[]
ifdef::VK_KHR_video_encode_av1[]
  * The behavior of emphasis maps used with an <<encode-av1-profile,AV1
    encode profile>> is described in the <<encode-av1-quantization>>
    section.
endif::VK_KHR_video_encode_av1[]
...
and implementations are required: to support the ename:VK_FORMAT_R8_UNORM
format for emphasis maps, as reported in
slink:VkVideoFormatPropertiesKHR::pname:format, when the video encode
profile supports ename:VK_VIDEO_ENCODE_CAPABILITY_EMPHASIS_MAP_BIT_KHR.


=== Quantization Map Capabilities

[open,refpage='VkVideoEncodeQuantizationMapCapabilitiesKHR',desc='Structure describing video encode quantization map capabilities for a video profile',type='structs']
--
When calling flink:vkGetPhysicalDeviceVideoCapabilitiesKHR with
pname:pVideoProfile->videoCodecOperation specifying an encode operation, the
slink:VkVideoEncodeQuantizationMapCapabilitiesKHR structure can: be included
in the pname:pNext chain of the slink:VkVideoCapabilitiesKHR structure to
retrieve capabilities specific to video encode quantization maps.

The sname:VkVideoEncodeQuantizationMapCapabilitiesKHR structure is defined
as:

include::{generated}/api/structs/VkVideoEncodeQuantizationMapCapabilitiesKHR.adoc[]

  * pname:sType is a elink:VkStructureType value identifying this structure.
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:maxQuantizationMapExtent indicates the maximum supported width and
    height of quantization maps.

include::{generated}/validity/structs/VkVideoEncodeQuantizationMapCapabilitiesKHR.adoc[]
--

ifdef::VK_KHR_video_encode_h264[]
[open,refpage='VkVideoEncodeH264QuantizationMapCapabilitiesKHR',desc='Structure describing H.264 encode quantization map capabilities',type='structs']
--
When calling flink:vkGetPhysicalDeviceVideoCapabilitiesKHR to query the
capabilities of an <<encode-h264-profile,H.264 encode profile>>, the
sname:VkVideoEncodeH264QuantizationMapCapabilitiesKHR structure can: be
...
The sname:VkVideoEncodeH264QuantizationMapCapabilitiesKHR structure is
defined as:

include::{generated}/api/structs/VkVideoEncodeH264QuantizationMapCapabilitiesKHR.adoc[]

  * pname:sType is a elink:VkStructureType value identifying this structure.
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:minQpDelta indicates the minimum QP delta value supported for
    <<encode-h264-qp-delta-map,H.264 QP delta maps>>.
  * pname:maxQpDelta indicates the maximum QP delta value supported for
    <<encode-h264-qp-delta-map,H.264 QP delta maps>>.

include::{generated}/validity/structs/VkVideoEncodeH264QuantizationMapCapabilitiesKHR.adoc[]
--
endif::VK_KHR_video_encode_h264[]

ifdef::VK_KHR_video_encode_h265[]
[open,refpage='VkVideoEncodeH265QuantizationMapCapabilitiesKHR',desc='Structure describing H.265 encode quantization map capabilities',type='structs']
--
When calling flink:vkGetPhysicalDeviceVideoCapabilitiesKHR to query the
capabilities of an <<encode-h265-profile,H.265 encode profile>>, the
sname:VkVideoEncodeH265QuantizationMapCapabilitiesKHR structure can: be
included in the pname:pNext chain of the slink:VkVideoCapabilitiesKHR
structure to retrieve additional video encode quantization map capabilities
specific to H.265 encode profiles.

The sname:VkVideoEncodeH265QuantizationMapCapabilitiesKHR structure is
defined as:

include::{generated}/api/structs/VkVideoEncodeH265QuantizationMapCapabilitiesKHR.adoc[]

  * pname:sType is a elink:VkStructureType value identifying this structure.
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:minQpDelta indicates the minimum QP delta value supported for
    <<encode-h265-qp-delta-map,H.265 QP delta maps>>.
  * pname:maxQpDelta indicates the maximum QP delta value supported for
    <<encode-h265-qp-delta-map,H.265 QP delta maps>>.

include::{generated}/validity/structs/VkVideoEncodeH265QuantizationMapCapabilitiesKHR.adoc[]
--
endif::VK_KHR_video_encode_h265[]

ifdef::VK_KHR_video_encode_av1[]
[open,refpage='VkVideoEncodeAV1QuantizationMapCapabilitiesKHR',desc='Structure describing AV1 encode quantization map capabilities',type='structs']
--
When calling flink:vkGetPhysicalDeviceVideoCapabilitiesKHR to query the
capabilities of an <<encode-av1-profile,AV1 encode profile>>, the
sname:VkVideoEncodeAV1QuantizationMapCapabilitiesKHR structure can: be
included in the pname:pNext chain of the slink:VkVideoCapabilitiesKHR
structure to retrieve additional video encode quantization map capabilities
specific to AV1 encode profiles.

The sname:VkVideoEncodeAV1QuantizationMapCapabilitiesKHR structure is
defined as:

include::{generated}/api/structs/VkVideoEncodeAV1QuantizationMapCapabilitiesKHR.adoc[]

  * pname:sType is a elink:VkStructureType value identifying this structure.
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:minQIndexDelta indicates the minimum quantizer index delta value
    supported for <<encode-av1-qindex-delta-map,AV1 quantizer index delta
    maps>>.
  * pname:maxQIndexDelta indicates the maximum quantizer index delta value
    supported for <<encode-av1-qindex-delta-map,AV1 quantizer index delta
    maps>>.

include::{generated}/validity/structs/VkVideoEncodeAV1QuantizationMapCapabilitiesKHR.adoc[]
--
endif::VK_KHR_video_encode_av1[]
...
[open,refpage='VkVideoFormatQuantizationMapPropertiesKHR',desc='Structure describing quantization map properties',type='structs']
--
When calling flink:vkGetPhysicalDeviceVideoFormatPropertiesKHR, the
slink:VkVideoFormatQuantizationMapPropertiesKHR structure can: be included
in the pname:pNext chain of the slink:VkVideoFormatPropertiesKHR structure
to retrieve video format properties specific to video encode quantization
maps.

The sname:VkVideoFormatQuantizationMapPropertiesKHR structure is defined as:

include::{generated}/api/structs/VkVideoFormatQuantizationMapPropertiesKHR.adoc[]

  * pname:sType is a elink:VkStructureType value identifying this structure.
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:quantizationMapTexelSize indicates the
    <<encode-quantization-map-texel-size,quantization map texel size>> of
...
slink:VkVideoFormatPropertiesKHR::pname:imageUsageFlags for this video
format include
...
flink:vkGetPhysicalDeviceVideoFormatPropertiesKHR returning multiple entries
with different pname:quantizationMapTexelSize values.

include::{generated}/validity/structs/VkVideoFormatQuantizationMapPropertiesKHR.adoc[]
--

ifdef::VK_KHR_video_encode_h265[]
[open,refpage='VkVideoFormatH265QuantizationMapPropertiesKHR',desc='Structure describing H.265 quantization map properties',type='structs']
--
When calling flink:vkGetPhysicalDeviceVideoFormatPropertiesKHR, the
sname:VkVideoFormatH265QuantizationMapPropertiesKHR structure can: be
included in the pname:pNext chain of the slink:VkVideoFormatPropertiesKHR
structure to retrieve video format properties specific to video encode
quantization maps used with an <<encode-h265-profile,H.265 encode profile>>.

The sname:VkVideoFormatH265QuantizationMapPropertiesKHR structure is defined
as:

include::{generated}/api/structs/VkVideoFormatH265QuantizationMapPropertiesKHR.adoc[]

  * pname:sType is a elink:VkStructureType value identifying this structure.
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:compatibleCtbSizes is a bitmask of
    elink:VkVideoEncodeH265CtbSizeFlagBitsKHR indicating the CTB sizes that
    quantization maps using this video format are compatible with.
+
[NOTE]
====
The value of pname:compatibleCtbSizes does not limit the use of the specific
quantization map format, but does limit the implementation in being able to
encode pictures with CTB sizes not included in pname:compatibleCtbSizes but
otherwise supported by the used video profile, as indicated by
slink:VkVideoEncodeH265CapabilitiesKHR::pname:ctbSizes.
In particular, using smaller
<<encode-quantization-map-texel-size,quantization map texel sizes>> may
...
include::{generated}/validity/structs/VkVideoFormatH265QuantizationMapPropertiesKHR.adoc[]
--
endif::VK_KHR_video_encode_h265[]

ifdef::VK_KHR_video_encode_av1[]
[open,refpage='VkVideoFormatAV1QuantizationMapPropertiesKHR',desc='Structure describing AV1 quantization map properties',type='structs']
--
When calling flink:vkGetPhysicalDeviceVideoFormatPropertiesKHR, the
sname:VkVideoFormatAV1QuantizationMapPropertiesKHR structure can: be
included in the pname:pNext chain of the slink:VkVideoFormatPropertiesKHR
structure to retrieve video format properties specific to video encode
quantization maps used with an <<encode-av1-profile,AV1 encode profile>>.

The sname:VkVideoFormatAV1QuantizationMapPropertiesKHR structure is defined
as:

include::{generated}/api/structs/VkVideoFormatAV1QuantizationMapPropertiesKHR.adoc[]

  * pname:sType is a elink:VkStructureType value identifying this structure.
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:compatibleSuperblockSizes is a bitmask of
    elink:VkVideoEncodeAV1SuperblockSizeFlagBitsKHR indicating the AV1
    superblock sizes that quantization maps using this video format are
    compatible with.
+
[NOTE]
====
The value of pname:compatibleSuperblockSizes does not limit the use of the
...
pname:compatibleSuperblockSizes but otherwise supported by the used video
profile, as indicated by
slink:VkVideoEncodeAV1CapabilitiesKHR::pname:superblockSizes.
In particular, using smaller
<<encode-quantization-map-texel-size,quantization map texel sizes>> may
...
include::{generated}/validity/structs/VkVideoFormatAV1QuantizationMapPropertiesKHR.adoc[]
--
endif::VK_KHR_video_encode_av1[]
...
[open,refpage='VkVideoEncodeQuantizationMapInfoKHR',desc='Structure specifying quantization map information to use for video encode operations',type='structs']
--
The sname:VkVideoEncodeQuantizationMapInfoKHR structure can: be included in
the pname:pNext chain of the slink:VkVideoEncodeInfoKHR structure passed to
the flink:vkCmdEncodeVideoKHR command to specify the quantization map used
by the issued video encode operations.

The sname:VkVideoEncodeQuantizationMapInfoKHR structure is defined as:

include::{generated}/api/structs/VkVideoEncodeQuantizationMapInfoKHR.adoc[]

  * pname:sType is a elink:VkStructureType value identifying this structure.
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:quantizationMap specifies the image view to use as the
    quantization map.
  * pname:quantizationMapExtent specifies the extent of the image subregion
    of pname:quantizationMap to use as the quantization map starting at
    offset [eq]#(0,0)#.

.Valid Usage
****
  * [[VUID-VkVideoEncodeQuantizationMapInfoKHR-quantizationMapExtent-10352]]
    pname:quantizationMapExtent.width must: be less than or equal to the
    width of pname:quantizationMap
  * [[VUID-VkVideoEncodeQuantizationMapInfoKHR-quantizationMapExtent-10353]]
    pname:quantizationMapExtent.height must: be less than or equal to the
    height of pname:quantizationMap
****

include::{generated}/validity/structs/VkVideoEncodeQuantizationMapInfoKHR.adoc[]
--
endif::VK_KHR_video_encode_quantization_map[]
...
== Video Encode {RGBprime} Conversion

{RGBprime} to {YCbCr} conversion performs the following operations to the
<<encode-input-picture,encode input picture>> before video coding:

* <<encode-rgb-conversion-range-compression,{RGBprime} Range Compression>>
* <<encode-rgb-conversion-model-conversion,{RGBprime} Model Conversion>>
* <<encode-rgb-conversion-chroma-subsampling-and-quantization,{RGBprime}
  Chroma Subsampling and Quantization>>


[[encode-rgb-conversion-capabilities]]
=== {RGBprime} Conversion Capabilities

[open,refpage='VkVideoEncodeRgbConversionCapabilitiesVALVE',desc='Structure describing video encode rgb conversion capabilities for a video profile',type='structs']
--
When calling flink:vkGetPhysicalDeviceVideoCapabilitiesKHR with
pname:pVideoProfile->videoCodecOperation specifying an encode operation, the
slink:VkVideoEncodeRgbConversionCapabilitiesVALVE structure can: be included
in the pname:pNext chain of the slink:VkVideoCapabilitiesKHR structure to
retrieve capabilities specific to video encode {RGBprime} conversion.

The sname:VkVideoEncodeRgbConversionCapabilitiesVALVE structure is defined
as:

include::{generated}/api/structs/VkVideoEncodeRgbConversionCapabilitiesVALVE.adoc[]

  * pname:sType is a elink:VkStructureType value identifying this structure.
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:rgbModels is a bitmask of
    elink:VkVideoEncodeRgbModelConversionFlagBitsVALVE describing supported
    <<encode-rgb-conversion-model-conversion,model conversions>> for video
    encode {RGBprime} conversion.
  * pname:rgbRanges is a bitmask of
    elink:VkVideoEncodeRgbRangeCompressionFlagBitsVALVE describing supported
    <<encode-rgb-conversion-range-compression,range compressions>> for video
    encode {RGBprime} conversion.
  * pname:xChromaOffsets is a bitmask of
    elink:VkVideoEncodeRgbChromaOffsetFlagBitsVALVE describing supported
    offsets of the output location of the downsampled chroma component on
    the X axis for video encode {RGBprime} conversion.
  * pname:yChromaOffsets is a bitmask of
    elink:VkVideoEncodeRgbChromaOffsetFlagBitsVALVE describing supported
    offsets of the output location of the downsampled chroma component on
    the Y axis for video encode {RGBprime} conversion.

include::{generated}/validity/structs/VkVideoEncodeRgbConversionCapabilitiesVALVE.adoc[]
--


[[encode-rgb-conversion-range-compression]]
==== {RGBprime} Range Compression

[open,refpage='VkVideoEncodeRgbRangeCompressionFlagBitsVALVE',desc='Range compression operation to perform for encode rgb conversion',type='enums']
--
The video encode {RGBprime} range compression to be applied to color
component values of the <<encode-input-picture,encode input picture>> before
video coding is defined by the pname:rgbRange member of the
slink:VkVideoEncodeSessionRgbConversionCreateInfoVALVE structure.

The elink:VkVideoEncodeRgbRangeCompressionFlagBitsVALVE enum describes
...
include::{generated}/api/enums/VkVideoEncodeRgbRangeCompressionFlagBitsVALVE.adoc[]

  * ename:VK_VIDEO_ENCODE_RGB_RANGE_COMPRESSION_FULL_RANGE_BIT_VALVE
...
====
  * ename:VK_VIDEO_ENCODE_RGB_RANGE_COMPRESSION_NARROW_RANGE_BIT_VALVE
    specifies the following transformations are applied:
+
[latexmath]
++++
...
Unlike <<textures-sampler-YCbCr-conversion-rangeexpand, sampler {YCbCr}
range expansion>>, no precision guarantees are made for video encode
{RGBprime} range compression.
====
  * _n_ is the bit-depth of the components in the bound video session's
    pname:pictureFormat.

[NOTE]
====
Video encode {RGBprime} range compression transformations have the inverse
definition of the <<textures-sampler-YCbCr-conversion-rangeexpand, sampler
{YCbCr} range expansion>> transformations.
====
--

[open,refpage='VkVideoEncodeRgbRangeCompressionFlagsVALVE',desc='Bitmask of VkVideoEncodeRgbRangeCompressionFlagBitsVALVE',type='flags']
--
include::{generated}/api/flags/VkVideoEncodeRgbRangeCompressionFlagsVALVE.adoc[]

tlink:VkVideoEncodeRgbRangeCompressionFlagsVALVE is a bitmask type for
setting a mask of zero or more
elink:VkVideoEncodeRgbRangeCompressionFlagBitsVALVE.
--


[[encode-rgb-conversion-model-conversion]]
==== {RGBprime} Model Conversion

[open,refpage='VkVideoEncodeRgbModelConversionFlagBitsVALVE',desc='Color model conversions for encode RGB conversion',type='enums']
--
The range-compressed values are converted between color models, according to
the color model conversion specified in the pname:rgbModel member of the
slink:VkVideoEncodeSessionRgbConversionCreateInfoVALVE structure.

elink:VkVideoEncodeRgbModelConversionFlagBitsVALVE defines the conversion
from the <<encode-input-picture-info,encode input picture>>'s color model to
the encode color model.

include::{generated}/api/enums/VkVideoEncodeRgbModelConversionFlagBitsVALVE.adoc[]

  * ename:VK_VIDEO_ENCODE_RGB_MODEL_CONVERSION_RGB_IDENTITY_BIT_VALVE
    ...
    for video coding; {RGBprime} range compression is applied to the
    components.
  * ename:VK_VIDEO_ENCODE_RGB_MODEL_CONVERSION_YCBCR_IDENTITY_BIT_VALVE
    specifies the color components are not modified by the color model
    conversion are assumed to be treated as though in {YCbCr} form; video
    encode {RGBprime} range compression and video encode {RGBprime} chroma
    subsampling is also ignored.
  * ename:VK_VIDEO_ENCODE_RGB_MODEL_CONVERSION_YCBCR_709_BIT_VALVE specifies
    the color components are transformed from an {RGBprime} representation
    to a {YCbCr} representation as described in the "`BT.709 {YCbCr}
    conversion`" section of the <<data-format,Khronos Data Format
    Specification>>.
  * ename:VK_VIDEO_ENCODE_RGB_MODEL_CONVERSION_YCBCR_601_BIT_VALVE specifies
    the color components are transformed from an {RGBprime} representation
    to a {YCbCr} representation as described in the "`BT.601 {YCbCr}
    conversion`" section of the <<data-format,Khronos Data Format
    Specification>>.
  * ename:VK_VIDEO_ENCODE_RGB_MODEL_CONVERSION_YCBCR_2020_BIT_VALVE
    specifies the color components are transformed from an {RGBprime}
    representation to a {YCbCr} representation as described in the "`BT.2020
    {YCbCr} conversion`" section of the <<data-format,Khronos Data Format
    Specification>>.

[NOTE]
====
Video encode {RGBprime} model conversion transformations have the inverse
definition of <<textures-sampler-YCbCr-conversion-modelconversion, sampler
{YCbCr} model conversion>> transformations.
====

[NOTE]
====
The video encode {RGBprime} model conversion step does not apply any
transfer function, only converting from {RGBprime} to {YCbCr} using the
primaries of the specified pname:rgbModel color model.
====
--

[open,refpage='VkVideoEncodeRgbModelConversionFlagsVALVE',desc='Bitmask of VkVideoEncodeRgbModelConversionFlagBitsVALVE',type='flags']
--
include::{generated}/api/flags/VkVideoEncodeRgbModelConversionFlagsVALVE.adoc[]

tlink:VkVideoEncodeRgbModelConversionFlagsVALVE is a bitmask type for
setting a mask of zero or more
elink:VkVideoEncodeRgbModelConversionFlagBitsVALVE.
--


[[encode-rgb-conversion-chroma-subsampling-and-quantization]]
==== {RGBprime} Chroma Subsampling and Quantization

[open,refpage='VkVideoEncodeRgbChromaOffsetFlagBitsVALVE',desc='Position of downsampled chroma samples for encode RGB conversion',type='enums']
--
The model-converted values are chroma subsampled and quantized, according to
the pname:chromaSubsampling, pname:lumaBitDepth and pname:chromaBitDepth
values specified by the bound video session.

The elink:VkVideoEncodeRgbChromaOffsetFlagBitsVALVE enum defines the
location of downsampled chroma component samples relative to the luma
samples for video encode {RGBprime} conversion, and is defined as:

include::{generated}/api/enums/VkVideoEncodeRgbChromaOffsetFlagBitsVALVE.adoc[]

  * ename:VK_VIDEO_ENCODE_RGB_CHROMA_OFFSET_COSITED_EVEN_BIT_VALVE specifies
    that downsampled chroma samples are aligned with luma samples with even
    coordinates.
  * ename:VK_VIDEO_ENCODE_RGB_CHROMA_OFFSET_MIDPOINT_BIT_VALVE specifies
...
pname:xChromaOffset and pname:yChromaOffset values of the
slink:VkVideoEncodeSessionRgbConversionCreateInfoVALVE structure:
...
[open,refpage='VkVideoEncodeRgbChromaOffsetFlagsVALVE',desc='Bitmask of VkVideoEncodeRgbChromaOffsetFlagBitsVALVE',type='flags']
--
include::{generated}/api/flags/VkVideoEncodeRgbChromaOffsetFlagsVALVE.adoc[]

tlink:VkVideoEncodeRgbChromaOffsetFlagsVALVE is a bitmask type for setting a
mask of zero or more elink:VkVideoEncodeRgbChromaOffsetFlagBitsVALVE.
--


=== Encoding Pictures with {RGBprime} Conversion

[open,refpage='VkVideoEncodeProfileRgbConversionInfoVALVE',desc='Structure describing video encode RGB conversion for a video profile',type='structs']
--
A video profile supporting video encode {RGBprime} conversion is specified
by an pname:pVideoProfile->videoCodecOperation specifying an encode
operation and including a sname:VkVideoEncodeProfileRgbConversionInfoVALVE
structure in the pname:pNext chain of the slink:VkVideoProfileInfoKHR
structure and enabling pname:performEncodeRgbConversion.

The sname:VkVideoEncodeProfileRgbConversionInfoVALVE structure is defined
as:

include::{generated}/api/structs/VkVideoEncodeProfileRgbConversionInfoVALVE.adoc[]

  * pname:sType is a elink:VkStructureType value identifying this structure.
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:performEncodeRgbConversion is a boolean value indicating whether
    video encode {RGBprime} conversion will be used for the encode
    operation.

include::{generated}/validity/structs/VkVideoEncodeProfileRgbConversionInfoVALVE.adoc[]
--

The sname:VkVideoEncodeSessionRgbConversionCreateInfoVALVE structure can: be
included in the pname:pNext chain of the slink:VkVideoSessionCreateInfoKHR
to specify the {RGBprime} conversion parameters used by the issued video
encode operations of the video session.

[open,refpage='VkVideoEncodeSessionRgbConversionCreateInfoVALVE',desc='Structure specifying video encode RGB conversion parameters for a video session',type='structs']
--
The sname:VkVideoEncodeSessionRgbConversionCreateInfoVALVE structure is
defined as:

include::{generated}/api/structs/VkVideoEncodeSessionRgbConversionCreateInfoVALVE.adoc[]

  * pname:sType is a elink:VkStructureType value identifying this structure.
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:rgbModel is the used
    <<encode-rgb-conversion-model-conversion,{RGBprime} model conversion>>
    for the {RGBprime} conversion.
  * pname:rgbRange is the used
    <<encode-rgb-conversion-range-compression,{RGBprime} range compression>>
    for the {RGBprime} conversion.
  * pname:xChromaOffset describes the output location of downsampled chroma
    components in the x dimension for the {RGBprime} conversion.
  * pname:yChromaOffset describes the output location of downsampled chroma
    components in the y dimension for the {RGBprime} conversion.

.Valid Usage
****
  * [[VUID-VkVideoEncodeSessionRgbConversionCreateInfoVALVE-rgbModel-10930]]
    pname:rgbModel must: only be a bit set in
    slink:VkVideoEncodeRgbConversionCapabilitiesVALVE::pname:rgbModels as
    returned by flink:vkGetPhysicalDeviceVideoCapabilitiesKHR with
    sname:VkVideoEncodeRgbConversionCapabilitiesVALVE in the pname:pNext
    chain of sname:VkVideoCapabilitiesKHR with the given pname:pVideoProfile
  * [[VUID-VkVideoEncodeSessionRgbConversionCreateInfoVALVE-rgbRange-10931]]
    pname:rgbRange must: only be a bit set in
    slink:VkVideoEncodeRgbConversionCapabilitiesVALVE::pname:rgbRanges as
    returned by flink:vkGetPhysicalDeviceVideoCapabilitiesKHR with
    sname:VkVideoEncodeRgbConversionCapabilitiesVALVE in the pname:pNext
    chain of sname:VkVideoCapabilitiesKHR with the given pname:pVideoProfile
  * [[VUID-VkVideoEncodeSessionRgbConversionCreateInfoVALVE-xChromaOffset-10932]]
    pname:xChromaOffset must: only be a bit set in
    slink:VkVideoEncodeRgbConversionCapabilitiesVALVE::pname:xChromaOffsets
    as returned by flink:vkGetPhysicalDeviceVideoCapabilitiesKHR with
    sname:VkVideoEncodeRgbConversionCapabilitiesVALVE in the pname:pNext
    chain of sname:VkVideoCapabilitiesKHR with the given pname:pVideoProfile
  * [[VUID-VkVideoEncodeSessionRgbConversionCreateInfoVALVE-yChromaOffset-10933]]
    pname:yChromaOffset must: only be a bit set in
    slink:VkVideoEncodeRgbConversionCapabilitiesVALVE::pname:yChromaOffsets
    as returned by flink:vkGetPhysicalDeviceVideoCapabilitiesKHR with
    sname:VkVideoEncodeRgbConversionCapabilitiesVALVE in the pname:pNext
    chain of sname:VkVideoCapabilitiesKHR with the given pname:pVideoProfile
****

include::{generated}/validity/structs/VkVideoEncodeSessionRgbConversionCreateInfoVALVE.adoc[]
--
