...
section 8 of the <<itu-t-h265,ITU-T H.265 Specification>> as follows:
  ...
  * The encoded bitstream data is written to the destination video bitstream
    buffer range as defined in the <<encode-h265-bitstream-data-access,H.265
    Encode Bitstream Data Access>> section.
  * Picture data in the <<video-picture-resources,video picture resources>>
    ...
    in the <<encode-h265-picture-data-access,H.265 Encode Picture Data
    Access>> section.
...
encode parameters:
  ...
  * the following parameters specified in the
    code:StdVideoH265ProfileTierLevel structure pointed to by
    code:StdVideoH265VideoParameterSet::code:pProfileTierLevel and
    code:StdVideoH265SequenceParameterSet::code:pProfileTierLevel:
  ** code:flags.general_tier_flag
  ** code:flags.general_progressive_source_flag, if set to `1`
  ** code:flags.general_interlaced_source_flag, if set to `0`
  ** code:flags.general_frame_only_constraint_flag
  ** code:general_profile_idc
  ** code:general_level_idc
  * the following parameters specified in
    code:StdVideoH265SequenceParameterSet:
  ** code:chroma_format_idc
  * the following parameters specified in the
    code:StdVideoH265SequenceParameterSetVui structure pointed to by
    code:StdVideoH265SequenceParameterSet::code:pSequenceParameterSetVui:
  ** code:flags.aspect_ratio_info_present_flag
  ** code:flags.overscan_info_present_flag
  ** code:flags.overscan_appropriate_flag
  ** code:flags.video_signal_type_present_flag
  ** code:flags.video_full_range_flag
  ** code:flags.colour_description_present_flag
  ** code:flags.chroma_loc_info_present_flag
  ** code:flags.neutral_chroma_indication_flag
  ** code:flags.field_seq_flag
  ** code:flags.frame_field_info_present_flag
  ** code:flags.default_display_window_flag
  ** code:flags.vui_timing_info_present_flag
  ** code:flags.vui_poc_proportional_to_timing_flag
  ** code:aspect_ratio_idc
  ** code:sar_width
  ** code:sar_height
  ** code:video_format
  ...
  ** code:matrix_coeffs
  ** code:chroma_sample_loc_type_top_field
  ** code:chroma_sample_loc_type_bottom_field
  ** code:def_disp_win_left_offset
  ** code:def_disp_win_right_offset
  ** code:def_disp_win_top_offset
  ** code:def_disp_win_bottom_offset
  ** code:vui_num_units_in_tick
  ** code:vui_time_scale
  ** code:vui_num_ticks_poc_diff_one_minus1
endif::VK_KHR_video_maintenance2[]

ifdef::VK_KHR_video_encode_quantization_map[]
In case of a <<encode-h265-parameter-sets,video session parameters>> object
created with
ename:VK_VIDEO_SESSION_PARAMETERS_CREATE_QUANTIZATION_MAP_COMPATIBLE_BIT_KHR,
the following H.265 <<encode-h265-sps,SPS>> and <<encode-h265-pps,PPS>>
...
session parameters object was created with:
  ...
  * code:StdVideoH265SequenceParameterSet::code:log2_min_luma_coding_block_size_minus3
  * code:StdVideoH265SequenceParameterSet::code:log2_diff_max_min_luma_coding_block_size
  * code:StdVideoH265SequenceParameterSet::code:log2_min_pcm_luma_coding_block_size_minus3
  * code:StdVideoH265SequenceParameterSet::code:log2_diff_max_min_pcm_luma_coding_block_size
  * code:StdVideoH265PictureParameterSet::code:diff_cu_qp_delta_depth
...
a picture are referred to as the _picture partitions_ of the picture.
...
[[encode-h265-coding-blocks]]
=== H.265 Coding Blocks

H.265 encode supports two types of coding blocks:

  * _Coding tree unit_, as defined in section 3.35 of the <<itu-t-h265,ITU-T
    H.265 Specification>>.
  * _Coding unit_, as defined in section 3.36 of the <<itu-t-h265,ITU-T
    H.265 Specification>>.

ifdef::VK_KHR_video_encode_feedback2[]
An H.265 coding unit is considered _skipped_ when the value of
code:cu_skip_flag for the coding unit equals `1`, as defined in section
7.4.95 of the <<itu-t-h265,ITU-T H.265 Specification>>.
...
include::{generated}/api/enums/VkVideoEncodeH265CapabilityFlagBitsKHR.adoc[]
  ...
  * ename:VK_VIDEO_ENCODE_H265_CAPABILITY_HRD_COMPLIANCE_BIT_KHR specifies
    whether the implementation may: be able to generate HRD compliant
    bitstreams if any of the code:nal_hrd_parameters_present_flag,
  ...
  * ename:VK_VIDEO_ENCODE_H265_CAPABILITY_PREDICTION_WEIGHT_TABLE_GENERATED_BIT_KHR
    specifies that if the code:weighted_pred_flag or the
...
ifdef::VK_KHR_video_encode_intra_refresh[]
    When a picture is encoded with <<encode-intra-refresh,intra refresh>>,
...
ifdef::VK_KHR_video_encode_intra_refresh[]
    ...
    code:slice_type code:STD_VIDEO_H265_SLICE_TYPE_I and can: differ from
    the code:slice_type of the other slice segments regardless of whether
    ename:VK_VIDEO_ENCODE_H265_CAPABILITY_DIFFERENT_SLICE_SEGMENT_TYPE_BIT_KHR
    is supported.
endif::VK_KHR_video_encode_intra_refresh[]
  * ename:VK_VIDEO_ENCODE_H265_CAPABILITY_B_FRAME_IN_L0_LIST_BIT_KHR
    specifies support for using a <<encode-h265-b-pic,B frame>> as L0
  ...
  * ename:VK_VIDEO_ENCODE_H265_CAPABILITY_B_FRAME_IN_L1_LIST_BIT_KHR
    specifies support for using a <<encode-h265-b-pic,B frame>> as L1
    reference, as specified in
  ...
  * ename:VK_VIDEO_ENCODE_H265_CAPABILITY_MULTIPLE_TILES_PER_SLICE_SEGMENT_BIT_KHR
    ...
    in section 6.3.1 of the <<itu-t-h265,ITU-T H.265 Specification>>, is
    supported.
...
ifdef::VK_KHR_video_encode_quantization_map[]
  * [[encode-h265-cu-qp-delta-wraparound]]
    ename:VK_VIDEO_ENCODE_H265_CAPABILITY_CU_QP_DIFF_WRAPAROUND_BIT_KHR
    ...
    the <<itu-t-h265,ITU-T H.265 Specification>>.
    If not supported, equation 8-283 of the <<itu-t-h265,ITU-T H.265
    Specification>> is effectively reduced to the following:
+
[eq]#Qp~Y~ = qP~Y_PRED~ {plus} code:CuQpDeltaVal#
+
[NOTE]
====
The effect of this is that the maximum QP difference across subsequent
coding units is limited to the [eq]#[-(26 {plus} QpBdOffset~Y~ / 2), 25
{plus} QpBdOffset~Y~ / 2]# range.
====
endif::VK_KHR_video_encode_quantization_map[]
ifdef::VK_KHR_video_encode_intra_refresh[]
  * ename:VK_VIDEO_ENCODE_H265_CAPABILITY_B_PICTURE_INTRA_REFRESH_BIT_KHR
    specifies support for encoding <<encode-h265-b-pic,B pictures>> with
    <<encode-intra-refresh,intra refresh>> enabled.
endif::VK_KHR_video_encode_intra_refresh[]
--
...
include::{generated}/api/enums/VkVideoEncodeH265StdFlagBitsKHR.adoc[]
  ...
  * ename:VK_VIDEO_ENCODE_H265_STD_PCM_ENABLED_FLAG_SET_BIT_KHR specifies
  ...
  * ename:VK_VIDEO_ENCODE_H265_STD_SPS_TEMPORAL_MVP_ENABLED_FLAG_SET_BIT_KHR
    ...
    <<encode-h265-sps,SPS>> when that value is `1`.
  * ename:VK_VIDEO_ENCODE_H265_STD_INIT_QP_MINUS26_BIT_KHR specifies whether
  ...
  * ename:VK_VIDEO_ENCODE_H265_STD_WEIGHTED_PRED_FLAG_SET_BIT_KHR specifies
    whether the implementation supports using the application-provided value
  ...
  * ename:VK_VIDEO_ENCODE_H265_STD_SLICE_QP_DELTA_BIT_KHR specifies whether
    the implementation supports using the application-provided value for
...
include::{generated}/api/structs/VkVideoEncodeH265SessionCreateInfoKHR.adoc[]
  ...
  * pname:useMaxLevelIdc indicates whether the value of pname:maxLevelIdc
    ...
    When it is ename:VK_FALSE, the implementation ignores the value of
    pname:maxLevelIdc and uses the value of
...
include::{generated}/api/structs/VkVideoEncodeH265SessionParametersGetInfoKHR.adoc[]
  ...
  * pname:stdSPSId specifies the H.265 sequence parameter set ID used to
    ...
    when pname:writeStdSPS and/or pname:writeStdPPS is ename:VK_TRUE.
  * pname:stdPPSId specifies the H.265 picture parameter set ID used to
    ...
    pname:writeStdPPS is ename:VK_TRUE.
...
parameter data to the output buffer in the following order:
  ...
  . The <<encode-h265-vps,H.265 video parameter set>> identified by
    pname:stdVPSId, if pname:writeStdVPS is ename:VK_TRUE.
  . The <<encode-h265-sps,H.265 sequence parameter set>> identified by the
    ...
    pname:writeStdSPS is ename:VK_TRUE.
  . The <<encode-h265-pps,H.265 picture parameter set>> identified by the
    ...
    pname:stdPPSId, if pname:writeStdPPS is ename:VK_TRUE.
...
****
  * [[VUID-VkVideoEncodeH265SessionParametersGetInfoKHR-writeStdVPS-08290]]
    ...
    pname:writeStdPPS must: be ename:VK_TRUE
****
...
the following algorithm:
  ...
  1. The first frame is always an I or an IDR frame.
...
image::{images}/h26x_regular_gop.svg[align="center",title="H.265 regular GOP starting with an I frame",opts="{imageopts}"]

[[encode-h265-idr-period]]
When <<encode-h265-idr-pic,IDR frames>> are used in a video sequence, the
_IDR period_ refers to the distance between subsequent IDR frames.
...
include::{generated}/api/structs/VkVideoEncodeH265RateControlInfoKHR.adoc[]
  ...
  * pname:gopFrameCount is the number of frames within a <<encode-h265-gop,
    ...
    If it is code:UINT32_MAX, the GOP length is treated as infinite.
  * pname:idrPeriod is the interval, in terms of number of frames, between
    ...
    If it is code:UINT32_MAX, the IDR period is treated as infinite.
  ...
  * pname:subLayerCount specifies the number of H.265 sub-layers that the
...
include::{generated}/api/structs/VkVideoEncodeH265RateControlLayerInfoKHR.adoc[]
  ...
  * pname:minQp specifies the lower bounds on the QP values, for each
    ...
    when pname:useMinQp is ename:VK_TRUE.
  ...
  * pname:maxQp specifies the upper bounds on the QP values, for each
    picture type, that the implementation's rate control algorithm will use
    when pname:useMaxQp is ename:VK_TRUE.
  ...
  * pname:maxFrameSize specifies the upper bounds on the encoded frame size,
    for each picture type, when pname:useMaxFrameSize is ename:VK_TRUE.
...
ifdef::VK_KHR_video_encode_quantization_map[]
[[encode-h265-qp-delta-map]]
==== H.265 QP Delta Maps

Quantization delta maps used with an <<encode-h265-profile,H.265 encode
profile>> are referred to as _QP delta maps_ and their texels contain
integer values representing QP delta values that are applied in the process
of determining the <<encode-h265-quantization,quantization parameters>> of
the encoded picture.

Accordingly, H.265 QP delta maps always have single channel integer formats,
as reported in slink:VkVideoFormatPropertiesKHR::pname:format.

When the <<encode-rate-control-modes,rate control mode>> is
ename:VK_VIDEO_ENCODE_RATE_CONTROL_MODE_DISABLED_BIT_KHR, the QP delta
...
For all other <<encode-rate-control-modes,rate control modes>>, the QP delta
values can: be used to offset the QP values that the rate control algorithm
would otherwise produce.
endif::VK_KHR_video_encode_quantization_map[]


[[encode-h265-quantization]]
...
below:
  ...
  * If the configured rate control mode is
    ename:VK_VIDEO_ENCODE_RATE_CONTROL_MODE_DEFAULT_KHR, then the QP value
    is initialized by the implementation-specific default rate control
    algorithm.
ifdef::VK_KHR_video_encode_quantization_map[]
  ** If the video encode operation is issued with a
     <<encode-quantization-delta-map,quantization delta map>>, the QP delta
     value corresponding to the coding unit, as
     <<encode-quantization-map-fetched-texel,fetched from the quantization
     map>>, is added to the previously determined QP value.
     If the fetched QP delta value falls outside the supported QP delta
     value range reported in the pname:minQpDelta and pname:maxQpDelta
     members of slink:VkVideoEncodeH265QuantizationMapCapabilitiesKHR, then
     the QP value used for the coding unit becomes undefined:.
endif::VK_KHR_video_encode_quantization_map[]
  * If the configured rate control mode is
    ename:VK_VIDEO_ENCODE_RATE_CONTROL_MODE_DISABLED_BIT_KHR, then the QP
    value is initialized from the constant QP value specified for the
    <<encode-h265-frame-picture-slice-type,H.265 slice segment>> the coding
    unit is part of.
ifdef::VK_KHR_video_encode_quantization_map[]
  ** If the video encode operation is issued with a
     <<encode-quantization-delta-map,quantization delta map>>, the QP delta
...
ifdef::VK_KHR_video_encode_quantization_map[]
  ...
  ** If the video encode operation is issued with an
     ...
     as <<encode-quantization-map-fetched-texel,fetched from the
     quantization map>>, according to the following equation:
+
[eq]#QP~new~ = f(QP~prev~,e)#
+
Where [eq]#QP~new~# is the resulting QP value, [eq]#QP~prev~# is the
previously determined QP value, [eq]#e# is the emphasis value corresponding
to the coding unit, and [eq]#f# is an implementation-defined function for
which the following implication is true:
+
[eq]#e~1~ < e~2~ => f(QP,e~1~) {geq} f(QP,e~2~)#
+
[NOTE]
====
...
====
  ...
  ** If clamping to maximum QP values is enabled in the applied rate control
     ...
     value.
ifdef::VK_KHR_video_encode_quantization_map[]
  * If ename:VK_VIDEO_ENCODE_H265_CAPABILITY_CU_QP_DIFF_WRAPAROUND_BIT_KHR
    is not supported, then the determined QP value is clamped in such a way
    that the code:CuQpDeltaVal value of the encoded coding unit complies to
    the <<encode-h265-cu-qp-delta-wraparound,modified version>> of equation
    8-283 of the <<itu-t-h265,ITU-T H.265 Specification>>.
+
[NOTE]
====
The effect of this is that the maximum QP difference across subsequent
coding units is limited to the [eq]#[-(26 {plus} QpBdOffset~Y~ / 2), 25
{plus} QpBdOffset~Y~ / 2]# range and only has an observable change in
behavior when the video encode operation is issued with a
<<encode-quantization-delta-map,QP delta map>>.
====
endif::VK_KHR_video_encode_quantization_map[]
  * In all cases, the final QP value is clamped to the QP value range
    supported by the video profile, as reported in the pname:minQp and
    pname:maxQp members of slink:VkVideoEncodeH265CapabilitiesKHR.
...
[[h265_encode-required-video-std-header-versions-table]]
...
[[h265_encode-required-video-capabilities-table]]
...
ifdef::VK_KHR_video_maintenance2[]
| ename:VK_VIDEO_ENCODE_RATE_CONTROL_MODE_DISABLED_BIT_KHR ^4^
endif::VK_KHR_video_maintenance2[]
| min
...
| pname:supportedEncodeFeedbackFlags | ename:VK_VIDEO_ENCODE_FEEDBACK_BITSTREAM_BUFFER_OFFSET_BIT_KHR +
                                       ename:VK_VIDEO_ENCODE_FEEDBACK_BITSTREAM_BYTES_WRITTEN_BIT_KHR
ifdef::VK_KHR_video_encode_feedback2[]
                                     + ename:VK_VIDEO_ENCODE_FEEDBACK_AVERAGE_QUANTIZATION_BIT_KHR ^6^
                                     + ename:VK_VIDEO_ENCODE_FEEDBACK_INTRA_PIXELS_BIT_KHR ^6^
                                     + ename:VK_VIDEO_ENCODE_FEEDBACK_INTER_PIXELS_BIT_KHR ^6^
                                     + ename:VK_VIDEO_ENCODE_FEEDBACK_PICTURE_PARTITION_COUNT_BIT_KHR ^6^
endif::VK_KHR_video_encode_feedback2[]
                                     | min
ifdef::VK_KHR_video_encode_feedback2[]
| **slink:VkVideoEncodeFeedback2CapabilitiesKHR** | |
| pname:maxPerPartitionFeedbackEntries | - | min
| pname:supportedPerPartitionEncodeFeedbackFlags | ename:VK_VIDEO_ENCODE_PER_PARTITION_FEEDBACK_STATUS_BIT_KHR ^7^
                                                 + ename:VK_VIDEO_ENCODE_PER_PARTITION_FEEDBACK_BITSTREAM_BUFFER_OFFSET_BIT_KHR ^7^
                                                 + ename:VK_VIDEO_ENCODE_PER_PARTITION_FEEDBACK_BITSTREAM_BYTES_WRITTEN_BIT_KHR ^7^ | min
endif::VK_KHR_video_encode_feedback2[]
...
| pname:maxQuantizationMapExtent | - ^2^ | min
| **slink:VkVideoEncodeH265QuantizationMapCapabilitiesKHR** | |
| pname:minQpDelta | - ^3^ | max
| pname:maxQpDelta | - ^3^ | min
endif::VK_KHR_video_encode_quantization_map[]
ifdef::VK_KHR_video_encode_intra_refresh[]
| **slink:VkVideoEncodeIntraRefreshCapabilitiesKHR** | |
| pname:intraRefreshModes | 0 | min
| pname:maxIntraRefreshCycleDuration | 2 ^5^ | min
| pname:maxIntraRefreshActiveReferencePictures | 1 ^5^ | min
| pname:partitionIndependentIntraRefreshRegions | - | implementation-dependent
| pname:nonRectangularIntraRefreshRegions | - | implementation-dependent
endif::VK_KHR_video_encode_intra_refresh[]
|====
...
3::
    ...
    pname:maxQpDelta must: be greater than pname:minQpDelta.
endif::VK_KHR_video_encode_quantization_map[]
...
ifdef::VK_KHR_video_encode_feedback2[]
6::
    If the <<features-videoEncodeFeedback2,pname:videoEncodeFeedback2>>
    feature is supported.

7::
    If pname:maxPerPartitionFeedbackEntries is greater than zero.
endif::VK_KHR_video_encode_feedback2[]
