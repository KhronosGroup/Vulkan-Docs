...
include::{generated}/api/enums/VkVideoEncodeH264StdFlagBitsKHR.adoc[]
  ...
  * ename:VK_VIDEO_ENCODE_H264_STD_CHROMA_QP_INDEX_OFFSET_BIT_KHR indicates
    whether the implementation supports using the the application-provided
    value for
    code:StdVideoH264PictureParameterSet::code:chroma_qp_index_offset in the
    <<encode-h264-pps,PPS>> when that value is non-zero.
