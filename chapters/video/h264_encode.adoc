...
section 8 of the <<itu-t-h264,ITU-T H.264 Specification>> as follows:
  ...
  * The encoded bitstream data is written to the destination video bitstream
    buffer range as defined in the <<encode-h264-bitstream-data-access,H.264
  ...
  * Picture data in the <<video-picture-resources,video picture resources>>
    ...
    in the <<encode-h264-picture-data-access,H.264 Encode Picture Data
    Access>> section.
...
encode parameters:
  ...
  * the following parameters specified in
    code:StdVideoH264SequenceParameterSet:
  ** code:flags.vui_parameters_present_flag
  ** code:profile_idc
  ** code:level_idc
  ** code:chroma_format_idc
  * the following parameters specified in the
    code:StdVideoH264SequenceParameterSetVui structure pointed to by
    code:StdVideoH264SequenceParameterSet::code:pSequenceParameterSetVui:
...
In addition, if
slink:VkVideoEncodeH264PictureInfoKHR::pname:generatePrefixNalu is
...
picture are referred to as the _picture partitions_ of the picture.
...
[[encode-h264-coding-blocks]]
=== H.264 Coding Blocks

H.264 encode supports a single type of coding block called a _macroblock_,
as defined in section 3.84 of the <<itu-t-h264,ITU-T H.264 Specification>>.

ifdef::VK_KHR_video_encode_feedback2[]
An H.264 macroblock is considered _skipped_ when the value of code:mb_type
for the macroblock equals code:P_Skip or code:B_Skip, as defined in section
7.4.5 of the <<itu-t-h264,ITU-T H.264 Specification>>.
...
include::{generated}/api/enums/VkVideoEncodeH264CapabilityFlagBitsKHR.adoc[]
  ...
  * ename:VK_VIDEO_ENCODE_H264_CAPABILITY_HRD_COMPLIANCE_BIT_KHR specifies
  ...
  * ename:VK_VIDEO_ENCODE_H264_CAPABILITY_PREDICTION_WEIGHT_TABLE_GENERATED_BIT_KHR
    specifies that if code:StdVideoH264PpsFlags::code:weighted_pred_flag is
    set to `1` or
...
ifdef::VK_KHR_video_encode_intra_refresh[]
    ...
    then the currently refreshed slice must: specify the code:slice_type
    code:STD_VIDEO_H264_SLICE_TYPE_I and can: differ from the
    code:slice_type of the other slices regardless of whether
    ename:VK_VIDEO_ENCODE_H264_CAPABILITY_DIFFERENT_SLICE_TYPE_BIT_KHR is
    supported.
endif::VK_KHR_video_encode_intra_refresh[]
  * ename:VK_VIDEO_ENCODE_H264_CAPABILITY_B_FRAME_IN_L0_LIST_BIT_KHR
    specifies support for using a <<encode-h264-b-pic,B frame>> as L0
  ...
  * ename:VK_VIDEO_ENCODE_H264_CAPABILITY_B_FRAME_IN_L1_LIST_BIT_KHR
    specifies support for using a <<encode-h264-b-pic,B frame>> as L1
    reference, as specified in
...
ifdef::VK_KHR_video_encode_quantization_map[]
  * [[encode-h264-mb-qp-delta-wraparound]]
    ename:VK_VIDEO_ENCODE_H264_CAPABILITY_MB_QP_DIFF_WRAPAROUND_BIT_KHR
    ...
    <<itu-t-h264,ITU-T H.264 Specification>>.
    If not supported, equation 7-37 of the <<itu-t-h264,ITU-T H.264
    Specification>> is effectively reduced to the following:
+
[eq]#QP~Y~ = QP~Y,PREV~ {plus} code:mb_qp_delta#
+
[NOTE]
====
The effect of this is that the maximum QP difference across subsequent
macroblocks is limited to the [eq]#[-(26 {plus} QpBdOffset~Y~ / 2), 25
...
ifdef::VK_KHR_video_encode_intra_refresh[]
  * ename:VK_VIDEO_ENCODE_H264_CAPABILITY_B_PICTURE_INTRA_REFRESH_BIT_KHR
    specifies support for encoding <<encode-h264-b-pic,B pictures>> with
...
include::{generated}/api/enums/VkVideoEncodeH264StdFlagBitsKHR.adoc[]
  ...
  * ename:VK_VIDEO_ENCODE_H264_STD_CHROMA_QP_INDEX_OFFSET_BIT_KHR specifies
  ...
  * ename:VK_VIDEO_ENCODE_H264_STD_PIC_INIT_QP_MINUS26_BIT_KHR specifies
    whether the implementation supports using the application-provided value
    for code:StdVideoH264PictureParameterSet::code:pic_init_qp_minus26 in
    the <<encode-h264-pps,PPS>> when that value is non-zero.
  * ename:VK_VIDEO_ENCODE_H264_STD_WEIGHTED_PRED_FLAG_SET_BIT_KHR specifies
    whether the implementation supports using the application-provided value
  ...
  * ename:VK_VIDEO_ENCODE_H264_STD_SLICE_QP_DELTA_BIT_KHR specifies whether
...
parameter data to the output buffer in the following order:
  ...
  . The <<encode-h264-sps,H.264 sequence parameter set>> identified by
    pname:stdSPSId, if pname:writeStdSPS is ename:VK_TRUE.
...
****
  * [[VUID-VkVideoEncodeH264SessionParametersGetInfoKHR-writeStdSPS-08279]]
    At least one of pname:writeStdSPS and pname:writeStdPPS must: be
    ename:VK_TRUE
****
...
image::{images}/h26x_regular_gop.svg[align="center",title="H.264 regular GOP starting with an I frame",opts="{imageopts}"]

[[encode-h264-idr-period]]
When <<encode-h264-idr-pic,IDR frames>> are used in a video sequence, the
...
ifdef::VK_KHR_video_encode_quantization_map[]
[[encode-h264-qp-delta-map]]
==== H.264 QP Delta Maps

Quantization delta maps used with an <<encode-h264-profile,H.264 encode
...
of determining the <<encode-h264-quantization,quantization parameters>> of
the encoded picture.
...
of the used <<encode-quantization-map-texel-size,quantization map texel
size>>.
...
ifdef::VK_KHR_video_encode_quantization_map[]
  ** If the video encode operation is issued with a
     ...
     members of slink:VkVideoEncodeH264QuantizationMapCapabilitiesKHR, then
  ...
  * If the configured rate control mode is
    ...
    <<encode-h264-frame-picture-slice,H.264 slice>> the macroblock is part
    of.
ifdef::VK_KHR_video_encode_quantization_map[]
  ** If the video encode operation is issued with a
     <<encode-quantization-delta-map,quantization delta map>>, the QP delta
...
to the macroblock, and [eq]#f# is an implementation-defined function for
...
====
  ...
  ** If clamping to maximum QP values is enabled in the applied rate control
     ...
     value.
ifdef::VK_KHR_video_encode_quantization_map[]
  * If ename:VK_VIDEO_ENCODE_H264_CAPABILITY_MB_QP_DIFF_WRAPAROUND_BIT_KHR
    is not supported, then the determined QP value is clamped in such a way
    that the code:mb_qp_delta value of the encoded macroblock complies to
    the <<encode-h264-mb-qp-delta-wraparound,modified version>> of equation
    7-37 of the <<itu-t-h264,ITU-T H.264 Specification>>.
+
[NOTE]
====
The effect of this is that the maximum QP difference across subsequent
...
====
  ...
  * In all cases, the final QP value is clamped to the QP value range
    ...
    pname:maxQp members of slink:VkVideoEncodeH264CapabilitiesKHR.
...
[[h264_encode-required-video-std-header-versions-table]]
...
[[h264_encode-required-video-capabilities-table]]
...
| **slink:VkVideoEncodeH264QuantizationMapCapabilitiesKHR** | |
