...
referred to as the _picture partitions_ of the frame.
...
An AV1 mode info block is considered _skipped_ when the value of code:skip
for the mode info block is set to `1`, as defined in section 5.11.11 of the
<<aomedia-av1,AV1 Specification>>.
...
reference name code:STD_VIDEO_AV1_REFERENCE_NAME_LAST_FRAME (i.e. there is
...
ifdef::VK_KHR_video_encode_intra_refresh[]
  * ename:VK_VIDEO_ENCODE_AV1_CAPABILITY_COMPOUND_PREDICTION_INTRA_REFRESH_BIT_KHR
    ...
    <<encode-av1-prediction-modes,unidirectional or bidirectional compound
    prediction mode>> with <<encode-intra-refresh,intra refresh>> enabled.
...
include::{generated}/api/enums/VkVideoEncodeAV1StdFlagBitsKHR.adoc[]
  ...
  * ename:VK_VIDEO_ENCODE_AV1_STD_UNIFORM_TILE_SPACING_FLAG_SET_BIT_KHR
    ...
    regardless of the coded extent of the <<encode-input-picture,encode
...
image::{images}/av1_regular_gop.svg[align="center",title="AV1 regular GOP starting with an intra-only frame",opts="{imageopts}"]

[[encode-av1-key-frame-period]]
When frames with the AV1 frame type code:STD_VIDEO_AV1_FRAME_TYPE_KEY are
used in a video sequence, the _key frame period_ refers to the distance
between subsequent frames with the AV1 frame type
code:STD_VIDEO_AV1_FRAME_TYPE_KEY.
...
[[av1_encode-required-video-std-header-versions-table]]
...
[[av1_encode-required-video-capabilities-table]]
...
ifdef::VK_KHR_video_encode_feedback2[]
                                     + ename:VK_VIDEO_ENCODE_FEEDBACK_AVERAGE_QUANTIZATION_BIT_KHR ^10^
                                     + ename:VK_VIDEO_ENCODE_FEEDBACK_INTRA_PIXELS_BIT_KHR ^10^
                                     + ename:VK_VIDEO_ENCODE_FEEDBACK_INTER_PIXELS_BIT_KHR ^10^
                                     + ename:VK_VIDEO_ENCODE_FEEDBACK_PICTURE_PARTITION_COUNT_BIT_KHR ^10^
endif::VK_KHR_video_encode_feedback2[]
                                     | min
ifdef::VK_KHR_video_encode_feedback2[]
| **slink:VkVideoEncodeFeedback2CapabilitiesKHR** | |
| pname:maxPerPartitionFeedbackEntries | - | min
| pname:supportedPerPartitionEncodeFeedbackFlags | ename:VK_VIDEO_ENCODE_PER_PARTITION_FEEDBACK_STATUS_BIT_KHR ^11^
                                                 + ename:VK_VIDEO_ENCODE_PER_PARTITION_FEEDBACK_BITSTREAM_BUFFER_OFFSET_BIT_KHR ^11^
                                                 + ename:VK_VIDEO_ENCODE_PER_PARTITION_FEEDBACK_BITSTREAM_BYTES_WRITTEN_BIT_KHR ^11^ | min
endif::VK_KHR_video_encode_feedback2[]
...
| pname:maxIntraRefreshCycleDuration | 2 ^9^ | min
| pname:maxIntraRefreshActiveReferencePictures | 1 ^9^ | min
