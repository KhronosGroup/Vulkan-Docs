...
<<opticalflow-usage,optical usage>>
...
for images with <<opticalflow-usage,optical usage>>
...
include::{generated}/api/structs/VkOpticalFlowSessionCreateInfoNV.adoc[]
  ...
  * pname:hintGridSize is one exactly bit of
    tlink:VkOpticalFlowGridSizeFlagsNV specifying the grid size of of the
    hint flow vector maps to be bound to this optical flow session.
...
include::{generated}/api/structs/VkOpticalFlowSessionCreatePrivateDataInfoNV.adoc[]
  ...
  * pname:size is is the size of data in bytes which is passed at a memory
