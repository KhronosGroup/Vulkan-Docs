...
`apiext:VK_NV_optical_flow` adds an optical flow queue type bit
...
pname:pFormatCount.
Otherwise, pname:pFormatCount must: point to a variable set by the
application to the number of elements in the pname:pImageFormatProperties
array, and on return the variable is overwritten with the number of values
actually written to pname:pImageFormatProperties.
...
ename:VK_FORMAT_R16G16_SFIXED5_NV is initially supported for images with
...
include::{generated}/api/protos/vkBindOpticalFlowSessionImageNV.adoc[]
  ...
  * pname:layout must: specify the layout that the image subresources
    accessible from pname:view will be in at the time the optical flow
    vectors are calculated with flink:vkCmdOpticalFlowExecuteNV on a
    sname:VkDevice.
...
include::{generated}/api/protos/vkCmdOpticalFlowExecuteNV.adoc[]
  ...
  * pname:session is the optical flow session object on which this command
    is operating.
  * pname:pExecuteInfo is a pointer to a slink:VkOpticalFlowExecuteInfoNV
    structure.

include::{generated}/validity/protos/vkCmdOpticalFlowExecuteNV.adoc[]
--

[open,refpage='VkOpticalFlowExecuteInfoNV',desc='Structure specifying parameters of an optical flow vector calculation',type='structs']
--
...
[open,refpage='VkOpticalFlowExecuteFlagBitsNV',desc='Bits specifying flags for an optical flow vector calculation',type='enums']
--
