...
Similarly for accesses to slink:VkMicromapBuildInfoEXT::pname:dstMicromap.
...
The data at pname:data is packed as either one bit per element for
ename:VK_OPACITY_MICROMAP_FORMAT_2_STATE_EXT or two bits per element for
ename:VK_OPACITY_MICROMAP_FORMAT_4_STATE_EXT and is packed from LSB to MSB
in each byte.
...
****
  ...
  * [[VUID-VkMicromapUsageEXT-format-07520]]
    ...
    must: be less than or equal to pname:maxOpacity2StateSubdivisionLevel of
    slink:VkPhysicalDeviceOpacityMicromapPropertiesEXT
  * [[VUID-VkMicromapUsageEXT-format-07521]]
    If the elink:VkMicromapTypeEXT of the micromap is
    ename:VK_MICROMAP_TYPE_OPACITY_MICROMAP_EXT and pname:format is
    ename:VK_OPACITY_MICROMAP_FORMAT_4_STATE_EXT then pname:subdivisionLevel
    must: be less than or equal to pname:maxOpacity4StateSubdivisionLevel of
    slink:VkPhysicalDeviceOpacityMicromapPropertiesEXT
****
...
<<features-micromapHostCommands,pname:micromapHostCommands>> feature is
enabled:
