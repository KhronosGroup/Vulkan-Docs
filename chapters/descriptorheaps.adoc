...
Each descriptor will be written to pname:pDescriptors[i].pname:address where
...
<<memory-external-android-hardware-buffer-external-formats,Android external
format>>
...
equal to
slink:VkSubsampledImageFormatPropertiesEXT::pname:subsampledImageDescriptorCount
for the image.
endif::VK_EXT_fragment_density_map[]

Descriptors using the same pname:type and written using a fully identical
slink:VkTexelBufferDescriptorInfoEXT or slink:VkDeviceAddressRangeKHR
structure on the same slink:VkDevice will always return the same bit
pattern.
If the <<features-descriptorHeapCaptureReplay,descriptorHeapCaptureReplay>>
...
****
  ...
  * [[VUID-vkWriteResourceDescriptorsEXT-pResources-11208]]
    ...
    slink:VkSamplerYcbcrConversionInfo structure
ifdef::VK_ANDROID_external_memory_android_hardware_buffer[]
ifdef::VK_COMPUTE_VERSION_1_4,VK_KHR_maintenance6[]
    without an
    ...
    in its pname:pNext chain, the corresponding element of
    pname:pDescriptors must: have a pname:size member that is greater than
    or equal to the product of the value returned by
    flink:vkGetPhysicalDeviceDescriptorSizeEXT with a pname:descriptorType
    equal to pname:type and
    slink:VkSamplerYcbcrConversionImageFormatProperties::pname:combinedImageSamplerDescriptorCount,
...
ifdef::VK_ANDROID_external_memory_android_hardware_buffer[]
ifdef::VK_COMPUTE_VERSION_1_4,VK_KHR_maintenance6[]
  * [[VUID-vkWriteResourceDescriptorsEXT-pResources-12522]]
    If any element of pname:pResources specifies a
    slink:VkImageViewCreateInfo structure with a
    slink:VkSamplerYcbcrConversionInfo structure with an
    <<memory-external-android-hardware-buffer-external-formats,Android
    external format>> in its pname:pNext chain, the corresponding element of
    pname:pDescriptors must: have a pname:size member that is greater than
    or equal to the product of the value returned by
    flink:vkGetPhysicalDeviceDescriptorSizeEXT with a pname:descriptorType
    equal to pname:type and
...
ifdef::VK_EXT_fragment_density_map[]
  ...
  * [[VUID-vkWriteResourceDescriptorsEXT-pResources-12494]]
    If any element of pname:pResources specifies a
    slink:VkImageViewCreateInfo structure, pname:device must: support at
...
ifdef::VK_KHR_video_decode_queue[]
  ...
  * [[VUID-vkWriteResourceDescriptorsEXT-pResources-12495]]
    If any element of pname:pResources specifies a
    slink:VkImageViewCreateInfo structure,
...
ifdef::VK_EXT_robustness2[]
  ...
  * [[VUID-VkResourceDescriptorInfoEXT-type-12349]]
    If pname:type is ename:VK_DESCRIPTOR_TYPE_UNIFORM_TEXEL_BUFFER or
    ename:VK_DESCRIPTOR_TYPE_STORAGE_TEXEL_BUFFER,
    pname:data->pTexelBuffer->addressRange.address must: be a multiple of
    the effective alignment requirement of pname:data->pTexelBuffer->format
    as determined by
  ...
  * [[VUID-VkResourceDescriptorInfoEXT-type-12350]]
    ...
    pname:data->pAddressRange->address must: be a multiple of
...
+
pname:data->pAddressRange->address must: be a multiple of 256
endif::VK_KHR_acceleration_structure,VK_NV_ray_tracing[]
  * [[VUID-VkResourceDescriptorInfoEXT-type-12496]]
    If pname:type is ename:VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,
    pname:data->pAddressRange->size must: be less than or equal to
    <<limits-maxUniformBufferRange, pname:maxUniformBufferRange>>
  * [[VUID-VkResourceDescriptorInfoEXT-type-12497]]
    If pname:type is ename:VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
...
ifdef::VK_EXT_image_2d_view_of_3d[]
  ...
  * [[VUID-VkResourceDescriptorInfoEXT-type-11458]]
    ...
    pname:data.pImage is not `NULL`, the
    <<resources-image-inherited-usage,effective pname:usage>> of the
    slink:VkImageViewCreateInfo must: include the
...
ifdef::VK_QCOM_image_processing[]
  * [[VUID-VkResourceDescriptorInfoEXT-type-12498]]
    If pname:type is ename:VK_DESCRIPTOR_TYPE_SAMPLE_WEIGHT_IMAGE_QCOM and
    pname:data.pImage is not `NULL`, the
    <<resources-image-inherited-usage,effective pname:usage>> of the
    slink:VkImageViewCreateInfo must: include the
    ename:VK_IMAGE_USAGE_SAMPLE_WEIGHT_BIT_QCOM usage flag
  * [[VUID-VkResourceDescriptorInfoEXT-type-12499]]
    If pname:type is ename:VK_DESCRIPTOR_TYPE_BLOCK_MATCH_IMAGE_QCOM and
    pname:data.pImage is not `NULL`, the
    <<resources-image-inherited-usage,effective pname:usage>> of the
    slink:VkImageViewCreateInfo must: include the
    ename:VK_IMAGE_USAGE_SAMPLE_BLOCK_MATCH_BIT_QCOM usage flag
endif::VK_QCOM_image_processing[]
...
include::{generated}/api/structs/VkResourceDescriptorDataEXT.adoc[]
  ...
  * pname:pAddressRange is a pointer to a slink:VkDeviceAddressRangeKHR
    structure.
ifdef::VK_ARM_tensors[]
...
include::{generated}/api/structs/VkTexelBufferDescriptorInfoEXT.adoc[]
  ...
  * pname:addressRange is a slink:VkDeviceAddressRangeKHR defining the range
...
pname:pBindInfo->reservedRangeSize)# are reserved for the implementation and
...
include::{generated}/api/structs/VkBindHeapInfoEXT.adoc[]
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:heapRange is a slink:VkDeviceAddressRangeKHR defining the device
...
shader using <<descriptorheaps-bindings, set and binding decorations>>,
...
****
  ...
  * [[VUID-VkDescriptorSetAndBindingMappingEXT-source-12452]]
    If pname:source is ename:VK_DESCRIPTOR_MAPPING_SOURCE_PUSH_DATA_EXT,
    pname:sourceData.pushDataOffset must: be less than or equal to
  ...
  * [[VUID-VkDescriptorSetAndBindingMappingEXT-source-12453]]
    If pname:source is ename:VK_DESCRIPTOR_MAPPING_SOURCE_PUSH_ADDRESS_EXT,
    pname:sourceData.pushAddressOffset must: be less than or equal to
...
ifdef::VK_KHR_ray_tracing_pipeline,VK_NV_ray_tracing[]
  ...
  * [[VUID-VkDescriptorSetAndBindingMappingEXT-source-11254]]
    ...
    or ename:VK_DESCRIPTOR_MAPPING_SOURCE_HEAP_WITH_SHADER_RECORD_INDEX_EXT,
    and pname:descriptorSet, pname:firstBinding, and pname:bindingCount
    identify any code:OpTypeSampledImage variables, and
    pname:pEmbeddedSampler is `NULL`, then any pname:samplerHeapOffset and
    pname:samplerHeapArrayStride members of the corresponding member of
    pname:sourceData must: be 0 or a multiple of
    <<limits-samplerDescriptorAlignment, pname:samplerDescriptorAlignment>>
  * [[VUID-VkDescriptorSetAndBindingMappingEXT-source-12406]]
    If pname:source is
    ename:VK_DESCRIPTOR_MAPPING_SOURCE_HEAP_WITH_CONSTANT_OFFSET_EXT,
    ename:VK_DESCRIPTOR_MAPPING_SOURCE_HEAP_WITH_PUSH_INDEX_EXT,
    ename:VK_DESCRIPTOR_MAPPING_SOURCE_HEAP_WITH_INDIRECT_INDEX_EXT,
    ...
    or ename:VK_DESCRIPTOR_MAPPING_SOURCE_HEAP_WITH_SHADER_RECORD_INDEX_EXT,
    and pname:descriptorSet, pname:firstBinding, and pname:bindingCount
    identify any code:OpTypeSampledImage variables, any pname:heapOffset and
    pname:heapArrayStride members of the corresponding member of
    pname:sourceData must: be 0 or a multiple of
    <<limits-imageDescriptorAlignment, pname:imageDescriptorAlignment>>
  * [[VUID-VkDescriptorSetAndBindingMappingEXT-source-12456]]
    If pname:source is
    ename:VK_DESCRIPTOR_MAPPING_SOURCE_HEAP_WITH_PUSH_INDEX_EXT, and
    pname:descriptorSet, pname:firstBinding, and pname:bindingCount identify
    any code:OpTypeSampledImage variables, and pname:pEmbeddedSampler is
    `NULL`, pname:samplerPushOffset must: be a multiple of 4
  * [[VUID-VkDescriptorSetAndBindingMappingEXT-source-12457]]
    If pname:source is
    ename:VK_DESCRIPTOR_MAPPING_SOURCE_HEAP_WITH_PUSH_INDEX_EXT, and
    pname:descriptorSet, pname:firstBinding, and pname:bindingCount identify
    any code:OpTypeSampledImage variables, and pname:pEmbeddedSampler is
    `NULL`, pname:samplerPushOffset must: be less than or equal to
    [eq]#pname:maxPushDataSize - 4#
  * [[VUID-VkDescriptorSetAndBindingMappingEXT-source-12458]]
    If pname:source is
    ename:VK_DESCRIPTOR_MAPPING_SOURCE_HEAP_WITH_INDIRECT_INDEX_EXT or
    ename:VK_DESCRIPTOR_MAPPING_SOURCE_HEAP_WITH_INDIRECT_INDEX_ARRAY_EXT,
    and pname:descriptorSet, pname:firstBinding, and pname:bindingCount
    identify any code:OpTypeSampledImage variables, and
    pname:pEmbeddedSampler is `NULL`, pname:samplerPushOffset must: be a
    multiple of 8
  * [[VUID-VkDescriptorSetAndBindingMappingEXT-source-12459]]
    If pname:source is
    ename:VK_DESCRIPTOR_MAPPING_SOURCE_HEAP_WITH_INDIRECT_INDEX_EXT or
    ename:VK_DESCRIPTOR_MAPPING_SOURCE_HEAP_WITH_INDIRECT_INDEX_ARRAY_EXT,
    and pname:descriptorSet, pname:firstBinding, and pname:bindingCount
    identify any code:OpTypeSampledImage variables, and
    pname:pEmbeddedSampler is `NULL`, pname:samplerPushOffset must: be less
    than or equal to [eq]#pname:maxPushDataSize - 8#
  * [[VUID-VkDescriptorSetAndBindingMappingEXT-source-12460]]
    If pname:source is
    ename:VK_DESCRIPTOR_MAPPING_SOURCE_HEAP_WITH_INDIRECT_INDEX_EXT or
    ename:VK_DESCRIPTOR_MAPPING_SOURCE_HEAP_WITH_INDIRECT_INDEX_ARRAY_EXT,
    and pname:descriptorSet, pname:firstBinding, and pname:bindingCount
    identify any code:OpTypeSampledImage variables, and
    pname:pEmbeddedSampler is `NULL`, pname:samplerAddressOffset must: be a
    multiple of 4
...
.Valid Usage
****
include::{chapters}/commonvalidity/descriptor_heap_embedded_sampler_common.adoc[]
****
