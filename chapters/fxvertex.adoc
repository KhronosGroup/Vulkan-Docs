...
ifdef::VK_KHR_maintenance5[]
  * [[VUID-vkCmdBindVertexBuffers2-pSizes-08769]]
    If pname:pSizes is not `NULL`, all elements of pname:pSizes that are not
    ename:VK_WHOLE_SIZE must: have the corresponding elements in
    pname:pOffsets plus pname:pSizes be less than or equal to the size of
    the corresponding element in pname:pBuffers
  * [[VUID-vkCmdBindVertexBuffers2-pSizes-08770]]
    If pname:pSizes is not `NULL`, all elements of pname:pSizes that are
    ename:VK_WHOLE_SIZE must: have the corresponding element in
    pname:pOffsets be less than the size of the corresponding element in
    pname:pBuffers
endif::VK_KHR_maintenance5[]
