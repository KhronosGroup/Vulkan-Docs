...
ifdef::VK_EXT_vertex_attribute_robustness,VK_KHR_maintenance9[]
In all the following component assignment specifications, if
ifdef::VK_EXT_vertex_attribute_robustness[]
the <<features-vertexAttributeRobustness, pname:vertexAttributeRobustness>>
feature is enabled,
endif::VK_EXT_vertex_attribute_robustness[]
ifdef::VK_EXT_vertex_attribute_robustness,VK_KHR_maintenance9[or]
ifdef::VK_KHR_maintenance9[]
the <<features-maintenance9, pname:maintenance9>> feature is enabled,
endif::VK_KHR_maintenance9[]
and there is no sname:VkVertexInputAttributeDescription::pname:location
specified for the shader vertex attribute code:Location being read, the
value (0,0,0,0) or (0,0,0,1) is used for each of the equivalent (x,y,z,w)
components consumed entries as specified below.
endif::VK_EXT_vertex_attribute_robustness,VK_KHR_maintenance9[]
...
ifdef::VK_GRAPHICS_VERSION_1_3,VK_EXT_extended_dynamic_state[or if]
endif::VK_EXT_shader_object[]
...
[eq]#[0,sname:VkPhysicalDeviceLimits::pname:maxVertexInputAttributes)# not
specified in the pname:pVertexAttributeDescriptions array becomes
undefined:.
...
ifndef::VK_GRAPHICS_VERSION_1_4,VK_KHR_vertex_attribute_divisor,VK_EXT_vertex_attribute_divisor[]
...
[open,refpage='vkCmdBindVertexBuffers3KHR',desc='Bind strided address ranges as vertex buffers to a command buffer and dynamically set strides',type='protos']
--
To bind strided address ranges as vertex buffers to a command buffer for use
in subsequent drawing commands, call:

include::{generated}/api/protos/vkCmdBindVertexBuffers3KHR.adoc[]
  ...
  * pname:pInfos is a pointer to an array of
    slink:VkBindVertexBuffer3InfoKHR structures defining address ranges to
    bind as vertex buffers.

The values taken from the address ranges defined by elements [eq]#i# of
pname:pInfos replace the current state for the vertex input binding
...
elements of pname:pInfos can: have a pname:addressRange.size and
pname:addressRange.address of 0.
If a vertex input attribute is bound to a vertex input binding with a
pname:size and pname:address of 0, the values taken from memory are
...
pname:pInfos[i] to the value of pname:pInfos[i].addressRange.stride
...
[NOTE]
====
Unlike the static state to set the same, pname:stride for each address range
...
****
  ...
  * [[VUID-vkCmdBindVertexBuffers3KHR-addressRange-13073]]
    The pname:addressRange.stride of all elements of pname:pInfos must: be
...
include::{generated}/validity/protos/vkCmdBindVertexBuffers3KHR.adoc[]
--

[open,refpage='VkBindVertexBuffer3InfoKHR',desc='Bind vertex buffer info',type='structs']
--
:refpage: VkBindVertexBuffer3InfoKHR

sname:VkBindVertexBuffer3InfoKHR is defined as:

include::{generated}/api/structs/VkBindVertexBuffer3InfoKHR.adoc[]

  * pname:sType is a elink:VkStructureType value identifying this structure.
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:setStride is a basetype:VkBool32 value indicating whether
    pname:addressRange.stride sets the stride for the buffer.
    If pname:setStride is ename:VK_TRUE, the dynamic stride is set for all
    attributes from this buffer.
    If pname:setStride is ename:VK_FALSE, the stride value is not set.
  * pname:addressRange is the slink:VkStridedDeviceAddressRangeKHR of the
...
include::{chapters}/commonvalidity/address_flags_common.adoc[]
  * [[VUID-VkBindVertexBuffer3InfoKHR-addressRange-13074]]
    ...
    pname:addressRange was queried must: have been created with the
    ename:VK_BUFFER_USAGE_VERTEX_BUFFER_BIT usage flag set
ifdef::VK_EXT_robustness2,VK_KHR_robustness2[]
...
ifdef::VK_EXT_robustness2,VK_KHR_robustness2[]
    ...
    pname:addressRange.size must: not be 0
  * [[VUID-VkBindVertexBuffer3InfoKHR-setStride-13126]]
    If pname:setStride is ename:VK_TRUE, pname:addressRange.stride must: be
    less than or equal to
    sname:VkPhysicalDeviceLimits::pname:maxVertexInputBindingStride
  * [[VUID-VkBindVertexBuffer3InfoKHR-setStride-13127]]
    If pname:setStride is ename:VK_FALSE, pname:addressRange.stride must: be
    0
****

include::{generated}/validity/structs/VkBindVertexBuffer3InfoKHR.adoc[]
--
...
ifdef::VK_GRAPHICS_VERSION_1_3,VK_EXT_extended_dynamic_state,VK_EXT_shader_object[]
[open,refpage='vkCmdBindVertexBuffers2',desc='Bind vertex buffers to a command buffer and dynamically set strides',type='protos']
--
...
:refpage: vkCmdBindVertexBuffers2
...
[open,refpage='VkPipelineVertexInputDivisorStateCreateInfo',desc='Structure specifying vertex attributes assignment during instanced rendering',type='structs']
--
If the <<features-vertexAttributeInstanceRateDivisor,
...
sname:VkPipelineVertexInputDivisorStateCreateInfo structure, then that
structure controls how vertex attributes are assigned to an instance when
instanced rendering is enabled.

The sname:VkPipelineVertexInputDivisorStateCreateInfo structure is defined
as:

include::{generated}/api/structs/VkPipelineVertexInputDivisorStateCreateInfo.adoc[]
...
include::{generated}/api/structs/VkPipelineVertexInputDivisorStateCreateInfoEXT.adoc[]
  ...
  * pname:pVertexBindingDivisors is a pointer to an array of
    slink:VkVertexInputBindingDivisorDescription structures specifying the
    divisor value for each binding.

include::{generated}/validity/structs/VkPipelineVertexInputDivisorStateCreateInfo.adoc[]
--

[open,refpage='VkVertexInputBindingDivisorDescription',desc='Structure specifying a divisor used in instanced rendering',type='structs']
--
The individual divisor values per binding are specified using the
sname:VkVertexInputBindingDivisorDescription structure which is defined as:

include::{generated}/api/structs/VkVertexInputBindingDivisorDescription.adoc[]
...
include::{generated}/api/structs/VkVertexInputBindingDivisorDescriptionEXT.adoc[]
  ...
  * pname:divisor is the number of successive instances that will use the
    ...
    The maximum value of pname:divisor is implementation-dependent and can
    be queried using
    slink:VkPhysicalDeviceVertexAttributeDivisorProperties::pname:maxVertexAttribDivisor.
    A value of `0` can: be used for the divisor if the
    <<features-vertexAttributeInstanceRateZeroDivisor,
    pname:vertexAttributeInstanceRateZeroDivisor>> feature is enabled.
    In this case, the same vertex attribute will be applied to all
...
include::{generated}/validity/structs/VkVertexInputBindingDivisorDescription.adoc[]
--
endif::VK_GRAPHICS_VERSION_1_4,VK_KHR_vertex_attribute_divisor,VK_EXT_vertex_attribute_divisor[]
...
code:instanceIndex is calculated as follows:
  ...
  * Let code:offset be an array of offsets into the bound vertex buffers
    specified during fname:vkCmdBindVertexBuffers
ifdef::VK_GRAPHICS_VERSION_1_3,VK_EXT_extended_dynamic_state[or fname:vkCmdBindVertexBuffers2]
    with pname:pOffsets.
ifdef::VK_GRAPHICS_VERSION_1_4,VK_KHR_vertex_attribute_divisor,VK_EXT_vertex_attribute_divisor[]
  * Let code:divisor be the member of
    slink:VkPipelineVertexInputDivisorStateCreateInfo::pname:pVertexBindingDivisors
    with slink:VkVertexInputBindingDivisorDescription::pname:binding equal
    to code:attribDesc.binding.
ifdef::VK_EXT_vertex_input_dynamic_state,VK_EXT_shader_object[]
...
ifdef::VK_GRAPHICS_VERSION_1_3,VK_EXT_extended_dynamic_state,VK_EXT_shader_object[]
  ** flink:vkCmdBindVertexBuffers2::pname:pStrides, if not `NULL`
endif::VK_GRAPHICS_VERSION_1_3,VK_EXT_extended_dynamic_state,VK_EXT_shader_object[]
...
converted from the slink:VkVertexInputAttributeDescription's pname:format in
the same manner as described for <<images-reads, image reads>> as if a texel
were read from that address.
The numeric type of the attribute's pname:format must: match the numeric
...
<<shaders-execution-memory-access-bounds>>.

If
ifdef::VK_EXT_legacy_vertex_attributes[]
either pname:format is a 64-bit format or the
<<features-legacyVertexAttributes, pname:legacyVertexAttributes>> feature is
not enabled, and
...
Otherwise,
ifdef::VK_EXT_legacy_vertex_attributes[]
if either pname:format is a 64-bit format or the
<<features-legacyVertexAttributes, pname:legacyVertexAttributes>> feature is
not enabled,
endif::VK_EXT_legacy_vertex_attributes[]
...
If the <<formats-numericformat, numeric format>> of pname:format uses sRGB
encoding, and the <<features-maintenance10,pname:maintenance10>> feature is
...
as described in the "`sRGB EOTF`" section of the <<data-format,Khronos Data
Format Specification>>.
endif::VK_KHR_maintenance10[]
Implementations
ifdef::VK_KHR_maintenance10[which do not support <<features-maintenance10,pname:maintenance10>>, and]
which do not convert nonlinear to linear for sRGB formats should: not expose
ename:VK_FORMAT_FEATURE_VERTEX_BUFFER_BIT for such formats.
