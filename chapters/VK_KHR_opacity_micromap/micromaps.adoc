...
Micromaps created with `apiext:VK_KHR_opacity_micromap`, which are
represented by slink:VkAccelerationStructureKHR objects, are built with
<<acceleration-structure-building,device build commands>>.

[open,refpage='VkAccelerationStructureGeometryMicromapDataKHR',desc='Structure specifying the data used to build a micromap',type='structs']
--
:refpage: VkAccelerationStructureGeometryMicromapDataKHR

When building micromaps with flink:vkCmdBuildAccelerationStructuresKHR, a
sname:VkAccelerationStructureGeometryMicromapDataKHR structure can be added
to the pname:pNext chain of slink:VkAccelerationStructureGeometryKHR to
specify the data used to build the micromap.

The sname:VkAccelerationStructureGeometryMicromapDataKHR structure is
defined as:

include::{generated}/api/structs/VkAccelerationStructureGeometryMicromapDataKHR.adoc[]
  ...
  * pname:pUsageCounts is a pointer to an array of slink:VkMicromapUsageKHR
    structures.
  * pname:ppUsageCounts is a pointer to an array of pointers to
    slink:VkMicromapUsageKHR structures.
  * pname:data is the device address to memory which contains the data for
    the micromap.
  * pname:triangleArray is the device address to memory containing the
    slink:VkMicromapTriangleKHR data
...
In practice, compilers compile slink:VkMicromapTriangleKHR to match this
pattern.

For opacity micromaps, the data at pname:data is packed as either one bit
per element for ename:VK_OPACITY_MICROMAP_FORMAT_2_STATE_KHR or two bits per
element for ename:VK_OPACITY_MICROMAP_FORMAT_4_STATE_KHR and is packed from
...
****
  ...
  * [[VUID-VkAccelerationStructureGeometryMicromapDataKHR-count-11643]]
    The total sum of pname:count from all elements of pname:pUsageCounts and
    pname:ppUsageCounts must: be less than or equal to
    <<limits-maxMicromapTriangles,pname:maxMicromapTriangles>>
  * [[VUID-VkAccelerationStructureGeometryMicromapDataKHR-subdivisionLevel-11645]]
    For each member of pname:pUsageCounts or pname:ppUsageCounts, if
    pname:format is ename:VK_OPACITY_MICROMAP_FORMAT_2_STATE_KHR,
  ...
  * [[VUID-VkAccelerationStructureGeometryMicromapDataKHR-triangleArrayStride-11646]]
    pname:triangleArrayStride must: be greater than or equal to `8` bytes
  * [[VUID-VkAccelerationStructureGeometryMicromapDataKHR-triangleArrayStride-11647]]
    pname:triangleArrayStride must: be a multiple of `4` bytes
****
include::{generated}/validity/structs/VkAccelerationStructureGeometryMicromapDataKHR.adoc[]
--

[open,refpage='VkMicromapUsageKHR',desc='Structure specifying the usage information used to build a micromap',type='structs']
--
:refpage: VkMicromapUsageKHR

The sname:VkMicromapUsageKHR structure is defined as:

include::{generated}/api/structs/VkMicromapUsageKHR.adoc[]
...
include::{generated}/validity/structs/VkMicromapUsageKHR.adoc[]
--
...
[open,refpage='VkMicromapTriangleKHR',desc='Structure specifying the micromap format and data for a triangle',type='structs',alias='VkMicromapTriangleEXT']
--
:refpage: VkMicromapTriangleKHR

The sname:VkMicromapTriangleKHR structure is defined as:

include::{generated}/api/structs/VkMicromapTriangleKHR.adoc[]
...
include::{generated}/validity/structs/VkMicromapTriangleKHR.adoc[]
--

[open,refpage='VkOpacityMicromapFormatKHR',desc='Format enum for opacity micromaps',type='enums',alias='VkOpacityMicromapFormatEXT']
--
:refpage: VkOpacityMicromapFormatKHR

Formats which can: be set in slink:VkMicromapUsageKHR::pname:format and
slink:VkMicromapTriangleKHR::pname:format for micromap builds, are:

include::{generated}/api/enums/VkOpacityMicromapFormatKHR.adoc[]

ifdef::VK_EXT_opacity_micromap[]
include::{generated}/api/enums/VkOpacityMicromapFormatEXT.adoc[]
endif::VK_EXT_opacity_micromap[]

  * ename:VK_OPACITY_MICROMAP_FORMAT_2_STATE_KHR specifies that the given
    micromap format has one bit per subtriangle encoding either fully opaque
    or fully transparent.
  * ename:VK_OPACITY_MICROMAP_FORMAT_4_STATE_KHR specifies that the given
    micromap format has two bits per subtriangle encoding four modes which
    can be interpreted as described in <<ray-opacity-micromap,Ray Opacity
    Micromap>>.
...
To build micromaps represented as a slink:VkMicromapEXT object call:
...
include::{chapters}/commonvalidity/build_micromap_common.adoc[]
  * [[VUID-vkCmdBuildMicromapsEXT-micromap-11648]]
    The
  ...
  * [[VUID-vkCmdBuildMicromapsEXT-pInfos-07512]]
    For each element of pname:pInfos, if the value of
    slink:VkMicromapBuildSizesInfoEXT::pname:buildScratchSize, returned from
    a call to flink:vkGetMicromapBuildSizesEXT with an identical
    slink:VkMicromapBuildInfoEXT structure, is not `0`,
    pname:scratchData.deviceAddress must: be a valid
    basetype:VkDeviceAddress
  * [[VUID-vkCmdBuildMicromapsEXT-pInfos-10896]]
    For each element of pname:pInfos, pname:data.deviceAddress must: be a
    valid basetype:VkDeviceAddress
  * [[VUID-vkCmdBuildMicromapsEXT-pInfos-10897]]
    For each element of pname:pInfos, pname:triangleArray.deviceAddress
  ...
  * [[VUID-vkCmdBuildMicromapsEXT-dstMicromap-11649]]
    The range of memory backing the pname:dstMicromap member of any element
    of pname:pInfos that is accessed by the execution of this command on the
    device must: not overlap the memory backing the pname:dstMicromap member
    of any other element of pname:pInfos, which is accessed by the execution
    of this command on the device
  * [[VUID-vkCmdBuildMicromapsEXT-dstMicromap-11650]]
    The range of memory backing the pname:dstMicromap member of any element
    of pname:pInfos that is accessed by the execution of this command on the
    device must: not overlap the memory backing the pname:scratchData member
    of any element of pname:pInfos (including the same element), which is
    accessed by the execution of this command on the device
  * [[VUID-vkCmdBuildMicromapsEXT-scratchData-11651]]
...
include::{generated}/api/structs/VkMicromapBuildInfoEXT.adoc[]
  ...
  * pname:data is the device or host address of memory which contains the
  ...
  * pname:triangleArray is the device or host address of memory containing
...
****
  ...
  * [[VUID-VkMicromapBuildInfoEXT-format-11652]]
    If pname:type is ename:VK_MICROMAP_TYPE_OPACITY_MICROMAP_EXT, for each
    member of pname:pUsageCounts or pname:ppUsageCounts, pname:format must:
    be a valid value from ename:VkOpacityMicromapFormatEXT
ifdef::VK_NV_displacement_micromap[]
  * [[VUID-VkMicromapBuildInfoEXT-format-11653]]
    If pname:type is ename:VK_MICROMAP_TYPE_DISPLACEMENT_MICROMAP_NV, for
    each member of pname:pUsageCounts or pname:ppUsageCounts, pname:format
    must: be a valid value from ename:VkDisplacementMicromapFormatNV
endif::VK_NV_displacement_micromap[]
  * [[VUID-VkMicromapBuildInfoEXT-subdivisionLevel-11654]]
    If pname:type is ename:VK_MICROMAP_TYPE_OPACITY_MICROMAP_EXT, for each
    member of pname:pUsageCounts or pname:ppUsageCounts, if pname:format is
    ename:VK_OPACITY_MICROMAP_FORMAT_2_STATE_EXT, pname:subdivisionLevel
  ...
  * [[VUID-VkMicromapBuildInfoEXT-subdivisionLevel-11655]]
    If pname:type is ename:VK_MICROMAP_TYPE_OPACITY_MICROMAP_EXT, for each
    member of pname:pUsageCounts or pname:ppUsageCounts, if pname:format is
    ename:VK_OPACITY_MICROMAP_FORMAT_4_STATE_EXT, pname:subdivisionLevel
    must: be less than or equal to
...
ifdef::VK_NV_displacement_micromap[]
  * [[VUID-VkMicromapBuildInfoEXT-subdivisionLevel-11656]]
    If pname:type is ename:VK_MICROMAP_TYPE_DISPLACEMENT_MICROMAP_NV, for
    each member of pname:pUsageCounts or pname:ppUsageCounts,
    pname:subdivisionLevel must: be less than or equal to
    slink:VkPhysicalDeviceDisplacementMicromapPropertiesNV::pname:maxDisplacementMicromapSubdivisionLevel
endif::VK_NV_displacement_micromap[]
  * [[VUID-VkMicromapBuildInfoEXT-format-11657]]
    If pname:type is ename:VK_MICROMAP_TYPE_OPACITY_MICROMAP_EXT, for each
    member of pname:triangleArray, pname:format must: be a valid value from
    ename:VkOpacityMicromapFormatEXT
ifdef::VK_NV_displacement_micromap[]
  * [[VUID-VkMicromapBuildInfoEXT-format-11658]]
    If pname:type is ename:VK_MICROMAP_TYPE_DISPLACEMENT_MICROMAP_NV, for
    each member of pname:triangleArray, pname:format must: be a valid value
    from ename:VkDisplacementMicromapFormatNV
endif::VK_NV_displacement_micromap[]
  * [[VUID-VkMicromapBuildInfoEXT-subdivisionLevel-11659]]
    If pname:type is ename:VK_MICROMAP_TYPE_OPACITY_MICROMAP_EXT, for each
    member of pname:triangleArray, if pname:format is
    ename:VK_OPACITY_MICROMAP_FORMAT_2_STATE_EXT, pname:subdivisionLevel
    must: be less than or equal to
    <<limits-maxOpacity2StateSubdivisionLevelEXT,
    slink:VkPhysicalDeviceOpacityMicromapPropertiesEXT::pname:maxOpacity2StateSubdivisionLevel>>
  * [[VUID-VkMicromapBuildInfoEXT-subdivisionLevel-11660]]
    If pname:type is ename:VK_MICROMAP_TYPE_OPACITY_MICROMAP_EXT, for each
    member of pname:triangleArray, if pname:format is
    ename:VK_OPACITY_MICROMAP_FORMAT_4_STATE_EXT, pname:subdivisionLevel
    must: be less than or equal to
    ...
    slink:VkPhysicalDeviceOpacityMicromapPropertiesEXT::pname:maxOpacity4StateSubdivisionLevel>>
ifdef::VK_NV_displacement_micromap[]
  * [[VUID-VkMicromapBuildInfoEXT-subdivisionLevel-11661]]
    If pname:type is ename:VK_MICROMAP_TYPE_DISPLACEMENT_MICROMAP_NV, for
    each member of pname:triangleArray, pname:subdivisionLevel must: be less
    than or equal to
    slink:VkPhysicalDeviceDisplacementMicromapPropertiesNV::pname:maxDisplacementMicromapSubdivisionLevel
endif::VK_NV_displacement_micromap[]
****
...
[open,refpage='VkBuildMicromapFlagBitsEXT',desc='Bitmask specifying additional parameters for micromap builds',type='enums']
--
...
include::{generated}/api/enums/VkBuildMicromapFlagBitsEXT.adoc[]
  ...
  * ename:VK_BUILD_MICROMAP_PREFER_FAST_TRACE_BIT_EXT specifies that the
    given micromap build should: prioritize trace performance over build
    time.
  * ename:VK_BUILD_MICROMAP_PREFER_FAST_BUILD_BIT_EXT specifies that the
    given micromap build should: prioritize build time over trace
    performance.
  * ename:VK_BUILD_MICROMAP_ALLOW_COMPACTION_BIT_EXT specifies that the
    micromap can: act as the source for a <<micromap-copying, copy micromap
    command>> with pname:mode of ename:VK_COPY_MICROMAP_MODE_COMPACT_EXT to
    produce a compacted micromap.
--

[open,refpage='VkBuildMicromapFlagsEXT',desc='Bitmask of VkBuildMicromapFlagBitsEXT',type='flags']
--
...
include::{generated}/api/enums/VkDisplacementMicromapFormatNV.adoc[]
  ...
  * ename:VK_DISPLACEMENT_MICROMAP_FORMAT_64_TRIANGLES_64_BYTES_NV specifies
...
ifdef::VK_KHR_opacity_micromap[]
Micromaps created with `apiext:VK_KHR_opacity_micromap`, which are
represented by slink:VkAccelerationStructureKHR objects, are copied with
<<acceleration-structure-copying,device copy commands>>.
endif::VK_KHR_opacity_micromap[]

ifdef::VK_EXT_opacity_micromap[]
For slink:VkMicromapEXT, an additional command exists for copying micromaps
...
To query slink:VkMicromapEXT size parameters call:
...
****
  ...
  * [[VUID-vkCmdWriteMicromapsPropertiesEXT-None-12417]]
    All queries used by this command must: be _unavailable_
  ...
  * [[VUID-vkCmdWriteMicromapsPropertiesEXT-pMicromaps-11662]]
...
To copy a slink:VkMicromapEXT call:
...
****
  ...
  * [[VUID-vkCmdCopyMicromapEXT-buffer-07530]]
    The pname:buffer used to create pname:pInfo->dst must: must: be bound to
    device memory
  * [[VUID-vkCmdCopyMicromapEXT-src-07532]]
...
To copy a slink:VkMicromapEXT to device memory call:
...
<<synchronization-access-types, access type>> of
ename:VK_ACCESS_2_MICROMAP_READ_BIT_EXT.
Accesses to the buffer indicated by pname:pInfo->dst must: be synchronized
with the ename:VK_PIPELINE_STAGE_2_MICROMAP_BUILD_BIT_EXT pipeline stage and
an access type of ename:VK_ACCESS_TRANSFER_WRITE_BIT.
...
compatible for use with either flink:vkCmdCopyMemoryToMicromapEXT or
...
<<synchronization-access-types, access type>> of
ename:VK_ACCESS_2_MICROMAP_WRITE_BIT_EXT.
Accesses to the buffer indicated by pname:pInfo->src must: be synchronized
with the ename:VK_PIPELINE_STAGE_2_MICROMAP_BUILD_BIT_EXT pipeline stage and
an access type of ename:VK_ACCESS_TRANSFER_READ_BIT.

This command can: accept micromaps produced by either
flink:vkCopyMicromapToMemoryEXT or flink:vkCmdCopyMicromapToMemoryEXT.
...
To check if a serialized slink:VkMicromapEXT is compatible with the current
device call:
...
.Valid Usage
****
  * [[VUID-vkGetDeviceMicromapCompatibilityEXT-micromap-11666]]
    The
    <<features-micromapEXT,sname:VkPhysicalDeviceOpacityMicromapFeaturesEXT::pname:micromap>>
    feature must: be enabled
****
...
To build slink:VkMicromapEXT on the host, call:
...
ifdef::VK_KHR_device_group,VK_BASE_VERSION_1_1[]
  ...
  * [[VUID-vkBuildMicromapsEXT-dstMicromap-11667]]
    The range of memory backing the pname:dstMicromap member of any element
  ...
  * [[VUID-vkBuildMicromapsEXT-dstMicromap-11668]]
    The range of memory backing the pname:dstMicromap member of any element
    of pname:pInfos that is accessed by this command must: not overlap the
    ...
    command
  * [[VUID-vkBuildMicromapsEXT-scratchData-11669]]
    The range of memory backing the pname:scratchData member of any element
    of pname:pInfos that is accessed by this command must: not overlap the
...
To copy or compact a slink:VkMicromapEXT on the host, call:
...
ifdef::VK_KHR_device_group,VK_BASE_VERSION_1_1[]
  ...
  * [[VUID-vkCopyMicromapEXT-src-11670]]
    The source micromap pname:src must: have been constructed prior to the
    execution of this command
****
...
To copy host accessible slink:VkMicromapEXT to a micromap, call:
...
To copy a slink:VkMicromapEXT to host accessible memory, call:
...
for use with either flink:vkCmdCopyMemoryToMicromapEXT or
flink:vkCopyMemoryToMicromapEXT.
...
To query slink:VkMicromapEXT size parameters on the host, call:
...
ifdef::VK_KHR_device_group,VK_BASE_VERSION_1_1[]
  ...
  * [[VUID-vkWriteMicromapsPropertiesEXT-pMicromaps-11671]]
    All micromaps in pname:pMicromaps must: have been constructed prior to
