...
and:
  ...
  ** pname:usage includes ename:VK_IMAGE_USAGE_SAMPLED_BIT, and
  ** pname:flags does not include any of
...
Then the result of calls to fname:vkGetPhysicalDeviceImageFormatProperties
with identical parameters except for the inclusion of
ename:VK_IMAGE_USAGE_HOST_TRANSFER_BIT in pname:usage must: be identical.
...
and:
  ...
  ** pname:pImageFormatInfo->usage includes ename:VK_IMAGE_USAGE_SAMPLED_BIT
  ** pname:pImageFormatInfo->flags does not include either of
     ename:VK_IMAGE_CREATE_SPARSE_BINDING_BIT,
     ename:VK_IMAGE_CREATE_SPARSE_RESIDENCY_BIT, or
     ename:VK_IMAGE_CREATE_SPARSE_ALIASED_BIT
ifdef::VK_BASE_VERSION_1_1,VK_KHR_external_memory_capabilities[]
  ** The pname:pNext chain of pname:pImageFormatInfo does not include a
     slink:VkPhysicalDeviceExternalImageFormatInfo structure with non-zero
     pname:handleType
endif::VK_BASE_VERSION_1_1,VK_KHR_external_memory_capabilities[]
ifdef::VK_EXT_image_drm_format_modifier[]
  ** pname:pImageFormatInfo->tiling is not
     ename:VK_IMAGE_TILING_DRM_FORMAT_MODIFIER_EXT
endif::VK_EXT_image_drm_format_modifier[]

Then the result of calls to fname:vkGetPhysicalDeviceImageFormatProperties2
with identical parameters except for the inclusion of
ename:VK_IMAGE_USAGE_HOST_TRANSFER_BIT in pname:pImageFormatInfo->usage
must: be identical.
endif::VK_BASE_VERSION_1_4,VK_EXT_host_image_copy[]

include::{chapters}/commonvalidity/no_dynamic_allocations_common.adoc[]

ifdef::VK_BASE_VERSION_1_4,VK_EXT_host_image_copy,VK_ANDROID_external_memory_android_hardware_buffer[]
.Valid Usage
****
...
ifdef::VK_BASE_VERSION_1_4,VK_EXT_host_image_copy[]
  * [[VUID-vkGetPhysicalDeviceImageFormatProperties2-pNext-09004]]
    If the pname:pNext chain of pname:pImageFormatProperties includes a
    slink:VkHostImageCopyDevicePerformanceQuery structure,
    pname:pImageFormatInfo->usage must: contain
    ename:VK_IMAGE_USAGE_HOST_TRANSFER_BIT
endif::VK_BASE_VERSION_1_4,VK_EXT_host_image_copy[]
****
endif::VK_BASE_VERSION_1_4,VK_EXT_host_image_copy,VK_ANDROID_external_memory_android_hardware_buffer[]
...
ifdef::VK_ARM_tensors[tensors,]
    ...
    and slink:VkMemoryDedicatedAllocateInfo
ifdef::VK_ARM_tensors[or slink:VkMemoryDedicatedAllocateInfoTensorARM]
    to create (or import) a dedicated allocation for the
ifdef::VK_ARM_tensors[tensor,]
    image or buffer.
...
ifdef::VK_EXT_external_memory_host[]
Implementations must: not report
ename:VK_EXTERNAL_MEMORY_FEATURE_DEDICATED_ONLY_BIT for
ifdef::VK_ARM_tensors[tensors,]
...
ifdef::VK_KHR_maintenance11[]
  * [[VUID-VkPhysicalDeviceImageDrmFormatModifierInfoEXT-maintenance11-13351]]
    If the <<features-maintenance11,pname:maintenance11>> feature is
    supported and pname:sharingMode is ename:VK_SHARING_MODE_CONCURRENT,
    then pname:queueFamilyIndexCount must: be greater than `0`
...
--
To determine the number of <<descriptors,descriptors>> required to support
an image format for sampled images with <<samplers-YCbCr-conversion,{YCbCr}
conversion>>, add slink:VkSamplerYcbcrConversionImageFormatProperties to the
pname:pNext chain of the slink:VkImageFormatProperties2 structure in a call
to fname:vkGetPhysicalDeviceImageFormatProperties2.
...
include::{generated}/api/structs/VkSamplerYcbcrConversionImageFormatPropertiesKHR.adoc[]
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:combinedImageSamplerDescriptorCount is the number of
    <<descriptors,descriptors>> that the implementation uses to access a
    sampled image with <<samplers-YCbCr-conversion,{YCbCr} conversion>>
...
The size an image descriptor with a slink:VkSamplerYcbcrConversionInfo
structure consumes in the resource heap is multiplied by the
pname:combinedImageSamplerDescriptorCount of its format.
...
layout that includes at least one immutable {YCbCr} conversion sampler will
internally use a number of descriptors equal to the maximum
pname:combinedImageSamplerDescriptorCount of the formats of all the
immutable {YCbCr} conversion samplers in that binding.
...
{YCbCr} samplers with formats that have
...
The maximum pname:combinedImageSamplerDescriptorCount for this binding
layout is therefore `3`.

If there are `3` descriptors in a descriptor set binding with this layout,
including one descriptor without an immutable {YCbCr} sampler, the binding
...
to a heap offset with slink:VkDescriptorSetAndBindingMappingEXT and should:
...
property to determine the maximum pname:combinedImageSamplerDescriptorCount
...
ifdef::VK_EXT_descriptor_heap[]
ifdef::VK_EXT_fragment_density_map[]
[open,refpage='VkSubsampledImageFormatPropertiesEXT',desc='Structure specifying image descriptor count for subsampled images',type='structs']
--
To determine the number of image descriptors required to support a
subsampled image created with ename:VK_IMAGE_CREATE_SUBSAMPLED_BIT_EXT, add
slink:VkSubsampledImageFormatPropertiesEXT to the pname:pNext chain of the
slink:VkImageFormatProperties2 structure in a call to
fname:vkGetPhysicalDeviceImageFormatProperties2.

The sname:VkSubsampledImageFormatPropertiesEXT structure is defined as:

include::{generated}/api/structs/VkSubsampledImageFormatPropertiesEXT.adoc[]

  * pname:sType is a elink:VkStructureType value identifying this structure.
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:subsampledImageDescriptorCount is the number of image descriptors
    that the implementation uses to access the image.

include::{generated}/validity/structs/VkSubsampledImageFormatPropertiesEXT.adoc[]
--
endif::VK_EXT_fragment_density_map[]
endif::VK_EXT_descriptor_heap[]
...
ifdef::VK_BASE_VERSION_1_4,VK_EXT_host_image_copy[]
[open,refpage='VkHostImageCopyDevicePerformanceQuery',desc='Struct containing information about optimality of device access',type='structs']
--
To query if using ename:VK_IMAGE_USAGE_HOST_TRANSFER_BIT has a negative
impact on device performance when accessing an image, add
ename:VK_IMAGE_USAGE_HOST_TRANSFER_BIT to
slink:VkPhysicalDeviceImageFormatInfo2::pname:usage, and add a
sname:VkHostImageCopyDevicePerformanceQuery structure to the pname:pNext
chain of a slink:VkImageFormatProperties2 structure passed to
flink:vkGetPhysicalDeviceImageFormatProperties2.
This structure is defined as:

include::{generated}/api/structs/VkHostImageCopyDevicePerformanceQuery.adoc[]
...
include::{generated}/api/structs/VkHostImageCopyDevicePerformanceQueryEXT.adoc[]
  ...
  * pname:optimalDeviceAccess returns ename:VK_TRUE if use of host image
    ...
    the same slink:VkDeviceMemory, except that the
    ename:VK_IMAGE_USAGE_HOST_TRANSFER_BIT usage flag is replaced with
  ...
  * pname:identicalMemoryLayout returns ename:VK_TRUE if use of host image
    ...
    slink:VkDeviceMemory, except that the
    ename:VK_IMAGE_USAGE_HOST_TRANSFER_BIT usage flag is replaced with
    ename:VK_IMAGE_USAGE_TRANSFER_SRC_BIT and
    ename:VK_IMAGE_USAGE_TRANSFER_DST_BIT.
...
include::{generated}/validity/structs/VkHostImageCopyDevicePerformanceQuery.adoc[]
--
endif::VK_BASE_VERSION_1_4,VK_EXT_host_image_copy[]
...
pname:sampleCounts will be ename:VK_SAMPLE_COUNT_1_BIT if at least one of
...
--
:refpage: VkPhysicalDeviceExternalBufferInfo
...
slink:VkBufferUsageFlags2CreateInfo::pname:usage.
...
[open,refpage='vkGetPhysicalDeviceExternalTensorPropertiesARM',desc='Function for querying external tensor handle capabilities.',type='protos']
--
To query the external handle types supported by tensors, call:

include::{generated}/api/protos/vkGetPhysicalDeviceExternalTensorPropertiesARM.adoc[]

  * pname:physicalDevice is the physical device from which to query the
    tensor capabilities.
  * pname:pExternalTensorInfo is a pointer to a
    slink:VkPhysicalDeviceExternalTensorInfoARM structure describing the
    parameters that would be consumed by flink:vkCreateTensorARM.
  * pname:pExternalTensorProperties is a pointer to a
    slink:VkExternalTensorPropertiesARM structure in which the capabilities
    are returned.

include::{generated}/validity/protos/vkGetPhysicalDeviceExternalTensorPropertiesARM.adoc[]
--

[open,refpage='VkPhysicalDeviceExternalTensorInfoARM',desc='Structure specifying tensor creation parameters.',type='structs']
--
The sname:VkPhysicalDeviceExternalTensorInfoARM structure is defined as:

include::{generated}/api/structs/VkPhysicalDeviceExternalTensorInfoARM.adoc[]
  ...
  * pname:pDescription is a slink:VkTensorDescriptionARM structure
    describing the tensor, corresponding to
    slink:VkTensorCreateInfoARM::pname:pDescription.
  * pname:handleType is a elink:VkExternalMemoryHandleTypeFlagBits value
    specifying the external memory handle type for which capabilities will
    be returned.

include::{generated}/validity/structs/VkPhysicalDeviceExternalTensorInfoARM.adoc[]
--

[open,refpage='VkExternalTensorPropertiesARM',desc='Structure specifying supported external handle capabilities for a tensor',type='structs']
--
The sname:VkExternalTensorPropertiesARM structure is defined as:

include::{generated}/api/structs/VkExternalTensorPropertiesARM.adoc[]
...
include::{generated}/validity/structs/VkExternalTensorPropertiesARM.adoc[]
--
...
ifdef::VK_NV_external_sci_sync,VK_NV_external_sci_sync2[]
  ...
  * ename:VK_EXTERNAL_FENCE_HANDLE_TYPE_SCI_SYNC_FENCE_BIT_NV specifies a
    structure of code:NvSciSyncFence that is a snapshot of a synchronization
...
If pname:handleType is not supported by the implementation, then
slink:VkExternalFenceProperties::pname:externalFenceFeatures will be zero.
...
[open,refpage='vkGetPhysicalDeviceCalibrateableTimeDomainsKHR',desc='Query calibrateable time domains',type='protos']
--
...
Otherwise, pname:pTimeDomainCount must: point to a variable set by the
application to the number of elements in the pname:pTimeDomains array, and
on return the variable is overwritten with the number of values actually
written to pname:pTimeDomains.
