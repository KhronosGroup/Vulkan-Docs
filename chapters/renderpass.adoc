...
ifdef::VK_VERSION_1_1,VK_KHR_device_group[]
  ...
  * [[VUID-VkRenderingInfo-pNext-07816]]
    ...
    slink:VkDeviceGroupRenderPassBeginInfo or its
    pname:deviceRenderAreaCount member is equal to 0, the sum of
    pname:renderArea.extent.height and pname:renderArea.offset.y must: be
    less than or equal to
...
into the same location in pname:resolveImageView.
...
ifdef::VK_VERSION_1_1,VK_KHR_multiview[]
  * [[VUID-VkRenderingFragmentDensityMapAttachmentInfoEXT-apiVersion-07908]]
    If apiext:VK_KHR_multiview is not enabled,
    slink:VkPhysicalDeviceProperties::pname:apiVersion is less than Vulkan
    1.1, and pname:imageView is not dlink:VK_NULL_HANDLE, it must: have a
    pname:layerCount equal to `1`
endif::VK_VERSION_1_1,VK_KHR_multiview[]
****
...
<<memory-model-access-data-race, data race>>, except in the following cases:
  ...
  * If a memory dependency is inserted between when the attachment is
    ...
    <<synchronization-pipeline-barriers, Pipeline barriers>> are the only
    way to achieve this, and one must: be inserted every time a fragment
    will read values at a particular sample (x, y, layer, sample)
    coordinate, if those values have been written since the most recent
    pipeline barrier; or since the start of the subpass, if there have been
    no pipeline barriers since the start of the subpass.
ifdef::VK_EXT_rasterization_order_attachment_access,VK_ARM_rasterization_order_attachment_access[]
...
conditions:
  ...
  * The fragment (p,q) is sampling the texel at (p,q), with a suitable
    <<synchronization-pipeline-barriers, pipeline barrier>> between the two
    accesses.
...
application must: specify which types of attachments in a render pass can:
use feedback loops.
...
To <<pipelines-dynamic-state, dynamically set>> whether attachment feedback
loops are enabled, call:
...
This command sets the enabled attachment feedback loops for subsequent
drawing commands.
...
ifdef::VK_VERSION_1_1,VK_KHR_multiview[]
  ...
  * [[VUID-VkFramebufferCreateInfo-renderPass-02747]]
    If pname:renderPass was specified with all view masks equal to zero,
    each element of pname:pAttachments that is referenced by
    pname:fragmentDensityMapAttachment must: have a pname:layerCount equal
    to `1`
endif::VK_VERSION_1_1,VK_KHR_multiview[]
