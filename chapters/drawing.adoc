...
ifdef::VK_GRAPHICS_VERSION_1_4[Vulkan 1.4 or]
ifdef::VK_KHR_maintenance5[fname:vkCmdBindIndexBuffer2 or]
...
ifdef::VK_GRAPHICS_VERSION_1_4,VK_KHR_index_type_uint8,VK_EXT_index_type_uint8[]
    0xFF when pname:indexType is equal to ename:VK_INDEX_TYPE_UINT8;
...
ifdef::VK_EXT_primitive_restart_index[]
    If the <<features-primitiveRestartIndex, pname:primitiveRestartIndex>>
    feature is enabled, flink:vkCmdSetPrimitiveRestartIndexEXT can: be used
...
ifdef::VK_GRAPHICS_VERSION_1_3,VK_EXT_extended_dynamic_state2,VK_EXT_shader_object[]
[open,refpage='vkCmdSetPrimitiveRestartEnable',desc='Set primitive assembly restart state dynamically for a command buffer',type='protos']
--
...
commands
ifdef::VK_EXT_shader_object[]
ifdef::VK_GRAPHICS_VERSION_1_3,VK_EXT_extended_dynamic_state2[when drawing using <<shaders-objects, shader objects>>, or]
ifndef::VK_GRAPHICS_VERSION_1_3,VK_EXT_extended_dynamic_state2[when drawing using <<shaders-objects, shader objects>>.]
endif::VK_EXT_shader_object[]
...
--
endif::VK_GRAPHICS_VERSION_1_3,VK_EXT_extended_dynamic_state2,VK_EXT_shader_object[]

ifdef::VK_EXT_primitive_restart_index[]
[open,refpage='vkCmdSetPrimitiveRestartIndexEXT',desc='Set primitive assembly restart index dynamically for a command buffer',type='protos']
--
To <<pipelines-dynamic-state, dynamically control>> which special vertex
index value is treated as restarting the assembly of primitives, call:

include::{generated}/api/protos/vkCmdSetPrimitiveRestartIndexEXT.adoc[]

  * pname:commandBuffer is the command buffer into which the command will be
    recorded.
  * pname:primitiveRestartIndex controls which special vertex index value is
...
:refpage: vkCmdSetPrimitiveRestartIndexEXT

.Valid Usage
****
  * [[VUID-vkCmdSetPrimitiveRestartIndexEXT-primitiveRestartIndex-12395]]
    The <<features-primitiveRestartIndex, pname:primitiveRestartIndex>>
    feature must: be enabled
****

include::{generated}/validity/protos/vkCmdSetPrimitiveRestartIndexEXT.adoc[]
--
endif::VK_EXT_primitive_restart_index[]
...
ifdef::VK_GRAPHICS_VERSION_1_3,VK_EXT_extended_dynamic_state,VK_EXT_shader_object[]
[open,refpage='vkCmdSetPrimitiveTopology',desc='Set primitive topology state dynamically for a command buffer',type='protos']
--
...
ifdef::VK_EXT_shader_object[]
ifdef::VK_GRAPHICS_VERSION_1_3,VK_EXT_extended_dynamic_state[when drawing using <<shaders-objects, shader objects>>, or]
ifndef::VK_GRAPHICS_VERSION_1_3,VK_EXT_extended_dynamic_state[when drawing using <<shaders-objects, shader objects>>.]
endif::VK_EXT_shader_object[]
...
|====
endif::VK_GRAPHICS_VERSION_1_3,VK_EXT_extended_dynamic_state,VK_EXT_shader_object[]
...
[[drawing-effective-primitive-topology]]
== Effective Primitive Topology

The effective primitive topology is the primitive topology that is consumed
or produced by a given stage of the graphics pipeline.
The interpretation of this topology depends on the active shader stages and
other state set in the graphics pipeline.

[[drawing-vertex-input-assembler-topology]]
=== Vertex Input Assembler Topology

This is the topology set when the ename:VK_SHADER_STAGE_VERTEX_BIT stage is
used and <<drawing-primitive-shading,Programmable Primitive Shading>>
occurs.
It can: be set with
slink:VkPipelineInputAssemblyStateCreateInfo::pname:topology
ifdef::VK_GRAPHICS_VERSION_1_3,VK_EXT_extended_dynamic_state,VK_EXT_shader_object[]
or flink:vkCmdSetPrimitiveTopology.
endif::VK_GRAPHICS_VERSION_1_3,VK_EXT_extended_dynamic_state,VK_EXT_shader_object[]

[[drawing-clip-space-topology]]
=== Clip Space Topology

This is the output of the last pre-rasterization shader stage involved.

  * If ename:VK_SHADER_STAGE_VERTEX_BIT is the last pre-rasterization stage,
    the topology is not changed from what was set in the
    <<drawing-vertex-input-assembler-topology,input assembly>>.
  * If ename:VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT is the last
    pre-rasterization stage, the topology depends on the
    code:OpExecutionMode found in either <<tessellation,tessellation
    stage>>.
  ** The code:Isolines {ExecutionMode} is in line
     <<drawing-primitive-topology-class,topology class>>.
  ** The code:Triangles and code:Quads {ExecutionMode} is in triangle
     <<drawing-primitive-topology-class,topology class>>.
  ** The code:PointMode {ExecutionMode} is in point
     <<drawing-primitive-topology-class,topology class>> and will take
     precedence over the other tessellation stage {ExecutionMode}.
  * If ename:VK_SHADER_STAGE_GEOMETRY_BIT is the last pre-rasterization
    stage, the topology depends on the code:OpExecutionMode found in this
    shader stage.
  ** The code:OutputPoints {ExecutionMode} is in point
     <<drawing-primitive-topology-class,topology class>>.
  ** The code:OutputLineStrip {ExecutionMode} is in line
     <<drawing-primitive-topology-class,topology class>>.
  ** The code:OutputTriangleStrip {ExecutionMode} is in triangle
     <<drawing-primitive-topology-class,topology class>>.
ifdef::VK_NV_mesh_shader,VK_EXT_mesh_shader[]
  * If ename:VK_SHADER_STAGE_MESH_BIT_EXT is the last pre-rasterization
    stage, the topology depends on the code:OpExecutionMode found in this
    shader stage.
  ** The code:OutputPoints {ExecutionMode} is in point
     <<drawing-primitive-topology-class,topology class>>.
  ** The code:OutputLinesEXT {ExecutionMode} is in line
     <<drawing-primitive-topology-class,topology class>>.
  ** The code:OutputTrianglesEXT {ExecutionMode} is in triangle
     <<drawing-primitive-topology-class,topology class>>.
endif::VK_NV_mesh_shader,VK_EXT_mesh_shader[]

[[drawing-rasterization-input-topology]]
=== Rasterization Input Topology

After clipping and viewport transformations, the <<primsrast-polygonmode,
Polygon Mode>> can: set a topology as an input for rasterization.
This can: be different from what was used in <<drawing-clip-space-topology,
Clip Space Topology>>.

If
slink:VkPipelineRasterizationStateCreateInfo::pname:rasterizerDiscardEnable
ifdef::VK_GRAPHICS_VERSION_1_3,VK_EXT_extended_dynamic_state2,VK_EXT_shader_object[]
or flink:vkCmdSetRasterizerDiscardEnableEXT::pname:rasterizerDiscardEnable
endif::VK_GRAPHICS_VERSION_1_3,VK_EXT_extended_dynamic_state2,VK_EXT_shader_object[]
is set to ename:VK_FALSE, then this Rasterization Input Topology is not
observed.

If the <<drawing-clip-space-topology,clip space topology>> is in triangle
<<drawing-primitive-topology-class,topology class>> then
ename:VK_POLYGON_MODE_LINE will turn it into line topology and
ename:VK_POLYGON_MODE_POINT will turn it into point topology.

If the <<drawing-clip-space-topology,clip space topology>> is in line
<<drawing-primitive-topology-class,topology class>> then
ename:VK_POLYGON_MODE_POINT will turn it into point topology.
...
[[index-buffer-range]]
For indexed drawing commands, indices are sourced from a range of memory
bound to the command buffer, referred to as the _bound index buffer range_,
and specified by one of the following commands.

ifdef::VK_KHR_device_address_commands[]
[open,refpage='vkCmdBindIndexBuffer3KHR',desc='Bind an address range as an index buffer to a command buffer',type='protos']
--
:refpage: vkCmdBindIndexBuffer3KHR

To bind an address range as an index buffer to a command buffer, call:

include::{generated}/api/protos/vkCmdBindIndexBuffer3KHR.adoc[]

  * pname:commandBuffer is the command buffer into which the command is
    recorded.
  * pname:pInfo is a pointer to a slink:VkBindIndexBuffer3InfoKHR structure
    defining parameters of this command.

The <<index-buffer-range,bound index buffer range>> is set to the range of
memory indicated by pname:pInfo->addressRange.

ifdef::VK_EXT_robustness2,VK_KHR_robustness2[]
If the <<features-nullDescriptor, pname:nullDescriptor>> feature is enabled,
pname:pInfo->addressRange.size and pname:pInfo->addressRange.address can: be 0.
When the size and address are 0, every index read from this binding will
return a value of zero.
endif::VK_EXT_robustness2,VK_KHR_robustness2[]

include::{generated}/validity/protos/vkCmdBindIndexBuffer3KHR.adoc[]
--

[open,refpage='VkBindIndexBuffer3InfoKHR',desc='Index buffer binding info',type='structs']
--
:refpage: VkBindIndexBuffer3InfoKHR

sname:VkBindIndexBuffer3InfoKHR is defined as:

include::{generated}/api/structs/VkBindIndexBuffer3InfoKHR.adoc[]

  * pname:sType is a elink:VkStructureType value identifying this structure.
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:addressRange is the slink:VkDeviceAddressRangeKHR of the address
    range to bind.
...
include::{chapters}/commonvalidity/address_flags_common.adoc[]
  * [[VUID-VkBindIndexBuffer3InfoKHR-addressRange-13051]]
    If pname:addressRange.address is not 0, the buffer from which
    pname:addressRange was queried must: have been created with
    ename:VK_BUFFER_USAGE_INDEX_BUFFER_BIT
  * [[VUID-VkBindIndexBuffer3InfoKHR-addressRange-13052]]
    pname:addressRange.address must: be a multiple of the size of the type
    indicated by pname:indexType
...
ifdef::VK_EXT_robustness2,VK_KHR_robustness2[]
    ...
    pname:addressRange.size must: be greater than 0
ifdef::VK_EXT_robustness2,VK_KHR_robustness2[]
  * [[VUID-VkBindIndexBuffer3InfoKHR-addressRange-13056]]
    If pname:addressRange.size is 0, pname:addressRange.address must: be 0
endif::VK_EXT_robustness2,VK_KHR_robustness2[]
****

include::{generated}/validity/structs/VkBindIndexBuffer3InfoKHR.adoc[]
--
endif::VK_KHR_device_address_commands[]


ifdef::VK_GRAPHICS_VERSION_1_4,VK_KHR_maintenance5[]
[open,refpage='vkCmdBindIndexBuffer2',desc='Bind an index buffer to a command buffer',type='protos']
--
:refpage: vkCmdBindIndexBuffer2

To bind an index buffer, along with its size, to a command buffer, call:

ifdef::VK_GRAPHICS_VERSION_1_4[]
include::{generated}/api/protos/vkCmdBindIndexBuffer2.adoc[]
...
pname:buffer, pname:offset, and pname:size specify the <<index-buffer-range,
bound index buffer range>>, with a range of memory bound from
[eq]#[code:base {plus} pname:offset, code:base {plus} pname:offset {plus}
pname:size)#, where code:base is the start of pname:buffer.
If pname:size is ename:VK_WHOLE_SIZE then the range is from pname:offset to
the end of the pname:buffer.

ifdef::VK_GRAPHICS_VERSION_1_4,VK_KHR_maintenance6[]
If the <<features-maintenance6,pname:maintenance6>> feature is enabled,
pname:buffer can: be dlink:VK_NULL_HANDLE.
ifdef::VK_EXT_robustness2,VK_KHR_robustness2[]
If pname:buffer is dlink:VK_NULL_HANDLE and the <<features-nullDescriptor,
pname:nullDescriptor>> feature is enabled, every index fetched results in a
value of zero.
endif::VK_EXT_robustness2,VK_KHR_robustness2[]
...
include::{generated}/validity/protos/vkCmdBindIndexBuffer2.adoc[]
--
...
pname:buffer and pname:offset specify the <<index-buffer-range, bound index
buffer range>>, with a range of memory bound from [eq]#[code:base {plus}
pname:offset, code:base {plus} pname:offset {plus} pname:size)#, where
code:size is from pname:offset to the end of the pname:buffer.
...
--
Possible values of
ifdef::VK_GRAPHICS_VERSION_1_4,VK_KHR_maintenance5[flink:vkCmdBindIndexBuffer2::pname:indexType and]
...
ifdef::VK_GRAPHICS_VERSION_1_4,VK_KHR_index_type_uint8,VK_EXT_index_type_uint8[]
  * ename:VK_INDEX_TYPE_UINT8 specifies that indices are 8-bit unsigned
    integer values.
endif::VK_GRAPHICS_VERSION_1_4,VK_KHR_index_type_uint8,VK_EXT_index_type_uint8[]
--
...
include::{chapters}/commonvalidity/draw_instance_common.adoc[]
****
...
ifdef::VK_GRAPHICS_VERSION_1_4,VK_KHR_maintenance5[flink:vkCmdBindIndexBuffer2::pname:indexType or the]
flink:vkCmdBindIndexBuffer::pname:indexType parameter with which the buffer
was bound.

The first vertex index is read from [eq]#code:indexAddress {plus}
(pname:firstIndex {times} code:indexSize)#, where pname:indexAddress is the
effective address specified for the index buffer by
ifdef::VK_KHR_device_address_commands[flink:vkCmdBindIndexBuffer3KHR,]
ifdef::VK_GRAPHICS_VERSION_1_4,VK_KHR_maintenance5[flink:vkCmdBindIndexBuffer2,]
ifdef::VK_GRAPHICS_VERSION_1_4,VK_KHR_maintenance5,VK_KHR_device_address_commands[or]
flink:vkCmdBindIndexBuffer, and code:indexSize is the byte size of the type
specified by pname:indexType.
...
include::{chapters}/commonvalidity/draw_instance_common.adoc[]
  ...
  * [[VUID-vkCmdDrawMultiEXT-drawCount-09628]]
    ...
    code:sizeof(sname:VkMultiDrawInfoEXT)
****
...
include::{chapters}/commonvalidity/draw_index_binding.adoc[]
  ...
  * [[VUID-vkCmdDrawMultiIndexedEXT-drawCount-09629]]
    If pname:drawCount is greater than `1`, pname:stride must: be a multiple
    of `4` and must: be greater than or equal to
    code:sizeof(sname:VkMultiDrawIndexedInfoEXT)
****
...
ifdef::VK_KHR_device_address_commands[]
[open,refpage='VkDrawIndirect2InfoKHR',desc='Draw indirect info',type='structs']
--
:refpage: VkDrawIndirect2InfoKHR

sname:VkDrawIndirect2InfoKHR is defined as:

include::{generated}/api/structs/VkDrawIndirect2InfoKHR.adoc[]

  * pname:sType is a elink:VkStructureType value identifying this structure.
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:addressRange is the slink:VkStridedDeviceAddressRangeKHR
...
.Valid Usage
****
include::{chapters}/commonvalidity/draw_dispatch_indirect_address_common.adoc[]
include::{chapters}/commonvalidity/draw_indirect_drawcount.adoc[]
****

include::{generated}/validity/structs/VkDrawIndirect2InfoKHR.adoc[]
--

[open,refpage='vkCmdDrawIndirect2KHR',desc='Draw primitives with indirect parameters through an address range',type='protos']
--
:refpage: vkCmdDrawIndirect2KHR

To record a non-indexed indirect drawing command, call:

include::{generated}/api/protos/vkCmdDrawIndirect2KHR.adoc[]

  * pname:commandBuffer is the command buffer into which the command is
    recorded.
  * pname:pInfo is a pointer to a slink:VkDrawIndirect2InfoKHR structure
    defining parameters of this command.

fname:vkCmdDrawIndirect2KHR behaves similarly to flink:vkCmdDraw except that
the parameters are read by the device from an address range during
execution.
pname:pInfo->drawCount draws are executed by the command, with parameters
taken from pname:pInfo->addressRange starting at pname:addressRange.address
and increasing by pname:addressRange.stride bytes for each successive draw.
The parameters of each draw are encoded in an array of
slink:VkDrawIndirectCommand structures.

:indirectStruct: VkDrawIndirectCommand
:drawCount: drawCount
...
include::{chapters}/commonvalidity/draw_common.adoc[]
include::{chapters}/commonvalidity/draw_vertex_binding.adoc[]
include::{chapters}/commonvalidity/draw_indirect_address_stride_common.adoc[]
  * [[VUID-vkCmdDrawIndirect2KHR-commandBuffer-13057]]
    pname:commandBuffer must: not be a protected command buffer
****

include::{generated}/validity/protos/vkCmdDrawIndirect2KHR.adoc[]
--
endif::VK_KHR_device_address_commands[]
...
[open,refpage='VkDrawIndirectCommand',desc='Structure specifying an indirect drawing command',type='structs',xrefs='vkCmdDrawIndirect']
--
:refpage: VkDrawIndirectCommand
...
ifdef::VK_GRAPHICS_VERSION_1_2,VK_KHR_draw_indirect_count,VK_AMD_draw_indirect_count[]

ifdef::VK_KHR_device_address_commands[]
[open,refpage='VkDrawIndirectCount2InfoKHR',desc='Draw indirect count info',type='structs']
--
:refpage: VkDrawIndirectCount2InfoKHR

sname:VkDrawIndirectCount2InfoKHR is defined as:

include::{generated}/api/structs/VkDrawIndirectCount2InfoKHR.adoc[]
  ...
  * pname:addressFlags is a tlink:VkAddressCommandFlagsKHR value defining
    the flags for the address range.
  * pname:countAddressRange is the slink:VkDeviceAddressRangeKHR containing
    the draw count.
  * pname:countAddressFlags is a tlink:VkAddressCommandFlagsKHR value
  ...
  * pname:maxDrawCount specifies the maximum number of draws that will be
    ...
    specified in pname:countAddressRange and pname:maxDrawCount.

.Valid Usage
****
include::{chapters}/commonvalidity/draw_dispatch_indirect_address_common.adoc[]
include::{chapters}/commonvalidity/draw_indirect_count_address_common.adoc[]
****

include::{generated}/validity/structs/VkDrawIndirectCount2InfoKHR.adoc[]
--

[open,refpage='vkCmdDrawIndirectCount2KHR',desc='Draw primitives with indirect parameters and draw count through address ranges',type='protos']
--
:refpage: vkCmdDrawIndirectCount2KHR

To record a non-indexed indirect drawing command, call:

include::{generated}/api/protos/vkCmdDrawIndirectCount2KHR.adoc[]

  * pname:commandBuffer is the command buffer into which the command is
    recorded.
  * pname:pInfo is a pointer to a slink:VkDrawIndirectCount2InfoKHR
    structure defining parameters of this command.

fname:vkCmdDrawIndirectCount2KHR behaves similarly to
flink:vkCmdDrawIndirect2KHR except that the draw count is read by the device
from an address range during execution.
The command will read an unsigned 32-bit integer from
pname:pInfo->countAddressRange and use this as the draw count.
...
include::{chapters}/commonvalidity/draw_vertex_binding.adoc[]
include::{chapters}/commonvalidity/draw_indirect_address_stride_common.adoc[]
  * [[VUID-vkCmdDrawIndirectCount2KHR-commandBuffer-13058]]
    pname:commandBuffer must: not be a protected command buffer
  * [[VUID-vkCmdDrawIndirectCount2KHR-drawIndirectCount-13119]]
    If the <<features-drawIndirectCount, pname:drawIndirectCount>> feature
    is not enabled this function must: not be used
****

include::{generated}/validity/protos/vkCmdDrawIndirectCount2KHR.adoc[]
--
endif::VK_KHR_device_address_commands[]

[open,refpage='vkCmdDrawIndirectCount',desc='Draw primitives with indirect parameters and draw count',type='protos']
--
...
--
endif::VK_GRAPHICS_VERSION_1_2,VK_KHR_draw_indirect_count,VK_AMD_draw_indirect_count[]

ifdef::VK_KHR_device_address_commands[]
[open,refpage='vkCmdDrawIndexedIndirect2KHR',desc='Draw primitives with indirect parameters through an address range and with indexed vertices',type='protos']
--
:refpage: vkCmdDrawIndexedIndirect2KHR

To record an indexed indirect drawing command, call:

include::{generated}/api/protos/vkCmdDrawIndexedIndirect2KHR.adoc[]
...
fname:vkCmdDrawIndexedIndirect2KHR behaves similarly to
flink:vkCmdDrawIndexed except that the parameters are read by the device
from an address range during execution.
pname:pInfo->drawCount draws are executed by the command, with parameters
taken from pname:pInfo->addressRange starting at pname:addressRange.address
and increasing by pname:addressRange.stride bytes for each successive draw.
...
:indirectStruct: VkDrawIndexedIndirectCommand
:drawCount: drawCount
...
include::{generated}/validity/protos/vkCmdDrawIndexedIndirect2KHR.adoc[]
--
endif::VK_KHR_device_address_commands[]
...
[open,refpage='VkDrawIndexedIndirectCommand',desc='Structure specifying an indexed indirect drawing command',type='structs',xrefs='vkCmdDrawIndexedIndirect']
--
...
ifdef::VK_GRAPHICS_VERSION_1_2,VK_KHR_draw_indirect_count,VK_AMD_draw_indirect_count[]
ifdef::VK_KHR_device_address_commands[]
[open,refpage='vkCmdDrawIndexedIndirectCount2KHR',desc='Draw primitives with indirect parameters and draw count through address ranges and with indexed vertices',type='protos']
--
:refpage: vkCmdDrawIndexedIndirectCount2KHR

To record a non-indexed indirect drawing command, call:

include::{generated}/api/protos/vkCmdDrawIndexedIndirectCount2KHR.adoc[]
...
fname:vkCmdDrawIndexedIndirectCount2KHR behaves similarly to
flink:vkCmdDrawIndexedIndirect2KHR except that the draw count is read by the
...
include::{generated}/validity/protos/vkCmdDrawIndexedIndirectCount2KHR.adoc[]
--
endif::VK_KHR_device_address_commands[]

[open,refpage='vkCmdDrawIndexedIndirectCount',desc='Draw parameters with indirect parameters, indexed vertices, and draw count',type='protos']
--
...
[open,refpage='vkCmdDrawIndirectByteCount2EXT',desc='Draw primitives with indirect parameters where the vertex count is derived from the counter byte value in the counter memory range',type='protos']
--
:refpage: vkCmdDrawIndirectByteCount2EXT
...
include::{generated}/api/protos/vkCmdDrawIndirectByteCount2EXT.adoc[]

  * pname:commandBuffer is the command buffer into which the command is
    recorded.
  * pname:instanceCount is the number of instances to draw.
  * pname:firstInstance is the instance ID of the first instance to draw.
  * pname:pCounterInfo is a pointer to a
    slink:VkBindTransformFeedbackBuffer2InfoEXT defining the range of memory
    that the byte count is read from.
  * pname:counterOffset is subtracted from the byte count.
...
include::{chapters}/commonvalidity/draw_vertex_binding.adoc[]
include::{chapters}/commonvalidity/draw_instance_common.adoc[]
include::{chapters}/commonvalidity/xform_feedback_draw_common.adoc[]
  * [[VUID-vkCmdDrawIndirectByteCount2EXT-pInfo-13061]]
    The slink:VkBuffer that pname:pInfo->counterRange was queried from must:
    have been created with the ename:VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT bit
    set
  * [[VUID-vkCmdDrawIndirectByteCount2EXT-pInfo-13062]]
    pname:pInfo->counterRange.address must: be a multiple of `4`
****

include::{generated}/validity/protos/vkCmdDrawIndirectByteCount2EXT.adoc[]
--
endif::VK_KHR_device_address_commands[]
...
include::{generated}/validity/conditionalrendering.adoc[]
...
****
  ...
  * [[VUID-VkConditionalRenderingBeginInfoEXT-buffer-01982]]
    ...
    ename:VK_BUFFER_USAGE_CONDITIONAL_RENDERING_BIT_EXT usage flag set
...
ifdef::VK_KHR_device_address_commands[]
[open,refpage='vkCmdBeginConditionalRendering2EXT',desc='Define the beginning of a conditional rendering block',type='protos']
--
To begin conditional rendering, call:

include::{generated}/api/protos/vkCmdBeginConditionalRendering2EXT.adoc[]

  * pname:commandBuffer is the command buffer into which this command will
    be recorded.
  * pname:pConditionalRenderingBegin is a pointer to a
    slink:VkConditionalRenderingBeginInfo2EXT structure specifying
...
include::{generated}/validity/protos/vkCmdBeginConditionalRendering2EXT.adoc[]
--

[open,refpage='VkConditionalRenderingBeginInfo2EXT',desc='Structure specifying conditional rendering begin information',type='structs']
--
:refpage: VkConditionalRenderingBeginInfo2EXT

The sname:VkConditionalRenderingBeginInfo2EXT structure is defined as:

include::{generated}/api/structs/VkConditionalRenderingBeginInfo2EXT.adoc[]

  * pname:sType is a elink:VkStructureType value identifying this structure.
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:addressRange is the slink:VkDeviceAddressRangeKHR containing the
...
If the 32-bit value at pname:addressRange.address is zero, then the
rendering commands are discarded, otherwise they are executed as normal.
...
include::{chapters}/commonvalidity/address_flags_common.adoc[]
  * [[VUID-VkConditionalRenderingBeginInfo2EXT-addressRange-13064]]
    The slink:VkBuffer that pname:addressRange was queried from must: have
    been created with ename:VK_BUFFER_USAGE_CONDITIONAL_RENDERING_BIT_EXT
  ...
  * [[VUID-VkConditionalRenderingBeginInfo2EXT-addressRange-13066]]
    pname:addressRange.size must: be greater than or equal to 4
****

include::{generated}/validity/structs/VkConditionalRenderingBeginInfo2EXT.adoc[]
--
endif::VK_KHR_device_address_commands[]
...
ifdef::VK_KHR_device_address_commands[]
[open,refpage='vkCmdDrawMeshTasksIndirect2EXT',desc='Issue a mesh tasks draw with indirect parameters through an address range',type='protos']
--
:refpage: vkCmdDrawMeshTasksIndirect2EXT

To record an indirect mesh tasks drawing command, call:

include::{generated}/api/protos/vkCmdDrawMeshTasksIndirect2EXT.adoc[]
...
fname:vkCmdDrawMeshTasksIndirect2EXT behaves similarly to
...
include::{chapters}/commonvalidity/draw_common.adoc[]
include::{chapters}/commonvalidity/draw_mesh_common.adoc[]
  * [[VUID-vkCmdDrawMeshTasksIndirect2EXT-commandBuffer-13067]]
    pname:commandBuffer must: not be a protected command buffer
****

include::{generated}/validity/protos/vkCmdDrawMeshTasksIndirect2EXT.adoc[]
--
endif::VK_KHR_device_address_commands[]
...
ifdef::VK_GRAPHICS_VERSION_1_2,VK_KHR_draw_indirect_count,VK_AMD_draw_indirect_count[]
ifdef::VK_KHR_device_address_commands[]
[open,refpage='vkCmdDrawMeshTasksIndirectCount2EXT',desc='Perform an indirect mesh tasks draw with the draw count sourced from an address range',type='protos']
--
:refpage: vkCmdDrawMeshTasksIndirectCount2EXT

To record an indirect mesh tasks drawing command with the draw count sourced
from an address range, call:

include::{generated}/api/protos/vkCmdDrawMeshTasksIndirectCount2EXT.adoc[]
...
fname:vkCmdDrawMeshTasksIndirectCount2EXT behaves similarly to
flink:vkCmdDrawMeshTasksIndirect2EXT except that the draw count is read by
...
include::{chapters}/commonvalidity/draw_common.adoc[]
include::{chapters}/commonvalidity/draw_mesh_common.adoc[]
  * [[VUID-vkCmdDrawMeshTasksIndirectCount2EXT-commandBuffer-13068]]
    pname:commandBuffer must: not be a protected command buffer
  * [[VUID-vkCmdDrawMeshTasksIndirectCount2EXT-drawIndirectCount-13069]]
    If the <<features-drawIndirectCount, pname:drawIndirectCount>> feature
    is not enabled this function must: not be used
****

include::{generated}/validity/protos/vkCmdDrawMeshTasksIndirectCount2EXT.adoc[]
--
endif::VK_KHR_device_address_commands[]
