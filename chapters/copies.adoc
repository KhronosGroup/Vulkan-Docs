...
[[copies-memory-ranges]]
== Copying Data Between Memory Ranges

[open,refpage='vkCmdCopyMemoryKHR',desc='Copy data between memory ranges',type='protos']
--
:refpage: vkCmdCopyMemoryKHR

To copy data between memory ranges, call:

include::{generated}/api/protos/vkCmdCopyMemoryKHR.adoc[]

  * pname:commandBuffer is the command buffer into which the command will be
    recorded.
  * pname:pCopyMemoryInfo a pointer to a slink:VkCopyDeviceMemoryInfoKHR
...
****
  * [[VUID-vkCmdCopyMemoryKHR-commandBuffer-13012]]
    ...
    the pname:srcCopyFlags member of all elements of
  ...
  * [[VUID-vkCmdCopyMemoryKHR-commandBuffer-13013]]
    If pname:commandBuffer is an unprotected command buffer and
    <<limits-protectedNoFault, pname:protectedNoFault>> is not supported,
    the pname:dstCopyFlags member of all elements of
    pname:pCopyMemoryInfo->pRegions must: not include
    ename:VK_ADDRESS_COMMAND_PROTECTED_BIT_KHR
  * [[VUID-vkCmdCopyMemoryKHR-commandBuffer-13014]]
    ...
    <<limits-protectedNoFault, pname:protectedNoFault>> is not supported,
    the pname:dstCopyFlags member of all elements of
    pname:pCopyMemoryInfo->pRegions must: include
    ename:VK_ADDRESS_COMMAND_PROTECTED_BIT_KHR
****

include::{generated}/validity/protos/vkCmdCopyMemoryKHR.adoc[]
--

[open,refpage='VkCopyDeviceMemoryInfoKHR',desc='Structure specifying a buffer copy operation',type='structs']
--
:refpage: VkCopyDeviceMemoryInfoKHR

The sname:VkCopyDeviceMemoryInfoKHR structure is defined as:

include::{generated}/api/structs/VkCopyDeviceMemoryInfoKHR.adoc[]

  * pname:sType is a elink:VkStructureType value identifying this structure.
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:regionCount is the number of copies to be performed.
  * pname:pRegions is a pointer to an array of slink:VkDeviceMemoryCopyKHR
...
****
  * [[VUID-VkCopyDeviceMemoryInfoKHR-srcRange-13015]]
    ...
    pname:srcRange member of any element of pname:pRegions must: not overlap
    the memory backing the address range defined by the pname:srcRange or
    pname:dstRange of any element of pname:pRegions
****

include::{generated}/validity/structs/VkCopyDeviceMemoryInfoKHR.adoc[]
--

[open,refpage='VkDeviceMemoryCopyKHR',desc='Structure specifying a memory range copy operation',type='structs']
--
:refpage: VkDeviceMemoryCopyKHR

The sname:VkDeviceMemoryCopyKHR structure is defined as:

include::{generated}/api/structs/VkDeviceMemoryCopyKHR.adoc[]

  * pname:sType is a elink:VkStructureType value identifying this structure.
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:srcRange is a slink:VkDeviceAddressRangeKHR structure defining the
    source memory to copy from.
  * pname:srcFlags is a tlink:VkAddressCommandFlagsKHR value defining the
    copy flags for the source address range.
  * pname:dstRange is a slink:VkDeviceAddressRangeKHR structure defining the
...
This structure defines a copy operation where pname:srcRange.size bytes will
be copied from pname:srcRange.address to pname:dstRange.address.

.Valid Usage
****
:addressRange: srcRange
:addressFlags: srcFlags
...
include::{chapters}/commonvalidity/address_flags_common.adoc[]
  * [[VUID-VkDeviceMemoryCopyKHR-size-13016]]
    The pname:size member of pname:dstRange must: be greater than or equal
    to the pname:size member of pname:srcRange
  * [[VUID-VkDeviceMemoryCopyKHR-srcRange-13017]]
    The buffer from which pname:srcRange was queried must: have been created
    with ename:VK_BUFFER_USAGE_TRANSFER_SRC_BIT
  * [[VUID-VkDeviceMemoryCopyKHR-dstRange-13018]]
    The buffer from which pname:dstRange was queried must: have been created
    with ename:VK_BUFFER_USAGE_TRANSFER_DST_BIT
****

include::{generated}/validity/structs/VkDeviceMemoryCopyKHR.adoc[]
--
...
[[copies-images-addressing]]
The texels in the images are accessed as follows.

Texels at each coordinate [eq]#(x,y,z,layer)# in a image subresource are
accessed, where:

  {empty}:: [eq]#x# is in the range [eq]#[offset.x, offset.x {plus}
            extent.width)#,
  {empty}:: [eq]#y# is in the range [eq]#[offset.y, offset.y {plus}
            extent.height)#,
  {empty}:: [eq]#z# is in the range [eq]#[offset.z, offset.z {plus}
            extent.depth)#,
  {empty}:: [eq]#layer# is in the range [eq]#[subresource.baseArrayLayer,
            subresource.baseArrayLayer {plus} subresource.layerCount)#

where for pname:srcImage, _offset_, _extent_, and _subresource_ are
pname:srcOffset, pname:extent, and pname:srcSubresource for each region,
respectively.

For pname:dstImage, _offset_, _extent_ , and _subresource_ are
pname:dstOffset, pname:extent, and pname:dstSubresource for each region,
respectively, where pname:dstOffset and pname:extent are first
<<formats-size-compatibility, size-adjusted>> if the format of the
destination image has a different <<formats-compatibility-classes,block
extent>> than the format of the source image (e.g. one is a compressed
format).
...
if they <<resources-memory-aliasing,alias each other>>, both
pname:srcSubresource and pname:dstSubresource overlap the aliased range, and
any of the following are true:

  * Their contents are not able to be interpreted in consistent ways
  * If the aliased regions are _host-accessible_, their accessed regions
    defined above overlap, where the memory locations accessed by the texels
    of a <<glossary-linear-resource,linear image>> is according to its
    slink:VkSubresourceLayout
...
ifdef::VK_KHR_device_address_commands[]
[[copies-memory-ranges-images]]
== Copying Data Between Memory Ranges and Images

[open,refpage='vkCmdCopyMemoryToImageKHR',desc='Copy data from memory ranges to an image',type='protos']
--
:refpage: vkCmdCopyMemoryToImageKHR

To copy data from an image to memory ranges, call:

include::{generated}/api/protos/vkCmdCopyMemoryToImageKHR.adoc[]

  * pname:commandBuffer is the command buffer into which the command will be
    recorded.
  * pname:pCopyMemoryInfo a pointer to a
    slink:VkCopyDeviceMemoryImageInfoKHR structure describing the copies to
    perform.

.Valid Usage
****
include::{chapters}/commonvalidity/copy_image_device_memory_common.adoc[]
  * [[VUID-vkCmdCopyMemoryToImageKHR-imageLayout-13019]]
    The pname:imageLayout member of each element of
    pname:pCopyMemoryInfo->pRegions must: be ename:VK_IMAGE_LAYOUT_GENERAL,
ifdef::VK_KHR_shared_presentable_image[]
    ename:VK_IMAGE_LAYOUT_SHARED_PRESENT_KHR,
endif::VK_KHR_shared_presentable_image[]
    or ename:VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL
  * [[VUID-vkCmdCopyMemoryToImageKHR-pCopyMemoryInfo-13020]]
    pname:pCopyMemoryInfo->image must: have been created with
...
ifdef::VK_EXT_depth_range_unrestricted[]
    ...
    each element of pname:pCopyMemoryInfo->pRegions whose
    pname:imageSubresource contains a depth aspect, each data element in its
    pname:addressRange must: be in the range [eq]#[0,1]#
  * [[VUID-vkCmdCopyMemoryToImageKHR-addressRange-13128]]
    At least one buffer containing the total address range specified by the
    pname:addressRange member of each element of
    pname:pCopyMemoryInfo->pRegions must: have been created with
    ename:VK_BUFFER_USAGE_TRANSFER_SRC_BIT
****

include::{generated}/validity/protos/vkCmdCopyMemoryToImageKHR.adoc[]
--

[open,refpage='vkCmdCopyImageToMemoryKHR',desc='Copy data from an image to memory ranges',type='protos']
--
:refpage: vkCmdCopyImageToMemoryKHR

To copy data from an image to memory ranges, call:

include::{generated}/api/protos/vkCmdCopyImageToMemoryKHR.adoc[]
...
ifdef::VK_KHR_shared_presentable_image[]
    ...
    or ename:VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL
...
include::{generated}/validity/protos/vkCmdCopyImageToMemoryKHR.adoc[]
--

[open,refpage='VkCopyDeviceMemoryImageInfoKHR',desc='Structure specifying copy operations between an image and memory ranges',type='structs']
--
:refpage: VkCopyDeviceMemoryImageInfoKHR

The sname:VkCopyDeviceMemoryImageInfoKHR structure is defined as:

include::{generated}/api/structs/VkCopyDeviceMemoryImageInfoKHR.adoc[]

  * pname:sType is a elink:VkStructureType value identifying this structure.
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:image is the base image accessed by the copy command.
  * pname:regionCount is the number of copies to be performed.
  * pname:pRegions is a pointer to an array of
    slink:VkDeviceMemoryImageCopyKHR structures describing individual copy
...
:bufferrowlength: addressRowLength
:bufferimageheight: addressImageHeight
:regionsparam: pname:pRegions
...
****
  ...
  * [[VUID-VkCopyDeviceMemoryImageInfoKHR-pRegions-12473]]
    Each memory location accessed by this command must: not be accessed by
  ...
  * [[VUID-VkCopyDeviceMemoryImageInfoKHR-imageLayout-13028]]
    The pname:imageLayout member of each element of pname:pRegions must:
    specify the layout of the image subresource specified for that element
    of pname:pRegions at the time this command is executed on a
    slink:VkDevice
include::{chapters}/commonvalidity/copy_anyimage_to_imageany_common.adoc[]
...
include::{chapters}/commonvalidity/copy_bufferimage_to_imagebuffer_not_both_image_common.adoc[]
  * [[VUID-VkCopyDeviceMemoryImageInfoKHR-image-13029]]
    If pname:image does not have either a depth/stencil format
ifdef::VK_VERSION_1_1,VK_KHR_sampler_ycbcr_conversion[]
    or a <<formats-requiring-sampler-ycbcr-conversion,multi-planar format>>,
endif::VK_VERSION_1_1,VK_KHR_sampler_ycbcr_conversion[]
    then for each element of pname:pRegions, pname:addressRange.address
    must: be a multiple of the <<formats-compatibility-classes,texel block
    size>>
ifdef::VK_VERSION_1_1,VK_KHR_sampler_ycbcr_conversion[]
  * [[VUID-VkCopyDeviceMemoryImageInfoKHR-image-13030]]
    If pname:image has a
    <<formats-requiring-sampler-ycbcr-conversion,multi-planar format>>, then
    for each element of pname:pRegions, pname:addressRange.address must: be
    a multiple of the element size of the compatible format for the format
    and the pname:aspectMask of the pname:imageSubresource as defined in
    <<formats-compatible-planes>>
endif::VK_VERSION_1_1,VK_KHR_sampler_ycbcr_conversion[]
  * [[VUID-VkCopyDeviceMemoryImageInfoKHR-image-13031]]
    If pname:image has a depth/stencil format, the
    pname:addressRange.address member of any element of pname:pRegions must:
    be a multiple of `4`
...
ifdef::VK_QCOM_rotated_copy_commands[]
  * [[VUID-VkCopyDeviceMemoryImageInfoKHR-pRegions-13033]]
    ...
    <<copies-buffers-images-rotation-addressing, rotated image region>>
    must: be contained within pname:image
  * [[VUID-VkCopyDeviceMemoryImageInfoKHR-pRegions-13034]]
    ...
    pname:image must: have a 1x1x1 <<formats-compatibility-classes,texel
    block extent>>
  * [[VUID-VkCopyDeviceMemoryImageInfoKHR-pRegions-13035]]
    If any element of pname:pRegions contains
    slink:VkCopyCommandTransformInfoQCOM in its pname:pNext chain, then
    pname:image must: be of type ename:VK_IMAGE_TYPE_2D
  * [[VUID-VkCopyDeviceMemoryImageInfoKHR-pRegions-13036]]
    If any element of pname:pRegions contains
    slink:VkCopyCommandTransformInfoQCOM in its pname:pNext chain, then
    pname:image must: not have a
    <<formats-requiring-sampler-ycbcr-conversion, multi-planar format>>
endif::VK_QCOM_rotated_copy_commands[]
****

include::{generated}/validity/structs/VkCopyDeviceMemoryImageInfoKHR.adoc[]
--

[open,refpage='VkDeviceMemoryImageCopyKHR',desc='Structure specifying a copy operation between a memory range and texels in an image',type='structs']
--
:refpage: VkDeviceMemoryImageCopyKHR

The sname:VkDeviceMemoryImageCopyKHR structure is defined as:

include::{generated}/api/structs/VkDeviceMemoryImageCopyKHR.adoc[]
  ...
  * pname:addressRowLength specifies the total number of texels in the
  ...
  * pname:addressImageHeight specifies the total number of rows of texels in
  ...
  * pname:imageLayout is the elink:VkImageLayout of the image subresource
...
include::{chapters}/commonvalidity/address_flags_common.adoc[]
  * [[VUID-VkDeviceMemoryImageCopyKHR-size-13037]]
    The pname:size member of pname:addressRange must: be large enough to
    contain all address locations that are accessed by this copy according
    to <<copies-buffers-images-addressing,Buffer and Image Addressing>>
include::{chapters}/commonvalidity/buffer_or_memory_image_copy_common.adoc[]
****

include::{generated}/validity/structs/VkDeviceMemoryImageCopyKHR.adoc[]
--
endif::VK_KHR_device_address_commands[]
...
ifdef::VK_EXT_host_image_copy,VK_KHR_device_address_commands,VK_NV_copy_memory_indirect[an address or]
...
For each [eq]#(x,y,z,layer)# coordinate in the image, bytes in memory are
accessed at offsets in the range [eq]#[texelOffset, texelOffset {plus}
blockSize)#, where:

  {empty}:: [eq]#texelOffset = baseAddress {plus} ({lfloor}(x -
...
and extents>> of the image's format, and [eq]#baseAddress# is
ifdef::VK_EXT_host_image_copy,VK_KHR_device_address_commands,VK_NV_copy_memory_indirect[]
either the supplied address for the copy, or
endif::VK_EXT_host_image_copy,VK_KHR_device_address_commands,VK_NV_copy_memory_indirect[]
the address of the buffer plus any supplied pname:bufferOffset value.
...
<<resources-memory-aliasing,alias each other>>, the pname:imageSubresource
overlaps the aliased range, and either:
  ...
  * their accessed regions defined above overlap, where the memory locations
    accessed by the texels of a <<glossary-linear-resource,linear image>> is
    according to its slink:VkSubresourceLayout
...
include::{chapters}/commonvalidity/copy_bufferimage_to_imagebuffer_common.adoc[]
include::{chapters}/commonvalidity/copy_bufferimage_to_imagebuffer_not_both_image_common.adoc[]
  * [[VUID-VkCopyMemoryToImageInfo-pRegions-09062]]
    Each memory location accessed by this command must: not be accessed in
    both a source region and a destination region, as specified in
    pname:pRegions and accessed according to
    <<copies-buffers-images-addressing,Buffer and Image Addressing>>
  * [[VUID-VkCopyMemoryToImageInfo-pRegions-12474]]
    Each memory location accessed by this command must: not be accessed in
    more than one destination region, as specified by pname:pRegions and
    accessed according to <<copies-buffers-images-addressing,Buffer and
    Image Addressing>>
...
include::{chapters}/commonvalidity/copy_bufferimage_to_imagebuffer_not_both_image_common.adoc[]
  * [[VUID-VkImageToMemoryCopy-pRegions-09067]]
    ...
    pname:pRegions and accessed according to
    <<copies-buffers-images-addressing,Buffer and Image Addressing>>
  * [[VUID-VkCopyImageToMemoryInfo-pRegions-12475]]
    Each memory location accessed by this command must: not be accessed in
    more than one destination region, as specified by pname:pRegions and
    accessed according to <<copies-buffers-images-addressing,Buffer and
    Image Addressing>>
...
****
  ...
  * [[VUID-VkCopyImageToImageInfo-pRegions-12476]]
    Each memory location accessed by this command must: not be accessed by
    ...
    Image Addressing>>
  * [[VUID-VkCopyImageToImageInfo-pRegions-12477]]
    Each memory location accessed by this command must: not be accessed by
    more than one destination region, as specified by pname:pRegions and
...
written.
A component in pname:dstOffsets[1] can: be smaller than the corresponding
component in pname:dstOffsets[0] and forms the same box as swapped
pname:dstOffsets.
A destination texel exists if the half-texel coordinates [eq]#u~base~#,
[eq]#v~base~# and [eq]#w~base~# are all contained inside the region.
...
ifdef::VK_KHR_device_address_commands[]
[open,refpage='VkMemoryMarkerInfoAMD',desc='Memory marker write info',type='structs']
--
:refpage: VkMemoryMarkerInfoAMD

sname:VkMemoryMarkerInfoAMD is defined as:

include::{generated}/api/structs/VkMemoryMarkerInfoAMD.adoc[]
  ...
  * pname:stage specifies the pipeline stage whose completion triggers the
    marker write.
  * pname:dstRange is the slink:VkDeviceAddressRangeKHR where the marker
    will be written.
  * pname:dstFlags is a tlink:VkAddressCommandFlagsKHR value defining the
    copy flags for the destination address range.
...
include::{chapters}/commonvalidity/stage_mask_2_common.adoc[]
  ...
  * [[VUID-VkMemoryMarkerInfoAMD-dstRange-13039]]
    The buffer from which pname:dstRange was queried must: have been created
    with ename:VK_BUFFER_USAGE_TRANSFER_DST_BIT usage flag
  * [[VUID-VkMemoryMarkerInfoAMD-dstRange-13040]]
    pname:dstRange.address must: be a multiple of 4
  * [[VUID-VkMemoryMarkerInfoAMD-dstRange-13041]]
    pname:dstRange.size must: be greater than or equal to 4
****

include::{generated}/validity/structs/VkMemoryMarkerInfoAMD.adoc[]
--

[open,refpage='vkCmdWriteMarkerToMemoryAMD',desc='Execute a pipelined write of a marker value into a memory range',type='protos']
--
:refpage: vkCmdWriteMarkerToMemoryAMD

To write a 32-bit marker value into memory as a pipelined operation, call:

include::{generated}/api/protos/vkCmdWriteMarkerToMemoryAMD.adoc[]

  * pname:commandBuffer is the command buffer into which the command will be
    recorded.
  * pname:pInfo specifies a pointer to an slink:VkMemoryMarkerInfoAMD
    structure defining parameters of this command.

When fname:vkCmdWriteMarkerToMemoryAMD is submitted to a queue, it defines
an execution dependency between prior operations and writing the marker
value, as well as a memory dependency from earlier
<<copies-buffer-markers,buffer marker write commands>>.
...
identified by pname:pInfo->stage.
...
submission order>> that specified either the same pname:pInfo->stage or a
...
pname:pInfo->stage or ename:VK_PIPELINE_STAGE_TRANSFER_BIT.
...
****
  ...
  * [[VUID-vkCmdWriteMarkerToMemoryAMD-pInfo-13043]]
    pname:pInfo->stage must: include only stages that are valid for the
  ...
  * [[VUID-vkCmdWriteMarkerToMemoryAMD-commandBuffer-13044]]
    If pname:commandBuffer is an unprotected command buffer and
    <<limits-protectedNoFault, pname:protectedNoFault>> is not supported,
    pname:pInfo->dstFlags must: not include
    ename:VK_ADDRESS_COMMAND_PROTECTED_BIT_KHR
  * [[VUID-vkCmdWriteMarkerToMemoryAMD-commandBuffer-13045]]
    If pname:commandBuffer is a protected command buffer and
    <<limits-protectedNoFault, pname:protectedNoFault>> is not supported,
    pname:pInfo->dstFlags must: include
    ename:VK_ADDRESS_COMMAND_PROTECTED_BIT_KHR
  * [[VUID-vkCmdWriteMarkerToMemoryAMD-pInfo-13046]]
    If pname:pInfo->dstFlags includes
    ename:VK_ADDRESS_COMMAND_PROTECTED_BIT_KHR, the buffer from which
    pname:pInfo->dstRange was queried must: have been created with
    ename:VK_BUFFER_CREATE_PROTECTED_BIT
  * [[VUID-vkCmdWriteMarkerToMemoryAMD-pInfo-13047]]
    If pname:pInfo->dstFlags does not include
    ename:VK_ADDRESS_COMMAND_PROTECTED_BIT_KHR, the buffer from which
    pname:pInfo->dstRange was queried must: have been created without
    ename:VK_BUFFER_CREATE_PROTECTED_BIT
****

include::{generated}/validity/protos/vkCmdWriteMarkerToMemoryAMD.adoc[]
--
endif::VK_KHR_device_address_commands[]
