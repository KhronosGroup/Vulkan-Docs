...
ifdef::VK_EXT_descriptor_heap[]
  * If the primary command buffer has a descriptor heap bound, and the
    ...
    slink:VkCommandBufferInheritanceDescriptorHeapInfoEXT for every
...
Initial::
    Command buffers in the _initial state_ can: only be transitioned to the
    ...
    transitioned to the initial state by calling flink:vkResetCommandPool on
    the pool they were allocated from.
    Command buffers allocated from a command pool created with
    ename:VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT and in any state
    other than the pending state can: also be transitioned to the initial
    state by calling flink:vkResetCommandBuffer.
Recording::
    Command buffers in the _recording state_ can: be used to record commands
    via ftext:vkCmd* commands, be reset, or be freed.
    Command buffers in the initial state can: be transitioned to the
    recording state by flink:vkBeginCommandBuffer.
    Command buffers allocated from a command pool created with
    ename:VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT in the invalid,
    recording, or executable state can: also be transitioned to the
    recording state by flink:vkBeginCommandBuffer.
Executable::
    A command buffer in the _executable state_ can: be
    <<commandbuffers-submission, submitted for execution>>, reset, freed, or
    <<commandbuffers-secondary, recorded to another command buffer>>.
    Command buffers in the recording state are transitioned to the
    executable state by flink:vkEndCommandBuffer.
    Command buffers in the pending state that were recorded without the
    ename:VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT flag immediately
...
Pending::
    A command buffer in the _pending state_ must: not be modified by the
    application, as it may: be executing on the device.
    Command buffers in the pending state that were recorded without the
    ename:VK_COMMAND_BUFFER_USAGE_SIMULTANEOUS_USE_BIT flag, or with the
    ename:VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT flag, must: not be
    submitted to the device for execution.
    Command buffers in the executable state can: be transitioned to the
    pending state by <<commandbuffers-submission,queue submission
    commands>>.
    ...
    ename:VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT flag.
    A <<synchronization, synchronization>> command can: be used to detect
    when command execution is complete.
Invalid::
    A command buffer in the _invalid_ state can: be reset or freed.
...
ifdef::VKSC_VERSION_1_0[]
    slink:VkQueryPool, slink:VkDescriptorPool, slink:VkDeviceMemory, or
endif::VKSC_VERSION_1_0[]
    slink:VkCommandPool, that may: be accessed when the command buffer is
...
execution (i.e. ename:VK_PIPELINE_STAGE_ALL_COMMANDS_BIT) with a
<<synchronization, synchronization command>> is sufficient to ensure the
...
ifdef::VK_ARM_data_graph[]
  * [[VUID-VkCommandPoolCreateInfo-pNext-09908]]
    ...
    slink:VkDataGraphProcessingEngineCreateInfoARM structure, then
    pname:queueFamilyIndex must: designate a queue family that supports
    ename:VK_QUEUE_DATA_GRAPH_BIT_ARM
  * [[VUID-VkCommandPoolCreateInfo-pNext-09909]]
    If the pname:pNext chain includes a
    slink:VkDataGraphProcessingEngineCreateInfoARM structure, each member of
    pname:pProcessingEngines must: be identical to
    ...
    pname:queueFamilyIndex and the pname:physicalDevice that was used to
    create pname:device
endif::VK_ARM_data_graph[]
ifdef::VK_QCOM_data_graph_model[]
  * [[VUID-VkCommandPoolCreateInfo-queueFamilyIndex-11830]]
    If pname:queueFamilyIndex designates a queue family that supports
    ename:VK_QUEUE_DATA_GRAPH_BIT_ARM and enumerates a foreign engine
    through flink:vkGetPhysicalDeviceQueueFamilyDataGraphPropertiesARM with
    type
    ...
    the pname:pNext chain must include
    slink:VkDataGraphProcessingEngineCreateInfoARM with
    slink:VkPhysicalDeviceDataGraphProcessingEngineARM::pname:isForeign set
    to ename:VK_TRUE for all elements of pname:pProcessingEngines
...
[NOTE]
====
The rules for <<glossary-identically-defined,identically defined>> objects
apply recursively, implying for example that if the command buffers are
created in different devices that those devices must have been created with
...
include::{generated}/api/structs/VkCommandPoolMemoryConsumption.adoc[]
  ...
  * pname:commandBufferAllocated is the number of bytes currently allocated
    ...
    If no command buffer is specified, then pname:commandBufferAllocated
    will be zero.
...
include::{generated}/api/protos/vkAllocateCommandBuffers.adoc[]
  ...
  * pname:pAllocateInfo is a pointer to a slink:VkCommandBufferAllocateInfo
    ...
    pname:commandPool may: be accessed any time one of the resulting command
    buffers is accessed.
...
****
  * [[VUID-vkBeginCommandBuffer-commandBuffer-00049]]
    pname:commandBuffer must: not be in the
    <<commandbuffers-lifecycle,pending state>>
...
ifdef::VK_QCOM_tile_shading[]
  * [[VUID-VkCommandBufferBeginInfo-flags-10617]]
    ...
    ename:VK_COMMAND_BUFFER_USAGE_RENDER_PASS_CONTINUE_BIT
ifdef::VK_GRAPHICS_VERSION_1_3,VK_KHR_dynamic_rendering[]
    , the pname:renderPass member of pname:pInheritanceInfo is not
    dlink:VK_NULL_HANDLE,
endif::VK_GRAPHICS_VERSION_1_3,VK_KHR_dynamic_rendering[]
    and pname:renderPass was created with <<renderpass-tile-shading,tile
    shading enabled>>, ename:VK_TILE_SHADING_RENDER_PASS_ENABLE_BIT_QCOM
    must: be included in
  ...
  * [[VUID-VkCommandBufferBeginInfo-flags-10618]]
    ...
    ename:VK_COMMAND_BUFFER_USAGE_RENDER_PASS_CONTINUE_BIT
ifdef::VK_GRAPHICS_VERSION_1_3,VK_KHR_dynamic_rendering[]
    , the pname:renderPass member of pname:pInheritanceInfo is
    dlink:VK_NULL_HANDLE,
endif::VK_GRAPHICS_VERSION_1_3,VK_KHR_dynamic_rendering[]
    or pname:renderPass was not created with tile shading enabled,
  ...
  * [[VUID-VkCommandBufferBeginInfo-flags-10619]]
    ...
    slink:VkRenderPassTileShadingCreateInfoQCOM::pname:flags,
    slink:VkRenderPassTileShadingCreateInfoQCOM::pname:tileApronSize must:
    be equal to the pname:tileApronSize used to create pname:renderPass
...
ifdef::VK_EXT_descriptor_heap[descriptor heaps and push data,]
...
ifndef::VK_GRAPHICS_VERSION_1_3,VK_EXT_extended_dynamic_state[state.]
ifdef::VK_GRAPHICS_VERSION_1_3,VK_EXT_extended_dynamic_state[]
...
ifdef::VK_GRAPHICS_VERSION_1_3,VK_KHR_dynamic_rendering[]
[open,refpage='VkCommandBufferInheritanceRenderingInfo',desc='Structure specifying command buffer inheritance info for dynamic render pass instances',type='structs']
--
...
ifdef::VK_AMD_mixed_attachment_samples,VK_NV_framebuffer_mixed_samples[]
[open,refpage='VkAttachmentSampleCountInfoAMD',desc='Structure specifying command buffer inheritance info for dynamic render pass instances',type='structs']
--
The
...
ifdef::VK_EXT_descriptor_heap[]
[open,refpage='VkCommandBufferInheritanceDescriptorHeapInfoEXT',desc='Structure specifying command buffer inheritance information',type='structs']
--
If the pname:pNext chain of slink:VkCommandBufferInheritanceInfo includes a
sname:VkCommandBufferInheritanceDescriptorHeapInfoEXT structure, then that
structure indicates that the secondary will use the same descriptor heaps as
the primary command buffer.

The sname:VkCommandBufferInheritanceDescriptorHeapInfoEXT structure is
defined as:

include::{generated}/api/structs/VkCommandBufferInheritanceDescriptorHeapInfoEXT.adoc[]

  * pname:sType is a elink:VkStructureType value identifying this structure.
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:pSamplerHeapBindInfo specifies the slink:VkBindHeapInfoEXT of the
    sampler heap bound using flink:vkCmdBindSamplerHeapEXT in the primary.
    If this is `NULL`, it indicates that no sampler heap is bound.
  * pname:pResourceHeapBindInfo specifies the slink:VkBindHeapInfoEXT of the
    resource heap bound using flink:vkCmdBindResourceHeapEXT in the primary.
    If this is `NULL`, it indicates that no resource heap is bound.

If this structure is not present, the behavior is as if
pname:pSamplerHeapBindInfo and pname:pResourceHeapBindInfo were both `NULL`.
...
****
  * [[VUID-VkCommandBufferInheritanceDescriptorHeapInfoEXT-descriptorHeap-11200]]
    ...
    enabled, pname:pSamplerHeapBindInfo must: be `NULL`
  * [[VUID-VkCommandBufferInheritanceDescriptorHeapInfoEXT-descriptorHeap-11201]]
    If the <<features-descriptorHeap, pname:descriptorHeap>> feature is not
    enabled, pname:pResourceHeapBindInfo must: be `NULL`
  * [[VUID-VkCommandBufferInheritanceDescriptorHeapInfoEXT-pSamplerHeapBindInfo-11470]]
    If pname:pSamplerHeapBindInfo is not `NULL`,
    pname:pSamplerHeapBindInfo->heapRange must: be a device address range
  ...
  * [[VUID-VkCommandBufferInheritanceDescriptorHeapInfoEXT-pResourceHeapBindInfo-11471]]
    If pname:pResourceHeapBindInfo is not `NULL`,
    pname:pResourceHeapBindInfo->heapRange must: be a device address range
    allocated to the application from a buffer created with the
    ename:VK_BUFFER_USAGE_DESCRIPTOR_HEAP_BIT_EXT usage flag set
****

include::{generated}/validity/structs/VkCommandBufferInheritanceDescriptorHeapInfoEXT.adoc[]
--
endif::VK_EXT_descriptor_heap[]
...
ifdef::VK_ARM_shader_instrumentation[]
  * [[VUID-vkEndCommandBuffer-commandBuffer-12372]]
    pname:commandBuffer must: not have any shader instrumentation
    <<shaders-instrumentation-active,active>>
...
ifdef::VK_BASE_VERSION_1_3,VK_KHR_synchronization2[]
[open,refpage='vkQueueSubmit2',desc='Submits command buffers to a queue',type='protos']
--
...
include::{generated}/api/protos/vkQueueSubmit2KHR.adoc[]
  ...
  * pname:pSubmits is a pointer to an array of slink:VkSubmitInfo2
    ...
    at any point until the <<devsandqueues-submission, queue operations>>
  ...
  * pname:fence is an optional: handle to a fence to be signaled once all
    ...
    If it is not dlink:VK_NULL_HANDLE, pname:fence may: be accessed at any
...
each <<synchronization-semaphores-signaling,semaphore signal operation>>
...
The scope is limited by the pname:stageMask member of the
slink:VkSemaphoreSubmitInfo used to define each such operation.
...
ifdef::VK_BASE_VERSION_1_2,VK_KHR_timeline_semaphore[]
  ...
  * [[VUID-vkQueueSubmit2-commandBuffer-10910]]
    ...
    of the pname:pCommandBufferInfos member of any element of pname:pSubmits
    includes a <<synchronization-queue-transfers-acquire, Queue Family
    Ownership Transfer Acquire Operation>>, the affected resource must: not
...
[open,refpage='VkSubmitInfo2',desc='Structure specifying a queue submit operation',type='structs']
--
...
ifdef::VK_ARM_data_graph[]
  * [[VUID-VkSubmitInfo2-pCommandBufferInfos-09933]]
    If at least one slink:VkCommandBufferSubmitInfo structure in
    pname:pCommandBufferInfos references a pname:commandBuffer allocated
    from a pool that was created with a
    slink:VkDataGraphProcessingEngineCreateInfoARM structure in the
    pname:pNext chain of slink:VkCommandPoolCreateInfo that included a
    foreign data graph processing engine in its pname:pProcessingEngines
    member, then pname:pWaitSemaphoreInfos and pname:pSignalSemaphoreInfos
    must: only reference pname:semaphore objects that were created from
    external handle types reported as supported in a
    slink:VkQueueFamilyDataGraphProcessingEnginePropertiesARM::pname:foreignSemaphoreHandleTypes
    structure via
    flink:vkGetPhysicalDeviceQueueFamilyDataGraphProcessingEnginePropertiesARM
    with a pname:queueFamilyIndex matching the one the command pool was
    created for, for all the foreign data graph processing engines that were
    part of the slink:VkDataGraphProcessingEngineCreateInfoARM used to
...
[open,refpage='VkSubmitFlagBits',desc='Bitmask specifying behavior of a submission',type='enums']
--
...
[open,refpage='VkSubmitFlags',desc='Bitmask of VkSubmitFlagBits',type='flags']
--
...
[open,refpage='VkSemaphoreSubmitInfo',desc='Structure specifying a semaphore signal or wait operation',type='structs']
--
...
The first <<synchronization-dependencies-scopes, synchronization scope>> of
a <<synchronization-semaphores-signaling,semaphore signal operation>> or the
second synchronization scope of a <<synchronization-semaphores-waiting,
semaphore wait operation>> defined by this structure are limited to
operations in stages indicated by pname:stageMask.
...
ifdef::VK_KHR_device_group_creation,VK_BASE_VERSION_1_1[]
...
endif::VK_KHR_device_group_creation,VK_BASE_VERSION_1_1[]
...
[open,refpage='VkCommandBufferSubmitInfo',desc='Structure specifying a command buffer submission',type='structs']
--
...
Fence operations submitted with flink:vkQueueSubmit have additional ordering
...
<<synchronization-fences, fence>> sections of <<synchronization, the
...
operation is defined in, and all commands that occur later in
<<synchronization-submission-order, submission order>>.
The scope is limited by the pname:pWaitDstStageMask for each batch, as
described in slink:VkSubmitInfo.
...
ifdef::VK_BASE_VERSION_1_2,VK_KHR_timeline_semaphore[]
  ...
  * [[VUID-vkQueueSubmit-pSubmits-10911]]
    If any element of pname:pSubmits->pCommandBuffers includes a
    <<synchronization-queue-transfers-acquire, Queue Family Ownership
    Transfer Acquire Operation>>, the affected resource must: not be
...
corresponding element of pname:pWaitDstStageMask.
...
ifdef::VK_BASE_VERSION_1_2,VK_KHR_timeline_semaphore[]
[open,refpage='VkTimelineSemaphoreSubmitInfo',desc='Structure specifying signal and wait values for timeline semaphores',type='structs']
--
...
****
  ...
  * [[VUID-vkCmdExecuteCommands-pCommandBuffers-00096]]
    ...
    pname:pCommandBuffers must: have been recorded with the
    ename:VK_COMMAND_BUFFER_USAGE_RENDER_PASS_CONTINUE_BIT
  * [[VUID-vkCmdExecuteCommands-pCommandBuffers-00099]]
    If this command is called within a render pass instance, and any element
    of pname:pCommandBuffers was recorded with
  ...
  * [[VUID-vkCmdExecuteCommands-contents-09680]]
    ...
    flink:vkCmdBeginRenderPass, and flink:vkCmdNextSubpass has not been
    called in the current render pass instance, the pname:contents parameter
    of flink:vkCmdBeginRenderPass must: have been
...
ifdef::VK_EXT_nested_command_buffer[, or ename:VK_SUBPASS_CONTENTS_INLINE_AND_SECONDARY_COMMAND_BUFFERS_EXT]
  * [[VUID-vkCmdExecuteCommands-None-09681]]
    If this command is called within a render pass instance begun with
    flink:vkCmdBeginRenderPass, and flink:vkCmdNextSubpass has been called
    in the current render pass instance, the pname:contents parameter of the
    last call to flink:vkCmdNextSubpass must: have been
    ename:VK_SUBPASS_CONTENTS_SECONDARY_COMMAND_BUFFERS
ifdef::VK_EXT_nested_command_buffer,VK_KHR_maintenance7[, or ename:VK_SUBPASS_CONTENTS_INLINE_AND_SECONDARY_COMMAND_BUFFERS_KHR]
  * [[VUID-vkCmdExecuteCommands-pCommandBuffers-06019]]
    If this command is called within a render pass instance begun with
    flink:vkCmdBeginRenderPass, each element of pname:pCommandBuffers must:
  ...
  * [[VUID-vkCmdExecuteCommands-pBeginInfo-06020]]
    If this command is called within a render pass instance begun with
    flink:vkCmdBeginRenderPass, the render passes specified in the
...
ifdef::VK_QCOM_render_pass_transform[]
  * [[VUID-vkCmdExecuteCommands-pNext-02865]]
    ...
    slink:VkRenderPassTransformBeginInfoQCOM in the pname:pNext chain of
    slink:VkRenderPassBeginInfo, then each element of pname:pCommandBuffers
...
ifdef::VK_EXT_transform_feedback[]
  ...
  * [[VUID-vkCmdExecuteCommands-commandBuffer-06533]]
    ...
    command in pname:commandBuffer in the current subpass will write to an
    image subresource as an attachment, commands recorded in elements of
    pname:pCommandBuffers must: not read from the memory backing that image
    subresource in any other way
  * [[VUID-vkCmdExecuteCommands-commandBuffer-06534]]
    If this command is called within a render pass instance and any recorded
    command in pname:commandBuffer in the current subpass will read from an
    image subresource used as an attachment in any way other than as an
    attachment, commands recorded in elements of pname:pCommandBuffers must:
    not write to that image subresource as an attachment
  * [[VUID-vkCmdExecuteCommands-pCommandBuffers-06535]]
    If this command is called within a render pass instance and any recorded
    command in a given element of pname:pCommandBuffers will write to an
    image subresource as an attachment, commands recorded in elements of
    pname:pCommandBuffers at a higher index must: not read from the memory
    backing that image subresource in any other way
  * [[VUID-vkCmdExecuteCommands-pCommandBuffers-06536]]
    If this command is called within a render pass instance and any recorded
    command in a given element of pname:pCommandBuffers will read from an
    image subresource used as an attachment in any way other than as an
...
ifdef::VK_EXT_sample_locations[]
  ...
  * [[VUID-vkCmdExecuteCommands-pBeginInfo-06025]]
    If this command is called within a render pass instance begun with
    flink:vkCmdBeginRendering, the render passes specified in the
    pname:pBeginInfo->pInheritanceInfo->renderPass members of the
    flink:vkBeginCommandBuffer commands used to begin recording each element
    of pname:pCommandBuffers must: be dlink:VK_NULL_HANDLE
  * [[VUID-vkCmdExecuteCommands-flags-06026]]
    If this command is called within a render pass instance begun with
    flink:vkCmdBeginRendering, the pname:flags member of the
  ...
  * [[VUID-vkCmdExecuteCommands-colorAttachmentCount-06027]]
    If this command is called within a render pass instance begun with
    flink:vkCmdBeginRendering, the pname:colorAttachmentCount member of the
    slink:VkCommandBufferInheritanceRenderingInfo structure included in the
    pname:pNext chain of
    slink:VkCommandBufferBeginInfo::pname:pInheritanceInfo used to begin
    recording each element of pname:pCommandBuffers must: be equal to the
  ...
  * [[VUID-vkCmdExecuteCommands-imageView-06028]]
    If this command is called within a render pass instance begun with
    flink:vkCmdBeginRendering, if the pname:imageView member of an element
    of the slink:VkRenderingInfo::pname:pColorAttachments parameter to
  ...
  * [[VUID-vkCmdExecuteCommands-pDepthAttachment-06029]]
    If this command is called within a render pass instance begun with
    flink:vkCmdBeginRendering, if the
  ...
  * [[VUID-vkCmdExecuteCommands-pDepthAttachment-06774]]
    If this command is called within a render pass instance begun with
    flink:vkCmdBeginRendering and the
    slink:VkRenderingInfo::pname:pDepthAttachment->imageView parameter to
...
ifdef::VK_EXT_custom_resolve[]
  * [[VUID-vkCmdExecuteCommands-pCommandBuffers-11500]]
    ...
    flink:vkCmdBeginRendering and contains a custom resolve, then each
    element of pname:pCommandBuffers must: have been recorded with a
    slink:VkCustomResolveCreateInfoEXT struct chained to its
    slink:VkCommandBufferInheritanceInfo
  * [[VUID-vkCmdExecuteCommands-pCommandBuffers-11501]]
    If the current render pass instance was begun with
    flink:vkCmdBeginRendering and does not contain a custom resolve, then
    each element of pname:pCommandBuffers must: not have been recorded with
    a slink:VkCustomResolveCreateInfoEXT struct chained to its
    slink:VkCommandBufferInheritanceInfo
  ...
  * [[VUID-vkCmdExecuteCommands-pCommandBuffers-11503]]
    ...
    pass instance, then each element of pname:pCommandBuffers must: have
    been recorded with
    slink:VkCustomResolveCreateInfoEXT::pname:customResolve as
    ename:VK_FALSE
  * [[VUID-vkCmdExecuteCommands-colorAttachmentCount-11532]]
    If this command is called within a render pass instance begun with
    flink:vkCmdBeginRendering that contains a custom resolve, the
    pname:colorAttachmentCount member of the
    slink:VkCustomResolveCreateInfoEXT structure included in the pname:pNext
    chain of slink:VkCommandBufferBeginInfo::pname:pInheritanceInfo used to
    begin recording each element of pname:pCommandBuffers must: be equal to
    the slink:VkRenderingInfo::pname:colorAttachmentCount parameter to
    flink:vkCmdBeginRendering
  * [[VUID-vkCmdExecuteCommands-resolveImageView-11533]]
    If this command is called within a render pass instance begun with
    flink:vkCmdBeginRendering that contains a custom resolve, if the
    pname:resolveImageView member of an element of the
  ...
  * [[VUID-vkCmdExecuteCommands-resolveImageView-11534]]
    ...
    slink:VkCustomResolveCreateInfoEXT structure included in the pname:pNext
    chain of slink:VkCommandBufferBeginInfo::pname:pInheritanceInfo used to
    begin recording each element of pname:pCommandBuffers must: be
    ename:VK_FORMAT_UNDEFINED
  ...
  * [[VUID-vkCmdExecuteCommands-pStencilAttachment-11536]]
    ...
    parameter to flink:vkCmdBeginRendering is not dlink:VK_NULL_HANDLE, the
    value of the pname:stencilAttachmentFormat member of the
    slink:VkCustomResolveCreateInfoEXT structure included in the pname:pNext
    chain of slink:VkCommandBufferBeginInfo::pname:pInheritanceInfo used to
    begin recording each element of pname:pCommandBuffers must: be equal to
    the format used to create that image view
  * [[VUID-vkCmdExecuteCommands-pDepthAttachment-11537]]
    If this command is called within a render pass instance begun with
    flink:vkCmdBeginRendering that contains a custom resolve and the
    slink:VkRenderingInfo::pname:pDepthAttachment->resolveImageView
    parameter to flink:vkCmdBeginRendering was dlink:VK_NULL_HANDLE, the
    value of the pname:depthAttachmentFormat member of the
    slink:VkCustomResolveCreateInfoEXT structure included in the pname:pNext
    chain of slink:VkCommandBufferBeginInfo::pname:pInheritanceInfo used to
    begin recording each element of pname:pCommandBuffers must: be
    ename:VK_FORMAT_UNDEFINED
  ...
  * [[VUID-vkCmdExecuteCommands-resolveImageView-11526]]
    ...
    recorded in the render pass instance, and the pname:resolveImageView
...
ifdef::VK_KHR_multiview,VK_GRAPHICS_VERSION_1_1[]
  * [[VUID-vkCmdExecuteCommands-viewMask-06031]]
    If this command is called within a render pass instance begun with
    flink:vkCmdBeginRendering, the pname:viewMask member of the
    slink:VkCommandBufferInheritanceRenderingInfo structure included in the
    pname:pNext chain of
    slink:VkCommandBufferBeginInfo::pname:pInheritanceInfo used to begin
    recording each element of pname:pCommandBuffers must: be equal to the
...
ifdef::VK_AMD_mixed_attachment_samples,VK_NV_framebuffer_mixed_samples[]
  * [[VUID-vkCmdExecuteCommands-pNext-06032]]
    ...
    slink:VkCommandBufferInheritanceInfo includes a
  ...
  * [[VUID-vkCmdExecuteCommands-pNext-06035]]
    If this command is called within a render pass instance begun with
    flink:vkCmdBeginRendering, and the pname:pNext chain of
    slink:VkCommandBufferInheritanceInfo does not include a
    slink:VkAttachmentSampleCountInfoAMD or
    slink:VkAttachmentSampleCountInfoNV structure,
ifdef::VK_EXT_custom_resolve[flink:vkCmdBeginCustomResolveEXT has not yet been recorded in the render pass instance, and]
    if the pname:imageView member of an element of the
    slink:VkRenderingInfo::pname:pColorAttachments parameter to
    flink:vkCmdBeginRendering is not dlink:VK_NULL_HANDLE, the value of
    slink:VkCommandBufferInheritanceRenderingInfo::pname:rasterizationSamples
    must: be equal to the sample count used to create that image view
  * [[VUID-vkCmdExecuteCommands-pNext-06036]]
    ...
    slink:VkAttachmentSampleCountInfoNV structure,
ifdef::VK_EXT_custom_resolve[flink:vkCmdBeginCustomResolveEXT has not yet been recorded in the render pass instance, and]
    if the slink:VkRenderingInfo::pname:pDepthAttachment->imageView
    parameter to flink:vkCmdBeginRendering is not dlink:VK_NULL_HANDLE, the
    value of
    slink:VkCommandBufferInheritanceRenderingInfo::pname:rasterizationSamples
    must: be equal to the sample count used to create that image view
  * [[VUID-vkCmdExecuteCommands-pNext-06037]]
    ...
    slink:VkAttachmentSampleCountInfoNV structure,
ifdef::VK_EXT_custom_resolve[flink:vkCmdBeginCustomResolveEXT has not yet been recorded in the render pass instance, and]
    if the slink:VkRenderingInfo::pname:pStencilAttachment->imageView
    parameter to flink:vkCmdBeginRendering is not dlink:VK_NULL_HANDLE, the
    value of
    slink:VkCommandBufferInheritanceRenderingInfo::pname:rasterizationSamples
    must: be equal to the sample count used to create that image view
...
ifdef::VK_ANDROID_external_format_resolve[]
  * [[VUID-vkCmdExecuteCommands-pNext-09299]]
    If this command is called within a render pass instance begun with
    flink:vkCmdBeginRendering, with any color attachment using a resolve
    mode of ename:VK_RESOLVE_MODE_EXTERNAL_FORMAT_DOWNSAMPLE_BIT_ANDROID,
    the pname:pNext chain of slink:VkCommandBufferInheritanceInfo used to
    create each element of pname:pCommandBuffers must: include a
  ...
  * [[VUID-vkCmdExecuteCommands-pNext-09300]]
    If this command is called within a render pass instance begun with
    flink:vkCmdBeginRendering with any color attachment using a resolve mode
    of ename:VK_RESOLVE_MODE_EXTERNAL_FORMAT_DOWNSAMPLE_BIT_ANDROID, and the
...
ifdef::VK_EXT_nested_command_buffer[]
  ...
  * [[VUID-vkCmdExecuteCommands-nestedCommandBuffer-09376]]
    ...
    feature is enabled, and pname:commandBuffer is a <<glossary, secondary
    command buffer>>, the <<glossary, command buffer nesting level>> of each
    element of pname:pCommandBuffers must: be less than
...
ifdef::VK_GRAPHICS_VERSION_1_4,VK_KHR_dynamic_rendering_local_read[]
  * [[VUID-vkCmdExecuteCommands-pCommandBuffers-09504]]
    If this command is called within a render pass instance begun with
    flink:vkCmdBeginRendering, the color attachment mapping state specified
    by slink:VkRenderingAttachmentLocationInfo in the inheritance info of
    each element of pname:pCommandBuffers and in the current state of
    pname:commandBuffer must: match
  * [[VUID-vkCmdExecuteCommands-pCommandBuffers-09505]]
    If this command is called within a render pass instance begun with
    flink:vkCmdBeginRendering, the input attachment mapping state specified
    by slink:VkRenderingInputAttachmentIndexInfo in the inheritance info of
    each element of pname:pCommandBuffers and in the current state of
    pname:commandBuffer must: match
...
ifdef::VK_QCOM_tile_memory_heap[]
  * [[VUID-vkCmdExecuteCommands-memory-10724]]
    ...
    pname:memory member of the slink:VkTileMemoryBindInfoQCOM structure
    included in the pname:pNext chain of
    slink:VkCommandBufferBeginInfo::pname:pInheritanceInfo used to begin
    recording each element of pname:pCommandBuffers must: be equal to the
    active bound <<memory-bind-tile-memory, bound tile memory object>> in
    pname:commandBuffer
...
ifdef::VK_QCOM_tile_shading[]
  * [[VUID-vkCmdExecuteCommands-pCommandBuffers-10620]]
    ...
    <<renderpass-tile-shading, tile shading>> enabled, all elements of
    pname:pCommandBuffers must: have been recorded with
    ename:VK_TILE_SHADING_RENDER_PASS_ENABLE_BIT_QCOM included in
    slink:VkRenderPassTileShadingCreateInfoQCOM::pname:flags
  * [[VUID-vkCmdExecuteCommands-pCommandBuffers-10621]]
    If the <<renderpass-per-tile-execution-model, per-tile execution model>>
    is enabled, all elements of pname:pCommandBuffers must: have been
    recorded with
    ename:VK_TILE_SHADING_RENDER_PASS_PER_TILE_EXECUTION_BIT_QCOM included
    in slink:VkRenderPassTileShadingCreateInfoQCOM::pname:flags
  * [[VUID-vkCmdExecuteCommands-tileApronSize-10622]]
    If this command is being recorded within a render pass instance, the
    pname:tileApronSize used to create the render pass instance must: equal
    the slink:VkRenderPassTileShadingCreateInfoQCOM::pname:tileApronSize
    used to record all elements of pname:pCommandBuffers
  * [[VUID-vkCmdExecuteCommands-pCommandBuffers-10623]]
    If any element of pname:pCommandBuffers was recorded with
    ename:VK_TILE_SHADING_RENDER_PASS_ENABLE_BIT_QCOM included in
    slink:VkRenderPassTileShadingCreateInfoQCOM::pname:flags, this command
  ...
  * [[VUID-vkCmdExecuteCommands-tileApronSize-10625]]
    ...
    slink:VkRenderPassTileShadingCreateInfoQCOM::pname:tileApronSize that
    was recorded into all elements of pname:pCommandBuffers must: equal
    `(0,0)`
endif::VK_QCOM_tile_shading[]
ifdef::VK_EXT_descriptor_heap[]
  * [[VUID-vkCmdExecuteCommands-commandBuffer-11351]]
    If there is a sampler descriptor heap bound to pname:commandBuffer, each
    element of pname:pCommandBuffers must: have been recorded with a value
    of
    ...
    that set via the last call to flink:vkCmdBindSamplerHeapEXT
  * [[VUID-vkCmdExecuteCommands-commandBuffer-11352]]
    If there is a resource descriptor heap bound to pname:commandBuffer,
    each element of pname:pCommandBuffers must: have been recorded with a
    value of
    slink:VkCommandBufferInheritanceDescriptorHeapInfoEXT::pname:pResourceHeapBindInfo
    that is either `NULL` or a pointer to a bind info that is identical to
    that set via the last call to flink:vkCmdBindResourceHeapEXT
  * [[VUID-vkCmdExecuteCommands-commandBuffer-11473]]
    If there is no sampler descriptor heap bound to pname:commandBuffer,
    each element of pname:pCommandBuffers must: have been recorded with a
    value of
    slink:VkCommandBufferInheritanceDescriptorHeapInfoEXT::pname:pSamplerHeapBindInfo
    set to `NULL`
  * [[VUID-vkCmdExecuteCommands-commandBuffer-11474]]
    If there is no resource descriptor heap bound to pname:commandBuffer,
    each element of pname:pCommandBuffers must: have been recorded with a
    value of
    slink:VkCommandBufferInheritanceDescriptorHeapInfoEXT::pname:pResourceHeapBindInfo
    set to `NULL`
