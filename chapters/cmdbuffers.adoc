...
ifdef::VKSC_VERSION_1_0[]
If the memory used for command recording exceeded the amount reserved by
slink:VkCommandPoolMemoryReservationCreateInfo::pname:commandPoolReservedSize
in the command pool that pname:commandBuffer was allocated from, then
fname:vkEndCommandBuffer must: return ename:VK_ERROR_OUT_OF_DEVICE_MEMORY.

[NOTE]
.Note
====
Implementations can also use <<fault-handling>> and in particular the
ename:VK_FAULT_TYPE_COMMAND_BUFFER_FULL fault to report ftext:vkCmd*
recording errors earlier than fname:vkEndCommandBuffer.
====
endif::VKSC_VERSION_1_0[]
