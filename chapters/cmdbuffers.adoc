...
ifdef::VK_ARM_data_graph[]
  ...
  * [[VUID-VkCommandPoolCreateInfo-pNext-09909]]
    If the pname:pNext chain includes a
    slink:VkDataGraphProcessingEngineCreateInfoARM structure, then the queue
    family designated by pname:queueFamilyIndex must: support the data graph
    processing engines specified in the
    slink:VkDataGraphProcessingEngineCreateInfoARM structure
