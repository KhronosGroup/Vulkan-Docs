...
ifdef::VK_KHR_acceleration_structure[]
  * ename:VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_KHR specifies an
    <<descriptors-accelerationstructure, acceleration structure
    descriptor>>.
endif::VK_KHR_acceleration_structure[]
ifdef::VK_NV_ray_tracing[]
  * ename:VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_NV specifies an
    <<descriptors-accelerationstructure, acceleration structure
    descriptor>>.
...
ifdef::VK_NV_partitioned_acceleration_structure[]
  * ename:VK_DESCRIPTOR_TYPE_PARTITIONED_ACCELERATION_STRUCTURE_NV specifies
    a <<descriptors-partitionedaccelerationstructure, partitioned
...
Block matching is supported in all shader stages if both the target view and
reference view specifies a format that supports
<<resources-image-view-format-features,format feature>>
...
code:opImageBlockMatchGatherSAD, and code:opImageBlockMatchGatherSSD
operations, only the single-component formats with the
ename:VK_FORMAT_FEATURE_2_BLOCK_MATCHING_BIT_QCOM flag set are supported.
endif::VK_QCOM_image_processing2[]

ifdef::VK_QCOM_image_processing3[]
For code:OpImageBlockMatchSSDQCOM and code:OpImageBlockMatchSADQCOM
operations, additional formats for target view and reference view are
specified with the
<<formats-properties,ename:VK_FORMAT_FEATURE_2_BLOCK_MATCHING_SXD_BIT_QCOM>>
format feature flag.

[NOTE]
====
Applications should: use
ename:VK_FORMAT_FEATURE_2_BLOCK_MATCHING_SXD_BIT_QCOM to determine the full
format support with code:OpImageBlockMatchSSDQCOM and
code:OpImageBlockMatchSADQCOM.
ifdef::VK_QCOM_image_processing2[]
Applications can: also use
ename:VK_FORMAT_FEATURE_2_BLOCK_MATCHING_BIT_QCOM, but it may: specify a
...
ifdef::VK_NV_partitioned_acceleration_structure[]
[[descriptors-partitionedaccelerationstructure]]
=== Partitioned Acceleration Structure

A _partitioned acceleration structure_
(ename:VK_DESCRIPTOR_TYPE_PARTITIONED_ACCELERATION_STRUCTURE_NV) is a
descriptor type that references a built partitioned top-level acceleration
structure (PTLAS), as defined in <<partitioned-tlas, Partitioned Top Level
