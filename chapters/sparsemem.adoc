...
slink:VkPhysicalDeviceFeatures.
  ...
  * _Sparse Residency_ builds on (and requires) the
    <<features-sparseBinding, pname:sparseBinding>> feature.
...
After creating a sname:VkBuffer with
ename:VK_BUFFER_CREATE_SPARSE_RESIDENCY_BIT, the entire buffer region is
considered unbound.
...
After creating a sname:VkImage with
ename:VK_IMAGE_CREATE_SPARSE_RESIDENCY_BIT, all image subresources are
considered unbound.
...
the code:VolatileTexel image operand, the code:Volatile memory semantic, or
endif::VK_COMPUTE_VERSION_1_2,VK_KHR_vulkan_memory_model[]
the code:Volatile decoration to load the value will prevent prior stored
...
bound simultaneously for a sname:VkImage created with the provided image
creation parameters.
...
arguments, pname:pPropertyCount will be zero upon return, and no data will
be written to pname:pProperties.
...
****
  * [[VUID-vkGetPhysicalDeviceSparseImageFormatProperties-samples-01094]]
    ...
    set in sname:VkImageFormatProperties::pname:sampleCounts returned by
    fname:vkGetPhysicalDeviceImageFormatProperties
ifdef::VK_BASE_VERSION_1_1,VK_KHR_get_physical_device_properties2[]
    ...
    with pname:format, pname:type, pname:tiling, and pname:usage equal to
...
sname:VkMemoryRequirements::pname:alignment field specifies the binding
...
pname:pSparseMemoryRequirements array, and on return the variable is
...
then pname:pSparseMemoryRequirementCount will be zero and
...
include::{generated}/api/protos/vkGetImageSparseMemoryRequirements2KHR.adoc[]
  ...
  * pname:device is the logical device that owns the image.
  * pname:pInfo is a pointer to a slink:VkImageSparseMemoryRequirementsInfo2
...
[open,refpage='vkGetDeviceImageSparseMemoryRequirements',desc='Query the memory requirements for a sparse image',type='protos']
--
...
****
  ...
  * [[VUID-VkSparseMemoryBind-resourceOffset-09491]]
    If the resource being bound is a sname:VkBuffer, pname:resourceOffset,
    pname:memoryOffset and pname:size must: be an integer multiple of the
    pname:alignment of the slink:VkMemoryRequirements structure returned
    from a call to flink:vkGetBufferMemoryRequirements with the buffer
    resource
  * [[VUID-VkSparseMemoryBind-resourceOffset-09492]]
    If the resource being bound is a sname:VkImage, pname:resourceOffset and
    pname:memoryOffset must: be an integer multiple of the pname:alignment
    of the slink:VkMemoryRequirements structure returned from a call to
    flink:vkGetImageMemoryRequirements with the image resource
...
If the pname:pBinds[i].flags of an element _i_ of pname:pBinds does not
contain ename:VK_SPARSE_MEMORY_BIND_METADATA_BIT, the pname:resourceOffset
is in the range [eq]#[0, slink:VkMemoryRequirements::pname:size)#, This
...
If the pname:pBinds[i].flags of an element _i_ of pname:pBinds contains
ename:VK_SPARSE_MEMORY_BIND_METADATA_BIT, the binding range specified must:
...
****
  ...
  * [[VUID-VkSparseImageMemoryBindInfo-subresource-01106]]
    The pname:subresource.aspectMask member of each element of pname:pBinds
    must: be valid for the pname:format specified in slink:VkImageCreateInfo
    when pname:image was created
...
****
  ...
  * [[VUID-vkQueueBindSparse-pWaitSemaphores-03245]]
    ...
    pname:pBindInfo referring to a semaphore
...
ifdef::VK_EXT_frame_boundary+VK_ARM_tensors[]
  * [[VUID-VkBindSparseInfo-pNext-09753]]
    If the pname:pNext chain of this structure includes a
