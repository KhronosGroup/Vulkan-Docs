...
If pname:customResolve is `VK_FALSE`, the pipeline can: only be used outside
the custom resolve section.
If pname:customResolve is `VK_TRUE`, the pipeline can: only be used inside
the custom resolve section.
...
slink:VkGraphicsPipelineCreateInfo, pname:customResolve is `VK_FALSE`,
...
When not passed as a pname:pNext member, pname:customResolve is `VK_FALSE`.
endif::VK_EXT_shader_object[]
endif::VK_EXT_fragment_density_map[]
