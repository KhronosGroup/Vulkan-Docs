...
To bind a pipeline in device generated commands, an implementation must: expose the pipeline's
device address. To query a pipeline's 64-bit device address, call:

[open,refpage='vkGetPipelineDeviceAddressNV',desc='Get pipeline's 64-bit device address',type='protos']
--
include::{generated}/api/protos/vkGetPipelineDeviceAddressNV.adoc[]
  * pname:device is the logical device on which the pipeline was created.
  * pname:pInfo is a pointer to a slink:VkPipelineDeviceAddressInfoNV structure
    specifying the pipeline to retrieve an address for.

include::{generated}/validity/protos/vkGetPipelineDeviceAddressNV.adoc[]
--

[open,refpage='VkPipelineDeviceAddressInfoNV',desc='Structure specifying the pipeline to query an address for',type='structs']
--

:refpage: VkPipelineDeviceAddressInfoNV
The sname:VkPipelineDeviceAddressInfoNV structure is defined
as:

include::{generated}/api/structs/VkPipelineDeviceAddressInfoNV.adoc[]
  * pname:sType is the type of this structure.
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:pipeline specifies the pipeline whose
    device address is being queried.

.Valid Usage
****
  * pname:pipeline must: be a compute pipeline
  * pname:pipeline must: have been created with flag ename:VK_PIPELINE_CREATE_INDIRECT_BINDABLE_BIT_NV set
****
include::{generated}/validity/structs/VkPipelineDeviceAddressInfoNV.adoc[]
--
...
ifdef::VKSC_VERSION_1_0[]
  ...
  * [[VUID-VkPipelineCacheCreateInfo-pInitialData-05045]]
    The contents of this structure and data pointed to by pname:pInitialData
    must: be the same as specified in one of the
