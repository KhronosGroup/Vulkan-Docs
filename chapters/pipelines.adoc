...
Device-Generated Commands>>, then that pipeline's associated metadata must:
be saved at a specified buffer device address for later use in indirect
command generation.
...
ifdef::VK_EXT_extended_dynamic_state3[]
    ...
    ename:VK_DYNAMIC_STATE_ALPHA_TO_COVERAGE_ENABLE_EXT, and
    ename:VK_DYNAMIC_STATE_ALPHA_TO_ONE_ENABLE_EXT dynamic states set, in
    which case
...
ifndef::VK_EXT_extended_dynamic_state3[]
  * [[VUID-VkGraphicsPipelineCreateInfo-topology-08773]]
    ...
    the pname:topology member of pname:pInputAssembly is
    ename:VK_PRIMITIVE_TOPOLOGY_POINT_LIST, a code:PointSize decorated
    variable must: be written to if <<features-maintenance5,
    pname:maintenance5>> is not enabled
endif::VK_EXT_extended_dynamic_state3[]
ifdef::VK_EXT_extended_dynamic_state3[]
  * [[VUID-VkGraphicsPipelineCreateInfo-topology-08808]]
    If the pipeline is being created with a code:Vertex {ExecutionModel} and
    no code:TessellationEvaluation or code:Geometry {ExecutionModel}, and
    the pname:topology member of pname:pInputAssembly is
    ename:VK_PRIMITIVE_TOPOLOGY_POINT_LIST,and either
    ...
    <<limits-dynamicPrimitiveTopologyUnrestricted,pname:dynamicPrimitiveTopologyUnrestricted>>
    is ename:VK_FALSE, a code:PointSize decorated variable must: be written
    to if <<features-maintenance5, pname:maintenance5>> is not enabled
endif::VK_EXT_extended_dynamic_state3[]
  * [[VUID-VkGraphicsPipelineCreateInfo-shaderTessellationAndGeometryPointSize-08774]]
    If the pipeline is being created with a code:TessellationEvaluation
    {ExecutionModel}, no code:Geometry {ExecutionModel}, uses the
    code:PointMode {ExecutionMode}, and
    <<features-shaderTessellationAndGeometryPointSize,
    pname:shaderTessellationAndGeometryPointSize>> is enabled, a
    code:PointSize decorated variable must: be written to if
    <<features-maintenance5, pname:maintenance5>> is not enabled
...
ifdef::VK_KHR_maintenance5[]
  * [[VUID-VkGraphicsPipelineCreateInfo-shaderTessellationAndGeometryPointSize-08776]]
    ...
    pname:shaderTessellationAndGeometryPointSize>> is enabled, a
    code:PointSize decorated variable must: be written to for every vertex
    emitted if <<features-maintenance5, pname:maintenance5>> is not enabled
endif::VK_KHR_maintenance5[]
...
ifdef::VK_EXT_extended_dynamic_state3[]
  * [[VUID-VkGraphicsPipelineCreateInfo-pMultisampleState-09026]]
    ...
    ename:VK_DYNAMIC_STATE_SAMPLE_MASK_EXT,
    ename:VK_DYNAMIC_STATE_ALPHA_TO_COVERAGE_ENABLE_EXT, or
    ename:VK_DYNAMIC_STATE_ALPHA_TO_ONE_ENABLE_EXT dynamic states is not
    set, pname:pMultisampleState must: be a valid pointer to a valid
...
ifdef::VK_EXT_extended_dynamic_state3[]
  * [[VUID-VkGraphicsPipelineCreateInfo-pRasterizationState-09039]]
    ...
    ename:VK_DYNAMIC_STATE_SAMPLE_MASK_EXT,
    ename:VK_DYNAMIC_STATE_ALPHA_TO_COVERAGE_ENABLE_EXT, or
    ename:VK_DYNAMIC_STATE_ALPHA_TO_ONE_ENABLE_EXT dynamic states are not
    set then pname:pRasterizationState must: be a valid pointer to a valid
  ...
  * [[VUID-VkGraphicsPipelineCreateInfo-renderPass-06603]]
    ...
    fragment output state>>, and pname:renderPass is not
    dlink:VK_NULL_HANDLE, pname:renderPass must: be a valid
    slink:VkRenderPass handle
...
ifdef::VK_EXT_extended_dynamic_state3[]
  * [[VUID-VkGraphicsPipelineCreateInfo-flags-09041]]
    ...
    ename:VK_DYNAMIC_STATE_ALPHA_TO_COVERAGE_ENABLE_EXT, or
    ename:VK_DYNAMIC_STATE_ALPHA_TO_ONE_ENABLE_EXT dynamic states are not
    set, pname:pRasterizationState must: be a valid pointer to a valid
    slink:VkPipelineRasterizationStateCreateInfo structure
...
ifdef::VK_NV_ray_tracing[]
  * ename:VK_PIPELINE_CREATE_2_DEFER_COMPILE_BIT_KHR specifies that a
...
ifdef::VK_NV_device_generated_commands[]
  * ename:VK_PIPELINE_CREATE_2_INDIRECT_BINDABLE_BIT_KHR specifies that the
...
ifdef::VK_NV_ray_tracing_motion_blur[]
  * ename:VK_PIPELINE_CREATE_2_RAY_TRACING_ALLOW_MOTION_BIT_KHR specifies
...
ifdef::VK_EXT_fragment_density_map[]
  * ename:VK_PIPELINE_CREATE_2_RENDERING_FRAGMENT_DENSITY_MAP_ATTACHMENT_BIT_KHR
    specifies that the pipeline will be used with a fragment density map
    attachment.
endif::VK_EXT_fragment_density_map[]
endif::VK_VERSION_1_3,VK_KHR_dynamic_rendering[]
ifdef::VK_EXT_graphics_pipeline_library[]
  * ename:VK_PIPELINE_CREATE_2_LINK_TIME_OPTIMIZATION_BIT_KHR specifies that
  ...
  * ename:VK_PIPELINE_CREATE_2_RETAIN_LINK_TIME_OPTIMIZATION_INFO_BIT_KHR
    specifies that pipeline libraries should retain any information
    necessary to later perform an optimal link with
    ename:VK_PIPELINE_CREATE_2_LINK_TIME_OPTIMIZATION_BIT_KHR.
endif::VK_EXT_graphics_pipeline_library[]
ifdef::VK_EXT_descriptor_buffer[]
  * ename:VK_PIPELINE_CREATE_2_DESCRIPTOR_BUFFER_BIT_KHR specifies that a
    pipeline will be used with <<descriptorbuffers,descriptor buffers>>,
    rather than <<descriptorsets,descriptor sets>>.
endif::VK_EXT_descriptor_buffer[]
ifdef::VK_EXT_attachment_feedback_loop_layout[]
  * ename:VK_PIPELINE_CREATE_2_COLOR_ATTACHMENT_FEEDBACK_LOOP_BIT_KHR
    specifies that the pipeline may: be used with an attachment feedback
    loop including color attachments.
  * ename:VK_PIPELINE_CREATE_2_DEPTH_STENCIL_ATTACHMENT_FEEDBACK_LOOP_BIT_KHR
    specifies that the pipeline may: be used with an attachment feedback
...
ifdef::VK_EXT_pipeline_protected_access[]
  * ename:VK_PIPELINE_CREATE_2_NO_PROTECTED_ACCESS_BIT_KHR specifies that
    the pipeline must: not be bound to a protected command buffer.
  * ename:VK_PIPELINE_CREATE_2_PROTECTED_ACCESS_ONLY_BIT_KHR specifies that
...
ename:VK_PIPELINE_CREATE_2_LINK_TIME_OPTIMIZATION_BIT_KHR if available,
whether or not that bit was specified.

[NOTE]
.Note
====
Using ename:VK_PIPELINE_CREATE_2_LINK_TIME_OPTIMIZATION_BIT_KHR (or not)
