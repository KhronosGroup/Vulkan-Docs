...
ifdef::VK_AMDX_shader_enqueue[]
  * [[VUID-VkGraphicsPipelineCreateInfo-shaderMeshEnqueue-10187]]
    If <<features-shaderMeshEnqueue,pname:shaderMeshEnqueue>> is not
    enabled, shaders specified by pname:pStages must: not declare the
...
ifdef::VK_EXT_graphics_pipeline_library[]
  * [[VUID-VkGraphicsPipelineCreateInfo-flags-06608]]
    {empty}
ifdef::VK_AMDX_shader_enqueue[]
    If <<features-shaderMeshEnqueue,pname:shaderMeshEnqueue feature>> is not
    enabled, and
endif::VK_AMDX_shader_enqueue[]
