...
ifdef::VK_AMDX_shader_enqueue[]
  * If <<features-shaderMeshEnqueue,pname:shaderMeshEnqueue>> is not enabled,
    shaders specified by pname:pStages must: not declare the
...
ifdef::VK_AMDX_shader_enqueue[]
  * ename:VK_PIPELINE_CREATE_2_EXECUTION_GRAPH_BIT_AMDX specifies that
    the pipeline will be used in an <<executiongraphs, execution graph>>
endif::VK_AMDX_shader_enqueue[]
