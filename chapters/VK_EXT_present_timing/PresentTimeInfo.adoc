...
include::{generated}/api/structs/VkPresentTimingInfoEXT.adoc[]
  ...
  * pname:targetTime is zero or a value specifying the target present
    time or duration, in nanoseconds, of the presentation request.
...
time domain pname:targetTime is specified for. Otherwise,
pname:targetTimeDomainPresentStage is ignored.
