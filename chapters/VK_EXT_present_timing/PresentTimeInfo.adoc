...
include::{generated}/api/structs/VkPresentTimingInfoEXT.adoc[]
  ...
  * pname:targetTimeDomainPresentStage is zero or a valid
    tlink:VkPresentStageFlagsEXT specifying a single present stage that a
    stage-local pname:timeDomainId refers to.
...
****
  ...
  * If pname:timeDomainId is associated with a
    ...
    pname:targetTimeDomainPresentStage must: not be zero.
****
