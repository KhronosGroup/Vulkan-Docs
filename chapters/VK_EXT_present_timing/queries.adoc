...
If pname:refreshDuration is zero, the presentation engine is not able
to determine the duration of the refresh cycle. Similarly, if
pname:refreshInterval is zero, the presentation engine is not able to
determine whether it is operating in VRR mode.

Otherwise, if pname:refreshInterval is the same as pname:refreshDuration,
the presentation engine is operating in FRR mode. In this case,
...
include::{generated}/api/structs/VkSwapchainTimeDomainPropertiesEXT.adoc[]
  ...
  * pname:pTimeDomains is a pointer to an array of elink:VkTimeDomainKHR values
...
the same elink:VkTimeDomainKHR value over time, for example when a surface is
...
fname:vkGetPastPresentationTimingEXT to return results in that same
order. Otherwise, results are returned in the order of their associated
...
****
  ...
  * If ename:VK_PAST_PRESENTATION_TIMING_ALLOW_OUT_OF_ORDER_RESULTS_BIT_EXT
    ...
    for which complete results have not been returned yet by a previous
    call.
****
...
results for. If
ename:VK_PAST_PRESENTATION_TIMING_ALLOW_PARTIAL_RESULTS_BIT_EXT was
...
stage results than were queried by the associated presentation
...
sname:VkPresentTimingInfoEXT::pname:timeDomainId if the requirements for using
