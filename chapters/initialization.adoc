...
2::
    ...
    implementation may: either return `NULL` or a function pointer, though
    the function pointer must: not be called.
...
ifdef::VK_LUNARG_direct_driver_loading[]
  * [[VUID-VkInstanceCreateInfo-pNext]]
    If the pname:pNext chain of sname:VkInstanceCreateInfo includes a
...
include::{generated}/api/structs/VkDirectDriverLoadingInfoLUNARG.adoc[]
  ...
  * pname:pfnGetInstanceProcAddr is a tlink:PFN_vkGetInstanceProcAddr
    pointer to the drivers `vkGetInstanceProcAddr` function.
...
--
Used to specify whether the provided drivers are used instead of
system-located drivers or if the provided drivers are used in addition.
...
[open,refpage='PFN_vkGetInstanceProcAddr',desc='Type definition for vkGetInstanceProcAddr',type='funcpointers']
--
include::{generated}/api/funcpointers/PFN_vkGetInstanceProcAddr.adoc[]
--
