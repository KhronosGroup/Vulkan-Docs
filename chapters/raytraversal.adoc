...
For traversals performed by <<glossary-pipeline-trace-ray, pipeline trace
ray>> instructions the pipeline must: be created with
ename:VK_PIPELINE_CREATE_RAY_TRACING_OPACITY_MICROMAP_BIT_KHR.
ifdef::VK_KHR_ray_query[]
ifdef::VK_KHR_opacity_micromap[]
For traversals performed by code:OpRayQueryProceedKHR
ifdef::VK_EXT_opacity_micromap[]
where apiext:VK_EXT_opacity_micromap is not enabled,
endif::VK_EXT_opacity_micromap[]
the shader must: declare the code:OpacityMicromapIdKHR execution mode with
its specialization-constant operand equal to ename:VK_TRUE.
endif::VK_KHR_opacity_micromap[]
ifdef::VK_EXT_opacity_micromap[]
If apiext:VK_EXT_opacity_micromap is enabled, code:OpRayQueryProceedKHR
traversal always uses opacity micromap data without the
code:OpacityMicromapIdKHR execution mode being declared.
ifdef::VK_KHR_opacity_micromap[]
This applies whenever apiext:VK_EXT_opacity_micromap is enabled, including
when apiext:VK_KHR_opacity_micromap is enabled as well.
endif::VK_KHR_opacity_micromap[]
endif::VK_EXT_opacity_micromap[]
ifdef::VK_KHR_opacity_micromap[]
...
micromaps, set ename:VK_GEOMETRY_INSTANCE_DISABLE_OPACITY_MICROMAPS_BIT_KHR
...
with the code:HitAttributeKHR storage class at the time
code:OpReportIntersectionKHR is executed.
