...
ifdef::VK_GRAPHICS_VERSION_1_0[  * <<queries-occlusion,Occlusion Queries>>]
ifdef::VK_COMPUTE_VERSION_1_0[  * <<queries-pipestats,Pipeline Statistics Queries>>]
  * <<queries-timestamps, Timestamp Queries>>
ifdef::VK_KHR_video_queue[  * <<queries-result-status-only, Result Status Queries>>]
ifdef::VK_KHR_video_encode_queue[  * <<queries-video-encode-feedback, Video Encode Feedback Queries>>]
ifdef::VK_KHR_performance_query[  * <<queries-performance, Performance Queries>>]
ifdef::VK_EXT_transform_feedback[  * <<queries-transform-feedback, Transform Feedback Queries>>]
ifdef::VK_INTEL_performance_query[  * <<queries-performance-intel, Intel Performance Queries>>]
ifdef::VK_EXT_mesh_shader[  * <<queries-mesh-shader, Mesh Shader Queries>>]
ifdef::VK_QCOM_elapsed_timer_query[  * <<queries-elapsed-timer, Elapsed Timer Queries>>]
...
include::{generated}/api/structs/VkQueryPoolCreateInfo.adoc[]
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:flags is a bitmask of elink:VkQueryPoolCreateFlagBits
  ...
  * pname:pipelineStatistics
ifdef::VK_COMPUTE_VERSION_1_0[]
    is a bitmask of elink:VkQueryPipelineStatisticFlagBits specifying which
    counters will be returned in queries on the new pool, as described below
    in <<queries-pipestats>>.
...
ifdef::VK_QCOM_elapsed_timer_query[]
  * [[VUID-VkQueryPoolCreateInfo-queryType-12416]]
    If the <<features-elapsedTimerQuery,elapsedTimerQuery>> feature is not
    enabled, pname:queryType must: not be
    ename:VK_QUERY_TYPE_TIME_ELAPSED_QCOM
...
ifdef::VK_KHR_video_queue[]
  * [[VUID-VkQueryPoolCreateInfo-queryType-11839]]
    If pname:queryType is ename:VK_QUERY_TYPE_RESULT_STATUS_ONLY_KHR, then
    at least one of the queue families of the device must: support
    <<queries-result-status-only,result status queries>>, as indicated by
    slink:VkQueueFamilyQueryResultStatusPropertiesKHR::pname:queryResultStatusSupport
endif::VK_KHR_video_queue[]
ifdef::VK_KHR_video_decode_vp9[]
  * [[VUID-VkQueryPoolCreateInfo-pNext-10779]]
    If the pname:pNext chain includes a slink:VkVideoProfileInfoKHR
    structure and its pname:videoCodecOperation member is
...
ifdef::VK_KHR_video_encode_feedback2[]
  * [[VUID-VkQueryPoolCreateInfo-queryType-12437]]
    If pname:queryType is ename:VK_QUERY_TYPE_VIDEO_ENCODE_FEEDBACK_KHR and
    the pname:pNext chain includes a
    slink:VkQueryPoolVideoEncodeFeedbackCreateInfoKHR structure with its
    pname:encodeFeedbackFlags member containing
    ...
    ename:VK_VIDEO_ENCODE_FEEDBACK_SKIPPED_PIXELS_BIT_KHR, or
    ename:VK_VIDEO_ENCODE_FEEDBACK_PICTURE_PARTITION_COUNT_BIT_KHR, then
    <<features-videoEncodeFeedback2,pname:videoEncodeFeedback2>> must: be
    enabled
  * [[VUID-VkQueryPoolCreateInfo-queryType-12438]]
    If pname:queryType is ename:VK_QUERY_TYPE_VIDEO_ENCODE_FEEDBACK_KHR and
    the pname:pNext chain includes a
    slink:VkQueryPoolVideoEncodePerPartitionFeedbackCreateInfoKHR structure
    with a non-zero pname:maxPerPartitionFeedbackEntries member, then
    <<features-videoEncodeFeedback2,pname:videoEncodeFeedback2>> must: be
    enabled
  * [[VUID-VkQueryPoolCreateInfo-queryType-12439]]
    If pname:queryType is ename:VK_QUERY_TYPE_VIDEO_ENCODE_FEEDBACK_KHR and
    the pname:pNext chain includes a
    slink:VkQueryPoolVideoEncodePerPartitionFeedbackCreateInfoKHR structure
    with a non-zero pname:maxPerPartitionFeedbackEntries member, then
    slink:VkQueryPoolVideoEncodePerPartitionFeedbackCreateInfoKHR::pname:perPartitionEncodeFeedbackFlags
    must: not be zero
  * [[VUID-VkQueryPoolCreateInfo-queryType-12440]]
    If pname:queryType is ename:VK_QUERY_TYPE_VIDEO_ENCODE_FEEDBACK_KHR and
    the pname:pNext chain includes a
    slink:VkQueryPoolVideoEncodePerPartitionFeedbackCreateInfoKHR structure,
    then its pname:maxPerPartitionFeedbackEntries must: be less than or
    equal to
    slink:VkVideoEncodeFeedback2CapabilitiesKHR::pname:maxPerPartitionFeedbackEntries,
    as returned by flink:vkGetPhysicalDeviceVideoCapabilitiesKHR for the
    <<video-profiles,video profile>> described by
    slink:VkVideoProfileInfoKHR and its pname:pNext chain
  * [[VUID-VkQueryPoolCreateInfo-queryType-12441]]
    If pname:queryType is ename:VK_QUERY_TYPE_VIDEO_ENCODE_FEEDBACK_KHR and
    the pname:pNext chain includes a
    slink:VkQueryPoolVideoEncodePerPartitionFeedbackCreateInfoKHR structure,
    then its pname:perPartitionEncodeFeedbackFlags must: not contain any
    bits that are not set in
    slink:VkVideoEncodeFeedback2CapabilitiesKHR::pname:supportedPerPartitionEncodeFeedbackFlags,
    as returned by flink:vkGetPhysicalDeviceVideoCapabilitiesKHR for the
    <<video-profiles,video profile>> described by
    slink:VkVideoProfileInfoKHR and its pname:pNext chain
...
ifdef::VK_VALVE_video_encode_rgb_conversion[]
  * [[VUID-VkQueryPoolCreateInfo-pNext-10918]]
...
[open,refpage='VkQueryPoolCreateFlagBits',desc='Bitmask specifying query pool properties',type='enums']
--
Bits which can: be set in slink:VkQueryPoolCreateInfo::pname:flags,
specifying options for query pools, are:

include::{generated}/api/enums/VkQueryPoolCreateFlagBits.adoc[]

ifdef::VK_KHR_maintenance9[]
  * ename:VK_QUERY_POOL_CREATE_RESET_BIT_KHR specifies that queries in the
...
[open,refpage='VkQueryPoolCreateFlags',desc='Bitmask of VkQueryPoolCreateFlagBits',type='flags']
--
include::{generated}/api/flags/VkQueryPoolCreateFlags.adoc[]

tname:VkQueryPoolCreateFlags is a bitmask type for setting a mask of zero or
more elink:VkQueryPoolCreateFlagBits.
--
...
ifdef::VK_QCOM_elapsed_timer_query[]
  * ename:VK_QUERY_TYPE_TIME_ELAPSED_QCOM specifies an
    <<queries-elapsed-timer, elapsed timer query>>.
endif::VK_QCOM_elapsed_timer_query[]
ifdef::VK_EXT_opacity_micromap[]
  * ename:VK_QUERY_TYPE_MICROMAP_SERIALIZATION_SIZE_EXT specifies a micromap
    query to get the size required by a serialized micromap.
  * ename:VK_QUERY_TYPE_MICROMAP_COMPACTED_SIZE_EXT specifies a micromap
...
ifdef::VK_KHR_device_address_commands[flink:vkCmdCopyQueryPoolResultsToMemoryKHR,]
...
ifdef::VK_KHR_maintenance9[]
where ename:VK_QUERY_POOL_CREATE_RESET_BIT_KHR was not set in
slink:VkQueryPoolCreateInfo::pname:flags
endif::VK_KHR_maintenance9[]
...
ifdef::VK_BASE_VERSION_1_2,VK_EXT_host_query_reset[]
[open,refpage='vkResetQueryPool',desc='Reset queries in a query pool',type='protos']
--
...
ifdef::VK_GRAPHICS_VERSION_1_0[occlusion queries count drawn samples,]
ifdef::VK_QCOM_elapsed_timer_query[elapsed timer queries count elapsed time]
resulting from commands that are recorded between a flink:vkCmdBeginQuery
command and a flink:vkCmdEndQuery command within a specified command buffer,
effectively scoping a set of dispatching
ifdef::VK_GRAPHICS_VERSION_1_0[and/or drawing]
commands.
endif::VK_COMPUTE_VERSION_1_0[]
...
pname:occlusionQueryEnable, pname:queryFlags, and/or
...
ifdef::VK_QCOM_elapsed_timer_query[]
If the pname:queryType of the pool is ename:VK_QUERY_TYPE_TIME_ELAPSED_QCOM,
...
ifdef::VK_COMPUTE_VERSION_1_0[]
  ** Pipeline statistics queries write one unsigned integer value for each
     bit that is enabled in the pname:pipelineStatistics when the pool is
...
ifdef::VK_QCOM_elapsed_timer_query[]
  ** Elapsed timer queries write one integer value - the elapsed time
     between fname:vkCmdBeginQuery and fname:vkCmdEndQuery.
endif::VK_QCOM_elapsed_timer_query[]
ifdef::VK_KHR_performance_query[]
...
ifdef::VK_KHR_video_encode_queue[]
  ...
  * If ename:VK_QUERY_RESULT_WITH_AVAILABILITY_BIT is specified, an
    ...
    it is available.
ifdef::VK_KHR_video_queue[]
  * If ename:VK_QUERY_RESULT_WITH_STATUS_BIT_KHR is specified, an additional
    ...
    Specific error codes are as defined in the elink:VkQueryResultStatusKHR
    enumeration.
endif::VK_KHR_video_queue[]
ifdef::VK_KHR_video_encode_feedback2[]
  ...
  ** If <<queries-video-encode-per-partition-feedback,per picture partition
     feedback>> is requested, video encode feedback queries write one or
     ...
     slink:VkQueryPoolVideoEncodePerPartitionFeedbackCreateInfoKHR::pname:maxPerPartitionFeedbackEntries
...
ename:VK_QUERY_RESULT_WITH_AVAILABILITY_BIT or
ename:VK_QUERY_RESULT_WITH_STATUS_BIT_KHR, respectively.
Implementations will never write more per picture partition results than the
pname:maxPerPartitionFeedbackEntries requested when the query pool was
created.
====
endif::VK_KHR_video_encode_feedback2[]
...
ifdef::VK_KHR_performance_query[]
    The only exceptions are the slink:VkPerformanceCounterResultKHR values
    written by performance queries whose type is determined by the
    pname:unit member of the corresponding slink:VkPerformanceCounterKHR.
endif::VK_KHR_performance_query[]
...
If the <<features-maintenance7,pname:maintenance7>> feature is enabled, for
...
ifdef::VK_KHR_performance_query[]
    ...
    then pname:pData must: be aligned to a multiple of `4`
  * [[VUID-vkGetQueryPoolResults-queryCount-12251]]
    If pname:queryCount is greater than 1, ename:VK_QUERY_RESULT_64_BIT is
    not set in pname:flags
ifdef::VK_KHR_performance_query[]
    and the pname:queryType used to create pname:queryPool was not
    ename:VK_QUERY_TYPE_PERFORMANCE_QUERY_KHR,
endif::VK_KHR_performance_query[]
    then pname:stride must: be a multiple of `4`
  ...
  * [[VUID-vkGetQueryPoolResults-queryCount-12252]]
    If pname:queryCount is greater than 1 and ename:VK_QUERY_RESULT_64_BIT
    is set in pname:flags, then pname:stride must: be a multiple of `8`
  * [[VUID-vkGetQueryPoolResults-stride-08993]]
    If pname:queryCount is greater than 1 and
    ename:VK_QUERY_RESULT_WITH_AVAILABILITY_BIT is set, pname:stride must:
...
ifdef::VK_KHR_performance_query[]
  * [[VUID-vkGetQueryPoolResults-queryType-03229]]
    If the pname:queryType used to create pname:queryPool was
    ename:VK_QUERY_TYPE_PERFORMANCE_QUERY_KHR, then pname:pData must: be
    aligned to a multiple of the size of slink:VkPerformanceCounterResultKHR
  * [[VUID-vkGetQueryPoolResults-queryCount-12253]]
    If pname:queryCount is greater than 1 and the pname:queryType used to
    create pname:queryPool was ename:VK_QUERY_TYPE_PERFORMANCE_QUERY_KHR,
  ...
  * [[VUID-vkGetQueryPoolResults-queryType-04519]]
    If pname:queryCount is greater than 1 and the pname:queryType used to
    create pname:queryPool was ename:VK_QUERY_TYPE_PERFORMANCE_QUERY_KHR,
    then pname:stride must: be large enough to contain the
...
[open,refpage='vkCmdCopyQueryPoolResultsToMemoryKHR',desc='Copy the results of queries in a query pool to a memory range',type='protos']
--
:refpage: vkCmdCopyQueryPoolResultsToMemoryKHR

To copy query statuses and numerical results directly to buffer memory,
call:

include::{generated}/api/protos/vkCmdCopyQueryPoolResultsToMemoryKHR.adoc[]
  ...
  * pname:queryCount is the number of queries.
    ...
    queries.
  * pname:pDstRange is a pointer to a slink:VkStridedDeviceAddressRangeKHR
    describing a range of memory addressed with a stride that will receive
    the results of the copy command.
  * pname:dstFlags is a tlink:VkAddressCommandFlagsKHR value defining the
    flags for the destination address range.
  * pname:queryResultFlags is a bitmask of elink:VkQueryResultFlagBits
...
pname:queryCount that is available are copied to pname:pDstRange.

If ename:VK_QUERY_RESULT_WITH_AVAILABILITY_BIT is set, results for all
queries in pname:queryPool identified by pname:firstQuery and
pname:queryCount are copied to pname:pDstRange, along with an extra
...
If ename:VK_QUERY_RESULT_WITH_STATUS_BIT_KHR is set, results for all queries
in pname:queryPool identified by pname:firstQuery and pname:queryCount are
copied to pname:pDstRange, along with an extra status value written directly
after the results of each query and interpreted as a signed integer.
A value of zero indicates that the results are not yet available.
Positive values indicate that the operations within the query completed
successfully, and the query results are valid.
...
fname:vkCmdCopyQueryPoolResultsToMemoryKHR is considered to be a transfer
...
.Valid Usage
****
:addressRange: pDstRange
:addressFlags: dstFlags
...
include::{chapters}/commonvalidity/query_results_common.adoc[]
  ...
  * [[VUID-vkCmdCopyQueryPoolResultsToMemoryKHR-flags-13077]]
    ...
    pname:pDstRange->address and pname:pDstRange->stride must: be multiples
    of 4
  ...
  * [[VUID-vkCmdCopyQueryPoolResultsToMemoryKHR-pDstRange-13079]]
    pname:pDstRange->size must: be large enough to contain the result of
    each query, as described <<queries-operation-memorylayout,here>>
  * [[VUID-vkCmdCopyQueryPoolResultsToMemoryKHR-pDstRange-13080]]
    The buffer from which the range defined by pname:pDstRange was queried
    must: have been created with ename:VK_BUFFER_USAGE_TRANSFER_DST_BIT
    usage flag
ifdef::VK_KHR_performance_query[]
...
ifdef::VK_INTEL_performance_query[]
  * [[VUID-vkCmdCopyQueryPoolResultsToMemoryKHR-queryType-13082]]
    fname:vkCmdCopyQueryPoolResultsToMemoryKHR must: not be called if the
  ...
  * [[VUID-vkCmdCopyQueryPoolResultsToMemoryKHR-dstFlags-13085]]
    pname:dstFlags must: not include
    ename:VK_ADDRESS_COMMAND_PROTECTED_BIT_KHR
****

include::{generated}/validity/protos/vkCmdCopyQueryPoolResultsToMemoryKHR.adoc[]
--
...
and flink:vkCmdWriteTimestamp are excluded from this scope.
...
include::{chapters}/commonvalidity/query_results_common.adoc[]
  ...
  * [[VUID-vkCmdCopyQueryPoolResults-queryCount-12254]]
    If pname:queryCount is greater than 1 and ename:VK_QUERY_RESULT_64_BIT
    is not set in pname:flags then pname:stride must: be a multiple of `4`
  * [[VUID-vkCmdCopyQueryPoolResults-flags-00823]]
    If ename:VK_QUERY_RESULT_64_BIT is set in pname:flags then
    pname:dstOffset must: be a multiple of `8`
...
ifdef::VK_GRAPHICS_VERSION_1_0[either drawing or]
...
supporting
ifdef::VK_GRAPHICS_VERSION_1_0[either graphics or]
compute operations.
...
include::{generated}/api/enums/VkQueryPipelineStatisticFlagBits.adoc[]
  ...
  ** If at least one vertex of the input primitive lies inside the clipping
     ...
     <<primsrast-polygons-basic, Basic Polygon Rasterization>> rules, or at
...
_Timestamps_ provide applications with a mechanism for monotonically
...
ifdef::VK_BASE_VERSION_1_3,VK_KHR_synchronization2[]
[open,refpage='vkCmdWriteTimestamp2',desc='Write a device timestamp into a query object',type='protos']
--
...
ifdef::VK_GRAPHICS_VERSION_1_3,VK_KHR_dynamic_rendering[or dynamic render pass]
...
ifdef::VK_GRAPHICS_VERSION_1_3,VK_KHR_dynamic_rendering[or dynamic render pass]
with multiview enabled, each of the [eq]#N# consecutive queries written for
...
ifdef::VK_QCOM_elapsed_timer_query[]
[[queries-elapsed-timer]]
== Elapsed Timer Queries

Elapsed timer queries provide a mechanism to query the elapsed time between
a flink:vkCmdBeginQuery and a flink:vkCmdEndQuery command.
The time calculated is generated by the sname:VkPhysicalDevice, and can: be
read from the host (using flink:vkGetQueryPoolResults) or copied to a
sname:VkBuffer (using flink:vkCmdCopyQueryPoolResults).
...
pname:timestampValidBits via flink:vkGetPhysicalDeviceQueueFamilyProperties.
If the <<limits-timestampComputeAndGraphics,
pname:timestampComputeAndGraphics>> limit is ename:VK_TRUE, elapsed timer
queries are supported by every queue family that supports either graphics or
compute operations (see slink:VkQueueFamilyProperties).
...
determined by the sname:VkQueueFamilyProperties::pname:timestampValidBits
...
other than pname:timestampValidBits to perform the elapsed time calculation.
...
sname:VkPhysicalDeviceLimits::pname:timestampPeriod.
endif::VK_QCOM_elapsed_timer_query[]
...
include::{generated}/api/structs/VkInitializePerformanceApiInfoINTEL.adoc[]
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:pUserData is NULL or a pointer for application data.
...
the slink:VkQueryPoolVideoEncodeFeedbackCreateInfoKHR structure included in
the pname:pNext chain of slink:VkQueryPoolCreateInfo.
...
ifdef::VK_KHR_video_encode_feedback2[]
  ...
  * ename:VK_VIDEO_ENCODE_FEEDBACK_INTRA_PIXELS_BIT_KHR specifies that
    ...
    below.
  * ename:VK_VIDEO_ENCODE_FEEDBACK_INTER_PIXELS_BIT_KHR specifies that
    queries managed by the pool will capture the number of pixels encoded as
    inter blocks, including or excluding any skipped blocks, as described
    below.
  * ename:VK_VIDEO_ENCODE_FEEDBACK_SKIPPED_PIXELS_BIT_KHR specifies that
    queries managed by the pool will capture the number of pixels encoded as
    skipped blocks.
  * ename:VK_VIDEO_ENCODE_FEEDBACK_PICTURE_PARTITION_COUNT_BIT_KHR specifies
...
ename:VK_VIDEO_ENCODE_FEEDBACK_AVERAGE_QUANTIZATION_BIT_KHR,
ename:VK_VIDEO_ENCODE_FEEDBACK_MIN_QUANTIZATION_BIT_KHR, and/or
ename:VK_VIDEO_ENCODE_FEEDBACK_MAX_QUANTIZATION_BIT_KHR are specified,
respectively, depend on the used video profile:

ifdef::VK_KHR_video_encode_h264[]
  * In case of <<encode-h264-profile,H.264 encode profiles>>, this value is
...
ifdef::VK_KHR_video_encode_h265[]
  * In case of <<encode-h265-profile,H.265 encode profiles>> this value is a
    signed integer indicating the average, minimum, or maximum QP used
    across the encoded coding blocks, respectively.
endif::VK_KHR_video_encode_h265[]
ifdef::VK_KHR_video_encode_av1[]
  * In case of <<encode-av1-profile,AV1 encode profiles>> this value is an
...
ename:VK_VIDEO_ENCODE_FEEDBACK_AVERAGE_QUANTIZATION_BIT_KHR is specified is
undefined:.

The number of pixels captured when
ename:VK_VIDEO_ENCODE_FEEDBACK_INTRA_PIXELS_BIT_KHR,
ename:VK_VIDEO_ENCODE_FEEDBACK_INTER_PIXELS_BIT_KHR, and/or
ename:VK_VIDEO_ENCODE_FEEDBACK_SKIPPED_PIXELS_BIT_KHR are specified include
padding pixels beyond the requested coded extent that are part of the
complete coding blocks encoded for the picture.

If ename:VK_VIDEO_ENCODE_FEEDBACK_SKIPPED_PIXELS_BIT_KHR is not supported
for a video encode profile, then the results captured for
ename:VK_VIDEO_ENCODE_FEEDBACK_INTRA_PIXELS_BIT_KHR and
ename:VK_VIDEO_ENCODE_FEEDBACK_INTER_PIXELS_BIT_KHR will always include
...
ifdef::VK_KHR_video_encode_h264[]
  * In case of <<encode-h264-profile,H.264 encode profiles>>, skipped blocks
...
ifdef::VK_KHR_video_encode_h265[]
  * In case of <<encode-h265-profile,H.265 encode profiles>>, skipped blocks
    are always inter blocks and the results captured for
    ename:VK_VIDEO_ENCODE_FEEDBACK_INTER_PIXELS_BIT_KHR will always include
    pixels encoded as skipped blocks.
+
...
ifdef::VK_KHR_video_encode_av1[]
  * In case of <<encode-av1-profile,AV1 encode profiles>>, both inter and
    ...
    ename:VK_VIDEO_ENCODE_FEEDBACK_INTRA_PIXELS_BIT_KHR and
    ename:VK_VIDEO_ENCODE_FEEDBACK_INTER_PIXELS_BIT_KHR, respectively, if
    possible.
    However, some implementations may: not be able to do so if they do not
    differentiate between skipped intra and inter blocks.
    Therefore, in case of <<encode-av1-profile,AV1 encode profiles>> that do
    support ename:VK_VIDEO_ENCODE_FEEDBACK_SKIPPED_PIXELS_BIT_KHR
    ...
    ename:VK_VIDEO_ENCODE_FEEDBACK_INTRA_PIXELS_BIT_KHR and
    ename:VK_VIDEO_ENCODE_FEEDBACK_INTER_PIXELS_BIT_KHR, respectively.
+
[NOTE]
====
...
ifdef::VK_KHR_video_encode_feedback2[]
If <<queries-video-encode-per-partition-feedback,per picture partition
feedback>> is enabled, this is followed by the values corresponding to each
...
[NOTE]
====
Applications should always specify ename:VK_QUERY_RESULT_WITH_STATUS_BIT_KHR
...
<<encode-unsuccessful,unsuccessfully>> completed video encode operations.
====
--
...
ifdef::VK_KHR_video_encode_feedback2[]
[[queries-video-encode-per-partition-feedback]]
=== Per Picture Partition Feedback

If the <<features-videoEncodeFeedback2,pname:videoEncodeFeedback2>> feature
...
pname:perPartitionEncodeFeedbackFlags member of the
slink:VkQueryPoolVideoEncodePerPartitionFeedbackCreateInfoKHR structure
included in the pname:pNext chain of slink:VkQueryPoolCreateInfo.

[open,refpage='VkQueryPoolVideoEncodePerPartitionFeedbackCreateInfoKHR',desc='Structure specifying enabled per picture partition video encode feedback values',type='structs']
--
The sname:VkQueryPoolVideoEncodePerPartitionFeedbackCreateInfoKHR structure
is defined as:

include::{generated}/api/structs/VkQueryPoolVideoEncodePerPartitionFeedbackCreateInfoKHR.adoc[]
  ...
  * pname:perPartitionEncodeFeedbackFlags is a bitmask of
...
include::{generated}/validity/structs/VkQueryPoolVideoEncodePerPartitionFeedbackCreateInfoKHR.adoc[]
--

[open,refpage='VkVideoEncodePerPartitionFeedbackFlagBitsKHR',desc='Bits specifying queried per picture partition video encode feedback values',type='enums']
--
Bits which can: be set in
slink:VkQueryPoolVideoEncodePerPartitionFeedbackCreateInfoKHR::pname:perPartitionEncodeFeedbackFlags
for video encode feedback query pools are:

include::{generated}/api/enums/VkVideoEncodePerPartitionFeedbackFlagBitsKHR.adoc[]

  * ename:VK_VIDEO_ENCODE_PER_PARTITION_FEEDBACK_STATUS_BIT_KHR specifies
    that queries managed by the pool will capture the result status of each
    encoded picture partition.
    These result status values are reported as elink:VkQueryResultStatusKHR
    values, similar to <<queries-result-status-only,result status queries>>
  ...
  * ename:VK_VIDEO_ENCODE_PER_PARTITION_FEEDBACK_BITSTREAM_BUFFER_OFFSET_BIT_KHR
    ...
    slink:VkVideoEncodeInfoKHR::pname:dstBuffer relative to the offset
    specified in slink:VkVideoEncodeInfoKHR::pname:dstBufferOffset.
  * ename:VK_VIDEO_ENCODE_PER_PARTITION_FEEDBACK_BITSTREAM_BYTES_WRITTEN_BIT_KHR
    ...
    slink:VkVideoEncodeInfoKHR::pname:dstBuffer.
...
the ename:VK_VIDEO_ENCODE_FEEDBACK_BITSTREAM_BUFFER_OFFSET_BIT_KHR and
ename:VK_VIDEO_ENCODE_FEEDBACK_BITSTREAM_BYTES_WRITTEN_BIT_KHR feedback
...
ifdef::VK_KHR_video_encode_av1[]
For example, AV1 bitstreams contain the code:tile_size_minus_1 syntax
...
than the pname:maxPerPartitionFeedbackEntries specified in the
slink:VkQueryPoolVideoEncodePerPartitionFeedbackCreateInfoKHR structure the
...
status of
ename:VK_QUERY_RESULT_STATUS_INSUFFICIENT_BITSTREAM_BUFFER_RANGE_KHR for the
picture partition(s) whose encoded bitstream data no longer fit into the
destination video bitstream buffer range.
--

[open,refpage='VkVideoEncodePerPartitionFeedbackFlagsKHR',desc='Bitmask of VkVideoEncodePerPartitionFeedbackFlagBitsKHR',type='flags']
--
include::{generated}/api/flags/VkVideoEncodePerPartitionFeedbackFlagsKHR.adoc[]

tname:VkVideoEncodePerPartitionFeedbackFlagsKHR is a bitmask type for
setting a mask of zero or more
elink:VkVideoEncodePerPartitionFeedbackFlagBitsKHR.
--
endif::VK_KHR_video_encode_feedback2[]
