...
Sampling is performed on <<images, images>>, typically in conjunction with a
...
ifdef::VK_QCOM_image_processing3[code:OpImageGatherQCOM,]
    and code:OpImageSparse*Gather read neighboring texels and
    <<textures-gather,return a single component>> of each.
...
ifdef::VK_QCOM_image_processing[]
  * code:OpImageSampleWeightedQCOM reads a 2D neighborhood of texels and
...
There are three sampling coordinate systems used in this chapter:
  ...
  * <<textures-sampling-coordinate-normalized,Normalized Sampling
    Coordinates>>
  * <<textures-sampling-coordinate-unnormalized,Unnormalized Sampling
    Coordinates>>
  * <<textures-sampling-coordinate-integer,Integer Sampling Coordinates>>
...
code:OpImageFetch, and code:OpImageSparseFetch instructions use integer
...
[[textures-sampling-coordinate-normalized]]
=== Normalized Sampling Coordinates

Normalized sampling coordinates are referred to as [eq]#(s,t,r,q,a)#, with
...
The values [eq]#s,t,r,q,a# are all floating-point values.
Values of [eq]#s,t,r# in the normalized range [eq]#[0.0,1.0]# and [eq]#a# in
the range [eq]#[0.0, layers - 1]# correspond to locations in the image being
sampled, where [eq]#layers# is the dimension of the image with the same
name.
The value [eq]#q# is used as a divisor for each of [eq]#s,t,r#, rather than
...
[[textures-sampling-coordinate-unnormalized]]
=== Unnormalized Sampling Coordinates

Unnormalized sampling coordinates are referred to as [eq]#(u,v,w,a)#, with
the coordinates having the following meanings:
...
The values [eq]#u,v,w,a# are all floating-point values, with values in the
ranges [eq]#[0.0,width)#, [eq]#[0.0, height)#, [eq]#[0.0, depth)#, and
[eq]#[0,layers-1]# corresponding to locations in the image being sampled.
The values [eq]#width#, [eq]#height#, [eq]#depth#, and [eq]#layers# are the
dimensions of the accessed <<images,image>>.
...
[[textures-sampling-coordinate-integer]]
...
The values [eq]#i,j,k,l,n# are all integer values, with values in the ranges
[eq]#[0,width)#, [eq]#[0, height)#, [eq]#[0, depth)#, [eq]#[0,layers)#, and
[eq]#[0,samples)# corresponding to locations in the image being sampled.
The values [eq]#width#, [eq]#height#, [eq]#depth#, [eq]#layers#, and
[eq]#samples# are the dimensions of the accessed <<images,image>>.
...
Integer coordinates are used as <<images, image coordinates>> to perform an
<<images-reads,image read>> after sampling calculations, directly
...
Integer sampling coordinates are used as <<images, image coordinates>> to
...
translating each coordinate as follows:
  ...
  * [eq]#y = j#
  * [eq]#z = k#
  * [eq]#layer = l + slink:VkImageSubresourceRange::pname:baseArrayLayer#
  * [eq]#sample = n#
...
They include the following steps, which are performed in the listed order:
  ...
  ** <<textures-operation-validation,Instruction/Format/Sampler/Image
     validation>>
...
ifdef::VK_QCOM_image_processing[]
The operations described in <<textures-blockmatch,block matching>> are
performed before <<images-component-substitution>> during
<<textures-texel-reads, texel read>>.

The operations described in <<textures-weightimage,weight image sampling>>
are performed before <<textures-component-swizzle,Component swizzle>>.
endif::VK_QCOM_image_processing[]
...
==== Instruction/Format/Sampler/Image View Validation
...
ifdef::VK_EXT_border_color_swizzle[]
  ...
  * The SPIR-V Image Format is not <<spirvenv-image-formats,compatible>>
    with the image view's or buffer view's pname:format.
...
ifdef::VK_COMPUTE_VERSION_1_3,VK_KHR_format_feature_flags2[]
  ...
  * The SPIR-V instruction's image variable's properties are not compatible
    with the image view:
  ** If the image view's pname:viewType is ename:VK_IMAGE_VIEW_TYPE_1D_ARRAY
     or ename:VK_IMAGE_VIEW_TYPE_2D_ARRAY,
ifdef::VK_KHR_maintenance11[and the image was not created with ename:VK_IMAGE_CREATE_ALIAS_SINGLE_LAYER_DESCRIPTOR_BIT_KHR,]
     then the instruction must: have code:Arrayed = 1.
  ** If the image view's pname:viewType is ename:VK_IMAGE_VIEW_TYPE_1D or
     ename:VK_IMAGE_VIEW_TYPE_2D,
ifdef::VK_KHR_maintenance11[and the image was not created with ename:VK_IMAGE_CREATE_ALIAS_SINGLE_LAYER_DESCRIPTOR_BIT_KHR,]
     the instruction must: have code:Arrayed = 0.
ifdef::VK_KHR_maintenance11[]
  ** If the image view's pname:viewType is ename:VK_IMAGE_VIEW_TYPE_1D_ARRAY
     or ename:VK_IMAGE_VIEW_TYPE_2D_ARRAY and code:Arrayed = 0, the image
     view must: have been created with a single layer.
endif::VK_KHR_maintenance11[]
  ** If the image view's pname:viewType is
     ename:VK_IMAGE_VIEW_TYPE_CUBE_ARRAY then the instruction must: have
     code:Arrayed = 1.
  ** If the image view's pname:viewType is ename:VK_IMAGE_VIEW_TYPE_CUBE
     then the instruction must: have code:Arrayed = 0.
...
statements that prevent the code:OpTypeImage and ename:VkFormat from being
different with respect to things such as numeric type, number of components,
etc.
Texel output validation covers cases such as using code:R32ui with
ename:VK_FORMAT_R8_UINT that for historical reasons are not considered
...
If <<limits-rgba4OpaqueBlackSwizzled,pname:rgba4OpaqueBlackSwizzled>> is
ename:VK_FALSE, the implementation
endif::VK_KHR_maintenance10[]
ifndef::VK_KHR_maintenance10[Implementations]
may: swap the blue and alpha channels when sampling non-custom border colors
with the ename:VK_FORMAT_B4G4R4A4_UNORM_PACK16 format, or the red and alpha
channels with the ename:VK_FORMAT_R4G4B4A4_UNORM_PACK16 format.
...
the sampled output.
However, etext:VK_BORDER_COLOR_*_OPAQUE_BLACK will instead be sampled as
...
ifndef::VK_GRAPHICS_VERSION_1_4,VK_KHR_maintenance5[]
...
[[textures-cube-map-face-and-coordinate-selection-table]]
...
[[textures-cube-map-derivative-selection-table]]
...
identify the four selected texels for code:OpImage*Gather
ifdef::VK_QCOM_image_processing3[]
, and code:OpImageGatherQCOM with modes code:GatherH2QCOM and
code:GatherV2QCOM
endif::VK_QCOM_image_processing3[]
.

If an code:OpImage*Gather operation does not use the code:ConstOffsets image
operand then the four texels form the 2 {times} 2 rectangle used for texture
filtering:
...
If an code:OpImage*Gather operation uses the code:ConstOffsets image operand
...
ifdef::VK_QCOM_image_processing3[]
If an code:OpImageGatherQCOM operation has a code:Mode equal to
code:GatherH2QCOM or code:GatherV2QCOM, then it uses a predefined set of
code:ConstOffsets.

The offsets used for code:GatherH2QCOM are:
...
The offsets used for code:GatherV2QCOM are:
...
The rules for the ename:VK_FILTER_NEAREST minification filter are applied to
identify the origin texel [eq]#\tau_{\{ij}}# for code:OpImageGatherQCOM with
modes equal to code:Gather4x1QCOM and code:GatherDQCOM.
The filter used for these modes is defined as:

[latexmath]
++++
\begin{aligned}
\tau[R] &= \tau_{ij + \Delta_0}[level_{base}][comp] \\
\tau[G] &= \tau_{ij + \Delta_1}[level_{base}][comp] \\
\tau[B] &= \tau_{ij + \Delta_2}[level_{base}][comp] \\
\tau[A] &= \tau_{ij + \Delta_3}[level_{base}][comp]
\end{aligned}
++++

The offsets used for code:Gather4x1QCOM are:
...
The offsets used for code:GatherDQCOM are:
...
ifndef::VK_COMPUTE_VERSION_1_2,VK_EXT_sampler_filter_minmax[]
...
[[textures-texel-footprint-granularity-values-table]]
...
The SPIR-V instruction code:OpImageSampleWeightedQCOM specifies a texture
...
The code:OpImageSampleWeightedQCOM support normalized or unnormalized texel
coordinates.
In addition to the inputs that would be accepted by an equivalent
code:OpImageSample* instruction, code:OpImageSampleWeightedQCOM accepts a
...
with the ename:VK_IMAGE_USAGE_SAMPLE_WEIGHT_BIT_QCOM usage flag set.
...
<<descriptors-weightimage,sample weight image>> descriptor type.
...
hPhase &= \left\lfloor\mathbin{frac}\left( u - 0.5 \right) \times phaseCount_{h} \right\rfloor  \\[1em]
vPhase &= \left\lfloor\mathbin{frac}\left( v - 0.5 \right) \times phaseCount_{v} \right\rfloor  \\[1em]
\psi &= \left(vPhase \times phaseCount_{h}\right) + hPhase
\end{aligned}
++++

where the number of fraction bits retained is
latexmath:[\mathbin{log2}\left( \sqrt{numPhases} \right)] specified by
...
<<textures-texel-reads>>, and their values are combined to produce a
filtered value.

[latexmath]
++++
\begin{aligned}
\tau_{weightSampling} &= \sum_{{j=j_0} \atop {l=l_0}}^{j_{filterHeight-1} \atop {l_{filterHeight-1}}}\quad \sum_{{i=i_0}\atop {k=k_0}}^{i_{filterWidth-1} \atop {k_{filterWidth-1}}}w(k,l,\psi)\tau_{ij} \\
\end{aligned}
++++
...
Finally, the operations described in <<textures-component-swizzle,Component
swizzle>> are performed and the final result is returned to the shader.
...
If code:blockSize is specified with either dimension `0`, code:Result will
...
with the _target_ and _reference_ images, which is defined as the _input_
sampler.

The _input_ sampler must: be created with:
  * For code:opImageBlockMatchSAD and code:opImageBlockMatchSSD,
    code:addressModeU and code:addressModeV equal to
...
ifdef::VK_QCOM_image_processing2[]
  * For code:opImageBlockMatchWindowSAD, code:opImageBlockMatchWindowSSD,
    code:opImageBlockMatchGatherSAD, or code:opImageBlockMatchGatherSSD
    instructions, code:addressModeU and code:addressModeV equal to
    ename:VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_BORDER with
    ename:VK_BORDER_COLOR_FLOAT_TRANSPARENT_BLACK
ifdef::VK_QCOM_image_processing3[]
    , or if the
    <<features-blockMatchExtendedClampToEdge,pname:blockMatchExtendedClampToEdge>>
    feature is enabled, equal to ename:VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE
  * For code:opImageBlockMatchWindowSAD, code:opImageBlockMatchWindowSSD,
    code:opImageBlockMatchGatherSAD, or code:opImageBlockMatchGatherSSD
    instructions, with either code:addressModeU and code:addressModeV equal
    to ename:VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE, code:addressModeU and
    code:addressModeV equal to ename:VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE
endif::VK_QCOM_image_processing3[]
endif::VK_QCOM_image_processing2[]
  * pname:unnormalizedCoordinates equal to ename:VK_TRUE
  * pname:components equal to ename:VK_COMPONENT_SWIZZLE_IDENTITY
  * pname:minFilter equal to ename:VK_FILTER_NEAREST
  * pname:mipmapMode equal to ename:VK_SAMPLER_MIPMAP_MODE_NEAREST
  * pname:minLod and pname:maxLod equal to `0`
  * pname:anisotropyEnable equal to ename:VK_FALSE
  * pname:compareEnable equal to ename:VK_FALSE
ifdef::VK_QCOM_image_processing2[]
  * For code:opImageBlockMatchWindowSAD and code:opImageBlockMatchWindowSSD
    instructions, slink:VkSamplerBlockMatchWindowCreateInfoQCOM must: be
...
[NOTE]
====
_Reference_ image does not support wrap modes and the coordinates must:
remain inside the image.
Application must: specify wrap modes in the sampler object because it must:
be identical with the _target_ image's sampler object, which does support
wrap modes.
====

The _target_ and _reference_ images must: be created with:
  * pname:imageType equal to ename:VK_IMAGE_TYPE_2D
  * pname:mipLevels equal to `1`
  * pname:arrayLayers equal to `1`
  * pname:samples equal to ename:VK_SAMPLE_COUNT_1_BIT
  * pname:format equal to a compatible <<descriptors-blockmatch,block
    matching format>>
  * pname:usage equal to `VK_IMAGE_USAGE_SAMPLE_BLOCK_MATCH_BIT_QCOM`
...
If the
<<features-blockMatchExtendedClampToEdge,pname:blockMatchExtendedClampToEdge>>
feature is enabled, and the _input_ sampler was created with
pname:addressModeU and pname:addressModeV equal to
ename:VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE, then any reference texel
coordinate latexmath:[(k,l)] in the set that fails
<<textures-integer-coordinate-validation,integer texel coordinate
validation>> is an invalid texel and <<textures-border-replacement,border
replacement>> is performed.
Otherwise, each
endif::VK_QCOM_image_processing3[]
ifndef::VK_QCOM_image_processing3[Each]
reference texel coordinate latexmath:[(k,l)] in the set must: not fail
...
texel from the target image latexmath:[\tau_{ij}].
Each pair of texel coordinates latexmath:[(k,l)] in the set selects a single
texel from the reference image latexmath:[\upsilon_{kl}].

Each selected texel is read with the operations specified in
<<textures-texel-reads>>, without performing
...
ifdef::VK_QCOM_image_processing3[code:OpImageGatherQCOM,]
    and code:OpImage*Gather instructions.
  * Texel Gathering: Performed by code:OpImage*Gather
ifdef::VK_QCOM_image_processing3[and code:OpImageGatherQCOM]
    instructions.
ifdef::VK_NV_shader_image_footprint[]
...
The return value is the vector [eq]#(d~l~ - level~base~, {lambda}')#.
These values may: be subject to implementation-specific maxima and minima
for very large, out-of-range values.

ifdef::VK_QCOM_image_processing[]
If the image is a <<textures-weightimage,weight image>>, the value returned
by code:OpImageQueryLod is undefined:.
endif::VK_QCOM_image_processing[]
