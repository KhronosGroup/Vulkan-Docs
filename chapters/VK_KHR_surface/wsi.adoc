...
ifdef::VK_SEC_ubm_surface[]
=== UBM Platform

[open,refpage='vkCreateUbmSurfaceSEC',desc='Create a slink:VkSurfaceKHR object for a UBM surface',type='protos']
--
To create a sname:VkSurfaceKHR object for a UBM surface, call:

include::{generated}/api/protos/vkCreateUbmSurfaceSEC.adoc[]

  * pname:instance is the instance to associate the surface with.
  * pname:pCreateInfo is a pointer to a slink:VkUbmSurfaceCreateInfoSEC
    structure containing parameters affecting the creation of the surface
    object.
  * pname:pAllocator is the allocator used for host memory allocated for the
    surface object when there is no more specific allocator available (see
...
include::{generated}/validity/protos/vkCreateUbmSurfaceSEC.adoc[]
--

[open,refpage='VkUbmSurfaceCreateInfoSEC', desc='Structure specifying parameters of a newly created UBM surface object', type='structs']
--
The sname:VkUbmSurfaceCreateInfoSEC structure is defined as:

include::{generated}/api/structs/VkUbmSurfaceCreateInfoSEC.adoc[]
...
[open,refpage='VkUbmSurfaceCreateFlagsSEC',desc='reserved flag for surface creation',type='flags']
--
include::{generated}/api/flags/VkUbmSurfaceCreateFlagsSEC.adoc[]

tname:VkUbmSurfaceCreateFlagsSEC is a bitmask type for setting a mask, but
is currently reserved for future use.
--
...
include::{generated}/api/protos/vkAcquireXlibDisplayEXT.adoc[]
  ...
  * pname:dpy is a connection to the X11 server that currently owns
    pname:display.
  * pname:display is the display the caller wishes to control in Vulkan.
...
include::{generated}/api/protos/vkGetRandROutputDisplayEXT.adoc[]
  ...
  * pname:physicalDevice is the physical device to query the display handle
    on.
  * pname:dpy is a connection to the X11 server from which pname:rrOutput
    was queried.
  * pname:rrOutput is an X11 RandR output ID.
  * pname:pDisplay is a pointer to a slink:VkDisplayKHR handle where the
    display is returned.

If there is no slink:VkDisplayKHR corresponding to pname:rrOutput on
pname:physicalDevice, dlink:VK_NULL_HANDLE will be returned in
pname:pDisplay.
...
include::{generated}/api/protos/vkGetWinrtDisplayNV.adoc[]
  ...
  * pname:physicalDevice is the physical device on which to query the
    display handle.
  * pname:deviceRelativeId is the value of the
...
include::{generated}/api/protos/vkAcquireDrmDisplayEXT.adoc[]
  ...
  * pname:physicalDevice is the physical device the display is on.
  * pname:drmFd is the DRM primary file descriptor.
  * pname:display is the display the caller wishes Vulkan to control.
...
include::{generated}/api/protos/vkGetDrmDisplayEXT.adoc[]
  ...
  * pname:physicalDevice is the physical device to query the display from.
  * pname:drmFd is the DRM primary file descriptor.
  * pname:connectorId is the identifier of the specified DRM connector.
  * pname:pDisplay is a pointer to a slink:VkDisplayKHR handle where the
    display is returned.

If there is no slink:VkDisplayKHR corresponding to the pname:connectorId on
the pname:physicalDevice, the returned pname:display will be
dlink:VK_NULL_HANDLE.
The provided pname:drmFd must: correspond to the one owned by the
pname:physicalDevice.
If not, the error code ename:VK_ERROR_UNKNOWN will be returned.
...
include::{generated}/api/protos/vkReleaseDisplayEXT.adoc[]
  ...
  * pname:physicalDevice is the physical device the display is on.
  * pname:display is the display to release control of.
...
ifdef::VK_SEC_ubm_surface[]
[[platformQuerySupport_ubm]]
=== UBM Platform

[open,refpage='vkGetPhysicalDeviceUbmPresentationSupportSEC',desc='Query physical device for presentation to UBM',type='protos']
--
To determine whether a queue family of a physical device supports
presentation to a UBM compositor, call:

include::{generated}/api/protos/vkGetPhysicalDeviceUbmPresentationSupportSEC.adoc[]
...
include::{generated}/validity/protos/vkGetPhysicalDeviceUbmPresentationSupportSEC.adoc[]
--
endif::VK_SEC_ubm_surface[]
...
ifdef::VK_KHR_swapchain[]
    See flink:vkCreateSwapchainKHR for details of swapchain creation.
endif::VK_KHR_swapchain[]
...
provided by
ifdef::VK_KHR_extended_flags[slink:VkSharedPresentSurfaceCapabilities2KHR or]
...
ifdef::VK_EXT_multisampled_render_to_swapchain[]
  * [[VUID-vkGetPhysicalDeviceSurfaceCapabilities2KHR-pNext-12446]]
    If a slink:VkSwapchainFlagsSurfaceCapabilitiesEXT structure is included
    in the pname:pNext chain of pname:pSurfaceCapabilities,
    pname:pSurfaceInfo->surface must: be a valid slink:VkSurfaceKHR handle
...
Only usage flags representable in elink:VkImageUsageFlagBits are returned in
pname:surfaceCapabilities.supportedUsageFlags.
If the pname:pNext chain includes a slink:VkImageUsageFlags2CreateInfoKHR
structure, all usage flags are returned in
slink:VkImageUsageFlags2CreateInfoKHR::pname:usage.
...
slink:VkSurfaceCapabilities2KHR to determine supported usage when using
ename:VK_PRESENT_MODE_SHARED_DEMAND_REFRESH_KHR or
ename:VK_PRESENT_MODE_SHARED_CONTINUOUS_REFRESH_KHR presentation mode.

include::{generated}/validity/structs/VkSharedPresentSurfaceCapabilitiesKHR.adoc[]
--

ifdef::VK_KHR_extended_flags[]
[open,refpage='VkSharedPresentSurfaceCapabilities2KHR',desc='Structure describing capabilities of a surface for shared presentation',type='structs']
--
The sname:VkSharedPresentSurfaceCapabilities2KHR structure is defined as:

include::{generated}/api/structs/VkSharedPresentSurfaceCapabilities2KHR.adoc[]

  * pname:sType is a elink:VkStructureType value identifying this structure.
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:sharedPresentSupportedUsageFlags is a bitmask of
    elink:VkImageUsageFlagBits2KHR representing the ways the application
    can: use the shared presentable image from a swapchain created with
    elink:VkPresentModeKHR set to
    ename:VK_PRESENT_MODE_SHARED_DEMAND_REFRESH_KHR or
    ename:VK_PRESENT_MODE_SHARED_CONTINUOUS_REFRESH_KHR for the surface on
    the specified device.
    ename:VK_IMAGE_USAGE_2_COLOR_ATTACHMENT_BIT_KHR must: be included in the
...
include::{generated}/validity/structs/VkSharedPresentSurfaceCapabilities2KHR.adoc[]
--
endif::VK_KHR_extended_flags[]
endif::VK_KHR_shared_presentable_image[]

ifdef::VK_EXT_multisampled_render_to_swapchain[]
[open,refpage='VkSwapchainFlagsSurfaceCapabilitiesEXT',desc='Structure describing supported swapchain create flags for a surface',type='structs']
--
The sname:VkSwapchainFlagsSurfaceCapabilitiesEXT structure is defined as:

include::{generated}/api/structs/VkSwapchainFlagsSurfaceCapabilitiesEXT.adoc[]

  * pname:sType is a elink:VkStructureType value identifying this structure.
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:swapchainSupportedFlags is a bitmask of
    tlink:VkSwapchainCreateFlagsKHR representing the supported flags in
    slink:VkSwapchainCreateInfoKHR when creating a swapchain for the surface
    on the specified device.
ifdef::VK_KHR_surface_maintenance1,VK_EXT_surface_maintenance1[]
    If a slink:VkSurfacePresentModeKHR structure is included in the
    pname:pNext chain of pname:pSurfaceInfo in
    flink:vkGetPhysicalDeviceSurfaceCapabilities2KHR, this value represents
    the supported flags for the specified present mode and those compatible
    with it as returned in slink:VkSurfacePresentModeCompatibilityKHR.
endif::VK_KHR_surface_maintenance1,VK_EXT_surface_maintenance1[]

include::{generated}/validity/structs/VkSwapchainFlagsSurfaceCapabilitiesEXT.adoc[]
--
endif::VK_EXT_multisampled_render_to_swapchain[]
...
If pname:presentTimingSupported is ename:VK_TRUE, the implementation must:
also advertise support for at least the
ename:VK_PRESENT_STAGE_QUEUE_OPERATIONS_END_BIT_EXT present stage in pname
pname:presentStageQueries.
...
include::{chapters}/VK_KHR_surface/wsi.adoc[tag=surface_capabilities_members]
  * pname:supportedSurfaceCounters is a bitmask of
    ...
    counter types.
    It must: not include ename:VK_SURFACE_COUNTER_VBLANK_BIT_EXT unless the
    surface queried is a <<wsi-display-surfaces,display surface>>
...
[[wsi-color-spaces-and-attributes-table]]
...
[[wsi-presentable-image-usage-queries-table]]
...
(pname:refreshDuration) that is a multiple of the vblank interval
(pname:refreshInterval) which may: be much smaller.
...
Note that sname:VkSwapchainTimingPropertiesEXT only represents the current
...
Otherwise, pname:timeDomainCount must: specify the number of elements in
pname:pTimeDomains and pname:pTimeDomainIds, and on return is overwritten
...
****
  * [[VUID-VkSwapchainTimeDomainPropertiesEXT-pTimeDomains-12370]]
    pname:pTimeDomains and pname:pTimeDomainIds must: both be `NULL` or both
    not be `NULL`
  * [[VUID-VkSwapchainTimeDomainPropertiesEXT-pTimeDomains-12371]]
    If pname:pTimeDomains and pname:pTimeDomainIds are not `NULL`, then
    pname:timeDomainCount must: not be zero
****
...
ifdef::VK_EXT_multisampled_render_to_swapchain[]
ename:VK_IMAGE_CREATE_MULTISAMPLED_RENDER_TO_SINGLE_SAMPLED_BIT_EXT is set if
ename:VK_SWAPCHAIN_CREATE_MULTISAMPLED_RENDER_TO_SINGLE_SAMPLED_BIT_EXT is set
...
[[wsi-swapchain-effective-image-usage]]
pname:imageUsage defines the <<fundamentals-effective-flags, effective usage
flags>> for the swapchain image(s).
ifdef::VK_KHR_extended_flags[]
If the pname:pNext chain includes a slink:VkImageUsageFlags2CreateInfoKHR
structure, pname:imageUsage is ignored, and the effective usage flags are
defined by slink:VkImageUsageFlags2CreateInfoKHR::pname:usage.
endif::VK_KHR_extended_flags[]
...
ifdef::VK_BASE_VERSION_1_1,VK_KHR_get_physical_device_properties2[]
    or flink:vkGetPhysicalDeviceImageFormatProperties2
endif::VK_BASE_VERSION_1_1,VK_KHR_get_physical_device_properties2[]
ifdef::VK_KHR_swapchain_mutable_format[]
...
ifdef::VK_EXT_multisampled_render_to_swapchain[]
  * [[VUID-VkSwapchainCreateInfoKHR-multisampledRenderToSwapchain-12447]]
    If the <<features-multisampledRenderToSwapchain,
    pname:multisampledRenderToSwapchain>> feature is not enabled,
    pname:flags must: not contain
    ename:VK_SWAPCHAIN_CREATE_MULTISAMPLED_RENDER_TO_SINGLE_SAMPLED_BIT_EXT
  * [[VUID-VkSwapchainCreateInfoKHR-flags-12448]]
    pname:flags must: be a subset of flags returned in
    slink:VkSwapchainFlagsSurfaceCapabilitiesEXT::pname:swapchainSupportedFlags
ifdef::VK_KHR_surface_maintenance1,VK_EXT_surface_maintenance1[]
...
ifdef::VK_EXT_present_timing[]
ename:VK_ERROR_PRESENT_TIMING_QUEUE_FULL_EXT,
endif::VK_EXT_present_timing[]
...
include::{generated}/api/structs/VkPresentTimingInfoEXT.adoc[]
  ...
  * pname:presentStageQueries is zero or a valid
    tlink:VkPresentStageFlagsEXT value indicating which present stages the
...
If pname:presentStageQueries is not zero, a slot in the swapchain's internal
...
flink:vkSetSwapchainPresentTimingQueueSizeEXT was never called, presentation
fails and returns ename:VK_ERROR_PRESENT_TIMING_QUEUE_FULL_EXT.
...
****
  ...
  * [[VUID-VkPresentTimingInfoEXT-timeDomainId-12400]]
    pname:timeDomainId must: be an id previously returned by
    flink:vkGetSwapchainTimeDomainPropertiesEXT
****
...
by a call to flink:vkQueuePresentKHR, an identifier needs to be associated
with them.
...
Each slink:VkSwapchainKHR has a presentId associated with it.
...
****
  * [[VUID-VkPresentIdKHR-swapchainCount-04998]]
    pname:swapchainCount must: be the same value as
    slink:VkPresentInfoKHR::pname:swapchainCount, where this
    sname:VkPresentIdKHR is in the pname:pNext chain of the
    slink:VkPresentInfoKHR structure
  * [[VUID-VkPresentIdKHR-presentIds-04999]]
    Each non-zero entry in pname:presentIds must: be greater than all
    previously submitted present ids for the associated swapchain in
    slink:VkPresentInfoKHR::pname:pSwapchains
****
...
slink:VkPresentIdKHR structure to the pname:pNext chain of the
...
one-to-one with fname:vkQueuePresentKHR calls.
...
include::{generated}/api/protos/vkLatencySleepNV.adoc[]
  ...
  * pname:device is the device associated with pname:swapchain.
  * pname:swapchain is the swapchain to delay associated CPU work for, based
    on the <<low-latency2-frame-attribution,frame attribution>> rules.
...
include::{generated}/api/structs/VkSetLatencyMarkerInfoNV.adoc[]
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:presentID is the application-defined identifier of the
...
flink:vkSetLatencyMarkerNV in the use cases described below:
  ...
  * ename:VK_LATENCY_MARKER_PRESENT_START_NV should: be called just before
    the fname:vkQueuePresentKHR call requesting presentation of an
    application-rendered frame.
  * ename:VK_LATENCY_MARKER_PRESENT_END_NV should: be called when that
    fname:vkQueuePresentKHR call returns.
    This may be used to delimit workload submissions between
    application-rendered frames according to the rules defined in
    <<low-latency2-frame-attribution>>.
...
[NOTE]
====
The ename:VK_LATENCY_MARKER_OUT_OF_BAND_RENDERSUBMIT_START_NV,
ename:VK_LATENCY_MARKER_OUT_OF_BAND_RENDERSUBMIT_END_NV,
ename:VK_LATENCY_MARKER_OUT_OF_BAND_PRESENT_START_NV, and
ename:VK_LATENCY_MARKER_OUT_OF_BAND_PRESENT_END_NV markers are not intended
for use by typical applications.
See <<low-latency2-frame-attribution>> for information on how the
...
replace fname:vkQueuePresentKHR calls.
Such layers should: leave the original
ename:VK_LATENCY_MARKER_PRESENT_START_NV and
ename:VK_LATENCY_MARKER_PRESENT_END_NV markers in place and emit
ename:VK_LATENCY_MARKER_OUT_OF_BAND_PRESENT_START_NV and
ename:VK_LATENCY_MARKER_OUT_OF_BAND_PRESENT_END_NV markers surrounding each
of their fname:vkQueuePresentKHR calls.
====
--
...
the following:
  ...
  * pname:presentID is the application-defined identifier of the
    ...
    flink:vkSetLatencyMarkerNV via
    slink:VkSetLatencyMarkerInfoNV::pname:presentID.
    It is not related to presentIds passed via
ifdef::VK_KHR_present_id[]
ifdef::VK_KHR_present_id2[slink:VkPresentIdKHR::pname:pPresentIds or]
ifndef::VK_KHR_present_id2[slink:VkPresentIdKHR::pname:pPresentIds.]
...
[open,refpage='VkLatencySubmissionPresentIdNV',desc='Structure used to associate a queue submission with an application-rendered frame',type='structs']
--
...
<<low-latency2-frame-attribution>> are not sufficient, an application can:
associate submissions with application-rendered frames by chaining
sname:VkLatencySubmissionPresentIdNV to its fname:vkQueueSubmit
...
calls.
A non-zero pname:presentID passed via sname:VkLatencySubmissionPresentIdNV
must: be greater than or equal to any non-zero pname:presentID previously
passed via sname:VkLatencySubmissionPresentIdNV on the same queue.

A logical device enters _explicit submission attribution_ mode when any
queue on that device receives a sname:VkLatencySubmissionPresentIdNV with a
non-zero pname:presentID.
While a device is in explicit submission attribution mode, each
fname:vkQueueSubmit
ifdef::VK_BASE_VERSION_1_3,VK_KHR_synchronization2[]
or fname:vkQueueSubmit2
endif::VK_BASE_VERSION_1_3,VK_KHR_synchronization2[]
call will be associated with the pname:presentID value of the first of these
rules to apply:

  * The pname:presentID in sname:VkLatencySubmissionPresentIdNV chained to
    the submission, if non-zero.
  * The most recent pname:presentID passed via
    sname:VkLatencySubmissionPresentIdNV on the same queue, if non-zero.
  * The maximum pname:presentID previously passed via
    sname:VkLatencySubmissionPresentIdNV on the device.

A fname:vkQueueSubmit
ifdef::VK_BASE_VERSION_1_3,VK_KHR_synchronization2[]
or fname:vkQueueSubmit2
endif::VK_BASE_VERSION_1_3,VK_KHR_synchronization2[]
call that chains sname:VkLatencySubmissionPresentIdNV with a pname:presentID
of zero clears the tracked pname:presentID for that queue.
The device will exit explicit submission attribution mode when all queues'
tracked pname:presentID values have been cleared.
...
advertising support for `apiext:VK_NV_low_latency2` with a
slink:VkExtensionProperties::pname:specVersion greater than or equal to 3.
Implementations advertising a lower pname:specVersion ignore
sname:VkLatencySubmissionPresentIdNV, so applications should: verify the
advertised pname:specVersion before relying on explicit submission
attribution.

[NOTE]
====
Most applications do not need to pass sname:VkLatencySubmissionPresentIdNV.
The implicit rules described in <<low-latency2-frame-attribution>> are
...
VK_NV_low_latency2 relies on attributing each fname:vkQueuePresentKHR and
fname:vkQueueSubmit call to an application-rendered frame, identified by the
pname:presentID values from slink:VkSetLatencyMarkerInfoNV.
...
A fname:vkQueuePresentKHR call is attributed to an application-rendered
frame according to the first of these rules to apply.

  * If a ename:VK_LATENCY_MARKER_OUT_OF_BAND_PRESENT_START_NV marker has
    been observed without an intervening
    ename:VK_LATENCY_MARKER_OUT_OF_BAND_PRESENT_END_NV, the frame's
    pname:presentID is the pname:presentID from that marker.
  * If the most recent ename:VK_LATENCY_MARKER_PRESENT_START_NV marker's
    pname:presentID has not already been mapped to a fname:vkQueuePresentKHR
    call, its pname:presentID is used.
  * Otherwise the fname:vkQueuePresentKHR call is considered a new
    application-rendered frame with no pname:presentID.

[NOTE]
====
The first rule allows frame generation implementations to map multiple
fname:vkQueuePresentKHR calls to the same pname:presentID, by emitting
multiple pairs of ename:VK_LATENCY_MARKER_OUT_OF_BAND_PRESENT_START_NV and
ename:VK_LATENCY_MARKER_OUT_OF_BAND_PRESENT_END_NV markers with the same
pname:presentID around each fname:vkQueuePresentKHR call.
====

Each fname:vkQueueSubmit
ifdef::VK_BASE_VERSION_1_3,VK_KHR_synchronization2[]
or fname:vkQueueSubmit2
endif::VK_BASE_VERSION_1_3,VK_KHR_synchronization2[]
...
of these rules that applies.
  ...
  * If the queue has been marked as
    ename:VK_OUT_OF_BAND_QUEUE_TYPE_PRESENT_NV via
    flink:vkQueueNotifyOutOfBandNV, the submission is ignored.
  * If the device is in explicit submission attribution mode as described
    under slink:VkLatencySubmissionPresentIdNV, the pname:presentID
    determined by that structure's rules is used.
  * If the queue has been marked as
    ename:VK_OUT_OF_BAND_QUEUE_TYPE_RENDER_NV, the submission is attributed
    to the pname:presentID of the most recent
    ename:VK_LATENCY_MARKER_OUT_OF_BAND_RENDERSUBMIT_START_NV marker that
    has been observed.
  * Otherwise the submission will be added to a list of pending submissions.
    ...
    flink:vkQueueNotifyOutOfBandNV, application-rendered frames are
    delimited by ename:VK_LATENCY_MARKER_PRESENT_END_NV markers, and the
    pending submission list is attributed to that marker's pname:presentID.
    Otherwise, application-rendered frames are delimited by
    fname:vkQueuePresentKHR calls, and the group is attributed to that
    present's application-rendered frame according to the
    fname:vkQueuePresentKHR mapping rules above.
...
include::{generated}/api/structs/VkLatencySurfaceCapabilitiesNV.adoc[]
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:presentModeCount is the number of presentation modes provided.
  * pname:pPresentModes is a list of presentation modes optimized for use
    with low latency mode with pname:presentModeCount entries.

If pname:pPresentModes is `NULL`, then the number of present modes that are
optimized for use with low latency mode is returned in
pname:presentModeCount.
Otherwise, pname:presentModeCount must: be set by the application to the
number of elements in the pname:pPresentModes array, and on return is
overwritten with the number of values actually written to
pname:pPresentModes.
