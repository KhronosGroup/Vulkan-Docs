...
ifdef::VK_EXT_shader_object[or shaders]
    with embedded samplers currently created on the device, there must: be
    less than
    [eq]#(<<limits-maxSamplerAllocationCount,pname:maxSamplerAllocationCount>>
    {minus} (<<limits-minSamplerHeapReservedRangeWithEmbedded,
...
include::{generated}/api/structs/VkSamplerCreateInfo.adoc[]
  ...
  * pname:addressModeU is a elink:VkSamplerAddressMode value specifying the
    <<textures-wrapping-operation,wrapping operation>> used when the [eq]#i#
  ...
  * pname:addressModeV is a elink:VkSamplerAddressMode value specifying the
    <<textures-wrapping-operation,wrapping operation>> used when the [eq]#j#
    coordinate used to sample the image would be out of bounds.
  * pname:addressModeW is a elink:VkSamplerAddressMode value specifying the
    <<textures-wrapping-operation,wrapping operation>> used when the [eq]#k#
    coordinate used to sample the image would be out of bounds.
    If pname:unnormalizedCoordinates is ename:VK_TRUE, pname:addressModeW is
    ignored.
  ...
  * [[samplers-unnormalizedCoordinates]] pname:unnormalizedCoordinates
    ...
    When pname:unnormalizedCoordinates is ename:VK_TRUE, the range of the
    image coordinates used to lookup the texel is in the range of zero to
    the image size in each dimension.
    When pname:unnormalizedCoordinates is ename:VK_FALSE, the range of image
...
flink:vkWriteSamplerDescriptorsEXT, applications can: give the descriptor a
debug name in a similar way to naming an object, via the
slink:VkDebugUtilsObjectNameInfoEXT structure.
However, as there is no actual object, slink:VkDebugUtilsObjectNameInfoEXT
must: be passed via the pname:pNext chain of this structure, with a
pname:objectType of ename:VK_OBJECT_TYPE_UNKNOWN and a pname:objectHandle of
...
slink:VkDescriptorSetAndBindingMappingEXT can: also be chained in the same
way when defining an embedded sampler via
slink:VkDescriptorSetAndBindingMappingEXT, naming the embedded sampler.
...
ifdef::VK_COMPUTE_VERSION_1_2,VK_EXT_sampler_filter_minmax[]
  * [[VUID-VkSamplerCreateInfo-None-01647]]
    ...
    reduction mode must: be ename:VK_SAMPLER_REDUCTION_MODE_WEIGHTED_AVERAGE
...
ifdef::VK_COMPUTE_VERSION_1_2[]
  * [[VUID-VkSamplerCreateInfo-pNext-06726]]
    If the <<features-samplerFilterMinmax, pname:samplerFilterMinmax>>
    feature is not enabled and the pname:pNext chain includes a
    slink:VkSamplerReductionModeCreateInfo structure, then the sampler
    reduction mode must: be ename:VK_SAMPLER_REDUCTION_MODE_WEIGHTED_AVERAGE
endif::VK_COMPUTE_VERSION_1_2[]
  * [[VUID-VkSamplerCreateInfo-addressModeU-01079]]
    If the <<features-samplerMirrorClampToEdge,
    pname:samplerMirrorClampToEdge>> feature is not enabled, and if the
    `apiext:VK_KHR_sampler_mirror_clamp_to_edge` extension is not enabled,
...
ifdef::VK_EXT_filter_cubic[]
    ...
    either pname:magFilter or pname:minFilter is ename:VK_FILTER_CUBIC_IMG,
...
ifdef::VK_QCOM_image_processing[]
  * [[samplers-imageprocessingsampler]]
    ...
    sampler will read from images using only code:OpImageSampleWeightedQCOM,
...
ifdef::VK_COMPUTE_VERSION_1_2,VK_EXT_sampler_filter_minmax[]
[open,refpage='VkSamplerReductionModeCreateInfo',desc='Structure specifying sampler reduction mode',type='structs']
--
...
[open,refpage='VkSamplerReductionMode',desc='Specify reduction mode for texture filtering',type='enums']
--
...
parameters, corresponding to different <<textures-wrapping-operation,
wrapping operations>> used during sampling, are:
...
ifdef::VK_COMPUTE_VERSION_1_2,VK_KHR_sampler_mirror_clamp_to_edge[]
  * ename:VK_SAMPLER_ADDRESS_MODE_MIRROR_CLAMP_TO_EDGE specifies that the
    mirror clamp to edge wrap mode will be used.
    This is only valid if
ifdef::VK_COMPUTE_VERSION_1_2[the <<features-samplerMirrorClampToEdge, pname:samplerMirrorClampToEdge>> feature is enabled, or if]
    the `apiext:VK_KHR_sampler_mirror_clamp_to_edge` extension is enabled.
endif::VK_COMPUTE_VERSION_1_2,VK_KHR_sampler_mirror_clamp_to_edge[]
--
...
include::{generated}/api/enums/VkBorderColor.adoc[]
  ...
  * ename:VK_BORDER_COLOR_INT_OPAQUE_WHITE specifies an opaque, integer
    format, white color.
ifdef::VK_EXT_custom_border_color[]
  * ename:VK_BORDER_COLOR_FLOAT_CUSTOM_EXT specifies that a
  ...
  * ename:VK_BORDER_COLOR_INT_CUSTOM_EXT specifies that a
    slink:VkSamplerCustomBorderColorCreateInfoEXT structure is included in
    the slink:VkSamplerCreateInfo::pname:pNext chain containing the color
    data in integer format.
endif::VK_EXT_custom_border_color[]

These colors are described in detail in <<textures-border-replacement,
Border Replacement>>.
--
...
ifdef::VK_EXT_descriptor_heap[]
a combined <<VkDescriptorSetAndBindingMappingEXT, embedded sampler and image
mapping>> if using descriptor heaps, or
endif::VK_EXT_descriptor_heap[]
...
the pname:forceExplicitReconstruction value behaves as if it were
ename:VK_TRUE.
====
...
ifdef::VK_ANDROID_external_memory_android_hardware_buffer[]
  ...
  * [[VUID-VkSamplerYcbcrConversionCreateInfo-ycbcrModel-01655]]
    ...
    consequence of <<images-component-substitution,component substitution>>
...
ifdef::VK_QCOM_ycbcr_degamma[]
  * [[VUID-VkSamplerYcbcrConversionCreateInfo-pNext-09207]]
    ...
    if the <<features-ycbcrDegamma,pname:ycbcrDegamma>> feature is not
    enabled, then
...
components of a {YCbCr} image during <<textures-YCbCr-degamma, sampling>> by
including sname:VkSamplerYcbcrConversionYcbcrDegammaCreateInfoQCOM structure
in the pname:pNext chain of slink:VkSamplerYcbcrConversionCreateInfo.
...
include::{generated}/api/structs/VkSamplerYcbcrConversionYcbcrDegammaCreateInfoQCOM.adoc[]
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:enableYDegamma indicates <<textures-YCbCr-degamma, sRGB to
    linear>> conversion is enabled for the G component.
...
slink:VkSamplerCreateInfo::pname:borderColor and should: source it from the
first component.
====

In some cases, implementations may: return undefined: values instead of the
specified custom border color when used on ename:VK_QUEUE_COMPUTE_BIT queues
that do not also support ename:VK_QUEUE_GRAPHICS_BIT.
...
[open,refpage='vkRegisterCustomBorderColorEXT',desc='Register a custom border color',type='protos']
--
To register a custom border color, call:

include::{generated}/api/protos/vkRegisterCustomBorderColorEXT.adoc[]

  * pname:device is the logical device where the border color is registered.
  * pname:pBorderColor is a pointer to a
    slink:VkSamplerCustomBorderColorCreateInfoEXT structure specifying the
    custom border color value to register.
  * pname:requestIndex is a Boolean value indicating if a specific index is
    requested or not.
  * pname:pIndex is a pointer to a code:uint32_t index value that will be
    written by the command upon success.

If pname:requestIndex is ename:VK_TRUE, the value present in pname:pIndex
...
ename:VK_ERROR_INVALID_OPAQUE_CAPTURE_ADDRESS will be returned.
...
If pname:requestIndex is ename:VK_FALSE, the value stored in pname:pIndex is
...
ename:VK_ERROR_TOO_MANY_OBJECTS will be returned.
...
border color, via slink:VkSamplerCustomBorderColorIndexCreateInfoEXT.

[NOTE]
====
The type of border color is not specified by this command
(ename:VK_BORDER_COLOR_FLOAT_CUSTOM_EXT vs.
ename:VK_BORDER_COLOR_INT_CUSTOM_EXT); the data will be interpreted at the
...
****
  * [[VUID-vkRegisterCustomBorderColorEXT-requestIndex-11287]]
    If pname:requestIndex is ename:VK_TRUE, the value stored in pname:pIndex
    must: be less than <<limits-maxCustomBorderColorSamplers,
    maxCustomBorderColorSamplers>>
****

include::{generated}/validity/protos/vkRegisterCustomBorderColorEXT.adoc[]
--

[open,refpage='vkUnregisterCustomBorderColorEXT',desc='Unregister a custom border color',type='protos']
--
To unregister a custom border color, call:

include::{generated}/api/protos/vkUnregisterCustomBorderColorEXT.adoc[]

  * pname:device is the logical device where the border color is registered.
  * pname:index is the code:uint32_t index value to unregister.

.Valid Usage
****
  * [[VUID-vkUnregisterCustomBorderColorEXT-index-11288]]
    pname:index must: be less than <<limits-maxCustomBorderColorSamplers,
    pname:maxCustomBorderColorSamplers>>
****

include::{generated}/validity/protos/vkUnregisterCustomBorderColorEXT.adoc[]
--

[open,refpage='VkSamplerCustomBorderColorIndexCreateInfoEXT',desc='Structure specifying the custom border color index for a sampler',type='structs']
--
The sname:VkSamplerCustomBorderColorIndexCreateInfoEXT structure is defined
as:

include::{generated}/api/structs/VkSamplerCustomBorderColorIndexCreateInfoEXT.adoc[]

  * pname:sType is a elink:VkStructureType value identifying this structure.
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:index is the code:uint32_t index value to use with the sampler

If this structure is included in the pname:pNext chain of
slink:VkSamplerCreateInfo, the value of pname:index will be used for the
custom border color registration.
pname:index does not need to be registered at the point that a sampler
...
color by calling flink:vkRegisterCustomBorderColorEXT with that custom
border color value, and using that value as pname:index in this structure.
...
custom border color, it is equivalent to setting pname:index to 0.
...
include::{generated}/validity/structs/VkSamplerCustomBorderColorIndexCreateInfoEXT.adoc[]
--
endif::VK_EXT_descriptor_heap[]
endif::VK_EXT_custom_border_color[]
