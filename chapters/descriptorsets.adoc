...
Additionally, when <<features-tileShadingColorAttachments,
pname:tileShadingColorAttachments>> is enabled, stores using
code:OpImageWrite are supported for color attachments in compute shaders
with the same set of image formats as for loads.
When the <<features-tileShadingAtomicOps, pname:tileShadingAtomicOps>> is
