...
ifdef::VK_NV_device_generated_commands_compute[]
  * ename:VK_DESCRIPTOR_SET_LAYOUT_CREATE_INDIRECT_BINDABLE_BIT_NV specifies
    that descriptor sets using this layout allows them to be bound with pipelines
    that are created with ename:VK_PIPELINE_CREATE_INDIRECT_BINDABLE_BIT_NV flag
    set to be used in <<device-generated-commands,Device-Generated Commands>>.
...
ifndef::VKSC_VERSION_1_0[]
  * [[VUID-VkDescriptorSetAllocateInfo-apiVersion-07895]]
    ...
    1.1, pname:descriptorSetCount must: not be greater than the number of
    sets that are currently available for allocation in pname:descriptorPool
  * [[VUID-VkDescriptorSetAllocateInfo-apiVersion-07896]]
    If the apiext:VK_KHR_maintenance1 extension is not enabled,
    slink:VkPhysicalDeviceProperties::pname:apiVersion is less than Vulkan
    1.1, pname:descriptorPool must: have enough free descriptor capacity
