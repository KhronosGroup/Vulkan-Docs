...
ifdef::VK_EXT_extended_dynamic_state3[]
  * pname:pAttachments is ignored if all of
...
attachments for subsequent drawing commands when the graphics pipeline is
created with ename:VK_DYNAMIC_STATE_COLOR_BLEND_ENABLE_EXT set in
...
attachments for subsequent drawing commands when the graphics pipeline is
created with ename:VK_DYNAMIC_STATE_COLOR_BLEND_EQUATION_EXT set in
slink:VkPipelineDynamicStateCreateInfo::pname:pDynamicStates.
Otherwise, this state is specified by the
