...
pname:pNext chain, and a sparse buffer is being created, the alignment for
buffer device addresses can: be controlled.
For non-sparse buffers, alignment requirements are specified at device
memory allocation time in flink:vkAllocateMemory instead.
...
ifdef::VK_VALVE_buffer_device_address_allocation_alignment[]
  ...
  * [[VUID-VkBufferCreateInfo-alignment-12516]]
    If
    slink:VkBufferDeviceAddressAlignmentAllocateInfoVALVE::pname:alignment
    is not 0, the <<resources-effective-buffer-usage, effective usage
    flags>> must: include ename:VK_BUFFER_USAGE_2_SHADER_DEVICE_ADDRESS_BIT
...
ename:VK_BUFFER_CREATE_SPARSE_BINDING_BIT, the returned address of the
buffer minus slink:VkBindBufferMemoryInfo::pname:memoryOffset or equivalent
flink:vkBindBufferMemory offset.
The required alignment is the maximum of the following values:

  * The value of slink:VkMemoryRequirements::pname:alignment queried via
    flink:vkGetBufferMemoryRequirements for pname:pInfo->buffer
  * <<limits-minStorageBufferOffsetAlignment,pname:minStorageBufferOffsetAlignment>>
    if pname:pInfo->buffer was created with
    ename:VK_BUFFER_USAGE_2_STORAGE_BUFFER_BIT
  * <<limits-minUniformBufferOffsetAlignment,pname:minUniformBufferOffsetAlignment>>
    if pname:pInfo->buffer was created with
    ename:VK_BUFFER_USAGE_2_UNIFORM_BUFFER_BIT
  * <<limits-minTexelBufferOffsetAlignment,pname:minTexelBufferOffsetAlignment>>
    if pname:pInfo->buffer was created with
    ename:VK_BUFFER_USAGE_2_STORAGE_TEXEL_BUFFER_BIT or
    ename:VK_BUFFER_USAGE_2_UNIFORM_TEXEL_BUFFER_BIT
ifdef::VK_EXT_descriptor_heap[]
  * <<limits-resourceHeapAlignment,pname:resourceHeapAlignment>> if
    pname:pInfo->buffer was created with
    ename:VK_BUFFER_USAGE_2_DESCRIPTOR_HEAP_BIT_EXT
  * <<limits-samplerHeapAlignment,pname:samplerHeapAlignment>> if
    pname:pInfo->buffer was created with
    ename:VK_BUFFER_USAGE_2_DESCRIPTOR_HEAP_BIT_EXT
endif::VK_EXT_descriptor_heap[]
ifdef::VK_EXT_descriptor_buffer[]
  * <<limits-descriptorBufferOffsetAlignment,pname:descriptorBufferOffsetAlignment>>
    if pname:pInfo->buffer was created with
    ename:VK_BUFFER_USAGE_2_SAMPLER_DESCRIPTOR_BUFFER_BIT_EXT or
    ename:VK_BUFFER_USAGE_2_RESOURCE_DESCRIPTOR_BUFFER_BIT_EXT
endif::VK_EXT_descriptor_buffer[]
ifdef::VK_VALVE_buffer_device_address_allocation_alignment[]
  * slink:VkBufferDeviceAddressAlignmentAllocateInfoVALVE::pname:alignment
    if pname:pInfo->buffer was created without
    ename:VK_BUFFER_CREATE_SPARSE_BINDING_BIT and the bound
    slink:VkDeviceMemory was created with specified non-zero alignment of
    slink:VkBufferDeviceAddressAlignmentAllocateInfoVALVE::pname:alignment
  * slink:VkBufferDeviceAddressAlignmentAllocateInfoVALVE::pname:alignment
    if pname:pInfo->buffer was created with
    ename:VK_BUFFER_CREATE_SPARSE_BINDING_BIT, and the buffer was created
    with non-zero
    slink:VkBufferDeviceAddressAlignmentAllocateInfoVALVE::pname:alignment
endif::VK_VALVE_buffer_device_address_allocation_alignment[]
ifdef::VK_KHR_acceleration_structure+VK_KHR_device_address_commands[]
  * 256, if pname:pInfo->buffer was created with
    ename:VK_BUFFER_USAGE_2_ACCELERATION_STRUCTURE_STORAGE_BIT_KHR, and
    `apiext:VK_KHR_device_address_commands` is supported
endif::VK_KHR_acceleration_structure+VK_KHR_device_address_commands[]
...
ifdef::VK_ANDROID_external_memory_android_hardware_buffer[]
  ** If pname:tiling is ename:VK_IMAGE_TILING_OPTIMAL, and if the
     ...
     pname:imageCreateFormatFeatures is empty.
endif::VK_ANDROID_external_memory_android_hardware_buffer[]
ifdef::VK_QNX_external_memory_screen_buffer[]
  ** If pname:tiling is ename:VK_IMAGE_TILING_OPTIMAL, and if the
...
ifdef::VK_ARM_tensors[]
  * [[VUID-VkImageCreateInfo-tiling-09711]]
    ...
    pname:usage
ifdef::VK_ANDROID_external_memory_android_hardware_buffer[]
  * [[VUID-VkImageCreateInfo-pNext-12503]]
    If the pname:pNext chain includes a slink:VkExternalFormatANDROID
    structure with non-zero pname:externalFormat, pname:usage must: not
    include ename:VK_IMAGE_USAGE_TENSOR_ALIASING_BIT_ARM
endif::VK_ANDROID_external_memory_android_hardware_buffer[]
endif::VK_ARM_tensors[]
ifdef::VK_KHR_maintenance11[]
...
ifdef::VK_QCOM_image_processing[]
  * ename:VK_IMAGE_USAGE_SAMPLE_WEIGHT_BIT_QCOM specifies that the image
    can: be used as a weight image in <<textures-weightimage,weight image
    sampling operations>>.
...
ename:VK_IMAGE_CREATE_MUTABLE_FORMAT_BIT, pname:format must: equal the
pname:format used to create pname:image.

If pname:image was created with ename:VK_IMAGE_CREATE_MUTABLE_FORMAT_BIT,
pname:format can: be set to a different format as follows:

ifdef::VK_BASE_VERSION_1_1,VK_KHR_maintenance2[]
  * If pname:image was created with the
    ename:VK_IMAGE_CREATE_BLOCK_TEXEL_VIEW_COMPATIBLE_BIT flag and
    pname:format is an uncompressed format, it must: be
    <<formats-size-compatibility, _size-compatible_>> with the pname:format
    used to create pname:image.
endif::VK_BASE_VERSION_1_1,VK_KHR_maintenance2[]
ifdef::VK_BASE_VERSION_1_1,VK_KHR_sampler_ycbcr_conversion[]
  * If creating a <<image-views-plane-view,single-plane view>>, pname:format
    must: equal one of the <<image-views-plane-view, _plane view formats_>>.
  * If creating a <<image-views-multi-plane-view,multi-plane view>>,
    pname:format must: equal the pname:format used to create pname:image.
endif::VK_BASE_VERSION_1_1,VK_KHR_sampler_ycbcr_conversion[]
  * {empty}
ifdef::VK_BASE_VERSION_1_1,VK_KHR_maintenance2,VK_KHR_sampler_ycbcr_conversion[Otherwise,]
    pname:format must: be <<formats-compatibility-classes, compatible>> with
    the pname:format of pname:image.
...
ifdef::VK_BASE_VERSION_1_1,VK_KHR_maintenance2,VK_KHR_sampler_ycbcr_conversion[]
The texel dimensions of the view are derived as follows:

ifdef::VK_BASE_VERSION_1_1,VK_KHR_maintenance2[]
  * If pname:image was created with the
    ename:VK_IMAGE_CREATE_BLOCK_TEXEL_VIEW_COMPATIBLE_BIT flag and
    pname:format is an uncompressed format, the dimensions equal the
    dimensions of the selected mip level divided by the compressed texel
    block size and rounded up.
endif::VK_BASE_VERSION_1_1,VK_KHR_maintenance2[]
ifdef::VK_BASE_VERSION_1_1,VK_KHR_sampler_ycbcr_conversion[]
  * If creating a <<image-views-plane-view,single-plane view>>, the
    dimensions equal the <<image-views-plane-view, _plane view
    dimensions_>>.
endif::VK_BASE_VERSION_1_1,VK_KHR_sampler_ycbcr_conversion[]
  * Otherwise, the dimensions are equal to the dimensions of the selected
    mip level as defined in <<resources-image-mip-level-sizing,Image Mip
    Level Sizing>>
endif::VK_BASE_VERSION_1_1,VK_KHR_maintenance2,VK_KHR_sampler_ycbcr_conversion[]

ifdef::VK_BASE_VERSION_1_1,VK_KHR_sampler_ycbcr_conversion[]
[[image-views-plane-view]]
If pname:subresourceRange.aspectMask is one of the
<<formats-multiplanar-image-aspect,multi-planar aspect mask>> bits, a
single-plane image view will be created.
The pname:image of a single-plane view must: have been created with a
<<formats-multiplanar, multi-planar format>> and the
ename:VK_IMAGE_CREATE_MUTABLE_FORMAT_BIT create flag.

The set of _plane view formats_ for the plane is the
<<formats-compatible-planes, compatible plane format>> and the formats
<<formats-compatibility-classes, compatible>> with it.

The _plane view dimensions_ are derived from the dimensions of the
pname:image in the manner listed for <<formats-compatible-planes,plane
compatibility>> for the plane.
...
[[image-views-multi-plane-view]]
If the pname:format of the pname:image is a
<<formats-multiplanar,multi-planar format>> and
pname:subresourceRange.aspectMask is equal to
ename:VK_IMAGE_ASPECT_COLOR_BIT, a multi-plane image view will be created.
endif::VK_BASE_VERSION_1_1,VK_KHR_sampler_ycbcr_conversion[]
...
require YC~B~C~R~ conversion>> and pname:usage containing
ename:VK_IMAGE_USAGE_SAMPLED_BIT
ifdef::VK_ANDROID_external_memory_android_hardware_buffer[]
...
ifdef::VK_EXT_fragment_density_map[]
  * [[VUID-VkImageViewCreateInfo-usage-12488]]
    If pname:usage contains
    ename:VK_IMAGE_USAGE_FRAGMENT_DENSITY_MAP_BIT_EXT, then the image view's
    <<resources-image-view-format-features,format features>> must: contain
    ename:VK_FORMAT_FEATURE_2_FRAGMENT_DENSITY_MAP_BIT_EXT
...
ifdef::VK_BASE_VERSION_1_1,VK_KHR_sampler_ycbcr_conversion[]
    and a <<image-views-plane-view,single-plane view>> and a
    <<image-views-multi-plane-view,multi-plane view>> is not being created,
endif::VK_BASE_VERSION_1_1,VK_KHR_sampler_ycbcr_conversion[]
...
ifdef::VK_BASE_VERSION_1_1,VK_KHR_sampler_ycbcr_conversion[]
  * [[VUID-VkImageViewCreateInfo-image-01586]]
    If creating a <<image-views-plane-view,single-plane view>>, pname:format
    must: be compatible with the elink:VkFormat for the plane of the
    pname:image pname:format indicated by pname:subresourceRange.aspectMask,
...
ifdef::VK_QCOM_image_processing[]
  * [[VUID-VkImageViewCreateInfo-usage-12506]]
    If pname:usage contains ename:VK_IMAGE_USAGE_SAMPLE_WEIGHT_BIT_QCOM,
    then the image view's <<resources-image-view-format-features,format
    features>> must: contain ename:VK_FORMAT_FEATURE_2_WEIGHT_IMAGE_BIT_QCOM
  * [[VUID-VkImageViewCreateInfo-usage-12507]]
    If pname:usage contains
    ename:VK_IMAGE_USAGE_SAMPLE_BLOCK_MATCH_BIT_QCOM, then the image view's
    <<resources-image-view-format-features,format features>> must: contain
  ...
  * [[VUID-VkImageViewCreateInfo-subresourceRange-09594]]
    pname:subresourceRange.aspectMask must: <<resources-valid-aspect-mask,be
    valid>> for the pname:format the pname:image was created with
ifdef::VK_KHR_maintenance11[]
...
ifdef::VK_KHR_extended_flags[]
  * [[VUID-VkImageViewCreateInfo-pNext-12508]]
    The pname:pNext chain must: contain at most one of the
    slink:VkImageViewUsageCreateInfo and
    slink:VkImageViewUsage2CreateInfoKHR structures
endif::VK_KHR_extended_flags[]
****
...
ifdef::VK_BASE_VERSION_1_1,VK_KHR_sampler_ycbcr_conversion[]
When creating a slink:VkImageView, if pname:image was created with
ename:VK_IMAGE_CREATE_MUTABLE_FORMAT_BIT, and the image pname:format is a
<<formats-multiplanar, multi-planar format>>, then pname:aspectMask can: be
set to one of the <<formats-multiplanar-image-aspect,multi-planar aspect>>
bits.
endif::VK_BASE_VERSION_1_1,VK_KHR_sampler_ycbcr_conversion[]

ifdef::VK_ANDROID_external_memory_android_hardware_buffer[]
When creating a slink:VkImageView, if pname:image was created with an
<<memory-external-android-hardware-buffer-external-formats,Android hardware
buffer external format>>, then pname:aspectMask must: equal
ename:VK_IMAGE_ASPECT_COLOR_BIT.
endif::VK_ANDROID_external_memory_android_hardware_buffer[]
...
<<descriptors,descriptor>> (e.g. for sampling in the shader, or for use as
an input attachment), the pname:aspectMask must: only include one bit, which
...
ifdef::VK_BASE_VERSION_1_1,VK_KHR_sampler_ycbcr_conversion[]
  * [[VUID-VkImageSubresourceRange-aspectMask-01670]]
    ...
    <<formats-multiplanar-image-aspect,multi-planar aspect mask>> bits
endif::VK_BASE_VERSION_1_1,VK_KHR_sampler_ycbcr_conversion[]
ifdef::VK_EXT_image_drm_format_modifier[]
...
ifdef::VK_EXT_image_drm_format_modifier[]
  * [[VUID-VkDeviceImageMemoryRequirements-pCreateInfo-12489]]
    pname:pCreateInfo->tiling must: not be
    ename:VK_IMAGE_TILING_DRM_FORMAT_MODIFIER_EXT
endif::VK_EXT_image_drm_format_modifier[]
ifdef::VK_ANDROID_external_memory_android_hardware_buffer[]
ifdef::VK_BASE_VERSION_1_3,VK_KHR_maintenance4[]
...
ifdef::VK_BASE_VERSION_1_3,VK_KHR_maintenance4[]
  ...
  * [[VUID-VkDeviceImageMemoryRequirements-planeAspect-12399]]
    If pname:planeAspect is not 0 and pname:pCreateInfo->flags does not have
    ename:VK_IMAGE_CREATE_DISJOINT_BIT set, then pname:planeAspect must be a
...
ifdef::VK_EXT_descriptor_heap[]
  ...
  * [[VUID-VkTensorCreateInfoARM-flags-11396]]
    ...
    ename:VK_TENSOR_CREATE_DESCRIPTOR_HEAP_CAPTURE_REPLAY_BIT_ARM,
    slink:VkOpaqueCaptureDataCreateInfoEXT::pname:pData->size must: be equal
    to <<limits-tensorCaptureReplayOpaqueDataSize,
    pname:tensorCaptureReplayOpaqueDataSize>>
endif::VK_EXT_descriptor_heap[]
ifdef::VK_ARM_tensor_controls[]
