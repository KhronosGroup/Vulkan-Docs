...
[open,refpage='vkGetBufferDeviceAddress',desc='Query an address of a buffer',type='protos',alias='vkGetBufferDeviceAddressKHR vkGetBufferDeviceAddressEXT']
--
...
include::{chapters}/VK_OHOS_external_memory.adoc[]
...
include::{chapters}/VK_OHOS_native_buffer.adoc[]
...
met:
  * The buffer referred to by the pname:buffer member of the
     ...
     slink:VkAccelerationStructureCreateInfoKHR that `AS_1` was created
     with.
  * The start of the memory range occupied by `AS_2`, defined by the sum of
     ...
     slink:VkDeviceMemory, must: match the start of the memory range
     occupied by `AS_1`.
  ...
  * The pname:size member of the slink:VkAccelerationStructureCreateInfoKHR
     that `AS_2` was created with must: be greater or equal to the
     pname:size returned by flink:vkGetAccelerationStructureBuildSizesKHR
     for the build parameters `AS_1` was built with.
