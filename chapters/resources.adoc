...
ifdef::VK_VALVE_buffer_device_address_allocation_alignment[]
If the buffer was created without ename:VK_BUFFER_CREATE_SPARSE_BINDING_BIT,
and the bound slink:VkDeviceMemory was created with specified non-zero
alignment of
slink:VkBufferDeviceAddressAlignmentAllocateInfoVALVE::pname:alignment,
either the returned address minus
slink:VkBindBufferMemoryInfo::pname:memoryOffset or equivalent
flink:vkBindBufferMemory offset must: satisfy that alignment requirement, or
the returned address must: satisfy that alignment requirement.

If the buffer was created with ename:VK_BUFFER_CREATE_SPARSE_BINDING_BIT,
and the buffer was created with non-zero
slink:VkBufferDeviceAddressAlignmentAllocateInfoVALVE::pname:alignment, the
returned address must: satisfy that alignment requirement.
endif::VK_VALVE_buffer_device_address_allocation_alignment[]
