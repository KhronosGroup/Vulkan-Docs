...
is ename:VK_TRUE, the ename:VK_IMAGE_LAYOUT_GENERAL image layout may: be
...
pname:pMemoryRequirements parameter of flink:vkGetBufferMemoryRequirements2
or flink:vkGetImageMemoryRequirements2,
ifdef::VK_ARM_tensors[]
or flink:vkGetTensorMemoryRequirementsARM,
endif::VK_ARM_tensors[]
respectively.
...
sname:VkTensorCreateInfoARM.
  ...
  * Let the [eq]#uint64_t tensorElements# define the number of data elements
    ...
    sname:VkTensorCreateInfoARM::pname:pDescription::pname:pDimensions[i]
    for i between 0 and
    sname:VkTensorCreateInfoARM::pname:pDescription::pname:dimensionCount - 1.
****

.Valid Usage
****
  * [[VUID-VkTensorCreateInfoARM-pDescription-09720]]
    If pname:pDescription::pname:tiling is
    ename:VK_TENSOR_TILING_OPTIMAL_ARM, pname:pDescription::pname:pStrides
    must: be `NULL`
...
ifdef::VK_EXT_descriptor_buffer[]
  ...
  * [[VUID-VkTensorCreateInfoARM-pDescription-09728]]
    If pname:pDescription::pname:usage does not have any of the following
    bits set (i.e. if it is not possible to create a tensor view for this
    tensor), then the <<resources-tensor-view-format-features,format
    features>> must: contain the format feature flags required by the
    pname:usage flags pname:pDescription::pname:format as indicated in the
