...
pname:usage defines the effective usage flags for the buffer.
...
structure is used as the effective usage instead of pname:usage from this
structure.
...
ifdef::VK_BASE_VERSION_1_2[** ename:VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT]
  ...
  * [[VUID-VkBufferCreateInfo-flags-09641]]
    ...
    <<resources-effective-buffer-usage, effective usage>> must: not contain
    bits other than
...
ifdef::VK_COMPUTE_VERSION_1_3,VK_EXT_texel_buffer_alignment[]
  * [[VUID-VkBufferViewCreateInfo-buffer-02750]]
    If the <<features-texelBufferAlignment, pname:texelBufferAlignment>>
    feature is enabled and if pname:buffer was created with the
    ename:VK_BUFFER_USAGE_STORAGE_TEXEL_BUFFER_BIT usage flag set,
    pname:offset must: be a multiple of the lesser of
