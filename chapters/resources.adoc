...
ifdef::VK_BASE_VERSION_1_4,VK_EXT_host_image_copy[]
  * [[VUID-VkImageCreateInfo-imageCreateFormatFeatures-09048]]
    ...
    <<resources-image-creation-limits,Image Creation Limits>>) does not
    contain ename:VK_FORMAT_FEATURE_2_HOST_IMAGE_TRANSFER_BIT
ifdef::VK_ANDROID_external_memory_android_hardware_buffer[]
    and the pname:pNext chain does not include a
    slink:VkExternalFormatANDROID structure with non-zero
    pname:externalFormat
endif::VK_ANDROID_external_memory_android_hardware_buffer[]
    , then pname:usage must: not contain
...
pname:format can: be set to a <<formats-compatibility-classes, format that
is compatible>> with the pname:format used to create pname:image
ifndef::VK_BASE_VERSION_1_1,VK_KHR_maintenance2,VK_KHR_sampler_ycbcr_conversion[.]
...
ifdef::VK_BASE_VERSION_1_1,VK_KHR_sampler_ycbcr_conversion[]
  * [[VUID-VkImageViewCreateInfo-image-12504]]
    If creating a <<image-views-plane-view,single-plane view>>, pname:image
    must: have been created with the
    ename:VK_IMAGE_CREATE_MUTABLE_FORMAT_BIT create flag
  * [[VUID-VkImageViewCreateInfo-format-12505]]
    If creating a <<image-views-plane-view,single-plane view>>, the
    pname:format used to create pname:image must: have a
