...
ifndef::VK_BASE_VERSION_1_2,VK_KHR_buffer_device_address[]
To
endif::VK_BASE_VERSION_1_2,VK_KHR_buffer_device_address[]
...
via <<descriptorsets, resource descriptors>>.
...
ename:VK_IMAGE_USAGE_TILE_MEMORY_BIT_QCOM have further restrictions on their
...
// Jon: logic needs to incorporate VK_BASE_VERSION_1_1
...
ifdef::VK_BASE_VERSION_1_4,VK_EXT_host_image_copy[]
[open,refpage='VkSubresourceHostMemcpySize',desc='Memory size needed to copy to or from an image on the host with VK_HOST_IMAGE_COPY_MEMCPY_BIT',type='structs',alias='VkSubresourceHostMemcpySizeEXT']
--
...
ifdef::VK_ARM_tensors[]
  * ename:VK_IMAGE_LAYOUT_TENSOR_ALIASING_ARM specifies the layout that an
    image created with ename:VK_IMAGE_TILING_OPTIMAL must: be in for it and
    a tensor bound to the same aliased range of memory to consistently
    interpret the data in memory.
    See <<resources-memory-aliasing>> for a complete set of rules for
    tensor/image aliasing.
    This layout is valid only for image subresources of images created with
    ename:VK_IMAGE_USAGE_TENSOR_ALIASING_BIT_ARM.
endif::VK_ARM_tensors[]
...
Constraints on the values returned for tensor resources are:
  ...
  * pname:requiresDedicatedAllocation will otherwise be ename:VK_FALSE.
  * When the implementation sets pname:requiresDedicatedAllocation to
    ename:VK_TRUE, it must: also set pname:prefersDedicatedAllocation to
    ename:VK_TRUE.
endif::VK_ARM_tensors[]
...
ifdef::VK_KHR_acceleration_structure[]
For an acceleration structure `AS_2` that is an alias of another acceleration
structure `AS_1`, `AS_2` can: be used in place of `AS_1` for operations acting
on acceleration structures if the following conditions are met:
  *  The buffer referred to by the pname:buffer member of the
     slink:VkAccelerationStructureCreateInfoKHR that
     `AS_2` was created with must: be bound to the same slink:VkDeviceMemory as
     the buffer referred to by the pname:buffer member of the
     slink:VkAccelerationStructureCreateInfoKHR that `AS_1` was created with.
  *  The start of the memory range occupied by `AS_2`, defined by the sum of the
     pname:offset member of the slink:VkAccelerationStructureCreateInfoKHR that
     `AS_2` was created with and the offset at which the buffer associated with `AS_2`
     is bound to a slink:VkDeviceMemory, must: match the start of the memory range
     occupied by `AS_1`.
  *  If the pname:deviceAddress member of the
     slink:VkAccelerationStructureCreateInfoKHR that `AS_2` was created with is
     non-zero, it must: match the pname:deviceAddress member of the
     slink:VkAccelerationStructureCreateInfoKHR that `AS_1` was created with.
  *  The pname:createFlags member of the
     ...
     slink:VkAccelerationStructureCreateInfoKHR that `AS_1` was created with.
  *  The pname:type member of the slink:VkAccelerationStructureCreateInfoKHR that `AS_2`
     was created with must: match the pname:type member of the
     slink:VkAccelerationStructureCreateInfoKHR that `AS_1` was created with.
  *  The pname:size member of the slink:VkAccelerationStructureCreateInfoKHR that `AS_2`
     was created with must: be greater or equal to the pname:size returned by
     flink:vkGetAccelerationStructureBuildSizesKHR for the build parameters `AS_1`
     was built with.

After an acceleration structure object is destroyed, aliased acceleration structures may:
continue being used to refer to that acceleration structure for operations acting on
acceleration structures.
endif::VK_KHR_acceleration_structure[]
...
only if:
  ...
  * The image was created with ename:VK_IMAGE_TILING_OPTIMAL and the tensor
    ...
    The image was created with ename:VK_IMAGE_USAGE_TENSOR_ALIASING_BIT_ARM
    and the tensor was created with
    ename:VK_TENSOR_USAGE_IMAGE_ALIASING_BIT_ARM.
