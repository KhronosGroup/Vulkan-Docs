...
ifdef::VK_VERSION_1_3,VK_KHR_synchronization2[]
  ...
  ** Queue family ownership transfers are no longer required when
     transitioning resources created with ename:VK_SHARING_MODE_EXCLUSIVE
     between queue families that support ename:VK_QUEUE_GRAPHICS_BIT and
     those that support only ename:VK_QUEUE_TRANSFER_BIT.
...
ifdef::VK_VERSION_1_4,VK_KHR_shader_expect_assume[]
...
--
endif::VK_VERSION_1_4,VK_KHR_shader_expect_assume[]
