...
ifdef::VK_EXT_mesh_shader+VK_KHR_fragment_shading_rate[]
  * `apiext:VK_KHR_fragment_shading_rate` if
    <<features-primitiveFragmentShadingRateMeshShader,
...
ifdef::VK_VERSION_1_4[]
  * If Vulkan 1.4 is supported, at least one queue family is reported by
    ...
    reported that support the ename:VK_QUEUE_TRANSFER_BIT flag,
    <<features-hostImageCopy,pname:hostImageCopy>> must: be supported.
endif::VK_VERSION_1_4[]
