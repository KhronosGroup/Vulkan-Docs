...
Each element of the array will consume all four code:Component slots in each
code:Location slot.

Matrices of size [eq]#n {times} m# are assigned locations identically to
arrays of size [eq]#n# of vectors of length 4 (consuming all code:Component
slots) with an identical element type.

When an code:OpVariable with a structure type is decorated with a
code:Location, the members in the structure type must: not be decorated with
a code:Location.
...
An code:OpVariable with a structure type that is not decorated with
code:Block must: be decorated with a code:Location.

When an code:OpVariable with a structure type decorated with code:Block is
declared without a code:Location decoration, each member in the structure
must: be decorated with a code:Location.
...
If there is no color attachment indicated by code:Location, the values that
...
ifdef::VK_QCOM_image_processing[]
    ...
    code:OpImage*Gather, or code:OpImageSparse*Gather.
  ...
  * _Storage tile attachment_ variables must: be declared with a
    ...
    code:OpImageTexelPointer for compatibility with atomic operations.
    _Sampled tile attachment_ variables are managed by the
...
or image resources, as described in the <<descriptorsets,Resource
Descriptors>> section, the shader resource variables must: be matched with
the <<descriptorsets-pipelinelayout,pipeline layout>> that is provided at
...
ifdef::VK_VERSION_1_2[]
  ...
  * [[VUID-{refpage}-Layer-10593]]
    If code:Layer is declared as an array of boolean values, the size of the
...
****
  ...
  * [[VUID-{refpage}-PrimitiveId-10595]]
    ...
    or decorate a variable of type code:OpTypeArray of 32-bit integer
    values.
...
****
  ...
  * [[VUID-{refpage}-PrimitiveShadingRateKHR-10599]]
    If code:PrimitiveShadingRateKHR is declared as an array of boolean
...
ifdef::VK_VERSION_1_2[]
  ...
  * [[VUID-{refpage}-ViewportIndex-10601]]
    ...
    decorate a scalar boolean member of a structure decorated as code:Block,
    or decorate a variable of type code:OpTypeArray of boolean values.
  * [[VUID-{refpage}-ViewportIndex-10602]]
    If code:ViewportIndex is declared as an array of boolean values, the
    size of the array must: match the value specified by
    code:OutputPrimitivesEXT
...
ifdef::VK_QCOM_tile_shading[]
  * [[VUID-{refpage}-TileShadingRateQCOM-10635]]
    ...
    variables decorated withcode:WorkgroupSize must: be declared using the
    code:Input {StorageClass}
endif::VK_QCOM_tile_shading[]
****
--
