...
[[interfaces-builtin-variables-fragmentcoveragemaskmesa]]
[open,refpage='FragmentCoverageMaskMESA',desc='Full coverage mask for a fragment',type='builtins']
--
:refpage: FragmentCoverageMaskMESA

code:FragmentCoverageMaskMESA::

Decorating a variable with the code:FragmentCoverageMaskMESA built-in
...
mode.
+
Unlike code:SampleMask, the value of code:FragmentCoverageMaskMESA is not
...
+
A variable decorated with code:FragmentCoverageMaskMESA is an array of
integers.
Bits are mapped to samples in a manner where bit B of mask M
(`FragmentCoverageMaskMESA[M]`) corresponds to sample
[eq]#32 {times} M {plus} B#.

.Valid Usage
****
  * The code:FragmentCoverageMaskMESA decoration must: be used only within
    the code:Fragment {ExecutionModel}
  * The variable decorated with code:FragmentCoverageMaskMESA must: be
    declared using the code:Input {StorageClass}
  * The variable decorated with code:FragmentCoverageMaskMESA must: be
...
ifdef::VK_EXT_mesh_shader[]
In mesh and task shaders, code:SubgroupSize must: be uniform with
<<shaders-scope-command, command scope>>.
In a single <<shaders-scope-command, command scope>>, the mesh and task
shader may: have different code:SubgroupSize.
endif::VK_EXT_mesh_shader[]
+
...
[NOTE]
====
The old behavior for code:SubgroupSize is considered legacy as certain
