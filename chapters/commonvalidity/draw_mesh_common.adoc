...
ifdef::VK_EXT_shader_object[]
  * [[VUID-{refpage}-None-08694]]
    ...
    <<features-meshShader,pname:meshShader>> features is enabled, and a
    valid sname:VkShaderEXT is bound the to the
    ename:VK_SHADER_STAGE_MESH_BIT_EXT stage, and that sname:VkShaderEXT was
    created without the ename:VK_SHADER_CREATE_NO_TASK_SHADER_BIT_EXT flag,
    a valid sname:VkShaderEXT must: be bound to the
  ...
  * [[VUID-{refpage}-None-08695]]
    ...
    pname:meshShader>> features is enabled, and a valid sname:VkShaderEXT is
    bound the to the ename:VK_SHADER_STAGE_MESH_BIT_EXT stage, and that
