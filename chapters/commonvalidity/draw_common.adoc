...
ifdef::VK_EXT_attachment_feedback_loop_layout[]
    and <<renderpass-feedbackloop,feedback loop>> is not enabled for
    ename:VK_IMAGE_ASPECT_COLOR_BIT on that attachment,
...
ifdef::VK_EXT_attachment_feedback_loop_layout[]
    and <<renderpass-feedbackloop,feedback loop>> is not enabled for
    ename:VK_IMAGE_ASPECT_DEPTH_BIT on that attachment,
endif::VK_EXT_attachment_feedback_loop_layout[]
    it must: not be accessed in any way other than as an attachment by this
    command
...
ifdef::VK_EXT_attachment_feedback_loop_layout[]
    and <<renderpass-feedbackloop,feedback loop>> is not enabled for
    ename:VK_IMAGE_ASPECT_STENCIL_BIT on that attachment,
endif::VK_EXT_attachment_feedback_loop_layout[]
    it must: not be accessed in any way other than as an attachment by this
    command
  * [[VUID-{refpage}-None-12338]]
...
ifdef::VK_EXT_shader_object[]
    ...
    topology>> is in the line topology class, then flink:vkCmdSetLineWidth
...
ifdef::VK_NV_viewport_swizzle[]
  * [[VUID-{refpage}-VkPipelineViewportCreateInfo-04141]]
    If the bound graphics pipeline state was created with the
    ename:VK_DYNAMIC_STATE_VIEWPORT_WITH_COUNT dynamic state enabled,
...
ifdef::VK_NV_scissor_exclusive[]
  * [[VUID-{refpage}-VkPipelineViewportCreateInfo-04142]]
    If the bound graphics pipeline state was created with the
...
ifdef::VK_EXT_shader_object[]
    ...
    mode, then the last call to flink:vkCmdSetColorBlendEnableEXT must: have
    set the blend enable to ename:VK_FALSE prior to this drawing command
...
ifdef::VK_EXT_shader_object[]
    ...
    the current render pass instance was begun with
    flink:vkCmdBeginRendering, and it includes a color attachment that uses
    the ename:VK_RESOLVE_MODE_EXTERNAL_FORMAT_DOWNSAMPLE_BIT_ANDROID resolve
    mode, then the last call to flink:vkCmdSetRasterizationSamplesEXT must:
    have set pname:rasterizationSamples to ename:VK_SAMPLE_COUNT_1_BIT prior
...
ifdef::VK_EXT_shader_object[]
    ...
    flink:vkCmdBeginRendering, and it includes a color attachment that uses
    the ename:VK_RESOLVE_MODE_EXTERNAL_FORMAT_DOWNSAMPLE_BIT_ANDROID resolve
    mode, then the last call to flink:vkCmdSetFragmentShadingRateKHR must:
    have set pname:pFragmentSize->width to `1` prior to this drawing command
...
ifdef::VK_EXT_shader_object[]
    ...
    the ename:VK_RESOLVE_MODE_EXTERNAL_FORMAT_DOWNSAMPLE_BIT_ANDROID resolve
    mode, then the last call to flink:vkCmdSetFragmentShadingRateKHR must:
    have set pname:pFragmentSize->height to `1` prior to this drawing
    command
endif::VK_KHR_fragment_shading_rate[]
...
ifdef::VK_EXT_shader_object[]
    ...
    <<drawing-rasterization-input-topology, effective rasterization input
    topology>> is in the line or point topology class, then the
    <<dynamic-state-current-value, current value>> of
...
ifdef::VK_EXT_extended_dynamic_state3[or]
endif::VK_EXT_shader_object[]
ifdef::VK_EXT_extended_dynamic_state3[]
    ...
    pname:rasterizerDiscardEnable is ename:VK_FALSE, and the
    <<drawing-rasterization-input-topology, effective rasterization input
    topology>> is in the line topology class, then
...
ifdef::VK_EXT_shader_object[]
    ...
    rasterization input topology>> is in the line topology class, and the
...
ifdef::VK_EXT_descriptor_heap[]
    the ename:VK_SHADER_CREATE_DESCRIPTOR_HEAP_BIT_EXT flag set, or with
endif::VK_EXT_descriptor_heap[]
    identical or <<glossary-identically-defined,identically defined>> arrays
...
ifdef::VK_EXT_shader_object[]
    ...
    a graphics pipeline is bound which was created with the
    ename:VK_DYNAMIC_STATE_STENCIL_WRITE_MASK dynamic state enabled, and the
    fragment shader declares the code:EarlyFragmentTests execution mode and
...
ifdef::VK_EXT_extended_dynamic_state3[]
    ...
    ename:VK_COLOR_COMPONENT_G_BIT, ename:VK_COLOR_COMPONENT_B_BIT, and
    ename:VK_COLOR_COMPONENT_A_BIT, or none of them
endif::VK_EXT_shader_object,VK_EXT_extended_dynamic_state3[]
...
ifdef::VK_EXT_shader_object[]
    ...
    in the bound pipeline, if either pname:pColorAttachmentInputIndices[i]
    or the value for the corresponding index set currently in the current
    render pass instance is <<shaders-staticuse,statically used>> in the
    bound pipeline in an code:InputAttachmentIndex decoration, the two
    values must: much
...
ifdef::VK_EXT_shader_object[]
    ...
    current render pass instance is <<shaders-staticuse,statically used>> in
    the bound pipeline in an code:InputAttachmentIndex decoration, the two
    values must: match
...
ifdef::VK_EXT_shader_object[]
  * [[VUID-{refpage}-flags-13361]]
    If there are any shader objects bound that were created with pname:flags
    that includes the ename:VK_SHADER_CREATE_INDEPENDENT_SETS_BIT_KHR bit,
    then all bound shader objects must: have been created with the
    ename:VK_SHADER_CREATE_INDEPENDENT_SETS_BIT_KHR bit
  * [[VUID-{refpage}-flags-13362]]
    If there are any shader objects bound that were created with pname:flags
    that includes the ename:VK_SHADER_CREATE_INDEPENDENT_SETS_BIT_KHR bit,
    with a pname:setLayoutCount not equal to 0, and with pname:pSetLayouts
    not equal to `NULL`, then the pipeline layout of the bound descriptor
    sets must: have been created with
    ename:VK_PIPELINE_LAYOUT_CREATE_INDEPENDENT_SETS_BIT_EXT set
  * [[VUID-{refpage}-flags-12491]]
    If there are any shader objects bound that were created with pname:flags
    that includes the ename:VK_SHADER_CREATE_INDEPENDENT_SETS_BIT_KHR bit,
    with a pname:pushConstantRangeCount not equal to 0, and with
    pname:pPushConstantRanges not equal to `NULL`, then the pipeline layout
    of the bound push constants must: have been created with
    ename:VK_PIPELINE_LAYOUT_CREATE_INDEPENDENT_SETS_BIT_EXT set
endif::VK_EXT_shader_object[]
ifdef::VK_EXT_shader_object+VK_EXT_mesh_shader[]
  * [[VUID-{refpage}-flags-13363]]
    If there are any shader objects bound that were created with pname:flags
    that includes the ename:VK_SHADER_CREATE_INDEPENDENT_SETS_BIT_KHR bit,
    ...
    slink:VkDescriptorSetLayoutBinding::pname:stageFlags set with
    ename:VK_SHADER_STAGE_VERTEX_BIT,
    ename:VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT,
    ename:VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT or
    ename:VK_SHADER_STAGE_GEOMETRY_BIT, then there must: be no binding with
    slink:VkDescriptorSetLayoutBinding::pname:stageFlags set with
    ename:VK_SHADER_STAGE_TASK_BIT_EXT or ename:VK_SHADER_STAGE_MESH_BIT_EXT
endif::VK_EXT_shader_object+VK_EXT_mesh_shader[]
