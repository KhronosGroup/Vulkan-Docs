...
ifdef::VK_EXT_shader_object+VK_EXT_mesh_shader[]
  ...
  * [[VUID-{refpage}-flags-13364]]
    If there are any shader objects bound that were created with pname:flags
    that includes both the ename:VK_SHADER_CREATE_INDEPENDENT_SETS_BIT_KHR
    and ename:VK_SHADER_CREATE_NO_TASK_SHADER_BIT_EXT bits, the pipeline
    layout of the bound descriptor sets must: have been created with
    ename:VK_PIPELINE_LAYOUT_CREATE_NO_TASK_SHADER_BIT_KHR set
  * [[VUID-{refpage}-flags-13365]]
    If there are any shader objects bound that were created with pname:flags
    that includes both the ename:VK_SHADER_CREATE_INDEPENDENT_SETS_BIT_KHR
    and ename:VK_SHADER_CREATE_NO_TASK_SHADER_BIT_EXT bits, there must: be
    no shader object bound to the ename:VK_SHADER_STAGE_TASK_BIT_EXT stage
