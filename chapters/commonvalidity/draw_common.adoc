...
ifdef::VK_VERSION_1_4,VK_KHR_line_rasterization[ename:VK_DYNAMIC_STATE_LINE_STIPPLE]
ifndef::VK_VERSION_1_4,VK_KHR_line_rasterization[ename:VK_DYNAMIC_STATE_LINE_STIPPLE_EXT]
    dynamic state enabled, the <<dynamic-state-current-value, current
    value>> of pname:rasterizerDiscardEnable is ename:VK_FALSE, and the
    <<dynamic-state-current-value, current value>> of
    pname:stippledLineEnable is ename:VK_TRUE, then
ifdef::VK_VERSION_1_4,VK_KHR_line_rasterization[flink:vkCmdSetLineStipple]
ifndef::VK_VERSION_1_4,VK_KHR_line_rasterization[flink:vkCmdSetLineStippleEXT]
    must: have been called and not subsequently <<dynamic-state-lifetime,
    invalidated>> in the current command buffer prior to this drawing
    command
