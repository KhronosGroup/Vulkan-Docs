...
ifdef::VK_BASE_VERSION_1_1,VK_KHR_sampler_ycbcr_conversion[]
  * [[VUID-{refpage}-None-01549]]
    In a copy to or from a plane of a <<formats-multiplanar, multi-planar
    image>>, the elink:VkFormat of the image and plane must: be compatible
    according to <<formats-compatible-planes,the description of compatible
    planes>> for the plane being copied
endif::VK_BASE_VERSION_1_1,VK_KHR_sampler_ycbcr_conversion[]
...
ifdef::VK_KHR_maintenance8[]
  * [[VUID-{refpage}-pRegions-12201]]
    For each element of pname:pRegions where pname:srcSubresource.aspectMask
    and pname:dstSubresource.aspectMask each contain at least one of
...
ifdef::VK_GRAPHICS_VERSION_1_2,VK_EXT_separate_stencil_usage[]
  * [[VUID-{refpage}-aspect-06664]]
    ...
    <<VkImageStencilUsageCreateInfo,separate stencil usage>>, pname:srcImage
