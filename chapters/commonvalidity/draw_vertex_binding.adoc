...
ifdef::VK_EXT_vertex_input_dynamic_state[]
  ...
  * [[VUID-{refpage}-Input-08734]]
    ...
    ename:VK_DYNAMIC_STATE_VERTEX_INPUT_EXT dynamic state enabled, then all
    variables with the code:Input storage class decorated with code:Location
    in the code:Vertex {ExecutionModel} code:OpEntryPoint must: be the same
    numeric type as the matching location in
    slink:VkVertexInputAttributeDescription2EXT
endif::VK_EXT_vertex_input_dynamic_state[]
ifdef::VK_EXT_shader_object[]
