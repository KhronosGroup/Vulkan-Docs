...
ifdef::VK_EXT_vertex_input_dynamic_state[]
    ...
    then flink:vkCmdBindVertexBuffers2 with a non-`NULL` pname:pStrides
    parameter
ifdef::VK_KHR_device_address_commands[]
    or flink:vkCmdBindVertexBuffers3KHR with pname:setStride set to
    ename:VK_TRUE
endif::VK_KHR_device_address_commands[]
    must: have been called and not subsequently <<dynamic-state-lifetime,
    invalidated>> in the current command buffer prior to this draw command
ifdef::VK_KHR_device_address_commands[]
  * [[VUID-{refpage}-None-13118]]
    If the bound graphics pipeline was created without the
    ename:VK_DYNAMIC_STATE_VERTEX_INPUT_BINDING_STRIDE dynamic state
    enabled,
ifdef::VK_EXT_vertex_input_dynamic_state[]
    without the ename:VK_DYNAMIC_STATE_VERTEX_INPUT_EXT dynamic state
    enabled,
endif::VK_EXT_vertex_input_dynamic_state[]
    and flink:vkCmdBindVertexBuffers3KHR was called and not subsequently
    <<dynamic-state-lifetime, invalidated>> in the current command buffer
    prior to this draw command, the value of pname:setStride in each of its
    pname:pBindInfos elements must have been ename:VK_FALSE
endif::VK_KHR_device_address_commands[]
...
ifdef::VK_EXT_shader_object[]
  ...
  * [[VUID-{refpage}-OpExecutionMode-12239]]
    If a shader is bound to both the
    ename:VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT and
    ename:VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT stages, and if both
    stages contain an code:OpExecutionMode instruction specifying the type
    of subdivision, they must: be the same
  * [[VUID-{refpage}-OpExecutionMode-12240]]
    If a shader is bound to both the
    ename:VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT and
    ename:VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT stages, and if both
    stages contain an code:OpExecutionMode instruction specifying the
    orientation of triangles, they must: be the same
  * [[VUID-{refpage}-OpExecutionMode-12241]]
    If a shader is bound to both the
    ename:VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT and
    ename:VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT stages, and if both
    stages contain an code:OpExecutionMode instruction specifying the
    segment spacing, they must: be the same
  * [[VUID-{refpage}-OpExecutionMode-12242]]
    If a shader is bound to both the
    ename:VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT and
    ename:VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT stages, and if both
    stages contain an code:OpExecutionMode instruction specifying the output
