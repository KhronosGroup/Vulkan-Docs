...
ifdef::VK_EXT_shader_object[]
 ...
 and
    ...
    <<descriptors-compatibility,compatible>> with the push constant range
...
ifdef::VK_QCOM_image_processing3[]
    or ename:VK_FORMAT_FEATURE_2_BLOCK_MATCHING_SXD_BIT_QCOM
endif::VK_QCOM_image_processing3[]
  * [[VUID-{refpage}-OpImageBlockMatchSADQCOM-12420]]
...
ifdef::VK_QCOM_image_processing3[]
    ...
    feature is not enabled or the _input_ sampler was not created with both
...
ifdef::VK_QCOM_image_processing3[]
    ...
    <<textures-integer-coordinate-validation,integer texel coordinate
    validation>>
  * [[VUID-{refpage}-OpImageBlockMatchWindow-12421]]
    If a code:OpImageBlockMatchWindow*QCOM or
    code:OpImageBlockMatchGather*QCOM instruction uses a slink:VkSampler as
...
ifdef::VK_QCOM_image_processing3[]
    ...
    the _input_ sampler must: not have been created with either
    pname:addressModeU or pname:addressModeV equal to
    ename:VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE
ifdef::VK_QCOM_image_processing3[]
  * [[VUID-{refpage}-addressModeU-12422]]
    If a code:OpImageBlockMatchWindow*QCOM or
    code:OpImageBlockMatchGather*QCOM instruction uses a slink:VkSampler as
    a result of this command, and the _input_ sampler was created with
    either pname:addressModeU or pname:addressModeV equal to
    ename:VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE, then the sampler must: have
    been created with both pname:addressModeU and pname:addressModeV equal
    to ename:VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE
endif::VK_QCOM_image_processing3[]
endif::VK_QCOM_image_processing2[]
...
ifdef::VK_KHR_ray_query[]
  * [[VUID-{refpage}-micromap-11636]]
    If a ename:VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_KHR descriptor is
    accessed as a result of this command through an
    code:OpRayQueryProceedKHR operation
ifdef::VK_EXT_opacity_micromap[]
    , the `apiext:VK_EXT_opacity_micromap` extension is not enabled,
endif::VK_EXT_opacity_micromap[]
    and the acceleration structure is built using geometry with a
    slink:VkAccelerationStructureTrianglesOpacityMicromapKHR structure
    included in its pname:pNext chain, the shader must: have specified a
    code:OpacityMicromapIdKHR execution mode with its specialization
    constant operand equal to ename:VK_TRUE
endif::VK_KHR_ray_query[]
  * [[VUID-{refpage}-micromap-11637]]
    If a ename:VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_KHR descriptor is
...
ifdef::VK_KHR_maintenance5,VK_BASE_VERSION_1_4[]
  * [[VUID-{refpage}-micromap-11638]]
    If a ename:VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_KHR descriptor is
    accessed as a result of this command and the acceleration structure is
    built using geometry with an pname:indexBuffer containing both
    elink:VkOpacityMicromapSpecialIndexKHR values and indices into an
    associated micromap array, any slink:VkPipeline bound to the pipeline
    bind point used by this command must: not have been created with
    pname:flags that includes
...
ifdef::VK_EXT_shader_object[]
  * [[VUID-{refpage}-micromap-11639]]
    If a ename:VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_KHR descriptor is
    accessed as a result of this command and the acceleration structure is
    built using geometry with an pname:indexBuffer containing both
    elink:VkOpacityMicromapSpecialIndexKHR values and indices into an
    associated micromap array, any slink:VkShaderEXT that accessed the
    descriptor must: not have been created with pname:flags that includes
    ename:VK_SHADER_CREATE_OPACITY_MICROMAP_DISALLOW_MIXED_SPECIAL_INDEX_BIT_EXT
endif::VK_EXT_shader_object[]
...
ifdef::VK_QCOM_tile_memory_heap[]
  * [[VUID-{refpage}-commandBuffer-10746]]
    ...
    <<memory-bind-tile-memory, bound tile memory object>> in
    pname:commandBuffer
...
ifdef::VK_ARM_tensor_controls[]
  * [[VUID-{refpage}-OpTensorReadARM-12462]]
    If a ename:VK_DESCRIPTOR_TYPE_TENSOR_ARM descriptor is accessed as a
    result of this command, using code:OpTensorReadARM or
    code:OpTensorWriteARM, and the underlying slink:VkTensorARM object was
    created with a ename:VK_TENSOR_TILING_BRICK_16_WIDE_ARM,
    ...
    elink:VkTensorTilingARM, and the object read or written is an array,
    then the innermost coordinate provided to code:OpTensorReadARM or
    code:OpTensorWriteARM via the code:Coordinates operand must: be a
    multiple of the largest power of 2 that is less than or equal to the
    number of components of the array
  * [[VUID-{refpage}-pNext-12463]]
    If a ename:VK_DESCRIPTOR_TYPE_TENSOR_ARM descriptor is accessed as a
    result of this command and the corresponding slink:VkTensorARM object
    was created with a slink:VkTensorRollingBackingCreateInfoARM structure
    in the pname:pNext chain of slink:VkTensorCreateInfoARM, then the number
    of elements accessed with one code:OpTensorReadARM or
    code:OpTensorWriteARM, must: be less than or equal to the number of
    elements available from the starting coordinate in the rolling backing
    along the innermost dimension, defined as
    [eq]#pname:wraps[pname:dimensionCount - 1] - InnermostCoordinate %
    pname:wraps[pname:dimensionCount - 1]# where InnermostCoordinate is the
    last component of the coordinates array provided via the
    code:Coordinates operand
...
ifdef::VK_EXT_robustness2[]
    ...
    and the <<features-robustBufferAccess, pname:robustBufferAccess>>
    feature is not enabled, that stage must: not access values outside of
    the range of the descriptor specified via slink:VkDeviceAddressRangeKHR
...
ifdef::VK_EXT_robustness2[]
    ...
    and the <<features-robustBufferAccess, pname:robustBufferAccess>>
    feature is not enabled, that stage must: not access values outside of
    the range of the descriptor specified by slink:VkDeviceAddressRangeKHR
    when the descriptor was written
ifdef::VK_EXT_shader_object[]
...
ifdef::VK_EXT_robustness2[]
    ...
    specified by slink:VkDeviceAddressRangeKHR when the descriptor was
    written
endif::VK_EXT_shader_object[]
...
ifdef::VK_KHR_ray_tracing_pipeline,VK_NV_ray_tracing[]
    ...
    ename:VK_DESCRIPTOR_MAPPING_SOURCE_RESOURCE_HEAP_DATA_EXT and a shader
    accesses a uniform buffer using that mapping, the address that the
