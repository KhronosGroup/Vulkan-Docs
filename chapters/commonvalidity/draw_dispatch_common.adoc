...
ifdef::VK_EXT_shader_object[]
    ...
    and it was created with a <<descriptorheaps-bindings,descriptor
    mapping>> using ename:VK_DESCRIPTOR_MAPPING_SOURCE_INDIRECT_ADDRESS_EXT,
    and a shader accesses a resource using that mapping, the indirect
    address used to load the resource address must: be aligned to 8 bytes
  * [[VUID-{refpage}-None-12451]]
    If a pipeline is bound to the pipeline bind point used by this command,
ifdef::VK_EXT_shader_object[]
    or shader is bound to a shader stage used by this command,
endif::VK_EXT_shader_object[]
    ...
    ename:VK_DESCRIPTOR_MAPPING_SOURCE_HEAP_WITH_INDIRECT_INDEX_ARRAY_EXT or
    ename:VK_DESCRIPTOR_MAPPING_SOURCE_HEAP_WITH_INDIRECT_INDEX_EXT, and a
    shader accesses a resource using that mapping, the indirect address used
    to load the indirect index must: be aligned to 4 bytes
