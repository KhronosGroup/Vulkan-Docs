...
// Common to acceleration structure build commands executed on the device
  ...
  * [[VUID-{refpage}-pInfos-03708]]
    ...
    pname:srcAccelerationStructure member must: be bound to device memory
  * [[VUID-{refpage}-pInfos-03709]]
    For each element of pname:pInfos, if an acceleration structure is
    referenced by the pname:geometry.instances.data member of any element of
    pname:pGeometries or pname:ppGeometries with a pname:geometryType of
    ename:VK_GEOMETRY_TYPE_INSTANCES_KHR, it must: be bound to device memory
