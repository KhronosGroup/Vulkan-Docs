...
within a _Video Coding Scope_, and submitting them to queues with video
decode or encode capabilities, respectively.
...
_Video Picture Resources_ represent multidimensional arrays of image data
which can: be used as the source or target of video coding operations.
They may: have associated metadata that can: include implementation-private
details required for the decode or encode operations and application managed
color-space related information.

In Vulkan, a video picture resource is represented by a slink:VkImage.
Individual slink:VkImageView objects created from such resources can: be
used as <<decoded-output-picture,Decode Output Pictures>>,
...
_Decoded Output Picture_, represented by a slink:VkImageView.
This may: be shared with the <<dpb,Decoded Picture Buffer>>.
It may: also be used as an input for video encode, graphics, or compute
operations
...
It may: also be a direct target of video decode, graphics, or compute
operations
ifdef::VK_KHR_surface[]
, or with <<wsi, Window System Integration>> APIs
endif::VK_KHR_surface[]
...
Such reconstructed pictures can: be used as <<reference-picture,Reference
...
implementation, and the application-specific use cases.
...
[[reference-picture]]
Previously reconstructed pictures can: be used by video codecs to provide
predictions of the values of samples in the subsequently decoded or encoded
pictures in which case they are referred to as _Reference Pictures_.

The _Decoded Picture Buffer (DPB)_ is an indexed set of such reference
pictures corresponding to a particular <<video-session,Video Session>>.

[[dpb-slot]]
Individual indexed entries of the <<dpb,DPB>> are referred to as the
_Decoded Picture Buffer (DPB) Slots_.
A valid DPB slot index starts from zero and goes up to the maximum of `N` -
1, where `N` is the number of <<reference-picture,Reference Picture>>
entries requested for a video session.
Each DPB slot can: hold a <<reference-picture,Reference Picture>>.

The DPB may: be represented by a single multi-layer image where subsequent
layers correspond to subsequent DPB slots, or by individual layers of
multiple images each separately associated with a particular DPB slot.
...
contain _Reference Picture Metadata_, present when the video picture
resource associated with the DPB slot is used as a
<<reference-picture,Reference Picture>> in one or more video decode or
encode operations.

An implementation or application may: maintain other metadata related to the
...
_Color Space Metadata_ is the additional static or dynamic state associated
...
of the video core specification.
...
A <<video-session,Video Session>> instance is created specifically:
  ...
  * With the maximum number of supported <<dpb,DPB>> or
    <<reconstructed-picture,Reconstructed Picture>> slots that can be
    allocated;
...
compression standard and profiles, <<encode-input-picture,Input>> /
...
by the video compression standard in use:
  ...
  * number of active <<dpb,DPB>> or <<reconstructed-picture,Reconstructed
    Picture>> slots
...
slink:VkVideoPictureResourceInfoKHR::pname:baseArrayLayer parameter.
...
<<encode-input-picture,Input>> / <<decoded-output-picture,Output>> /
...
slink:VkVideoPictureResourceInfoKHR::pname:extent parameter of each resource
in use.
...
slink:VkVideoSessionCreateInfoKHR::pname:maxDpbSlots must: be set to a value
bigger than `0` when the instance of the <<video-session,Video Session>>
object is created.

Up to slink:VkVideoSessionCreateInfoKHR::pname:maxDpbSlots slots can: be
activated with <<video-picture-resources,Video Picture Resources>> for a
video session and up to
slink:VkVideoSessionCreateInfoKHR::pname:maxActiveReferencePictures active
slots can: be used as DPB or Reconstructed <<reference-picture,Reference
Pictures>> within a single decode or encode operation.
...
change, dynamically, via slink:VkVideoPictureResourceInfoKHR::pname:extent,
...
<<encode-input-picture, Input Picture>>, their coded extent can differ.
...
device and host memory resources are used in an optimal way:
  ...
  * The video application can: allocate or destroy the number of allocated
    <<decoded-output-picture, Output>> or <<encode-input-picture, Input>>
  ...
  * <<reference-picture,Reference Pictures>> can: be shared with the decoded
    <<decoded-output-picture, Output>> or encoded <<encode-input-picture,
    Input>> pictures.
  * The application can: use <<sparsememory, sparse memory>> for the
    ...
    Furthermore, if the <<features-sparseResidency, pname:sparseResidency>>
...
backing of the <<resources-images, image resources>> must: be resident.

When a <<dpb-slot,DPB Slot>> were to be activated, the
slink:VkVideoDecodeInfoKHR::pname:slotIndex for decode, or
slink:VkVideoEncodeInfoKHR::pname:slotIndex for encode, must: be set to the
...
slink:VkVideoPictureResourceInfoKHR::pname:imageViewBinding for that decode
or encode operation.
...
<<encode-input-picture,Input>>, <<dpb,DPB>> or
<<reconstructed-picture,Reconstructed>> <<video-picture-resources,Video
...
[cols="10%h,30%a,50%a,10%",options="header"]
|====
...
The structure slink:VkQueueFamilyVideoPropertiesKHR may: be chained to
...
[open,refpage='VkQueueFamilyVideoPropertiesKHR',desc='Structure specifying support for video codec operations',type='structs']
--
...
ifdef::VK_EXT_video_decode_h264[]
  * ename:VK_VIDEO_CODEC_OPERATION_DECODE_H264_BIT_EXT specifies support for
    H.264 video decode operations.
endif::VK_EXT_video_decode_h264[]
ifdef::VK_EXT_video_decode_h265[]
  * ename:VK_VIDEO_CODEC_OPERATION_DECODE_H265_BIT_EXT specifies support for
...
[open,refpage='VkVideoProfileInfoKHR',desc='Structure specifying the codec video profile',type='structs']
--
A video profile is defined by slink:VkVideoProfileInfoKHR structure as:
...
When a sname:VkVideoProfileInfoKHR structure is specified through the
...
ifdef::VK_EXT_video_decode_h264[]
    ...
    slink:VkVideoDecodeH264ProfileInfoEXT structure included in the
    pname:pNext chain of sname:VkVideoProfileInfoKHR)
endif::VK_EXT_video_decode_h264[]
    is not supported.
  * ename:VK_ERROR_VIDEO_PROFILE_OPERATION_NOT_SUPPORTED_KHR A video profile
    operation specified by pname:videoCodecOperation is not supported.
  * ename:VK_ERROR_VIDEO_PROFILE_FORMAT_NOT_SUPPORTED_KHR Format parameters
    specified by pname:chromaSubsampling, pname:lumaBitDepth, or
    pname:chromaBitDepth are not supported.
  * ename:VK_ERROR_VIDEO_PROFILE_CODEC_NOT_SUPPORTED_KHR Codec-specific
    parameters corresponding to the pname:videoCodecOperation are not
    provided through the pname:pNext chain of sname:VkVideoProfileInfoKHR,
...
Each instance of slink:VkVideoProfileInfoKHR must: chain a codec-operation
specific video profile extension structure, corresponding to the
codec-operation specified in
slink:VkVideoProfileInfoKHR::pname:videoCodecOperation.
...
If pname:pVideoProfile and the chained codec-specific parameters are not
supported, then one of the <<video-profile-error-codes,
video-profile-specific error codes>> are returned.
...
include::{generated}/api/structs/VkVideoCapabilitiesKHR.adoc[]
  ...
  * pname:minCodedExtent is the minimum width and height of the decoded or
    encoded video.
  * pname:maxCodedExtent is the maximum width and height of the decoded or
    encoded video.
  * pname:maxDpbSlots is the maximum number of DPB Slots supported by the
    implementation for a single video session instance.
  * pname:maxActiveReferencePictures is the maximum slots that can be used
    as <<reference-picture,Reference Pictures>> with a single decode or
    encode operation.
  * pname:stdHeaderVersion is a slink:VkExtensionProperties structure
    reporting the Video Std header version supported for the
    pname:codecOperation requested in
    fname:vkGetPhysicalDeviceVideoCapabilitiesKHR::pname:pVideoProfile.
...
ename:VK_VIDEO_DECODE_CAPABILITY_DPB_AND_OUTPUT_COINCIDE_BIT_KHR is
supported but
ename:VK_VIDEO_DECODE_CAPABILITY_DPB_AND_OUTPUT_DISTINCT_BIT_KHR is not
supported in slink:VkVideoDecodeCapabilitiesKHR::pname:flags, then to query
for video format properties for decode DPB or output,
slink:VkPhysicalDeviceVideoFormatInfoKHR::pname:imageUsage must: have both
ename:VK_IMAGE_USAGE_VIDEO_DECODE_DPB_BIT_KHR and
ename:VK_IMAGE_USAGE_VIDEO_DECODE_DST_BIT_KHR set.
Otherwise, the call will fail with
ename:VK_ERROR_IMAGE_USAGE_NOT_SUPPORTED_KHR.

If an implementation reports
ename:VK_VIDEO_DECODE_CAPABILITY_DPB_AND_OUTPUT_DISTINCT_BIT_KHR is
supported but
ename:VK_VIDEO_DECODE_CAPABILITY_DPB_AND_OUTPUT_COINCIDE_BIT_KHR is not
supported in slink:VkVideoDecodeCapabilitiesKHR::pname:flags, then to query
for video format properties for decode DPB,
slink:VkPhysicalDeviceVideoFormatInfoKHR::pname:imageUsage must: have
ename:VK_IMAGE_USAGE_VIDEO_DECODE_DPB_BIT_KHR set and
ename:VK_IMAGE_USAGE_VIDEO_DECODE_DST_BIT_KHR not set.
Otherwise, the call will fail with
ename:VK_ERROR_IMAGE_USAGE_NOT_SUPPORTED_KHR.
Similarly, to query for video format properties for decode output,
slink:VkPhysicalDeviceVideoFormatInfoKHR::pname:imageUsage must: have
ename:VK_IMAGE_USAGE_VIDEO_DECODE_DST_BIT_KHR set and
ename:VK_IMAGE_USAGE_VIDEO_DECODE_DPB_BIT_KHR not set.
Otherwise, the call will fail with
ename:VK_ERROR_IMAGE_USAGE_NOT_SUPPORTED_KHR.
endif::VK_KHR_video_decode_queue[]
...
video profiles, as defined by the slink:VkVideoProfileInfoKHR structure.
...
For a use case such as transcode, where a decode session output image can:
be used as encode input in one or more encode sessions, multiple video
profiles corresponding to the video sessions that will share the image must:
be provided.

If any of the profiles specified via
slink:VkVideoProfileListInfoKHR::pname:pProfiles are not supported, then
fname:vkGetPhysicalDeviceVideoFormatPropertiesKHR returns one of the
<<video-profile-error-codes, video-profile-specific error codes>>.
...
then fname:vkGetPhysicalDeviceVideoFormatPropertiesKHR returns
ename:VK_ERROR_IMAGE_USAGE_NOT_SUPPORTED_KHR.
...
the video codec operations specified in sname:VkVideoProfileListInfoKHR.
...
****
  * [[VUID-vkGetPhysicalDeviceVideoFormatPropertiesKHR-pNext-06812]]
    The pname:pNext chain of pname:pVideoFormatInfo must: include a valid
...
[open,refpage='VkVideoProfileListInfoKHR',desc='Structure enumerating the video profiles',type='structs']
--
The slink:VkVideoProfileListInfoKHR structure is defined as:
...
include::{generated}/api/structs/VkVideoProfileListInfoKHR.adoc[]
  ...
  * pname:pProfiles is a pointer to an array of slink:VkVideoProfileInfoKHR
    structures.
    Each slink:VkVideoProfileInfoKHR structure must: chain the corresponding
    codec-operation specific extension video profile structure.

Video transcoding is an example of a use case that necessitates numerous
profiles.
...
use cases without the need for manual transformation.

.Valid Usage
****
  * [[VUID-VkVideoProfileListInfoKHR-pProfiles-06813]]
    If the pname:pProfiles list contains more than one
    slink:VkVideoProfileInfoKHR entry, then it must: not contain more than
    one entry whose pname:videoCodecOperation member specifies a decode
    operation
****
...
application must: issue a flink:vkCmdControlVideoCodingKHR command with the
pname:flags member of slink:VkVideoCodingControlInfoKHR including
...
include::{generated}/api/structs/VkVideoSessionCreateInfoKHR.adoc[]
  ...
  * pname:pictureFormat is the format of the <<resources-image-views, image
    ...
    encoded <<encode-input-picture, Input>> pictures.
  ...
  * pname:maxDpbSlots is the maximum number of <<dpb-slot,DPB Slots>> that
    can be activated with associated <<video-picture-resources,Video Picture
    Resources>> for the created video session.
  * pname:maxActiveReferencePictures is the maximum number of active
  ...
  * pname:pStdHeaderVersion is a pointer to a slink:VkExtensionProperties
    ...
    pname:codecOperation in pname:pVideoProfile.

.Valid Usage
****
  * [[VUID-VkVideoSessionCreateInfoKHR-pVideoProfile-04845]]
    pname:pVideoProfile must: be a pointer to a valid
    slink:VkVideoProfileInfoKHR structure whose pname:pNext chain must:
    include a valid codec-specific profile structure
  * [[VUID-VkVideoSessionCreateInfoKHR-maxDpbSlots-04846]]
    If <<reference-picture,Reference Pictures>> are required for use with
    the created video session, the pname:maxDpbSlots must: be set to a value
    bigger than `0`
  * [[VUID-VkVideoSessionCreateInfoKHR-maxDpbSlots-04847]]
    pname:maxDpbSlots cannot: exceed the implementation reported
    slink:VkVideoCapabilitiesKHR::pname:maxDpbSlots
  * [[VUID-VkVideoSessionCreateInfoKHR-maxActiveReferencePictures-04848]]
    If <<reference-picture,Reference Pictures>> are required for use with
    the created video session, the pname:maxActiveReferencePictures must: be
    set to a value bigger than `0`
  * [[VUID-VkVideoSessionCreateInfoKHR-maxActiveReferencePictures-04849]]
    pname:maxActiveReferencePictures cannot: exceed the implementation
    reported slink:VkVideoCapabilitiesKHR::pname:maxActiveReferencePictures
  * [[VUID-VkVideoSessionCreateInfoKHR-maxActiveReferencePictures-04850]]
    pname:maxActiveReferencePictures cannot: exceed the pname:maxDpbSlots
  ...
  * [[VUID-VkVideoSessionCreateInfoKHR-referencePicturesFormat-04852]]
    If the pname:videoCodecOperation member of pname:pVideoProfile specifies
    a decode operation then pname:referencePicturesFormat must: be one of
    the supported decode DPB formats as reported by
    flink:vkGetPhysicalDeviceVideoFormatPropertiesKHR in the pname:format
    member of slink:VkVideoFormatPropertiesKHR when called with the
    ...
    slink:VkVideoProfileListInfoKHR structure chained to its
    pname:pVideoFormatInfo parameter whose pname:pProfiles member contains
    an entry matching pname:pVideoProfile
  * [[VUID-VkVideoSessionCreateInfoKHR-videoCodecOperation-06814]]
    If the pname:videoCodecOperation member of pname:pVideoProfile specifies
    an encode operation then pname:referencePicturesFormat must: be one of
  ...
  * [[VUID-VkVideoSessionCreateInfoKHR-pictureFormat-04853]]
    If the pname:videoCodecOperation member of pname:pVideoProfile specifies
    a decode operation then pname:pictureFormat must: be one of the
  ...
  * [[VUID-VkVideoSessionCreateInfoKHR-pictureFormat-04854]]
    If the pname:videoCodecOperation member of pname:pVideoProfile specifies
    an encode operation then pname:pictureFormat must: be one of the
...
[open,refpage='VkVideoSessionMemoryRequirementsKHR',desc='Structure specifying video session memory requirements',type='structs']
--
The slink:VkVideoSessionMemoryRequirementsKHR structure is defined as:
...
--
The slink:VkBindVideoSessionMemoryInfoKHR structure is defined as:
...
include::{generated}/api/structs/VkBindVideoSessionMemoryInfoKHR.adoc[]
  ...
  * pname:pNext is `NULL` or a pointer to a structure extending this
    structure.
  * pname:memoryBindIndex is the index of the memory binding returned in
    slink:VkVideoSessionMemoryRequirementsKHR::pname:memoryBindIndex from
...
include::{generated}/api/protos/vkUpdateVideoSessionParametersKHR.adoc[]
  ...
  * pname:device is the logical device that was used for the creation of the
    video session object.
  * pname:videoSessionParameters is the video session parameters object that
...
include::{generated}/api/protos/vkDestroyVideoSessionParametersKHR.adoc[]
  ...
  * pname:device is the device the video session parameters object was
    created with.
...
pname:videoSession member of slink:VkVideoBeginCodingInfoKHR is _bound_ to
...
include::{generated}/api/structs/VkVideoBeginCodingInfoKHR.adoc[]
  ...
  * pname:pReferenceSlots is a pointer to an array of
    slink:VkVideoReferenceSlotInfoKHR structures specifying reference slots,
    used within the video command context between this
    flink:vkCmdBeginVideoCodingKHR command and the
    flink:vkCmdEndVideoCodingKHR command that follows.
    Each reference slot provides a slot index and the
    slink:VkVideoPictureResourceInfoKHR specifying the reference picture
...
****
  * [[VUID-VkVideoBeginCodingInfoKHR-referenceSlotCount-04856]]
    ...
    slink:VkVideoSessionCreateInfoKHR::pname:maxDpbSlots when creating the
    video session object that is being provided in pname:videoSession
...
[open,refpage='VkVideoReferenceSlotInfoKHR',desc='Structure specifying the reference picture slot',type='structs']
--
The slink:VkVideoReferenceSlotInfoKHR structure is defined as:
...
include::{generated}/api/structs/VkVideoReferenceSlotInfoKHR.adoc[]
  ...
  * pname:slotIndex is the unique reference slot index used for the encode
    or decode operation.
  * pname:pPictureResource is a pointer to a
    slink:VkVideoPictureResourceInfoKHR structure describing the picture
    resource bound to this slot index.

include::{generated}/validity/structs/VkVideoReferenceSlotInfoKHR.adoc[]
--

[open,refpage='VkVideoPictureResourceInfoKHR',desc='Structure specifying the picture resources',type='structs']
--
The slink:VkVideoPictureResourceInfoKHR structure is defined as:
...
After ending a video coding scope, the session object previously bound by
the corresponding flink:vkCmdBeginVideoCodingKHR command is _unbound_.
...
until a subsequent fname:vkCmdControlVideoCodingKHR command changes the
corresponding control parameters.

[open,refpage='VkVideoCodingControlInfoKHR',desc='Structure specifying parameters of coding control',type='structs']
--
...
encode operations by including ename:VK_VIDEO_CODING_CONTROL_RESET_BIT_KHR
in pname:flags.
...
operation may: be used to return the video session object to the same
_initial_ state as after the reset of a newly created video session.
...
****
  * [[VUID-VkVideoCodingControlInfoKHR-flags-07016]]
    pname:flags must: include at least one bit
  * [[VUID-VkVideoCodingControlInfoKHR-flags-07017]]
    If the bound video session object is in _uninitialized_ state, then
    pname:flags must: include ename:VK_VIDEO_CODING_CONTROL_RESET_BIT_KHR
ifdef::VK_KHR_video_encode_queue[]
  * [[VUID-VkVideoCodingControlInfoKHR-flags-07018]]
    If pname:flags includes
    ename:VK_VIDEO_CODING_CONTROL_ENCODE_RATE_CONTROL_BIT_KHR, then the
    pname:pNext chain must: include a valid
    slink:VkVideoEncodeRateControlInfoKHR structure
...
ifdef::VK_EXT_video_encode_h264[]
  * [[VUID-VkVideoCodingControlInfoKHR-flags-07021]]
    ...
    slink:VkVideoProfileInfoKHR::pname:videoCodecOperation set to
...
[open,refpage='VkVideoCodingControlFlagsKHR',desc='Bitmask specifying the video coding control flags',type='flags']
--
