...
ifdef::VK_COMPUTE_VERSION_1_4,VK_KHR_maintenance6[]
    ...
    in its pname:pNext chain, pname:dataSize must: equal the size of
...
ifdef::VK_ANDROID_external_memory_android_hardware_buffer[]
ifdef::VK_COMPUTE_VERSION_1_4,VK_KHR_maintenance6[]
  * [[VUID-vkGetDescriptorEXT-pDescriptorInfo-12521]]
    If pname:pDescriptorInfo->type is
    ename:VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER and
    pname:pDescriptorInfo->data.pCombinedImageSampler has an pname:imageView
    member that was created with a sname:VkSamplerYcbcrConversionInfo
    structure with an
    <<memory-external-android-hardware-buffer-external-formats,Android
    external format>> in its pname:pNext chain, pname:dataSize must: equal
    the size of
    slink:VkPhysicalDeviceDescriptorBufferPropertiesEXT::pname:combinedImageSamplerDescriptorSize
    times
...
ifdef::VK_ARM_tensors[]
  ...
  * [[VUID-VkDescriptorGetInfoEXT-type-12270]]
    ...
    alignment requirement as determined by
    <<limits-minTexelBufferOffsetAlignment,pname:minTexelBufferOffsetAlignment>>
  * [[VUID-VkDescriptorGetInfoEXT-type-12492]]
    If pname:type is ename:VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER and
    pname:pUniformBuffer->address is not 0, pname:pUniformBuffer->range
    must: be less than or equal to <<limits-maxUniformBufferRange,
    pname:maxUniformBufferRange>>
  * [[VUID-VkDescriptorGetInfoEXT-type-12493]]
    If pname:type is ename:VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
ifdef::VK_EXT_shader_64bit_indexing[]
    the <<features-shader64BitIndexing, pname:shader64BitIndexing>> feature
    is not enabled,
endif::VK_EXT_shader_64bit_indexing[]
    and pname:pUniformBuffer->address is not 0, pname:pStorageBuffer->range
    must: be less than or equal to <<limits-maxStorageBufferRange,
...
fname:vkCmdBindDescriptorBuffersEXT causes any offsets previously set by
...
fname:vkCmdBindDescriptorBufferEmbeddedSamplersEXT binds the embedded
immutable samplers in pname:set of pname:layout to pname:set for the command
buffer for subsequent <<pipelines-bindpoint-commands, bound pipeline
commands>> set by pname:pipelineBindPoint.
...
ifdef::VK_KHR_acceleration_structure[]
    ...
    pname:accelerationStructure must: be dlink:VK_NULL_HANDLE
  * [[VUID-VkAccelerationStructureCaptureDescriptorDataInfoEXT-accelerationStructureNV-12431]]
    If the `apiext:VK_NV_ray_tracing extension` is not enabled, then
    pname:accelerationStructureNV must: be dlink:VK_NULL_HANDLE
****
...
pname:pNext chain of a
ifdef::VK_KHR_acceleration_structure[slink:VkAccelerationStructureCreateInfoKHR,]
ifdef::VK_NV_ray_tracing[slink:VkAccelerationStructureCreateInfoNV,]
ifdef::VK_ARM_tensors[slink:VkTensorCreateInfoARM, slink:VkTensorViewCreateInfoARM,]
slink:VkBufferCreateInfo, slink:VkImageCreateInfo,
slink:VkImageViewCreateInfo, or slink:VkSamplerCreateInfo structure.
