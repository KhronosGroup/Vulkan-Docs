...
=== Should this extension be built by adding Metal platform variations to the `VK_KHR_external_memory` family of extensions?
...
=== Should this extension be split into two, one for resources and one for other objects?
...
=== Should the application be required to indicate its intention to export Metal objects when the corresponding Vulkan object is created?
...
=== Should a `MTLBuffer` be imported and exported through a `VkBuffer` or a `VkDeviceMemory`?
