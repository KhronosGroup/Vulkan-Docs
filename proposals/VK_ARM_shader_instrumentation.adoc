...
link:{extensions}VK_KHR_pipeline_executable_properties[VK_KHR_pipeline_executable_properties] is insufficient to address these concerns since the metrics returned here can only cover values known or estimated at compile time.
...
3. Something that matches how link:{extensions}VK_KHR_pipeline_executable_properties[VK_KHR_pipeline_executable_properties] exposes data to the user, but make it apply to an instrumentation object rather than a pipeline/shader object.
