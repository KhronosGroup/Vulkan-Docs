...
colorspace to `eglCreateWindowSurface` at the same time.

In Vulkan, the format and colorspace information are retrievable from the
`VkSurface` through
...
On some implementations and platforms, the surface formats, colorspaces,
