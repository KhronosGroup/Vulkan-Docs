...
While link:{extensions}VK_EXT_metal_objects[VK_EXT_metal_objects] provides a way to expose underlying Metal resources,
...
link:{extensions}VK_EXT_metal_objects[VK_EXT_metal_objects] differently.
...
link:{extensions}VK_KHR_external_memory[VK_KHR_external_memory] as baseline to conform to what other platforms have done with
extensions like link:{extensions}VK_KHR_external_memory_win32[VK_KHR_external_memory_win32] and
link:{extensions}VK_ANDROID_external_memory_android_hardware_buffer[VK_ANDROID_external_memory_android_hardware_buffer] to name a few.

This means that external Metal resources will be imported through link:{docs}chapters/memory.html#VkDeviceMemory[VkDeviceMemory] to later be bound.
