...
This proposal builds on two existing extensions:
  ...
  * xref:{refpages}VK_QCOM_tile_properties.html[VK_QCOM_tile_properties] which exposes tile locations
    and dimensions to the application.
  * xref:{refpages}VK_KHR_dynamic_rendering_local_read.html[VK_KHR_dynamic_rendering_local_read]
...
enabled for a render pass instance, these are the highlights of the new functionality:
  ...
  * Fragment shaders can declare xref:{docs}chapters/renderpass.html#renderpass-tile-shading-attachment-access[tile image attachment]
  ...
  * xref:{docs}chapters/renderpass.html#renderpass-tile-shading-aprons[Tile aprons] can be enabled and pixels in the apron
    region can be accessed by the fragment shader.
  * A new state command is added that enables/disables
    xref:{docs}chapters/renderpass.html#renderpass-per-tile-execution-model[per-tile execution model].
  ...
  ** Compute shaders have all the same functionality described above for fragment shaders.
     This includes load/store/sample of xref:{docs}chapters/renderpass.html#renderpass-tile-shading-attachment-access[tile image attachments],
     <<spirv-changes,built-in input variables>>, and xref:{docs}chapters/renderpass.html#renderpass-tile-shading-aprons[tile aprons].
...
xref:{docs}chapters/renderpass.html#renderpass-tile-shading-aprons[tile apron].
...
Within a render pass that xref:{docs}chapters/renderpass.html#renderpass-tile-shading[enables tile shading],
the xref:{docs}chapters/renderpass.html#renderpass-per-tile-execution-model[per-tile execution mode]
...
xref:{docs}chapters/renderpass.html#renderpass-tile-shading-command-restrictions[Per-Tile Command Restrictions] apply.
...
When xref:{docs}chapters/renderpass.html#renderpass-per-tile-execution-model[per-tile execution mode]
...
Where _per-tile execution model_ is enabled the following are disallowed:
  ...
  * Access of an attachment with layout `VK_IMAGE_LAYOUT_ATTACHMENT_FEEDBACK_LOOP_OPTIMAL`
    as provided by xref:{refpages}VK_EXT_attachment_feedback_loop_layout.html[VK_EXT_attachment_feedback_loop_layout].
...
xref:{refpages}VK_QCOM_tile_properties.html[VK_QCOM_tile_properties]
...
of two and less than or equal to the xref:{docs}chapters/limits.html#limits-maxTileShadingRate[maxTileShadingRate] limit.
...
xref:{docs}chapters/renderpass.html#renderpass-tile-shading-attachment-access[Tile attachment] variables are declared
...
The Vulkan SPIR-V environment will specify that:
  ...
  * If the xref:{docs}chapters/renderpass.html#renderpass-tile-shading-aprons[tile apron] has width or height greater than zero, then loads
 ...
 * The framebuffer regions defined by `VK_DEPENDENCY_BY_REGION_BIT` are enlarged to
   be tile-sized regions, where xref:{refpages}VK_QCOM_tile_properties.html[VK_QCOM_tile_properties]
...
A few notable features are documented below.
  ...
  * `tileShadingPerTileDispatch` indicates the implementation supports
    ...
    xref:{docs}chapters/renderpass.html#renderpass-per-tile-execution-model[per-tile execution] is enabled.
...
xref:{refpages}VK_QCOM_tile_properties.html[VK_QCOM_tile_properties] reports
...
xref:{docs}chapters/renderpass.html#renderpass-per-tile-execution-model[per-tile execution model], then
...
Yes, on some Adreno (TM) GPUs and for some use cases, the tile-sized
...
xref:{docs}chapters/renderpass.html#renderpass-per-tile-execution-model[per-tile execution mode] is enabled. Similarly,
...
=== How does this extension differ from xref:{refpages}VK_EXT_shader_tile_image.html[VK_EXT_shader_tile_image]?
...
=== How does this extension interact with xref:{refpages}VK_QCOM_image_processing.html[VK_QCOM_image_processing]?
