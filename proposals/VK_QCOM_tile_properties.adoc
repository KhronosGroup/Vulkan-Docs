...
The reported value for `apronSize` is described in xref:{refpages}VK_QCOM_tile_shading.html[VK_QCOM_tile_shading].
...
=== How to handle dynamic rendering?
...
=== This extension returns only one set of dimensions for tile size so how to handle the case of non-merged subpasses where each subpass can have a different tile size?
...
=== Adreno implementation may decide to execute certain workloads in direct rendering mode a.k.a Flex render. What is the interaction of this extension with Flex render?
