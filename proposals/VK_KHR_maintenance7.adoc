...
For example, running on Mesa/Venus, driver ID is returned as ename:VK_DRIVER_ID_MESA_VENUS, but it may be necessary to know what the real driver under the hood is.
...
----
  ...
  * `robustFragmentShadingRateAttachmentAccess` indicates whether a
    ...
    slink:VkImageSubresourceRange::pname:baseMipLevel equal to 0 can
