...
xref:{refpages}VK_QUEUE_FAMILY_FOREIGN_EXT.html[VK_QUEUE_FAMILY_FOREIGN_EXT]).
...
of xref:{refpages}VkDeviceMemory.html[VkDeviceMemory] bound to the resource,
where the xref:{refpages}VkDeviceMemory.html[VkDeviceMemory] has been imported or
exported with an API related to
xref:{refpages}VK_KHR_external_memory.html[VK_KHR_external_memory].
...
and does not reside in any range of xref:{refpages}VkDeviceMemory.html[VkDeviceMemory] bound to the resource.
For example, if the external resource is a xref:{refpages}VkImage.html[VkImage],
...
For example, each xref:{refpages}VkDevice.html[VkDevice]
...
xref:{refpages}VkImageMemoryBarrier.html[VkImageMemoryBarrier].
...
during the execution of the xref:{refpages}VkImageMemoryBarrier.html[VkImageMemoryBarrier].

The problem is especially relevant for systems that implement
xref:{refpages}VkSwapchainKHR.html[VkSwapchainKHR]
by layering it atop the external memory extensions such as
xref:{refpages}VK_EXT_external_memory_dma_buf.html[VK_EXT_external_memory_dma_buf].
...
xref:{refpages}VkImageLayout.html[VK_IMAGE_LAYOUT_PRESENT_SRC_KHR]
after xref:{refpages}vkAcquireNextImageKHR.html[vkAcquireNextImageKHR].
...
- The solution should not require the implementation to internally track the
  xref:{refpages}VkImageLayout.html[VkImageLayout] of external images.
...
- The solution must work well for systems that implement
  xref:{refpages}VkSwapchainKHR.html[VkSwapchainKHR] by layering it atop external
...
- After acquiring a swapchain image but before using the image, applications
  ...
  xref:{refpages}VkImageLayout.html[VK_IMAGE_LAYOUT_PRESENT_SRC_LAYOUT_KHR]
  (which preserves the image's pixels) or xref:{refpages}VkImageLayout.html[VK_IMAGE_LAYOUT_UNDEFINED]
...
- **B**. Extend resource creation, such as xref:{refpages}VkImageCreateInfo.html[VkImageCreateInfo],
...
- **C**. Extend the ownership-release memory barrier, such as
  xref:{refpages}VkImageMemoryBarrier.html[VkImageMemoryBarrier], with a new
...
- A presentation engine (such as a window system) that only reads the image
  ...
  engine, such as xref:{refpages}vkCreateWaylandSurfaceKHR.html[vkCreateWaylandSurfaceKHR],
  xref:{refpages}VkSwapchainKHR.html[VkSwapchainKHR], and
  xref:{refpages}vkQueuePresentKHR.html[vkQueuePresentKHR].
  Instead, the application effectively implements these
  functions itself with a combination of
  xref:{refpages}VK_EXT_external_memory_dma_buf.html[VK_EXT_external_memory_dma_buf],
  xref:{refpages}VK_EXT_queue_family_foreign.html[VK_EXT_queue_family_foreign],
...
performance hint to xref:{refpages}VkImageMemoryBarrier.html[VkImageMemoryBarrier] in step (g).
...
xref:{refpages}VkSwapchainKHR.html[VkSwapchainKHR] atop
xref:{refpages}VK_KHR_external_memory.html[VK_KHR_external_memory] and native
window system APIs.
...
and transfer commands such as xref:{refpages}vkCmdCopyImage.html[vkCmdCopyImage].
...
=== OUT OF SCOPE. How to determine if a resource's external memory is modified?
