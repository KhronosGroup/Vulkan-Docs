...
=== What are the requirements for the codec-specific input parameters?
...
=== Are OBU extension headers generated by the implementation when multiple temporal or spatial layers are used?
...
=== What codec-specific parameters are guaranteed to not be overridden by implementations?
...
=== How is reference picture setup requested for AV1 encode operations?
...
=== Should we have separate rate control configuration parameters (quantizer indices, frame sizes) for each AV1 prediction mode?
...
=== How can the application indicate the use of a `primary_ref_frame` that is used for CDF data but not for picture prediction?
...
=== Why there is no `maxUnidirectionalCompoundGroup2ReferenceCount` capability?
...
=== Why are implementations allowed to override the coded resolution?
