...
`vkCmdDrawMesh*` commands are added as an interaction with link:{extensions}VK_EXT_mesh_shader[VK_EXT_mesh_shader], and `vkCmdDraw*IndirectCount2KHR` commands are added as an interaction with link:{extensions}VK_KHR_draw_indirect_count[VK_KHR_draw_indirect_count] or Vulkan 1.2.
...
Copy commands are based on link:{extensions}VK_KHR_copy_commands2[VK_KHR_copy_commands2], which is required by this extension.
...
A revised version of link:{extensions}vkCmdBeginConditionalRenderingEXT[vkCmdBeginConditionalRenderingEXT] is added when link:{extensions}VK_EXT_conditional_rendering[VK_EXT_conditional_rendering] is supported, that functions identically but takes an address range instead of a buffer and offset:
...
Transform feedback commands added by link:{extensions}VK_EXT_transform_feedback[VK_EXT_transform_feedback] now accept address ranges rather than buffer objects, but are otherwise unmodified.
...
If link:{extensions}VK_AMD_buffer_marker[VK_AMD_buffer_marker] is supported, a new function is added that accepts an address rather than a buffer.
...
If link:{extensions}VK_KHR_acceleration_structure[VK_KHR_acceleration_structure] is supported, a new function is added that accepts an address rather than a buffer.
