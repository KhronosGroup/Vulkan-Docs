...
=== Why is there no `VK_PIPELINE_STAGE_VIDEO_ENCODE_BIT_KHR`?
...
=== How can layered codec-specific encode extensions enable applications to provide the necessary codec-specific picture information, parameter sets, etc. that may be needed to perform the video coding operations?
...
=== Can `vkCmdVideoEncodeKHR` only encode frames? What about field encoding, slice encoding, etc.?
...
=== What is the effect of the flags provided in `VkVideoEncodeUsageInfoKHR::videoUsageHints` and `VkVideoEncodeUsageInfoKHR::videoContentHints`?
...
=== What is the effect of the tuning mode provided in `VkVideoEncodeUsageInfoKHR::tuningMode`?
...
=== How should we expose video encoding feedback values (e.g. encoded bitstream size)?
...
=== Do result status queries need to be used in conjunction with video encode feedback queries?
...
=== Why is there a need to allow implementations to override codec-specific parameters?
...
=== Can the application disable all implementation overrides?
...
=== Can implementations override any codec-specific parameter?
...
=== Do all implementations have to implement the same rate control algorithms corresponding to the rate control modes defined by this proposal?
...
=== Do rate control implementations guarantee to respect the average/max bitrates, or frame sizes configured for the video session?
...
=== Are video session parameters objects dependent on the used video encode quality level?
...
=== Are video encode quality levels and rate control mutually exclusive?
...
=== Does specifying `VkVideoEncodeRateControlInfoKHR` in the `pNext` chain of the `pBeginCodingInfo` parameter of `vkCmdBeginVideoCodingKHR` change the current rate control configuration?
...
=== When is it mandatory to specify reconstructed picture information in `VkVideoEncodeInfoKHR::pSetupReferenceSlot`?
