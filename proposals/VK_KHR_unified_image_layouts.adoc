...
Since then however, `VK_QUEUE_FAMILY_EXTERNAL` and `VK_QUEUE_FAMILY_FOREIGN_EXT` in xref:{docs}chapters/synchronization.html#synchronization-queue-transfers[queue family ownership transfers] provided a better mechanism to interact with such external entities.
