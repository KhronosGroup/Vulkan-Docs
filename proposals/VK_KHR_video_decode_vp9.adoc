...
=== What type of VP9 parameter sets do we want to store in video session parameters objects?

None. VP9 does not define any parameter sets.


=== What should be the contents of the input bitstream buffer when decoding an VP9 frame?
...
=== Does the application also need to specify the offset of the frame header?
...
=== How do VP9 references map to DPB slot indices?
...
=== Does the application has to send frames that have `show_existing_frame` set for decoding?
...
=== How is reference picture setup requested for VP9 decode operations?
