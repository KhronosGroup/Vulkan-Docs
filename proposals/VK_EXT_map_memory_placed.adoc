...
=== How should the requested address be passed to `vkMapMemory2KHR()`?
...
=== What should happen if the implementation cannot place the memory map at the requested address?

The memory object should be left unmapped and
`vkMapMemory2KHR` should return `VK_ERROR_MEMORY_MAP_FAILED`.

=== How can an application atomically re-reserve the address range on unmap?
...
=== Should a placed memory map replace existing maps in the specified range or fail if a map exists?
