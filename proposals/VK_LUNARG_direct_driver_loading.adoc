...
xref:{refpages}VkInstanceCreateInfo.html[VkInstanceCreateInfo] to provide a list
of structures which contain the information needed by the loader to load the drivers.
This is achieved by providing the driver's xref:{refpages}vkGetInstanceProcAddr.html[vkGetInstanceProcAddr],
...
=== Global functions are not extensible
...
=== Should this extension also handle layer loading?
...
=== Do drivers implement this extension?

No, this would be implemented by the Vulkan Loader.

=== Are there any changes drivers need to make to allow being used in this extension?
...
=== What kinds of drivers are expected to be used with this extension?
