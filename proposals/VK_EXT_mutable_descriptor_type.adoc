...
NOTE: This extension is a direct promotion of xref:{refpages}VK_VALVE_mutable_descriptor_type.html[VK_VALVE_mutable_descriptor_type]. As that extension already shipped before proposal documents existed, this document has been written retroactively during promotion to EXT.
...
Typically when specifying a xref:{refpages}VkDescriptorSetLayoutBinding.html[descriptor set layout binding], applications have to choose one of the available xref:{refpages}VkDescriptorType.html[descriptor types] that will occupy that binding.
...
`VkMutableDescriptorTypeCreateInfoEXT` can be added to the `pNext` chain of xref:{refpages}VkDescriptorSetLayoutCreateInfo.html[VkDescriptorSetLayoutCreateInfo], where each entry in `pMutableDescriptorTypeLists` corresponds to a binding at the same index in `pBindings`.
...
It is possible to go beyond the minimum supported set. For this purpose, the desired descriptor set layout can be queried with xref:{refpages}vkGetDescriptorSetLayoutSupport.html[vkGetDescriptorSetLayoutSupport].
...
Host-only descriptor sets cannot be bound to a command buffer, and their contents must be copied to a non-host-only set using xref:{refpages}vkUpdateDescriptorSets.html[vkUpdateDescriptorSets] before those descriptors can be used.

Descriptor pools are specified as host-only using a new xref:{refpages}VkDescriptorSetLayoutCreateFlagBits.html[create flag]:
...
The memory layout of a descriptor set may also be optimized for device access rather than host access, so a new xref:{refpages}VkDescriptorSetLayoutCreateFlagBits.html[create flag] is provided to specify when a layout will be used with a host-only pool:
...
In addition, as such layouts are not valid for device access, xref:{refpages}VkPipelineLayout.html[VkPipelineLayout] objects cannot be created with such descriptor set layouts.
