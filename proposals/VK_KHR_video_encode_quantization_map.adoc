...
=== Do we need separate extensions for each video codec?
...
=== How should quantization maps be provided to the implementation?
...
=== Can all implementations support quantization delta maps, or do some implementations only support absolute quantization maps?
...
=== Are quantization parameters clamped to the allowed range before or after applying the quantization map?
...
=== What is the granularity of quantization maps (i.e. the texel size of the rectangular blocks each quantization control value applies to)?
...
=== Should we report the supported quantization map granularities through individual entries as a `VkExtent2D` or should we use a bitmask where each flag corresponds to a supported block size?
...
=== Do we need a capability for the maximum extent of quantization maps?
...
=== Can subregions of a larger image be used as a quantization map?
...
=== Should we require an opt-in at video session creation time to use quantization maps?
...
=== Can applications use quantization maps only on a subset of the pictures encoded within a video stream?

Yes. Implementations do not require quantization maps to be provided for every video encode operation.

=== Why does this proposal introduce the `VK_VIDEO_ENCODE_H264_CAPABILITY_MB_QP_DIFF_WRAPAROUND_BIT_KHR` capability flag?
...
=== Why does this proposal introduce the `VK_VIDEO_ENCODE_H265_CAPABILITY_CU_QP_DIFF_WRAPAROUND_BIT_KHR` capability flag?
