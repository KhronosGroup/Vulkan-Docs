...
=== In what form should codec-specific parameters be provided?
...
=== What are the requirements for the codec-specific input parameters and bitstream data?
...
=== What type of AV1 parameter sets do we want to store in video session parameters objects?
...
=== What should be the contents of the input bitstream buffer when decoding an AV1 frame?
...
=== Does the application need to specify the offsets of individual tiles?

Yes.


=== Does the application also need to specify the sizes of individual tiles?

Yes. Especially as the tiles may be spread across multiple tile group OBUs.


=== Does the application also need to specify the offset of the frame header OBU?
...
=== Does the application also need to specify the offsets of the tile group OBUs and the range of tiles they cover?
...
=== How do AV1 references map to DPB slot indices?
...
=== Does AV1 decode support the _coincide_ mode?
...
=== Should film grain be a capability or part of the video profile definition?
...
=== Do the additional memory requirements imposed by AV1 film grain support affect the memory requirements of images created with `VK_IMAGE_CREATE_VIDEO_PROFILE_INDEPENDENT_BIT_KHR`?
...
=== Does the application has to send frame OBUs that have `show_existing_frame` set for decoding?

No. Such frame OBUs do not contain any actual payload that is relevant to implementations.


=== Are any of the `frame_width_minus_1`, `frame_height_minus_1`, `render_width_minus_1`, and `render_height_minus_1` parameters from the frame header redundant with respect to the decoded frame size specified in `VkVideoDecodeInfoKHR::dstPictureResource.codedExtent`?
...
=== Are the extents in superblocks specified by the `width_in_sbs_minus_1` and `height_in_sbs_minus_1` tile parameters necessary?
...
=== What codec-specific parameters need to be specified for the active reference pictures?
...
=== How is reference picture setup requested for AV1 decode operations?
...
=== Are film grain parameters tracked as part of DPB slot reference picture metadata?
...
=== Should the size of the `GmType` and `gm_params` arrays be `STD_VIDEO_AV1_NUM_REF_FRAMES` or `STD_VIDEO_AV1_REFS_PER_FRAME`?
