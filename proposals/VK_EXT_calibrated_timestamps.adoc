...
Finally, the `VK_TIME_DOMAIN_DEVICE_EXT` is the time domain used by the device for all xref:{refpages}vkCmdWriteTimestamp.html[vkCmdWriteTimestamp] commands on any queue that supports timestamps.
