...
=== Should the depth clamp range be a per-viewport parameter?
...
=== Should this pipeline state be dynamic?
...
=== Can the depth clamp range be ignored when depth clamping is disabled?
...
xref:{refpages}VK_EXT_depth_clamp_control.html[VK_EXT_depth_clamp_control]
