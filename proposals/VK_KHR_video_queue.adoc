...
=== What is within the scope of this extension?
...
=== Are Vulkan video profiles equivalent to the corresponding concepts of video compression standards?
...
=== Do we need a query to be able to enumerate all supported video profiles?
...
=== Do we need queries that allow determining how multiple video profiles can be used in conjunction?
...
=== What kind of capabilitity queries do we need?
...
=== What kind of command buffer commands do we need?
...
=== How can the application get feedback about the status of video coding operations?
...
=== Do we need to introduce new `vkCmdBeginQueryRangeKHR` and `vkCmdEndQueryRangeKHR` commands to allow capturing feedback about multiple video coding operations using a single scope?
...
=== Can resources be shared across video sessions, potentially ones using different video profiles?
...
=== How are video coding operations synchronized with respect to other Vulkan operations?
...
=== Why do some of the members of `VkVideoProfileInfoKHR` have `Flags` types instead of `FlagBits` types when only a single bit can be used?
...
=== Can the application create video sessions with any `maxDpbSlots` and `maxActiveReferencePictures` values within the supported capabilities?
...
=== Are `VkVideoSessionParametersKHR` objects internally or externally synchronized?
