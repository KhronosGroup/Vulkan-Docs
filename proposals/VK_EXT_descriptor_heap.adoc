...
A tensor with this flag can be recreated from a previously captured tensor by passing `data` back into tensor creation by chaining `VkOpaqueCaptureDataCreateInfoEXT` to xref:{refpages}VkTensorCreateInfo.html[VkTensorCreateInfo], with all other creation parameters matching.
