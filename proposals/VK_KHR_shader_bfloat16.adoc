...
`shaderBFloat16CooperativeMatrix` must be `VK_FALSE` if xref:{refpages}VK_KHR_cooperative_matrix.html[VK_KHR_cooperative_matrix] is not supported.
...
A new xref:{refpages}VkComponentTypeKHR.html[VkComponentTypeKHR] is added that can be reported as supported by xref:{refpages}vkGetPhysicalDeviceCooperativeMatrixPropertiesKHR.html[vkGetPhysicalDeviceCooperativeMatrixPropertiesKHR]:
...
If `shaderBFloat16CooperativeMatrix` is supported, at least one entry in xref:{refpages}vkGetPhysicalDeviceCooperativeMatrixPropertiesKHR.html[vkGetPhysicalDeviceCooperativeMatrixPropertiesKHR::pProperties] must include this type in all of its `AType`, `BType`, and `CType` members.
