...
- `VK_INDIRECT_COMMANDS_LAYOUT_USAGE_UNORDERED_SEQUENCES_BIT_EXT` specifies that xref:{docs}chapters/synchronization.html#synchronization-submission-order[submission order] is not affected by the ordering of sequences, and sequences may be processed in any order.
