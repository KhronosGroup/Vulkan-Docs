...
=== Should additional state be included?

No additional state changes are permitted in order to enable fast and broad adoption.

=== What shader stages or pipeline states should be allowed to change?
...
=== Should additional alignment properties be added?
...
=== Should index type values be remappable?
...
=== Should indirect buffers be reusable?

Yes, indirect buffers can be reused.

=== How should commands with less than 32-bits of data be handled?

No such commands are provided.

=== How should applications provide data to the preprocess command in order for drivers to optimize indirect execution?
