...
:sectnums:
:VK_KHR_device_fault: link:{extensions}VK_KHR_device_fault[VK_KHR_device_fault]
...
Execution of `OpAbortKHR` will trigger device loss in the API within finite time, making the data in _Message_ available via link:{docs}chapters/debugging.html#vkGetDeviceFaultDebugInfoKHR[vkGetDeviceFaultDebugInfoKHR] for further analysis.
...
If {VK_KHR_device_fault} is supported and enabled, new structures to retrieve shader abort messages can be chained into link:{docs}chapters/debugging.html#VkDeviceFaultDebugInfoKHR[VkDeviceFaultDebugInfoKHR] are added:
