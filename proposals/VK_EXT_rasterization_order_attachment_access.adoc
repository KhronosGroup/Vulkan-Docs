...
For simplicity, this proposal extends the original render pass API and not the xref:{refpages}VK_KHR_dynamic_rendering.html[VK_KHR_dynamic_rendering] API.
...
=== Is there any interaction with the xref:{refpages}VK_KHR_dynamic_rendering.html[VK_KHR_dynamic_rendering] extension?
...
=== What are the differences to the xref:{refpages}VK_ARM_rasterization_order_attachment_access.html[VK_ARM_rasterization_order_attachment_access] extension?
...
=== What is the interaction with the `VK_EXT_shader_object` extension?
