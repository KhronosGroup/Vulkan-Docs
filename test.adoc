...
The `VkPipelineShaderStageNodeCreateInfoAMDX` provides options for specifying how the shader is specified with regards to its entry point name and index, and can be chained to the xref:{docs}chapters/pipelines.html#VkPipelineShaderStageCreateInfo[VkPipelineShaderStageCreateInfo] structure.
