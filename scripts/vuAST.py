...
def createSymbolAvailabilityMap(registry, aliasMap):
    ...
    for versionAndExtension, elem in versionAndExtensionList:
        for require in elem.findall('require'):
            ...
            for enumDecl in require.findall('.//enum'):
                ...
                # If aliased, add the alias too but don't mark it as visited.
                # The alias would be introduced by another extension / version
    ...
    for struct, versionsAndExtensions in featureStructMap.items():
        ...
        for member in info.getMembers():
            ...
            # need to codify VUs involving those features, they are not
            # special-cased.  More cases like this have been added to the spec
            # since, so need to find a way to automatically disambiguate.
