...
from vuAST import VuAST, isCodifiedVU
...
def test_builtin_loop_index(registry):
  ...
  if binding.descriptorType == VK_DESCRIPTOR_TYPE_MUTABLE_EXT:
    bindingIndex = loop_index(binding)
    if bindingIndex != -1:
...
def test_builtin_version(registry):
    """Basic test of the is_version() builtin."""
...
def test_negative_parse(registry):
    ...
    # Invalid character `
    vu = """require(is_ext_enabled(`VK_EXT_foo`))"""
    assert(not ast.parse(vu, 'no_file', 100))
...
while index < correlatedViewMaskCount:
 ...
 # Mutation currently not supported!
 # index = index + 1"""
    assertParseAndVerify(registry, 'VkRenderPassCreateInfo2', vu, 'renderpass.adoc')
