...
ifdef::VK_VERSION_1_1[]
  * [[VUID-{refpage}-subgroupSize-10141]]
    If <<limits-subgroupSize, pname:subgroupSize>> is 1, the
    code:UniformBufferArrayNonUniformIndexing capability is not declared,
  ...
  * [[VUID-{refpage}-subgroupSize-10142]]
    If <<limits-subgroupSize, pname:subgroupSize>> is 1, the
    code:SampledImageArrayNonUniformIndexing capability is not declared, and
  ...
  * [[VUID-{refpage}-subgroupSize-10143]]
    If <<limits-subgroupSize, pname:subgroupSize>> is 1, the
    code:StorageBufferArrayNonUniformIndexing capability is not declared,
    and an instruction accesses memory through a storage buffer, the storage
    buffer through which that memory is accessed must: be dynamically
    uniform within the invocation group
  * [[VUID-{refpage}-subgroupSize-10144]]
    If <<limits-subgroupSize, pname:subgroupSize>> is 1, the
    code:StorageImageArrayNonUniformIndexing capability is not declared, and
  ...
  * [[VUID-{refpage}-subgroupSize-10145]]
    If <<limits-subgroupSize, pname:subgroupSize>> is 1, the
    code:InputAttachmentArrayNonUniformIndexing capability is not declared,
  ...
  * [[VUID-{refpage}-subgroupSize-10146]]
    If <<limits-subgroupSize, pname:subgroupSize>> is 1, the
    code:UniformTexelBufferArrayNonUniformIndexing capability is not
    declared, and an instruction accesses memory through a uniform texel
    buffer, the uniform texel buffer through which that memory is accessed
    must: be dynamically uniform within the invocation group
  * [[VUID-{refpage}-subgroupSize-10147]]
    If <<limits-subgroupSize, pname:subgroupSize>> is 1, the
    code:StorageTexelBufferArrayNonUniformIndexing capability is not is not
    declared, and an instruction accesses memory through a storage texel
    buffer, the storage texel buffer through which that memory is accessed
    must: be dynamically uniform within the invocation group
endif::VK_VERSION_1_1[]
...
ifdef::VK_VERSION_1_1[]
    <<limits-subgroupSize, pname:subgroupSize>> is 1,
endif::VK_VERSION_1_1[]
...
ifdef::VK_VERSION_1_1[]
  * [[VUID-{refpage}-subgroupSize-10149]]
    If an instruction accesses memory through any resource,
    <<limits-subgroupSize, pname:subgroupSize>> is greater than 1, and the
    ...
    operand) must: be decorated with code:NonUniform
endif::VK_VERSION_1_1[]
...
ifdef::VK_KHR_shader_subgroup_uniform_control_flow[]
  * [[VUID-{refpage}-SubgroupUniformControlFlowKHR-06379]]
    ...
    pname:shaderSubgroupUniformControlFlow>> is enabled, the corresponding
    shader stage bit is set in <<limits-subgroupSupportedStages,
    pname:subgroupSupportedStages>>, and the entry point does not execute
    any <<ray-tracing-repack,_invocation repack instructions_>>
...
floating-point instructions:
  ...
  * Positive and negative infinities and positive and negative zeros are
    ...
    precisions allowed in the <<spirvenv-precision-core-table,table below>>.
...
ifdef::VK_VERSION_1_2,VK_KHR_shader_float_controls[]
  ...
  ** The following core SPIR-V instructions must: respect the
     code:DenormPreserve {ExecutionMode}: code:OpSpecConstantOp,
...
These execution modes do not affect code:OpQuantizeToF16 and
code:PackHalf2x16.
