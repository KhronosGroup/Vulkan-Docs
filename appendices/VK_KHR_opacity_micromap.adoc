...
Traversal performed by <<glossary-pipeline-trace-ray, pipeline trace ray>>
...
Traversal performed by code:OpRayQueryProceedKHR instead requires the shader
to declare the code:OpacityMicromapIdKHR execution mode, added by
`SPV_KHR_opacity_micromap`, with its specialization-constant operand equal
to ename:VK_TRUE.
endif::VK_KHR_ray_query[]
See <<ray-opacity-micromap,Ray Opacity Micromap>> for details.
