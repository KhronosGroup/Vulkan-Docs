...
extensions, two notable differences to be aware of are:
  ...
  * The pname:hostMicromapCommands feature and its associated host-side
    build, copy, and query commands are only exposed by
    apiext:VK_EXT_opacity_micromap; those commands are <<legacy-host-builds,
    legacy>> and are not included in apiext:VK_KHR_opacity_micromap.

  * The two extensions differ in how opacity micromap data is consumed by
    ray query traversal.
    With apiext:VK_EXT_opacity_micromap enabled, opacity micromap data
    present in the acceleration structure is always consumed.
    With only apiext:VK_KHR_opacity_micromap enabled and its pname:micromap
    feature enabled, opacity micromap data is consumed only when the shader
    declares the code:OpacityMicromapIdKHR execution mode with its
    specialization-constant operand equal to ename:VK_TRUE.
    If both extensions are enabled, the apiext:VK_EXT_opacity_micromap
    behavior applies and the execution mode is not required.
    Traversal initiated by <<glossary-pipeline-trace-ray, pipeline trace
    ray>> instructions is unaffected by this difference; it requires
    ename:VK_PIPELINE_CREATE_RAY_TRACING_OPACITY_MICROMAP_BIT_KHR under
    either extension.
