...
If the `<<VK_EXT_4444_formats>>` extension is not supported, support for all
formats defined by it are optional in Vulkan 1.3.
...
`<<VK_EXT_extended_dynamic_state2>>` for patch control points and logic op
are not promoted in Vulkan 1.3.
There are no members in the slink:VkPhysicalDeviceVulkan13Features structure
...
If the `<<VK_EXT_texture_compression_astc_hdr>>` extension is not supported,
...
include::{generated}/meta/promoted_extensions_VK_VERSION_1_2.adoc[]
...
The code:ShaderViewportIndexLayerEXT SPIR-V capability was replaced the with
the code:ShaderViewportIndex and code:ShaderLayer capability. Declaring both
...
include::{generated}/meta/promoted_extensions_VK_VERSION_1_1.adoc[]
