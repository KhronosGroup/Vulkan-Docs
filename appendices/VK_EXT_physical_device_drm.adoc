...
device, which have one primary node and may have one render node.

A physical devices always has the primary node if it is associated with a DRM
...
[NOTE]
====
This extension only returns primary node and render node from the same
