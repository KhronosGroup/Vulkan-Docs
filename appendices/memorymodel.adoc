...
ifdef::VK_GRAPHICS_VERSION_1_0[  ** fixed-function stage of the pipeline,]
ifdef::VK_COMPUTE_VERSION_1_0[  ** shader invocation, or]
  ** host thread.
...
Buffers and <<images,images>> are explicitly mapped to memory locations
allocated in slink:VkDeviceMemory objects.
...
ename:VK_MEMORY_ALLOCATE_ZERO_INITIALIZE_BIT_EXT flag, otherwise they are
...
ifdef::VK_COMPUTE_VERSION_1_2[in code:PhysicalStorageBuffer storage class,]
...
ifdef::VK_COMPUTE_VERSION_1_0[]
// TBD - can we do without this for Vulkan base?
// maybe move the scope definitions here, and phrase them in terms of
// operations instead of invocations?
[[memory-model-scope]]
== Scope
...
// TBD - not sure what to do with this for Vulkan base..
...
code:OpMemoryBarrier, and code:OpMemoryNamedBarrier.
...
code:OpMemoryBarrier, and code:OpMemoryNamedBarrier are _memory barrier_
instructions in SPIR-V.
ifdef::VK_EXT_shader_split_barrier[]
code:OpControlBarrierArriveEXT is a _control barrier arrival_ and
code:OpControlBarrierWaitEXT is a _control barrier wait_.
endif::VK_EXT_shader_split_barrier[]
endif::VK_COMPUTE_VERSION_1_0[]
...
_Non-private_ memory operations are intended to be used for communication
with other agents.
Atomic operations are always considered _non-private_.
...
bit set are treated as _non-private_
endif::VK_COMPUTE_VERSION_1_0[]
